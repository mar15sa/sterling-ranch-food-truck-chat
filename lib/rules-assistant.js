...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "", residentQuestion = query) {
  ...
  // Fee amounts and an outdated pet-limit summary need the source-derived
  // evidence path instead of a general clause with a non-factual excerpt.
  const sourceAnswer = (isFeeQuery(query) || (
    isPetKeepingQuery(query) && summaryHasVolatileFacts(originalAnswer)
  ))
    ? ""
    : sourceGroundedRuleAnswer(query, sources);
