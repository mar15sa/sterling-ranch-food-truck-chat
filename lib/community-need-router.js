...
function joinReadableDetails(details = []) {
  ...
  // dense. This preserves the exact evidence-backed wording while making the
  // answer easy to scan on a phone.
  return prose.length > 240 && values.length > 1 ? values.join("\n\n") : prose;
}
...
function supportedAnswerText(result) {
  ...
  if (result.need.task === "specification") {
    ...
    if (facets.length === 1) {
      ...
      if (selected.length) return joinReadableDetails(selected.map((claim) => focusedClaimText(claim, result.need.task)));
    }
  }
