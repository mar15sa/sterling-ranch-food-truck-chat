...
const { observeCanonicalSource, revalidateApprovedEvidence } = require("./community-approved-revalidation");
...
async function revalidateDueApprovedEvidence(options = {}) {
  ...
  const result = await revalidateApprovedEvidence(trusted, {
    ...
    concurrency: options.concurrency || Number(process.env.COMMUNITY_REVALIDATION_CONCURRENCY || 4),
