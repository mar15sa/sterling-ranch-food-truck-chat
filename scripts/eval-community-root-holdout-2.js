...
const cases = require("../test/fixtures/community-root-holdout-2-2026-09-18.json");
...
async function main() {
  ...
  for (const item of cases) {
    ...
    const failures = [];
    const expectedOutcome = item.expectedOutcome || "complete";
    if (result.completion?.outcome !== expectedOutcome) failures.push(`outcome=${result.completion?.outcome || "missing"}`);
  ...
  assert.equal(passed, cases.length, `${passed}/${cases.length} second holdout behaviors passed`);
  console.log(`Second holdout: ${passed}/${cases.length} passed, including the approved-source refusal boundary.`);
}
