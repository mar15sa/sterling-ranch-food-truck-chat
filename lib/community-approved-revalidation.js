...
function projectedActionProof(sources = [], rawLinks = [], text = "") {
  ...
  const observed = expected.map(({ sourceId, action }) => ({
    sourceId,
    action,
    matches: rawLinks.filter((link) => actionUrlIdentity(link.url) === actionUrlIdentity(action.url)),
    contextMatches: Boolean(action.context && text.includes(action.context)),
  }));
  return {
...
async function observeCanonicalSource(sourceUrl, approvedSources, { fetchImpl = globalThis.fetch, extractPdfTextImpl = extractPdfText, timeoutMs = 30_000 } = {}) {
  ...
  if (scopedProjections.length && !ordinarySources.length) {
    const projectionProof = projectedActionProof(scopedProjections, observedLinks, text);
    return {
  ...
  if (scopedProjections.length) {
    ...
    return {
      ...
      // owner-approved full-page projection. Prove both identities so the
      // projection does not expire merely because the crawler also retained
      // ordinary chunks from the same unchanged page.
      observedHashes: [...new Set([...chunkText(text).map(sourceHash), sourceHash(text)])],
