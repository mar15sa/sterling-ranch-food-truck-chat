...
function fenceAndShedOverviewAnswer(query, sources = [], index) {
  ...
  const height = shedText.match(/overall height of\s+([^.]+)\./i)?.[1]?.trim();
  const footprint = shedText.match(/shed footprint maximum is\s+([^.]+)\./i)?.[1]?.trim();
...
function simpleTopicOverviewAnswer(query, sources = []) {
  ...
  if (isAmenityReservationQuery(query)) {
    const answer = facilityRentalOverviewAnswer(query, sources);
    if (answer) return answer;
  }
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (searchMode === "ai-hybrid") {
    ...
    if (
      ...
      !["facility_reservation", "rental_cancellation"].includes(searchPlan?.intent)
    ) {
  ...
  const derived = compoundDerived || (needsSourceDerivedFacts
    ? sourceDerivedAnswerParts(routingQuery, answerSources, plainAnswer)
