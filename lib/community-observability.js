...
const counters = {
  ...
  routingContractFailures: 0,
  lastRoutingContractFailureAt: "",
  totalDurationMs: 0,
  routingDecisions: {},
  routingGoals: {},
...
function supportedLiveRoutingPlan(plan = {}) {
  const text = `${plan.subject || ""} ${(plan.searchQueries || []).join(" ")}`;
  const goals = new Set([plan.goal, ...(plan.goals || [])]);
  return ["schedule", "cost", "information"].some((goal) => goals.has(goal))
    && /\bfood\s*trucks?\b/i.test(text)
    || ["schedule", "status", "information"].some((goal) => goals.has(goal))
      && /\b(?:trash|garbage|recycl(?:e|ing)|waste)\b/i.test(text);
}
...
function recordCommunityAnswer({ answer, resolvedQuestion, usedPriorContext, durationMs, aiUsage } = {}) {
  ...
  const trace = {
    ...
    completionOutcome: answer.completion?.outcome || "unknown",
    requestedDetailCount: answer.completion?.requestedDetails?.length || 0,
    unresolvedDetailKeys: (answer.completion?.missingDetails || []).map((detail) => detail.key).slice(0, 8),
    completionBlockerTypes: (answer.completion?.blockers || []).map((blocker) => blocker.type).slice(0, 8),
    ...
    interpretationMode: answer._interpretation?.mode || "legacy",
    interpretationOutcome: answer._interpretation?.outcome || "legacy",
    clarificationReason: answer._interpretation?.clarificationReason || "",
    appliedFilterCount: (answer._interpretation?.appliedFilters || answer._connectorDiagnostics?.appliedFilters || []).length,
    connectorOutcome: answer._connectorDiagnostics?.sourceOutcome || "",
    beforeFilterCount: Number.isFinite(answer._connectorDiagnostics?.beforeFilterCount) ? answer._connectorDiagnostics.beforeFilterCount : null,
    afterFilterCount: Number.isFinite(answer._connectorDiagnostics?.afterFilterCount) ? answer._connectorDiagnostics.afterFilterCount : null,
    questionFingerprint,
  ...
  increment(counters.interpretationOutcomes, trace.interpretationOutcome);
  increment(counters.connectorOutcomes, trace.connectorOutcome);
  increment(counters.completionOutcomes, trace.completionOutcome);
  if (trace.appliedFilterCount) counters.filteredAnswers += 1;
  ...
  if (supportedLiveRoutingPlan(answer.routingPlan)
    && (answer.answerStatus === "out-of-scope" || answer.inputClassification === "unrelated")) {
    counters.routingContractFailures += 1;
    counters.lastRoutingContractFailureAt = new Date().toISOString();
    console.warn(JSON.stringify({
      event: "community_routing_contract_failure",
      questionFingerprint,
      routingGoal,
      routingIntent: answer.routingPlan.intent,
    }));
...
function communityAnswerMetrics() {
  return {
    ...
    interpretationOutcomes: { ...counters.interpretationOutcomes },
    connectorOutcomes: { ...counters.connectorOutcomes },
    averageDurationMs: counters.answers ? Math.round(counters.totalDurationMs / counters.answers) : 0,
    recent: recent.slice(-10),
  };
}

module.exports = { communityAnswerMetrics, privacyFingerprint, recordCommunityAnswer, sourceAgeMs, supportedLiveRoutingPlan };
