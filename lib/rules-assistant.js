...
function landscapeOverviewAnswer(sources = []) {
  return evidenceFacetOverviewAnswer("landscape yard requirements", sources, [
    /Landscape and irrigation plans[\s\S]{0,180}must be submitted[\s\S]{0,180}(?:review and approval|DRC)/i,
    /front yard (?:shall consist|design and installation shall include)|live coverage/i,
    /backyard landscaping|backyard must contain/i,
    /automatic, underground irrigation system is required/i,
    /Landscaping is to be kept healthy/i,
  ]);
}

function parksOpenSpaceOverviewAnswer(sources = []) {
  return evidenceFacetOverviewAnswer("parks trails open space rules", sources, [
    /open from[\s\S]{0,80}(?:a\.m\.|p\.m\.)/i,
    /dog owners must leash|physical control of their dog/i,
    /Motorized vehicles are prohibited/i,
    /Glass containers, littering, dumping/i,
    /Fires are only permitted|Fires in Open Space areas/i,
    /Swimming or the use of watercraft/i,
    /fishing is permitted[\s\S]{0,180}permit/i,
