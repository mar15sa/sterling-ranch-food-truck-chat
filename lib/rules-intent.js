...
const TYPO_CORRECTIONS = Object.freeze([
  ...
  [/\bscrens?\b/gi, "screens"],
  [/\blibary\b/gi, "library"],
  [/\blibray\b/gi, "library"],
  [/\blitte\b/gi, "little"],
]);
...
const COMMUNITY_TOPIC_TERMS = new RegExp(
  [
    ...
    "holiday displays?|holiday decorations?|hallowe+en decor(?:ations?)?|hallowen decoratons?|christmas decorations?",
    "backyards?",
...
function negativeCatalogMembershipClaims(value = "", options = {}) {
  ...
  const resourceFirst = new RegExp(
    `^(${resource})[^.!?\\n]{0,25}?\\s+(?:does|do)(?:n't| not)\\s+` +
      `(?:include|contain|list|name|show)\\s+(.{1,120})$`,
    "i"
  );
...
// Small resident-installed sharing fixtures use many informal names. Route
// the family only when the wording asks about placing one on residential
// property (or is the familiar short-form name), so public-library services,
// book donations, and online resources remain outside the property-rule path.
function isResidentSharingFixtureQuery(value = "") {
  const text = normalizeResidentQuestion(value);
  const namedFixture = /\b(?:(?:little|lil)\s+free\s+librar(?:y|ies)|free\s+little\s+librar(?:y|ies)|neighbou?rhood\s+(?:lending|sharing)\s+librar(?:y|ies)|(?:book|seed)\s+(?:swap|exchange|sharing|lending)\s+(?:boxes|box|cabinets?|stations?))\b/i;
  if (!namedFixture.test(text)) return false;
  if (/\b(?:public|school|digital|online)\s+librar(?:y|ies)|\blibrary cards?|\bbook clubs?|\b(?:donate|borrow|return|check out)\s+books?\b|\b(?:hours?|open|close[sd]?)\b/i.test(text)) return false;
  const placement = /\b(?:build|construct|install|place|put|set up|setup|add|mount|have|do)\b|\b(?:front|back|rear|side)\s+yards?\b|\b(?:my|our|residential)\s+(?:yards?|lots?|propert(?:y|ies))\b|\b(?:sidewalk|tree lawn|driveway)\b/i;
  const shortForm = /^\s*(?:a\s+)?(?:(?:little|lil)\s+free\s+library|free\s+little\s+library|neighbou?rhood\s+(?:lending|sharing)\s+library|(?:book|seed)\s+(?:swap|exchange|sharing|lending)\s+(?:box|cabinet|station))\s*[?!.]*\s*$/i;
  return placement.test(text) || shortForm.test(text);
}
...
function unsupportedResourceAbsenceIssues(answer = "", sources = []) {
  ...
  const membershipClaims = negativeCatalogMembershipClaims(answer, { catalogContext });
...
function hasCommunityTopicSignal(value = "") {
  return COMMUNITY_TOPIC_TERMS.test(normalizeResidentQuestion(value))
    ...
    || isResidentSharingFixtureQuery(value);
}
...
function answerCoverageIssues(question, answer = "", sources = []) {
  ...
  if (facets.includes("duration") && !/\b(?:hours?|days?|nights?|weeks?|duration|time|limit|does not (?:give|state|specify))\b|\b\d{1,2}(?::\d{2})?\s*[ap]\.?m\.?/i.test(text)) issues.push("requested-duration-missing");
  if (facets.includes("permission") && !hasPermissionStatement(text)
    && !/\b(?:does not (?:say|state|specify)|evaluated on an individual basis)\b/i.test(text)) issues.push("direct-permission-answer-missing");
