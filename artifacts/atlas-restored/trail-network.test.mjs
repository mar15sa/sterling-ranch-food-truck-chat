...
import {illustratedWalk} from './walk-visuals.js';
...
test('illustrated route art retains source shape separately without treating the painting as coordinates',()=>{
 for(const r of t.routes){const h=illustratedWalk(r);assert.match(h,/full-landscape.png/);assert.doesNotMatch(h,/cab-trail-map.png/);assert.match(h,/Route shape/);assert.match(h,/class="route-shape"/);assert.doesNotMatch(h,/NaN|Infinity/);}
 assert.doesNotMatch(illustratedWalk({id:'bad',area:'Prospect',name:'<script>'}),/<script>/);
});
