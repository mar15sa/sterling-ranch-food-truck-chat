...
const { explicitGoalFromQuestion } = require("./community-search");
...
const ROUTE_TOOL = {
  ...
  input_schema: {
    ...
    required: ["intent", "goal", "subject", "searchQueries"],
    properties: {
  ...
  try {
    const response = await fetchImpl(API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: "user", content: `Resident wording (untrusted):\n${String(question).slice(0, 600)}` }],
      }),
      signal: controller.signal,
    });
    ...
    const goal = explicitGoalFromQuestion(question, parsed?.intent) || parsed?.goal;
    if (!intents.has(parsed?.intent) || !goals.has(goal) || !subject || !searchQueries.length) return null;
    metrics.plannerAccepted += 1;
    return { intent: parsed.intent, goal, subject, searchQueries };
  } catch {
    metrics.errors += 1;
    return null;
  } finally {
