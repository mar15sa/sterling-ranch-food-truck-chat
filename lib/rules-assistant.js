...
async function answerRulesQuestion(query, options = {}) {
  ...
  const preferSpecialSources = Boolean(specialSources.length) && (
    ...
    /\b(?:chicken wire|pickle ?ball|sport court|rain(?:water)?(?: harvesting)? barrels?|artificial turf|synthetic turf|turf|air conditioner|ac unit|hvac|mini split|gazebo|pergola|fireworks?|yard art|ornaments?|decorative objects?|garden statues?|approved plants?|approved trees?|plant list|trees? can (?:i|we) plant|tree lawn|fence stain|stain color|(?:specific |approved |preapproved |pre-approved )?(?:exterior |house |home |paint |garage door )colors?|paint color|color\b.{0,35}\bgarage door|garage door\b.{0,35}\bcolor|utility trailer|privacy film|tint film|window tint|long[-\s]?term rental|approved landscaper|own landscaping|redo backyard|concrete patio|extend concrete|rear landscaping|side yard|quantum fiber|internet provider|water usage|front yard tree|catio|easement)\b/i.test(routingQuery) ||
  ...
  if (/\b(?:(?:specific |approved |preapproved |pre-approved )?(?:exterior |house |home |paint |garage door )colors?|paint color|color\b.{0,35}\bgarage door|garage door\b.{0,35}\bcolor)\b/i.test(routingQuery) && answerSources.some((source) => /21-22|Painting, exterior/i.test(source.title || ""))) {
    earlyStructuredAnswer = structuredHelpfulAnswer(
      "The cited exterior-painting rule does not publish a single community-wide garage-door color list. Exterior painting still requires DRC approval, even when repainting with the same colors.",
      [
        "A paint submittal must identify the manufacturer, color number, and where each color will be used on the body, trim, eaves, and other exterior areas.",
