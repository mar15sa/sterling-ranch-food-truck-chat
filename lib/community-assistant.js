...
const { GOALS, addDays, correctNavigationSubject, deterministicRequestedDetails, highConfidenceDateRange, isOfficialInformationPageRequest, isReportedFindingsRequest, isWaterUsageAccessRequest, instructionalRequestShape, normalizeInterpretation, resolveInterpretationMode } = require("./community-interpretation");
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (isOfficialInformationPageRequest(question, routingPlan)) {
    ...
    const navigationTerms = new Set(["could", "details", "find", "info", "information", "look", "official", "open", "page", "please", "resource", "service", "show", "site", "website", "where", "would"]);
    ...
    const subjectText = correctNavigationSubject(question.split(/\s+/)
