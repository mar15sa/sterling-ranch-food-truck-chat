...
async function answerCommunityQuestion(query, options = {}) {
  ...
  const proactive = proactiveCommunityAnswer(question, { index: options.index, now: options.now });
  if (proactive) return { ...proactive, communityIntent: classifyCommunityIntent(question) };
...
module.exports = { answerCommunityQuestion, bestContactContext, cleanAnswerText, conciseRecurringSchedule, contactDirectAnswer, eventsAnswer, extractiveAnswer, liveRecyclingScheduleAnswer, poolStatusAnswer, relevantActions, sourcedAnswer, unanchoredRecurringScheduleAnswer, usefulSentences };
