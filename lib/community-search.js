...
function normalizedRoutingPlan(plan = {}, question = "", options = {}) {
  ...
  const goal = compoundGoal || explicitGoalFromQuestion(question, proposedIntent) || modelGoal;
  ...
  const intent = fixedIntent || (goal === "schedule"
    ? (proposedIntent === "rules" ? "rules" : proposedIntent === "events" || /\b(?:events?|classes?|activities|programs?|meetings?|calendar)\b/i.test(question) ? "events" : "services")
