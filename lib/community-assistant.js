const { buildAnswerContract, detectFactConflicts, reviewedFactCoversDetail, reviewedDetailIsRendered } = require("./community-contracts");
const { isContextDependentQuestion } = require("./community-conversation");
const { answerStatusForCompletion, resolveAnswerCompletion } = require("./community-completion");
const { runNeedFirstShadow } = require("./community-need-router");
const { assessResidentNeeds, buildResidentRequestContract } = require("./community-request-contract");
const { foodTruckAnswer, isFoodTruckQuestion, isFoodTruckRequest, normalizeFoodTruckQuestion } = require("./community-food-trucks");
const { GOALS, addDays, correctNavigationSubject, deterministicRequestedDetails, highConfidenceDateRange, isInternetServiceSetupRequest, isOfficialInformationPageRequest, isReportedFindingsRequest, isWaterUsageAccessRequest, instructionalRequestShape, normalizeInterpretation, resolveInterpretationMode } = require("./community-interpretation");
...
function sourceForDisplay(source) {
  const ownerObservation = source.connectorType === "owner-observation"
    || source.provenanceType === "owner-observation";
  return {
    ...source,
    nodeId: source.nodeId || `COMMUNITY_${String(source.id || "SOURCE").toUpperCase().replace(/[^A-Z0-9]+/g, "_")}`,
    sourceName: source.sourceName || (ownerObservation ? "Community-provided onsite information" : "Official community website"),
    isOfficialResource: ownerObservation ? false : source.isOfficialResource !== false,
  };
}
...
function cleanSafeRulesBoundaryText(value = "") {
  ...
  if (!/mentions? [^.\n]{1,120} only as an example in a different rule/i.test(cleaned)) return cleaned;
...
const GENERIC_EVIDENCE_TERMS = new Set([
  "account", "alert", "alerts", "answer", "approved", "cab", "community", "contact", "directory", "does", "help", "hoa", "information",
  "home", "list", "listed", "listing", "lost", "number", "official", "phone", "question", "ranch", "restore", "rule", "rules",
  "sterling", "subscribe", "system", "their", "notification", "notifications", "notify", "notified",
  "there", "have", "available", "nearby", "find", "looking",
]);
...
const DEFINITION_GRAMMAR_TERMS = new Set([
  "that", "these", "this", "those", "what", "which", "where", "who",
]);

const OPERATIONAL_REQUEST_HEAD_TERMS = new Set([
  "address", "application", "cost", "date", "email", "form", "hours", "link",
  "page", "portal", "price", "schedule", "time", "website",
]);

function requiredDefinitionHeadTerm(question = "") {
  const match = String(question).toLowerCase().match(/^\s*(?:what|who)\s+(?:is|are)\s+(.+?)[?.!]*\s*$/i);
  if (!match) return "";
  const terms = (match[1].match(/[a-z0-9][a-z0-9'-]*/g) || []).filter((term) =>
    term.length >= 4
    && !GENERIC_EVIDENCE_TERMS.has(term)
    && !DEFINITION_GRAMMAR_TERMS.has(term)
  );
  const head = terms.at(-1) || "";
  return OPERATIONAL_REQUEST_HEAD_TERMS.has(head) ? "" : head;
}

function evidenceHasTerm(evidenceTokens, term) {
  if (!term) return true;
  if (evidenceTokens.has(term)) return true;
  if (term.endsWith("s") && evidenceTokens.has(term.slice(0, -1))) return true;
  return evidenceTokens.has(`${term}s`);
}

function hasDistinctiveCommunityEvidence(question, sources = [], { includeApprovedActions = false } = {}) {
  const distinctive = tokens(question).filter((token) => token.length >= 4 && !GENERIC_EVIDENCE_TERMS.has(token));
  const requiredDefinitionHead = requiredDefinitionHeadTerm(question);
  if (!distinctive.length) return false;
  return sources.some((source) => {
    const approvedSubjects = source.canonicalFactualProjection ? (source.facts || []).filter(fact => fact.approvalClaim)
      .map(fact => `${fact.scopeKey || ''} ${fact.subjectKey || ''}`.replace(/[-_]/g, ' ')).join(' ') : '';
    const approvedActionEvidence = (source.actions || [])
      .filter((action) => action.approvalClaim)
      .map((action) => `${action.context || ""} ${action.evidence?.label || ""} ${action.evidence?.context || ""}`)
      .join(" ");
    const evidence = `${source.title || ""} ${source.text || ""} ${approvedSubjects} ${includeApprovedActions ? approvedActionEvidence : ""}`.toLowerCase();
    // The reviewed general architectural form uses the governing term
    // “accessory building,” while residents normally say “shed.” Treat the
    // two as the same project only when that wording appears inside the
    // approved action evidence itself.
    if (/\bsheds?\b/i.test(question) && /\baccessory buildings?\b/i.test(approvedActionEvidence)) return true;
    const evidenceTokens = new Set(tokens(evidence));
    // In a short definition question, the final subject word is the thing the
    // resident asked us to identify. A shared brand or place name alone is not
    // enough (for example, a coffee page cannot define a similarly named WiFi
    // service). This protects other named products and services from the same
    // single-word collision without adding a stored answer for any one name.
    if (!evidenceHasTerm(evidenceTokens, requiredDefinitionHead)) return false;
    return distinctive.some((token) => evidenceTokens.has(token));
  });
}
...
function isWaterFeeStructureQuestion(question = "") {
  const text = String(question);
  const asksBasePurpose = /\b(?:fixed|monthly|base)\b.{0,35}\b(?:water )?(?:fee|charge)\b.{0,30}\b(?:pay for|include|cover)\b/i.test(text)
    || /\bwhat does\b.{0,55}\b(?:fixed|monthly|base)\b.{0,25}\b(?:fee|charge)\b/i.test(text);
  const asksUsageStructure = /\busage charge\b.{0,35}\b(?:fixed|variable|tiered)\b/i.test(text)
    || /\b(?:fixed|variable|tiered)\b.{0,35}\busage charge\b/i.test(text);
  return asksBasePurpose || asksUsageStructure;
}

function hasApprovedWaterFeeStructureFact(sources = [], question = "") {
  const text = String(question);
  const facts = sources.flatMap((source) => (source.facts || [])
    .filter((fact) => fact.approvalClaim)
    .map((fact) => String(fact.context || fact.value || "")));
  const asksBasePurpose = /\b(?:fixed|monthly|base)\b.{0,35}\b(?:water )?(?:fee|charge)\b.{0,30}\b(?:pay for|include|cover)\b/i.test(text)
    || /\bwhat does\b.{0,55}\b(?:fixed|monthly|base)\b.{0,25}\b(?:fee|charge)\b/i.test(text);
  const asksUsageStructure = /\busage charge\b.{0,35}\b(?:fixed|variable|tiered)\b/i.test(text)
    || /\b(?:fixed|variable|tiered)\b.{0,35}\busage charge\b/i.test(text);
  const supportsBasePurpose = facts.some((fact) => /\b(?:fixed|base)\b.{0,80}\b(?:fixed and operating costs|costs before any service|covers? the cost)\b/i.test(fact));
  const supportsUsageStructure = facts.some((fact) => /\busage charge\b.{0,80}\b(?:tiered|variable|based (?:upon|on) use)\b|\b(?:tiered|variable)\b.{0,80}\busage charge\b/i.test(fact));
  return (!asksBasePurpose || supportsBasePurpose) && (!asksUsageStructure || supportsUsageStructure);
}

const FACILITY_SUBJECT_TERMS = new Set(["pool", "clubhouse", "hall", "pavilion", "park", "shelter", "court", "amenity"]);

function sourceMatchesRequestedFacility(question, source = {}) {
  if (isUtilityInfrastructureFeeRequest(question) && source.sourceType === "facilities") return false;
  // Exact reviewed subject identity is stronger than an incidental facility
  // noun in display prose. A scoped membership price need not repeat the
  // building name; inferred IDs and raw page text cannot grant this exception.
  const withoutFacilityNouns = String(question).replace(new RegExp(`\\b(?:${[...FACILITY_SUBJECT_TERMS].join('|')})s?\\b`, 'gi'), ' ');
  if (source.canonicalFactualProjection === true && source.reviewedSubjectMatch === true
    && hasOriginalOperationalSubject(withoutFacilityNouns, source)) return true;
  const terms = tokens(question).filter(term => FACILITY_SUBJECT_TERMS.has(term));
  const evidence = `${source.title || ""} ${source.text || source.excerpt || ""}`.toLowerCase();
  return !terms.length || terms.some(term => new RegExp(`\\b${term}s?\\b`).test(evidence));
}
...
function hasSafeWithheldHandoffEvidence(question, sources = [], routingPlan = null) {
  ...
  const facilityTerms = [...new Set(tokens(`${question} ${plan.subject || ""}`).filter((term) =>
    FACILITY_SUBJECT_TERMS.has(term)
  ))];
...
function directlyRelevantOfficialPdf(question, sources = []) {
  ...
  const facetGroups = [
    [/\b(?:color|colour|paint|stain|finish(?:es)?)\b/i, /\b(?:color|colour|paint|stain|finish(?:es|ed|ing)?)\b/i,
      (source) => (source.facts || []).some((fact) => fact.facet === "specification"
        && /#[0-9]{2,}\b|\b(?:Sherwin Williams|Solomon)\b/i.test(fact.context || fact.value || ""))],
  ...
  return sources.find((source) => {
    ...
    return requestedObjects.every((pattern) => pattern.test(evidence))
      && requestedFacets.every(([, evidencePattern, structuredMatch]) => evidencePattern.test(evidence)
        || structuredMatch?.(source));
  }) || null;
}
...
function isPublicPickleballOperationsQuestion(question = "") {
  ...
  return /\bpickleball\b/.test(normalized)
    && (/\b(?:community|neighborhood|public)\b/.test(normalized)
      || /\b(?:play|hours|open play|reserve|reservation|paddles?|balls?|shoes?|equipment|children|kids?|pets?|snow|wet|parking|located|where|cost|price|fee|nonresidents?|residents?)\b/.test(normalized))
...
function isParkingPriceRequest(question = "") {
  return /\b(?:pay|cost|fee|free|price)\b[\s\S]{0,40}\bpark(?:ing)?\b|\bpark(?:ing)?\b[\s\S]{0,40}\b(?:pay|cost|fee|free|price)\b/i.test(question);
}
...
function operationalFacilitySource(question, sources = []) {
  ...
  return sources.find((source) => source.sourceType === "facilities"
    && sourceMatchesRequestedFacility(question, source)
...
function isWasteStorageRuleQuestion(question = "") {
  ...
  return /\b(?:trash|garbage|recycling|waste|bins?|carts?|containers?)\b/i.test(text)
    && (/(?:bring|take)(?:\s+\w+){0,4}\s+(?:in|back)\b/i.test(text)
      || /\b(?:store|stored|storage|keep|leave|left|overnight|curb|bring back|take back|taking|put out|outside|exact hour|what time)\b/i.test(text))
    && !/\b(?:pickup|pick up|collection|schedule|which day|what day)\b/i.test(text);
}

function asksBindingRule(question = "", routingPlan = null) {
  if (routingPlan?.intent === "rules" || routingPlan?.requestedDetails?.includes("permission")
    || originalQuestionNeedsRule(question)) return true;
...
function actionGoalStrength(action, goal) {
  if (!action || !actionSupportsGoal(action, goal)) return 0;
  const exactType = { booking: 'booking', payment: 'payment', application: 'form', registration: 'registration', 'account-access': 'account' }[goal];
  if (action.actionType === exactType) return 3;
  const directVerbs = { booking: /\b(?:book(?:ing)?|reserve|check availability)\b/i,
    payment: /\b(?:pay|make a payment)\b/i, application: /\b(?:apply|submit)\b/i,
    registration: /\b(?:register|sign up|enroll)\b/i, 'account-access': /\b(?:log ?in|sign ?in|reset|open a ticket)\b/i };
  return directVerbs[goal]?.test(`${action.label || ''} ${action.context || ''} ${action.evidence?.label || ''} ${action.evidence?.context || ''}`) ? 2 : 1;
}

// Authority follows the resident's requested decision, not the confidence or
// permission-like language of a retrieved rule. Operational facts and live
// observations cannot become binding-rule questions merely through a plan.
function originalQuestionNeedsRule(question = "") {
  const details = deterministicRequestedDetails(question);
  const normativeSpecification = details.includes('specification') && !instructionalRequestShape(question).calculation
    && /\b(?:allowed|permitted|maximum|minimum|can I|may I|should I)\b/i.test(question);
  const generalResidentFees = /\b(?:fees?|charges?)\b/i.test(question) && /\b(?:residents?|homeowners?)\b/i.test(question)
    && !/\b(?:membership|pass|ticket|rental|booking)\b/i.test(question) && !isOperationalFacilityQuestion(question);
  return (details.includes('permission') && !isParkingPriceRequest(question)) || normativeSpecification || generalResidentFees
    || /\b(?:rules?|regulations?|requirements?|allowed|permitted|prohibited|prohibition|drc|design review|approval|section|sec\.)\b/i.test(question)
    || isResidentialPropertyRentalRequest(question)
    || isWasteStorageRuleQuestion(question);
}

function hasOriginalOperationalSubject(question, source = {}, { requireAll = false } = {}) {
  const generic = new Set([...GENERIC_PROJECT_TERMS, 'cost', 'costs', 'price', 'prices', 'fees', 'fee',
    'much', 'does', 'about', 'what', 'which', 'could', 'should', 'check', 'whether', 'running',
    'right', 'now', 'schedule', 'scheduling', 'report', 'reported', 'findings', 'violations',
    'inspection', 'information', 'service', 'services', 'support', 'eligible', 'eligibility',
    'alert', 'alerts', 'notification', 'notifications', 'notify', 'notified', 'subscribe',
    'sterling', 'ranch', 'community', 'authority', 'board', 'cab', 'job', 'house', 'home',
    'front', 'missed', 'friend', 'here', 'first']);
  const normalize = text => String(text || '').toLowerCase().replace(/\bnon[-\s]+resident/g, 'nonresident');
  const normalizedQuestion = normalize(question);
  const normalizedEvidence = normalize(`${source.title || ''} ${(source.facts || []).filter(fact => fact.approvalClaim)
    .map(fact => `${fact.context || fact.value || ''} ${fact.subjectKey || ''}`).join(' ')}`);
  const subjectFamilies = [
    [/\b(?:leak|excessive water use|water use)\b.{0,80}\b(?:relief|adjustment|forgiveness|form)\b|\b(?:relief|adjustment|forgiveness)\b.{0,80}\b(?:leak|excessive water use)\b/i,
      /\b(?:unintentional excessive water use|leak relief|leak forgiveness|relief form)\b/i],
    [/\b(?:snow\s*plow|snow removal|plow(?:ed|ing)?|clear(?:s|ed|ing)?\s+snow)\b/i,
      /\b(?:snow removal|removes? snow|handles? the public roadways|shared drives?|townhomes?.{0,40}(?:hoa|snow service))\b/i],
    [/\bmailbox(?:es)?\b/i, /\bmailbox(?:es)?\b/i],
    [/\bleak(?:s|ed|ing)?\b/i, /\b(?:leak|excessive water use|forgiveness)\b/i],
    [/\b(?:watch(?:es|ing)?|care(?:s|d|ing)? for|look(?:s|ed|ing)? after)\b.{0,40}\b(?:kids?|children|child)\b|\bcaregiver\b/i, /\bcaregiver\b/i],
    [/\bpickleball\b/i, /\bpickleball\b/i],
    [/\bsolar\b/i, /\bsolar\b/i],
    [/\bpool\b/i, /\bpool\b/i],
    [/\b(?:services?|utilities?)\b[\s\S]{0,100}\bautomatically\b|\bautomatically\b[\s\S]{0,100}\b(?:services?|utilities?)\b|\bclose\b[\s\S]{0,80}\b(?:set ?up|start|activate)\b/i,
      /\b(?:automatically\s+(?:be\s+)?set up|responsible for setting up personal accounts|service setup after closing)\b/i],
  ];
  if (subjectFamilies.some(([requestPattern, evidencePattern]) => requestPattern.test(normalizedQuestion)
    && !evidencePattern.test(normalizedEvidence))) return false;
  const subjectTerms = tokens(normalize(question)).filter(term => term.length >= 4 && !generic.has(term));
  if (!subjectTerms.length) return false;
  return requireAll
    ? subjectTerms.every(term => tokens(normalizedEvidence).includes(term))
    : subjectTerms.some(term => normalizedEvidence.includes(term));
}

function hasApprovedCommunityEvidenceMatch(question, plan, options = {}) {
  const interpretation = plan ? { ...plan, scope: "community" } : null;
  const evidence = searchCommunityIndex(question, {
    index: options.index,
    indexPath: options.indexPath,
    communityId: options.communityId,
    intent: plan?.intent || classifyCommunityIntent(question),
    ...(interpretation ? { interpretation } : {}),
    now: options.now,
    limit: 10,
  });
  return evidence.sources.some((source) => source.canonicalFactualProjection
    && hasOriginalOperationalSubject(question, source, { requireAll: true })
    && (source.facts || []).some((fact) => fact.approvalClaim));
}

function actionForGoal(source = {}, goal = "") {
  const action = (source?.actions || []).filter(action => actionSupportsGoal(action, goal))
    .sort((left, right) => actionGoalStrength(right, goal) - actionGoalStrength(left, goal))[0] || null;
  if (goal === "application" && /APPLICATION FOR APPROVAL OF GENERAL ARCHITECTURAL IMPROVEMENTS/i.test(action?.evidence?.context || "")) {
    return { ...action, label: "Open the general architectural improvement application" };
  }
  return action;
}

function preferredSourceForGoal(sources, goal) {
  return sources.filter(source => actionForGoal(source, goal))
    .sort((left, right) => actionGoalStrength(actionForGoal(right, goal), goal)
      - actionGoalStrength(actionForGoal(left, goal), goal))[0] || null;
}

function recoveryNeedsProcessEvidence(question = '', sources = []) {
  const recovery = /\b(?:restor(?:e|ing)|recover(?:y|ing)?|regain(?:ing)?|reset(?:ting)?|troubleshoot(?:ing)?|repair(?:ing)?|fix(?:ing)?)\b/i.test(question)
    || /\b(?:lost access|locked out|cannot log in|can't log in|cannot sign in|can't sign in)\b/i.test(question);
  if (!recovery) return false;
  // A support destination can be the next move without being the answer to
  // how to recover access or repair a device. Continue grounded retrieval if
  // the shortcut has no reviewed explanation of its own.
  return !sources.some(source => hasDistinctiveCommunityEvidence(question, [source])
    && (source.facts || []).some(fact => {
      if (!fact.approvalClaim || fact.type !== 'information') return false;
      const text = String(fact.context || fact.value || '');
      const recoveryContext = /\b(?:restor(?:e|ing)|recover(?:y|ing)?|regain(?:ing)?|reset(?:ting)?|troubleshoot(?:ing)?|repair(?:ing)?|fix(?:ing)?|support|help|password)\b/i.test(text);
      return recoveryContext && (claimTextCoversDetail(text, 'action') || claimTextCoversDetail(text, 'contact'));
    }));
}

function recoveryInstructionBoundary(question, result, options = {}) {
  const sources = (result.sources || []).map(sourceForDisplay);
  const actions = relevantActions(question, sources, 1, options.routingPlan);
  return buildAnswerContract({
    ...
    sources,
    status: 'could-not-verify',
    requestedDetails: [...new Set([...(result.requestedDetails || []), 'methods'])],
    coveredDetails: actions.length ? ['action'] : [],
    answerMode: 'community-recovery-instructions-unavailable',
  });
}
...
function relevantActions(question, sources, limit = 3, routingPlan = null) {
  ...
  const actionEvidenceMatchesToken = (token, evidence) => token === "shed"
    ? /\b(?:sheds?|accessory buildings?)\b/i.test(evidence)
    : evidence.includes(token);
  const needsReviewedActionSubject = /\b(?:sheds?|accessory buildings?)\b/i.test(question);
  ...
  const actions = sources.flatMap((source) => {
    ...
    return sourceActions.map((action) => ({
      ...
      sourceEvidence: `${source.title || ""} ${source.text || ""} ${source.excerpt || ""} ${needsReviewedActionSubject ? `${action.evidence?.label || ""} ${action.evidence?.context || ""}` : ""}`.toLowerCase(),
  ...
  const ranked = actions
    ...
    .map((action) => ({
      ...
      coreMatch: !coreTokens.length || coreTokens.some((token) => actionEvidenceMatchesToken(token, action.actionText))
        || (action.sourceActionCount === 1 && coreTokens.some((token) => actionEvidenceMatchesToken(token, action.sourceEvidence))),
    }))
...
function isWastePickupScheduleRequest(question = "", plan = null) {
  ...
  return /\b(?:week|next|date|today|tomorrow|pickup|pick up|picked up|collected|collection|schedule|delayed|delay|holiday)\b/i.test(text)
    || /\bwhen\s+(?:is|are)\s+(?:the\s+)?(?:next\s+)?(?:trash|garbage|recycling)\b/i.test(text);
}

function isRecurringWasteDayQuestion(question = "", now = new Date()) {
  if (!/\b(?:trash|garbage|recycling|waste)\b/i.test(question)) return false;
  if (highConfidenceDateRange(question, now)) return false;
  if (deterministicRequestedDetails(question).some(detail => detail !== 'date'
    && !(detail === 'examples' && /\bwhich days?\b/i.test(question) && !/\bexamples?\b/i.test(question)))) return false;
...
function liveWasteScheduleAnswer(question, schedule) {
  ...
  return { ...buildAnswerContract({
    ...
    actions,
    sources,
    claims: [directAnswer, ...keyDetails].map((text) => ({
      text,
      evidenceSourceIds: [dateEvidence.evidenceId],
    })),
    status: "verified",
...
function approvedWasteHolidayContext(question, index = {}) {
  if (!/\b(?:holiday|delay(?:ed)?|late|on\s+time)\b/i.test(question)) return null;
  const source = (index.sources || []).find((item) => item.id === "approved-trash-recurring-service");
  const fact = (source?.facts || []).find((item) => item.scopeKey === "trash-holiday-delay"
    && item.reviewStatus === "approved");
  if (!source || !fact?.value) return null;
  return { source: sourceForDisplay(source), fact };
}

function isBinStorageLocationRequest(question = "") {
  return /\bwhere\b.{0,80}\b(?:keep|place|put|store)\b.{0,50}\b(?:bins?|cans?|carts?|containers?)\b/i.test(question);
}

function isWastePickupStatusRequest(question = "", plan = null) {
  if (plan?.goal === "status" || (plan?.requestedDetails || []).includes("status")) return true;
  return /\b(?:delay(?:ed)?|late|on\s+time)\b/i.test(question)
    || /\bholiday\b.{0,40}\b(?:change|affect)s?\b|\b(?:change|affect)s?\b.{0,40}\bholiday\b/i.test(question);
}

function unavailableWasteScheduleAnswer(question, profile = {}, plan = {}, candidate = null, options = {}) {
  ...
  const delayRequested = isWastePickupStatusRequest(question, plan);
  const service = /\brecycl/i.test(question) ? "recycling" : /\b(?:trash|garbage)\b/i.test(question) ? "trash" : "waste";
  // The hosted server supplies an index path and lets the assistant load it,
  // while focused tests often inject an already-loaded index. Support both so
  // approved holiday guidance cannot disappear only after deployment.
  const holidayContext = approvedWasteHolidayContext(
    question,
    options.index || loadCommunityIndex(options.indexPath)
  );
  const candidateActions = hasVerifiedDate && (candidate.actions || []).length
    ? candidate.actions
    : action ? [{ label: action.label, url: action.url, actionType: action.type }] : [];
  const pickupAction = candidateActions[0]
    ? {
        ...candidateActions[0],
        label: /\bpickup\b/i.test(candidateActions[0].label || "")
          ? candidateActions[0].label
          : "Check your address in the official pickup calendar",
      }
    : null;
  const actions = pickupAction ? [pickupAction, ...candidateActions.slice(1)] : [];
  const sources = [
    ...(hasVerifiedDate ? candidate.sources : []),
    ...(holidayContext ? [holidayContext.source] : []),
  ].filter((source, index, all) => all.findIndex((item) => (item.id || item.nodeId) === (source.id || source.nodeId)) === index);
  const claims = [
    ...(hasVerifiedDate ? candidate.claims : []),
    ...(holidayContext ? [{ text: holidayContext.fact.value, evidenceSourceIds: [holidayContext.source.id] }] : []),
  ];
  const holidayLead = holidayContext?.fact?.value
    ? `Holiday pickup changes apply only after these named holidays: ${holidayContext.fact.value}`
    : "";
  const directAnswer = hasVerifiedDate && holidayLead
    ? holidayLead
    : hasVerifiedDate
      ? candidate.directAnswer
      : delayRequested
        ? `I couldn’t verify whether ${service} pickup was delayed for the requested period from the live pickup calendar.`
        : "I couldn’t find a current official pickup date.";
  const responseDetails = [];
  if (hasVerifiedDate && holidayLead) {
    responseDetails.push(candidate.directAnswer, (candidate.keyDetails || []).join("; "));
  } else if (hasVerifiedDate) {
    responseDetails.push(...(candidate.keyDetails || []));
  }
  return {
    ...buildAnswerContract({
      directAnswer,
      keyDetails: responseDetails.filter(Boolean),
      nextStep: actions[0]?.label || "",
      actions,
      sources,
      claims: [
        ...(holidayLead ? [{ text: holidayLead, evidenceSourceIds: [holidayContext.source.id] }] : []),
        ...claims,
      ],
      status: "source-unavailable",
      requestedDetails: requested,
      coveredDetails: hasVerifiedDate ? ["date"] : [],
      checkedAt: hasVerifiedDate ? candidate.checkedAt : undefined,
...
async function approvedRecurringWasteDayAnswer(question, plan = {}, options = {}) {
  if (!isRecurringWasteDayQuestion(question, options.now)) return null;
  const recurringPlan = {
    ...plan,
    scope: "community",
    intent: "services",
  ...
  const livePickupSummary = livePresentation
    ? [livePresentation.nextPickupLabel, ...livePresentation.nextPickups.map((detail) => `- ${detail}`)].join(String.fromCharCode(10))
    : "";
...
function coveredDetails(requested, sources, actions, question = "") {
  ...
  return requested.filter((detail) => {
    if (detail === "action") return actions.length > 0 || facts.some((fact) =>
      fact.type === "information" && /\b(?:ask|call|check|contact|go|open|select|register|sign in|log in|submit|visit|use)\b/i.test(fact.context || fact.value || "")
    );
    if (detail === "information") return facts.length > 0;
    if (detail === "reimbursement") return facts.some((fact) =>
      /\b(?:reimburs|refund|receipt|pay(?:ing)? back)\b/i.test(`${fact.context || fact.value || ""} ${fact.subjectKey || ""}`)
    );
    if (detail === "price") return facts.some((fact) => fact.type === "money"
      || /\b(?:free|no (?:additional )?(?:charge|cost)|no fee|complimentary)\b/i.test(fact.context || fact.value || ''));
    if (['eligibility', 'quantity'].includes(detail)) return facts.some(fact => reviewedFactCoversDetail(fact, detail));
    if (detail === "contact") return facts.some((fact) => ["phone", "email"].includes(fact.type)
      || /@|(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]\d{3}[-.\s]\d{4}/.test(fact.context || fact.value || ""));
    if (detail === "permission") return facts.some((fact) => fact.facet === "restriction");
    ...
    if (detail === "hours") return !isHolidayHoursRequest(question) && facts.some((fact) =>
      fact.type === "time" || (["schedule", "information"].includes(fact.type)
        && /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)/i.test(fact.context || fact.value || ""))
    );
    ...
    if (detail === "specification") return sources.some((source) => (source.authorityFacets || []).includes("specification")
      || (source.facts || []).some((fact) => fact.facet === "specification"
        || /\b(?:(?:one|two|three|four|five|six|seven|eight|nine|ten|twelve|forty[- ]eight)\s*(?:\(\s*\d+(?:\.\d+)?\s*\))?|\d+(?:\.\d+)?)\s*(?:business\s+)?(?:"|(?:seconds?|minutes?|hours?|days?|weeks?|months?|years?|feet|foot|inches|inch|ft\.?|in\.?)\b)/i.test(fact.context || fact.value || "")));
    if (detail === "methods") return facts.some((fact) => fact.facet === "method");
    return false;
  });
}

function contactSubjectMatchesEvidence(question, evidence = "") {
  const contactBoilerplate = new Set([...GENERIC_PROJECT_TERMS,
    "who", "whom", "service", "services", "support", "team", "person", "reach",
    "billing", "bill", "bills", "payment", "payments", "customer", "technical", "assistance", "setup",
  ]);
  const subjectTokens = tokens(question).filter(token => token.length >= 3 && !contactBoilerplate.has(token));
  return !subjectTokens.length || subjectTokens.some(token => String(evidence).toLowerCase().includes(token));
}
...
function bestContactCandidate(question, sources) {
  ...
  const candidates = sources.flatMap((source, sourceIndex) => (source.facts || [])
    ...
    // Approval proves the contact value, not that every contact on a broad
    // directory serves every topic. Match the selected fact or dedicated
    // page title; unrelated neighboring directory entries are not proof.
    // Shared tasks such as billing are not the service subject: internet
    // billing must not match water billing merely because both say billing.
    .filter((fact) => contactSubjectMatchesEvidence(question, `${source.title || ""} ${fact.context || ""}`))
    .map((fact) => ({
      source,
      fact,
...
function bestContactContext(question, sources) {
  ...
  const values = ["phone", "email"].map((type) => (best.source.facts || [])
    ...
    .filter((fact) => bestContactCandidate(question, [{ ...best.source, facts: [fact] }]))
    .map((fact) => ({
      fact,
...
function missingContactAnswer(organization, routingPlan, options = {}) {
  const approvedResources = options.index && options.question ? searchCommunityIndex(options.question, {
    index: options.index, intent: "services", interpretation: { requestedDetails: [] },
    includeActionOnlyProjections: true, now: options.now,
  }).sources.map(sourceForDisplay) : [];
  ...
  const relevantResources = [...approvedResources, ...contactCandidates].filter(source =>
    contactSubjectMatchesEvidence(options.question, `${source.title || ""} ${source.text || source.excerpt || ""}`))
    .sort((left, right) => Number(contactSubjectMatchesEvidence(options.question, right.title))
      - Number(contactSubjectMatchesEvidence(options.question, left.title)));
  const contactDirectory = configuredCommunityResource(options.index, options.profile, relevantResources, options.question);
  const boundary = buildAnswerContract({
...
function extractiveAnswer(question, result, options = {}) {
  ...
  if (recoveryNeedsProcessEvidence(question, sources)) return recoveryInstructionBoundary(question, result, options);
  ...
  if (result.requestedDetails.includes("contact") || options.routingPlan?.goal === "contact") {
    ...
    if (!contactCandidate) return missingContactAnswer("the requested service", options.routingPlan,
      { index: result.index, profile: options.communityProfile, sources, question });
  ...
  const stale = sources.some((source) => isFreshnessTrackedSource(source)
    && source.staleAfter && new Date(source.staleAfter).getTime() < new Date(options.now || Date.now()).getTime());
  return buildAnswerContract({
    directAnswer,
    keyDetails,
...
function approvedOperationalProjectionAnswer(question, result, options = {}) {
  let sources = (result.sources || []).filter((source) => source.canonicalScopedProjection
    && sourceMatchesRequestedFacility(question, source));
  if (/\b(?:alerts?|notifications?)\b/i.test(question)) {
    // “Alert” describes many unrelated systems. Require the resident's
    // actual subject (for example water usage) to match the reviewed source,
    // rather than letting that generic delivery word choose the answer.
    sources = sources.filter((source) => hasOriginalOperationalSubject(question, source));
  }
  if (isBinStorageLocationRequest(question)) {
    // Pickup-day return guidance does not answer where carts may be stored
    // between collections. Let the governing storage rule answer that scope.
    sources = sources.filter((source) => source.id !== "approved-trash-recurring-service");
  }
  // A report request is about its reviewed findings, not neighboring advice
  // that happens to share a service word. Retrieval already selected the
  // requested subjects and reporting periods within each approved projection.
  const reportSources = isReportedFindingsRequest(question) ? sources.filter(source =>
    /\b(?:reports?|findings|audit)\b/i.test(source.title || '')
    && (source.facts || []).some(fact => fact.subjectKey && fact.approvalClaim)) : [];
  const completeReportFindings = reportSources.length > 0;
  if (completeReportFindings) sources = reportSources;
  ...
  if (!sources.length || !hasDistinctiveCommunityEvidence(question, sources)
    || recoveryNeedsProcessEvidence(question, sources)) return null;
  if (result.requestedDetails?.length === 1 && result.requestedDetails[0] === "hours"
    && !isHolidayHoursRequest(question)) {
    const timingSource = sources.find((source) => source.sourceType === "services"
      && (source.facts || []).some((fact) => fact.approvalClaim
      && ["information", "schedule", "time"].includes(fact.type)
      && /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b/i.test(fact.context || fact.value || "")));
    const timingFact = (timingSource?.facts || []).find((fact) => fact.approvalClaim
      && ["information", "schedule", "time"].includes(fact.type)
      && /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b/i.test(fact.context || fact.value || ""));
    if (timingSource && timingFact) {
      const displayed = sourceForDisplay(timingSource);
      const text = String(timingFact.context || timingFact.value || "").trim();
      return buildAnswerContract({
        directAnswer: text,
        nextStep: timingSource.title,
        sources: [displayed],
        status: "verified",
        requestedDetails: ["hours"],
        coveredDetails: ["hours"],
        checkedAt: timingSource.checkedAt,
        answerMode: "community-approved-operational",
        claims: [{ text, evidenceSourceIds: [displayed.id], approvalClaimIds: [timingFact.approvalClaim] }],
      });
    }
  }
  const recurringWeekdayClosing = recurringWeekdayClosingAnswer(question, { ...result, sources });
  if (recurringWeekdayClosing) return recurringWeekdayClosing;
  ...
  const scopedAction = actions.find((action) =>
    sources.some((source) => (source.actions || []).some((candidate) => candidate.url === action.url))
  );
  // For an hours, price, equipment, or rules-of-use question, answer the
  // requested fact first and keep the booking link as the next useful move.
  // An action becomes the direct answer only when the resident asked to act.
  const preferredAction = options.preferredAction || (options.routingPlan?.goal
    && ACTION_GOALS.has(options.routingPlan.goal) ? scopedAction : null);
  const supplementalAction = preferredAction ? null : scopedAction;
  ...
  const parkingPriceRequest = isParkingPriceRequest(question);
  const requestFactScore = (fact) => {
    const evidence = `${fact.context || fact.value || ""} ${fact.scopeKey || ""} ${fact.subjectKey || ""}`;
    const requestTerms = tokens(question).filter((term) => term.length >= 3);
    let score = requestTerms.filter((term) => tokens(evidence).includes(term)).length;
    if (/\b(?:\d{1,2}[- ]?year[- ]old|child|kid|minor)\b/i.test(question)
      && /\b(?:children? under|child supervision|resident age 16)\b/i.test(evidence)) score += 10;
    if (/\b(?:friend|guest|out-of-neighborhood|non[- ]?resident)\b/i.test(question)
      && /\bnonresidents?\b/i.test(evidence)) score += 6;
    if (/\b(?:dog|cat|pet|animal)\b/i.test(question) && /\bpets?\b/i.test(evidence)) score += 10;
    if (/\b(?:park|parking)\b/i.test(question) && /\b(?:street )?parking\b/i.test(evidence)) score += 10;
    if (/\b(?:ball machine|glass)\b/i.test(question)
      && /\b(?:ball machines?|glass items?)\b/i.test(evidence)) score += 10;
    if (/\b(?:gear|equipment|paddles?|balls?|shoes?|footwear)\b/i.test(question)
      && /\b(?:bring their own paddles and balls|tennis or court shoes)\b/i.test(evidence)) score += 12;
    if (/\b(?:intersection|where (?:are|is)|located|location)\b/i.test(question)
      && /\b(?:Sterling Ranch Avenue|Middle Fork Street|courts are at)\b/i.test(evidence)) score += 12;
    if (/\b(?:sidewalk|driveway)\b/i.test(question)
      && /\bhomeowners? are responsible\b.{0,80}\b(?:sidewalk|driveway)\b/i.test(evidence)) score += 12;
    if (/\bsolar\b/i.test(question) && /\b(?:ground[- ]?mounted|roof[- ]?mounted)\b/i.test(question)
      && /\broof mounted\b/i.test(evidence)) score += 12;
    if (/\bsolar\b/i.test(question) && /\b(?:silver|black|color|colour)\b/i.test(question)
      && /\bblack\b/i.test(evidence)) score += 12;
    if (/\b(?:score|points?|rotate|waiting players?)\b/i.test(question)
      && /\b(?:11 points|paddle rack|players rotate)\b/i.test(evidence)) score += 10;
    if (/\b(?:bill|billed|arrears|past usage|manages?|administers?)\b/i.test(question)
      && /\b(?:bills?|billed|arrears|administers?|billing partner)\b/i.test(evidence)) score += 10;
    if (/\b(?:autopay|automatic withdrawal|bank account|ach)\b/i.test(question)
      && /\b(?:automatic withdrawal|ach)\b.{0,80}\bfree of charge\b/i.test(evidence)) score += 12;
    if (/\b(?:card|credit|debit)\b/i.test(question)
      && /\b(?:card|credit|debit)\b.{0,80}\b2\.95%/i.test(evidence)) score += 12;
    if (/\bSnow Angels?\b/i.test(question)
      && /\bclear sidewalks and driveways\b.{0,60}\bunable to shovel\b/i.test(evidence)) score += 12;
    if (/\b(?:lesson|tournament|priority|precedence|conflict)\b/i.test(question)
      && /\btournaments and lessons take precedence\b/i.test(evidence)) score += 12;
    if (/\bchase drain\b/i.test(question)
      && /\bpublic road\b.{0,120}\bDouglas County\b.{0,180}\bshared drive\b.{0,120}\bInspection Request Form\b/i.test(evidence)) score += 14;
    if (/\b(?:fixed monthly|base fee|usage charge)\b/i.test(question)
      && /\b(?:fixed and operating costs|residential usage charge|tiered rates|variable)\b/i.test(evidence)) score += 10;
    if (/\btownhomes?\b/i.test(question) && /\btownhomes?\b.{0,80}\bhoa\b/i.test(evidence)) score += 12;
    if (/\b(?:whose|who|which)\b.{0,50}\b(?:job|responsib)|\b(?:is|was)\b.{0,30}\b(?:the\s+)?(?:cab|hoa|county)(?:'s|s)?\s+job\b/i.test(question)
      && /\b(?:handles?|responsible for|removes?)\b/i.test(evidence)) score += 10;
    return score;
  };
  const factMatchesRequestedDetail = (fact, detail) => {
    const evidence = `${fact.context || fact.value || ""} ${fact.scopeKey || ""} ${fact.subjectKey || ""}`;
    if (detail === "price" && parkingPriceRequest && !/\b(?:street )?parking\b/i.test(evidence)) return false;
    return reviewedFactCoversDetail(fact, detail)
    || (detail === "price" && (fact.type === "money" || fact.facet === "fee"
      || /\b(?:free|no charge|no fee|complimentary)\b/i.test(fact.context || fact.value || "")))
    || (detail === "contact" && (fact.type === "phone" || fact.type === "email"
      || /@|(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]\d{3}[-.\s]\d{4}/.test(fact.context || fact.value || "")))
    || (detail === "hours" && (fact.type === "time" || fact.facet === "facility-hours"
      || (fact.type === "information" && /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b/i.test(fact.context || fact.value || ""))))
    || (detail === "permission" && fact.facet === "restriction")
    || (detail === "specification" && (fact.facet === "specification"
      || /\b(?:(?:one|two|three|four|five|six|seven|eight|nine|ten|twelve|forty[- ]eight)\s*(?:\(\s*\d+(?:\.\d+)?\s*\))?|\d+(?:\.\d+)?)\s*(?:business\s+)?(?:"|(?:seconds?|minutes?|hours?|days?|weeks?|months?|years?|feet|foot|inches|inch|ft\.?|in\.?)\b)/i.test(fact.context || fact.value || "")))
    || (detail === "reimbursement" && /\b(?:reimburs|receipt|refund)\b/i.test(`${fact.context || fact.value || ""} ${fact.subjectKey || ""}`));
  };
  const factMatchesRequestedSubject = (fact) => {
    if (!parkingPriceRequest) return true;
    const evidence = `${fact.context || fact.value || ""} ${fact.scopeKey || ""} ${fact.subjectKey || ""}`;
    const priceLike = fact.type === "money" || fact.facet === "fee"
      || /\b(?:free|no charge|no fee|complimentary)\b/i.test(evidence);
    return !priceLike || /\b(?:street )?parking\b/i.test(evidence);
  };
  const details = displayedSources.flatMap((source, sourceIndex) => (source.facts || [])
    .filter(factMatchesRequestedSubject).map((fact) => ({
    text: String(fact.context || "").trim(),
    factId: fact.id,
    sourceId: source.id,
    sourceIndex,
    reviewedSubjectMatch: source.reviewedSubjectMatch === true,
    requestedDetailMatch: result.requestedDetails.some((detail) => factMatchesRequestedDetail(fact, detail)),
    requestFactScore: requestFactScore(fact),
    subjectKey: fact.subjectKey || source.id,
    explicitlyScoped: Boolean(fact.subjectKey),
    approvalClaimIds: fact.approvalClaim ? [fact.approvalClaim] : [],
  }))).filter((detail) => detail.text && displayedIds.has(detail.sourceId));
  const requestedMethodLeadFact = options._fixedRoutingPlan && /\b(?:credit|debit|card)\b/i.test(question)
    ? displayedSources.flatMap((source) => source.facts || []).find((fact) =>
      /\b(?:credit|debit|card)\b.{0,100}\b2\.95%/i.test(fact.context || fact.value || ""))
    : options._fixedRoutingPlan && /\bcaregiver pass\b/i.test(question) && /\b(?:check|card|cash|payment option)\b/i.test(question)
      ? displayedSources.flatMap((source) => source.facts || []).find((fact) =>
        /\bacceptable payment options include credit\/debit card, check, and cash\b/i.test(fact.context || fact.value || ""))
      : null;
  const requestedLeadFact = options.preferredLeadFact || requestedMethodLeadFact || (instructionalRequestShape(question).check
    ? displayedSources.flatMap(source => source.facts || []).find(fact => fact.type === 'information'
      && /^(?:go|open|select|click|use)\b/i.test(String(fact.context || '').trim())) : null);
  const detailCandidates = (requestedLeadFact && options.excludePreferredLeadFromDetails
    ...
    : details).sort((left, right) => options._fixedRoutingPlan
      ? Number(right.requestedDetailMatch) - Number(left.requestedDetailMatch)
        || right.requestFactScore - left.requestFactScore
        || Number(right.reviewedSubjectMatch) - Number(left.reviewedSubjectMatch)
        || left.sourceIndex - right.sourceIndex
        || Number(right.explicitlyScoped) - Number(left.explicitlyScoped)
      : Number(right.explicitlyScoped) - Number(left.explicitlyScoped));
  let groupedDetails = [];
  for (const detail of detailCandidates) {
    const current = groupedDetails[groupedDetails.length - 1];
    if (current && current.sourceId === detail.sourceId && current.subjectKey === detail.subjectKey && `${current.text} ${detail.text}`.length <= 400) {
      current.text = `${current.text} ${detail.text}`;
      current.approvalClaimIds = [...new Set([...current.approvalClaimIds, ...detail.approvalClaimIds])];
    } else groupedDetails.push({ ...detail });
  }
  // Represent each selected reviewed subject before taking a second finding
  // from one of them; otherwise a multi-entity answer can silently omit one.
  const representedSubjects = new Set();
  const firstDetails = [], additionalDetails = [];
  for (const detail of groupedDetails) {
    (representedSubjects.has(detail.subjectKey) ? additionalDetails : firstDetails).push(detail);
    representedSubjects.add(detail.subjectKey);
  }
  groupedDetails = [...firstDetails, ...additionalDetails];
  if (options._fixedRoutingPlan
    && /\bautomatically\b[\s\S]{0,140}\b(?:myself|ourselves|yourself|separately|personal account)\b/i.test(question)) {
    const automatic = detailCandidates.find((detail) => /\bautomatically\b.{0,80}\bset up\b/i.test(detail.text));
    const personal = detailCandidates.find((detail) => /\bresponsible for setting up personal accounts\b/i.test(detail.text));
    if (automatic && personal) groupedDetails = [automatic, personal];
  }
  // All selected findings are coverage obligations. Put them together in the
  // direct answer so the generic three supporting-detail presentation limit
  // cannot silently remove a second violation, response, or another subject.
  const reportFindings = completeReportFindings ? detailCandidates : [];
  const reportLead = reportFindings.length ? {
    text: reportFindings.map(detail => detail.text).join('\n\n'),
    evidenceSourceIds: [...new Set(reportFindings.map(detail => detail.sourceId))],
    approvalClaimIds: [...new Set(reportFindings.flatMap(detail => detail.approvalClaimIds))],
  } : null;
  groupedDetails = reportLead ? [] : groupedDetails.slice(0, 3);
  ...
  const directDetail = reportLead || processLead || (preferredAction ? null : groupedDetails.shift());
  if (!preferredAction && !directDetail?.text) return null;
  const directAnswer = reportLead ? reportLead.text : processLead
    ? processLead.text
    : preferredAction
  ...
  if (result.requestedDetails.length && !covered.length) return null;
  // Binding permission and dated/live facets have dedicated answer contracts.
  // An action-only partial must not intercept those unresolved authority gaps.
  if (result.requestedDetails.some(detail => ['permission', 'date', 'hours', 'status'].includes(detail))
    && covered.length !== result.requestedDetails.length) return null;
  const directEvidenceSourceIds = reportLead ? reportLead.evidenceSourceIds : processLead
  ...
  const answer = buildAnswerContract({
    ...
    nextStep: preferredAction?.label || supplementalAction?.label || displayedSources[0].title,
    actions: preferredAction ? [preferredAction] : supplementalAction ? [supplementalAction] : [],
    sources: displayedSources,
    status: covered.length === result.requestedDetails.length ? "verified" : "verified-incomplete",
    requestedDetails: result.requestedDetails,
    coveredDetails: covered,
    checkedAt: displayedSources[0].checkedAt,
    answerMode: reportLead ? "community-approved-report-findings" : "community-approved-operational",
    claims: [
      ...(reportLead ? reportFindings.map(detail => ({ text: detail.text, evidenceSourceIds: [detail.sourceId], approvalClaimIds: detail.approvalClaimIds }))
  ...
  }, answer.claims || [], displayedSources, covered)
    || !preservesRequestedContactValues({
      directAnswer,
      keyDetails: groupedDetails.map((detail) => detail.text),
      nextStep: answer.nextStep,
    }, displayedSources, covered)) return null;
  return {
    ...answer,
    answerMode: answer.answerStatus === 'verified-incomplete' ? 'community-approved-operational-partial' : answer.answerMode,
...
function claimTextCoversDetail(text = "", detail = "") {
  const checks = {
    action: /\b(?:apply|ask|book|call|check|click|contact|continue|log in|open|pay|register|renew|reserve|select|sign in|submit|use|visit)\b/i,
    information: /\S/,
    ...
    eligibility: /\b(?:eligible|eligibility|qualif(?:y|ies|ied)|restricted to|available (?:only )?to|open (?:only )?to|(?:non)?residents? can (?:access|book|reserve|use|visit))\b/i,
    reimbursement: /\b(?:reimburs|refund|receipt|pay(?:ing)? back)\b/i,
    ...
    specification: /\b(?:color|dimension|distance|duration|finish|height|material|setback|size)\b|\b(?:(?:one|two|three|four|five|six|seven|eight|nine|ten|twelve|forty[- ]eight)\s*(?:\(\s*\d+(?:\.\d+)?\s*\))?|\d+(?:\.\d+)?)\s*(?:business\s+)?(?:"|(?:seconds?|minutes?|hours?|days?|weeks?|months?|years?|feet|foot|inches|inch|ft\.?|in\.?)\b)/i,
...
function approvedDraftCoversEveryFacet(question, draft, claims, sources, requested = []) {
  ...
  return requestedDetails.every((detail) => claims.some((claim) => {
    ...
    if (detail === 'methods') return cited.some(source => (source.facts || []).some(fact => fact.facet === 'method'
      && claim.text.includes(fact.context || fact.value)));
    const reviewedSpecification = detail === 'specification' && cited.some(source => (source.facts || []).some(fact =>
      fact.facet === 'specification' && fact.approvalClaim
      && String(claim.text || '').includes(String(fact.context || fact.value || '').trim())));
    if (!reviewedSpecification && !claimTextCoversDetail(claim.text, detail)) return false;
...
async function composeApprovedOperationalProjection(question, result, options = {}) {
  ...
  if (["community-approved-operational-overview", "community-approved-report-findings"].includes(fallback.answerMode)) return fallback;
...
async function approvedOperationalActionAnswer(question, routingPlan, options = {}) {
  ...
  const goalSources = approvedSources.filter((source) => sourceSupportsGoal(source, routingPlan.goal)
    && sourceMatchesRequestedFacility(question, source));
  let actionSource = preferredSourceForGoal(goalSources, routingPlan.goal);
...
function conditionalInstructionMatchesQuestion(question, fact = {}) {
  const generic = new Set(["approval", "application", "architectural", "contact", "controlling", "deliver", "design", "drop", "drc", "email", "fee", "fees", "form", "forms", "how", "needed", "need", "pay", "required", "resident", "residents", "review", "rule", "send", "submit", "where"]);
...
function isAccountAccessInstruction(source = {}, fact = {}) {
  const evidence = `${fact.context || ""} ${fact.value || ""}`;
  // A usage calculation is not an instruction to access an online account.
  // The selected claim itself must describe that interface/process; a page
  // title or nearby monitoring vocabulary cannot supply the missing proof.
  return fact.facet !== "method"
    && /\b(?:account|dashboard|portal|register|registration|sign in|log ?in)\b/i.test(evidence)
...
function approvedOperationalSubmissionAnswer(question, routingPlan, options = {}) {
  ...
  const coveredSubmissionDetails = ["action"];
  if (requested.includes("contact") && facts.some((fact) => fact.type === "email"
    || /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i.test(fact.context || fact.value || ""))) {
    coveredSubmissionDetails.push("contact");
  }
  const displayAction = {
    id: action.id,
    label: action.label,
  ...
  const answer = buildAnswerContract({
    ...
    status: "verified",
    requestedDetails: requested,
    coveredDetails: coveredSubmissionDetails,
    checkedAt: source.checkedAt,
...
async function approvedOperationalContactAnswer(question, routingPlan, options = {}) {
  ...
  if (!fact?.value || !fact.context) return missingContactAnswer("the requested service", routingPlan, {
    index: selected.result.index || options.index, profile: options.communityProfile, now: options.now,
    sources: candidates.map(candidate => candidate.source), question,
  });
...
function namedHolidayDate(holidayName = "", now = Date.now(), referenceText = "") {
  ...
  const requestsPast = /\b(?:was|were|did|last)\b/i.test(String(referenceText));
  if (requestsPast) return current && current <= referenceDate ? current : dateForYear(year - 1);
...
function regularScheduleForDate(text = "", date = "") {
  ...
  const labels = weekday === 6 ? ["Saturday", "Weekends"]
    : weekday === 0 ? ["Sunday", "Weekends"]
    : ["Monday-Friday", "Monday to Friday", "Weekdays", dayNames[weekday]];
...
function recurringWeekdayClosingAnswer(question, result) {
  if (!/\b(?:normally|regular(?:ly)?|usual(?:ly)?|typically)\b/i.test(question)
    || !/\bclos(?:e|es|ing)\b/i.test(question)
    || /\b(?:this|next|last)\s+(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)\b/i.test(question)) return null;
  const dayNames = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
  const weekday = dayNames.findIndex((day) => new RegExp(`\\b${day}\\b`, "i").test(question));
  if (weekday < 0) return null;
  const representativeDates = ["2026-09-20", "2026-09-21", "2026-09-22", "2026-09-23", "2026-09-24", "2026-09-25", "2026-09-26"];
  for (const rawSource of result.sources || []) {
    const approvedSchedule = (rawSource.facts || []).some((fact) => fact.reviewStatus === "approved"
      && fact.sourceVersion === rawSource.contentHash
      && fact.type === "schedule");
    if (rawSource.canonicalScopedProjection !== true && !approvedSchedule) continue;
    const source = sourceForDisplay(rawSource);
    const text = String(rawSource.text || rawSource.excerpt || "").replace(/\s+/g, " ").trim();
    const schedule = regularScheduleForDate(text, representativeDates[weekday]);
    if (!schedule) continue;
    const times = [...schedule.detail.matchAll(/\b(\d{1,2})(?::(\d{2}))?\s*(a\.?m\.?|p\.?m\.?)\b/gi)].map((match) => {
      let hour = Number(match[1]) % 12;
      if (/^p/i.test(match[3])) hour += 12;
      return { minutes: hour * 60 + Number(match[2] || 0), label: match[0] };
    });
    if (!times.length) continue;
    const closing = times.sort((left, right) => right.minutes - left.minutes)[0].label
      .replace(/\ba\.?m\.?\b/i, "a.m.").replace(/\bp\.?m\.?\b/i, "p.m.");
    const season = text.split(/(?<=[.!?])\s+/).find((sentence) => /\bopen\b.{0,100}\bthrough\b/i.test(sentence)) || "";
    const subject = /\bpool\b/i.test(question) ? "The pool’s" : "The facility’s";
    const directAnswer = `${subject} published ${dayNames[weekday]} schedule runs until ${closing}${/[.!?]$/.test(closing) ? "" : "."}${season ? ` ${season}` : ""}`;
    return buildAnswerContract({
      directAnswer,
      sources: [source],
      status: "verified",
      ...
      coveredDetails: ["hours"],
      checkedAt: source.checkedAt,
      answerMode: "community-recurring-weekday-closing",
      claims: [{ text: directAnswer, evidenceSourceIds: [source.id] }],
    });
  }
  return null;
}
...
function holidayFacilityHoursAnswer(question, result, options = {}) {
  ...
  const holidayDate = options.routingPlan?.dateRange?.start || namedHolidayDate(holidayName, options.now, question);
  ...
  for (const source of result.sources || []) {
    ...
    if (holidayContext && requested.includes("date")) coveredDetails.push("date");
    if (season.declared && season.covers && requested.includes("status")) coveredDetails.push("status");
    return buildAnswerContract({
      directAnswer,
      keyDetails,
      nextStep: `Open ${source.title}`,
...
function datedFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    if (season.declared && !season.covers && options.statusConfirmsOpen !== true) {
    ...
    if (holidayRequest) {
      ...
      const coveredDetails = [];
      if (holidayContext && plan.requestedDetails.includes("date")) coveredDetails.push("date");
      if (season.declared && season.covers && plan.requestedDetails.includes("status")) coveredDetails.push("status");
      return buildAnswerContract({
        directAnswer,
...
function localIsoDay(value = new Date(), timeZone = "UTC") {
...
function isPoolSeasonReopeningRequest(question = "", routingPlan = {}) {
  ...
  if (/\b(?:when|what date)\b.{0,60}\bpool\b.{0,30}\bopen\b/i.test(text)
    && !/\b(?:what time|hours?|daily|each day|today|tomorrow|normally|regular|usual|weekly)\b/i.test(text)) return true;
  return false;
}
...
function eventsAnswer(result, requested = [], routingPlan = {}) {
  ...
  const namedFilterLabel = [...new Set((diagnostics.appliedFilters || []).map((filter) => filter.value).filter(Boolean))].join(" and ");
  ...
  if (!details.length) {
    ...
    if (diagnostics.appliedFilters?.length && result.alternatives?.length) {
      ...
      const boundary = `I couldn’t find an event matching “${filterLabel}” on the official calendar for ${result.range.label}.`;
      return { ...buildAnswerContract({
        ...
        requestedDetails,
        coveredDetails: [],
        claims: calendarEvidence ? [{ text: boundary, evidenceSourceIds: [calendarEvidence.evidenceId] }] : [],
        checkedAt: result.checkedAt,
        answerMode: "community-live-events",
      }), _connectorDiagnostics: diagnostics };
    }
  ...
  return { ...buildAnswerContract({
    directAnswer: namedFilterLabel && details[0]
      ? details[0]
      : `The official calendar has ${details.length} ${details.length === 1 ? "event" : "events"} ${result.range.label}.`,
    // A named-event answer already promotes the first matching event into the
    // lead sentence. Keep only additional matches in the detail list so the
    // resident does not see the same event twice.
    keyDetails: namedFilterLabel ? details.slice(1) : details,
    ...
    claims: calendarEvidence ? result.events.map((event) => eventDateClaims.get(`event-${event.id}`)).filter(Boolean).map((claim) => ({
      text: claim.text,
      kind: "event-date",
      evidenceSourceIds: [`${claim.controllingEvidenceId}:${claim.id}`],
      verified: true,
    })) : [],
    status: healthy && coveredDetails.length === requestedDetails.length ? "verified" : "verified-incomplete",
    requestedDetails,
...
function fallbackStructuredInterpretation(question, intent, options = {}) {
      ...
      : details.includes("permission") ? "permission"
        : details.includes("price") ? "cost"
          : details.includes("contact") ? "contact"
            ...
            : "information";
  const fallback = normalizedRoutingPlan({
    intent,
    goal,
    goals: [goal],
    subject: question,
  ...
  if (!fallback?.dateRange?.start && details.includes("hours") && isHolidayHoursRequest(question)) {
    const holidayLabel = holidayNameFromText(question);
    const holidayDate = namedHolidayDate(holidayLabel, options.now, question);
    if (holidayLabel && holidayDate) {
      fallback.dateRange = {
        kind: "explicit-date",
        start: holidayDate,
        end: holidayDate,
  ...
  if (fallback?.intent === "events" && fallback.goal === "schedule"
    && fallback.filters?.category && !highConfidenceDateRange(question, options.now)
    && fallback.dateRange?.start) {
    return {
      ...fallback,
      dateRange: {
        kind: "next-31-days",
        start: fallback.dateRange.start,
        end: addDays(fallback.dateRange.start, 30),
...
function hasAuthoritativeSpecification(answer, question = "") {
  ...
  const rendered = /#[0-9]{2,}\b|\b(?:\d+(?:[ -]\d+\/\d+)?|one|two|three|four|five|six|seven|eight|nine|ten|twelve|forty[- ]eight)\s*(?:\(\s*\d+(?:\.\d+)?\s*\))?\s*(?:business\s+)?(?:"|(?:seconds?|minutes?|hours?|days?|weeks?|months?|years?|feet|foot|inches|inch|ft\.?|in\.?)\b)|\b(?:color|colour|paint|stain|finish|material|height|size|dimension|setback|distance|duration)\b.{0,180}\b(?:approved|required|requires|must|match|prohibited|not allowed|no more than|at least|feet|foot|inches|#[0-9]{2,})\b/i.test(text);
...
function missingDetailsFromQuality(requested, issues = [], answer = {}, question = "") {
  return requested.filter((detail) => {
    ...
    if (['quantity', 'eligibility'].includes(detail)) {
      if (reviewedDetailIsRendered(detail, answer.answer, answer.sources, answer.claims)) return false;
      // Governing answers have their own citation/grounding contract. They do
      // not inherit community approval IDs, but must display the actual
      // count or eligibility statement present in that controlling evidence.
      const normalize = value => String(value || '').toLowerCase().replace(/[^a-z0-9\s-]/g, ' ').replace(/\s+/g, ' ').trim();
      const rendered = normalize(answer.answer);
      const evidence = (answer.sources || []).filter(governingRuleSource).map(source => normalize(source.text || source.excerpt)).join(' ');
      if (answer.confidence?.canAnswer !== true || !evidence) return true;
      const pattern = detail === 'quantity'
        ? /\b(?:\d+|zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|unlimited)\s+(?!(?:january|february|march|april|may|june|july|august|september|october|november|december|am|pm)\b)[a-z0-9-]+(?:\s+[a-z-]+){0,3}/g
        : /\b(?:eligible|qualify|qualifies|restricted to|available only to|open only to)\b(?:\s+[a-z0-9-]+){1,8}/g;
      return !(rendered.match(pattern) || []).some(phrase => evidence.includes(phrase));
    }
...
async function answerCommunityQuestionCore(query, options = {}) {
  const rawQuestion = normalizeInput(query);
  if (hasPromptInjectionSignals(rawQuestion)) {
  ...
  const question = normalizeFoodTruckQuestion(rawQuestion);
  ...
  let routingPlan = options._fixedRoutingPlan
    ? (normalizedRoutingPlan(options._fixedRoutingPlan, question, { now: options.now }) || {
      ...options._fixedRoutingPlan,
      dateRange: options._fixedRoutingPlan.dateRange || highConfidenceDateRange(question, options.now),
      filters: options._fixedRoutingPlan.filters || {},
      searchQueries: options._fixedRoutingPlan.searchQueries?.length ? options._fixedRoutingPlan.searchQueries : [question],
    })
    : null;
  if (routingPlan && options._fixedRoutingPlan) {
    // Need-first routing has already classified the resident's exact facet.
    // A retrieval-friendly rewritten query may omit words such as “receipt”
    // or “how many days”; do not let normalization erase that obligation.
    routingPlan = {
      ...routingPlan,
      intent: options._fixedRoutingPlan.intent || routingPlan.intent,
      goal: options._fixedRoutingPlan.goal || routingPlan.goal,
      goals: options._fixedRoutingPlan.goals || routingPlan.goals,
      subject: options._fixedRoutingPlan.subject || routingPlan.subject,
      requestedDetails: options._fixedRoutingPlan.requestedDetails || routingPlan.requestedDetails,
    };
  }
  if (routingPlan && isUtilityInfrastructureFeeRequest(question)) {
    // A broad planner rewrite such as “facility fees” must not erase the
    // resident's utility-infrastructure subject and redirect a tap-fee
    // question to clubhouse or park rentals.
    routingPlan = {
      ...routingPlan,
      intent: "services",
      subject: question,
  ...
  let interpretationOutcome = routingPlan ? "need-contract" : "legacy";
  let shadowPlan = null;
  ...
  if (!routingPlan && shouldInterpret && options.planCommunitySearch !== false) {
  ...
  if (!routingPlan && isOfficialInformationPageRequest(question)) {
    // Preserve the same navigation semantics when planning is disabled or
    // unavailable. In particular, imperative “Open” does not request hours.
    routingPlan = normalizedRoutingPlan({
      intent, goal: "information", goals: ["information"], subject: question,
      requestedDetails: ["action"], searchQueries: [question], scope: "community",
    }, question, { now: options.now });
    interpretationOutcome = "fallback";
  }
  const stateParksPassQuestion = isStateParksPassQuestion(question)
    && !routingPlan?.requestedDetails?.includes("reimbursement");
  ...
  if (routingPlan && supportsKnownLiveCommunityRequest(question, routingPlan)
    && (!options._fixedRoutingPlan || routingPlan.requestedDetails.some((detail) => ["date", "status"].includes(detail)))) {
  ...
  if (routingPlan?.scope === 'unrelated' && !originalQuestionNeedsRule(question)) {
    // The scope label is advisory. Repair it only with current approved
    // claims about the original subject, never raw pages or expanded queries.
    const evidence = searchCommunityIndex(question, {
      index: options.index, indexPath: options.indexPath, communityId: options.communityId,
      intent: routingPlan.intent, interpretation: { ...routingPlan, scope: 'community' },
      now: options.now, limit: 10,
    });
    if (evidence.sources.some(source => source.canonicalFactualProjection
      && hasOriginalOperationalSubject(question, source, { requireAll: true })
      && (source.facts || []).some(fact => fact.approvalClaim))) {
      routingPlan = { ...routingPlan, scope: 'community', needsClarification: false, clarificationQuestion: '' };
    }
  }
  if (
  ...
  if (sourceAnswerPlan?.intent === "services"
    && sourceAnswerPlan.requestedDetails?.length === 1
    && sourceAnswerPlan.requestedDetails[0] === "hours") {
    const serviceTimingSearch = searchCommunityIndex(question, {
      index: options.index,
      indexPath: options.indexPath,
      communityId: options.communityId,
      intent: "services",
      interpretation: sourceAnswerPlan,
      limit: 5,
      now: options.now,
    });
    const serviceTimingAnswer = approvedOperationalProjectionAnswer(question, serviceTimingSearch, {
      ...options,
      routingPlan: sourceAnswerPlan,
    });
    if (serviceTimingAnswer) return { ...serviceTimingAnswer, communityIntent: "services" };
  }
  ...
  if (isOfficialInformationPageRequest(question, routingPlan)) {
    ...
    const internetSetupRequest = isInternetServiceSetupRequest(question);
    const navigationTerms = new Set(["activate", "activation", "closing", "connect", "connected", "could", "details", "find", "info", "information", "look", "move", "moved", "moving", "official", "online", "open", "page", "please", "resource", "service", "setup", "show", "site", "start", "website", "where", "would"]);
    // Strip navigation words before synonym expansion: “open the page” must
    // not add status/hours, and “page” must not make an unrelated page relevant.
    const navigationIndex = options.index || loadCommunityIndex(options.indexPath);
    const titles = (navigationIndex?.sources || []).filter(source => source.communityId === (options.communityId || navigationIndex.communityId))
      .map(source => source.title);
    const normalizedNavigationQuestion = internetSetupRequest
      ? question.replace(/\bwi[-\s]?fi\b/gi, "internet")
      : question;
    const subjectText = correctNavigationSubject(normalizedNavigationQuestion.split(/\s+/)
      .filter(word => !navigationTerms.has(word.toLowerCase().replace(/[^a-z]/g, ""))).join(" "), titles);
    const topicTerms = tokens(subjectText).filter(term => term.length >= 4 && !navigationTerms.has(term));
    const informationQueries = [topicTerms.join(" "), ...(routingPlan?.searchQueries || [])].filter(Boolean).slice(0, 3);
    const informationSearch = searchCommunityIndexWithQueries(question, informationQueries, {
      ...
      interpretation: routingPlan,
      // A reviewed destination can satisfy navigation without permission to
      // quote factual content. This option remains off for factual retrieval.
      includeActionOnlyProjections: true,
      limit: 10,
      now: options.now,
    });
    const resourceTerms = [...new Set([...topicTerms, ...tokens((routingPlan?.subject || "").split(/\s+/)
      .filter(word => !navigationTerms.has(word.toLowerCase().replace(/[^a-z]/g, ""))).join(" "))])]
      .filter(term => term.length >= 4 && !navigationTerms.has(term));
    const resource = informationSearch.sources.filter((source) => source.sourceUrl
      && source.lifecycle === "current"
      && source.canonicalScopedProjection === true
      && hasDistinctiveCommunityEvidence(resourceTerms.join(" "), [source], { includeApprovedActions: true }))
      .sort((left, right) => {
        // “Utility” is a broad resident category, so it cannot make a
        // specialized UtilityHawk tool outrank the higher-scoring water and
        // monthly-billing hubs. More specific resident terms still control.
        const titleScore = (source) => topicTerms.filter((term) => !["utility", "utilities"].includes(term)
          && String(source.title || "").toLowerCase().includes(term)).length;
        const broadUtilityRequest = topicTerms.length > 0
          && topicTerms.every((term) => ["utility", "utilities"].includes(term));
        const utilityHubScore = (source) => {
          if (!broadUtilityRequest) return 0;
          const title = String(source.title || "").toLowerCase();
          if (/\b(?:water billing|monthly fee billing|utilities?)\b/.test(title) && !/\b(?:monitoring|utilityhawk)\b/.test(title)) return 1;
          if (/\b(?:monitoring|utilityhawk)\b/.test(title)) return -1;
          return 0;
        };
        return titleScore(right) - titleScore(left)
          || utilityHubScore(right) - utilityHubScore(left)
    ...
    if (resource) {
      ...
      const approvedSetupAction = internetSetupRequest
        ? (resource.actions || []).find((candidate) => candidate.reviewStatus === "approved"
          && /\b(?:activat(?:e|ion)|set\s*up|setup|start)\b/i.test(`${candidate.label || ""} ${candidate.context || ""}`))
        : null;
      const action = approvedSetupAction
        ? { ...approvedSetupAction, label: approvedSetupAction.label, url: approvedSetupAction.url, actionType: approvedSetupAction.actionType || "information" }
        : { label: `Open ${resource.title}`, url: resource.sourceUrl, actionType: "information" };
      return {
        ...buildAnswerContract({
          directAnswer,
    ...
    const informationWithheld = (informationSearch.withheldSources || []).find((source) => hasSafeWithheldHandoffEvidence(resourceTerms.join(" "), [source], routingPlan));
    if (informationWithheld) {
      return withheldSourceAnswer(informationWithheld, informationIntent, ["action"], routingPlan, question);
    }
    // Never fall through to broad extractive retrieval after recognizing a
    // navigation request: generic page words cannot establish topic relevance.
    return buildAnswerContract({
      directAnswer: "I couldn’t find a current official answer for that.",
      status: "could-not-verify", requestedDetails: ["action"], coveredDetails: [],
      answerMode: "community-information-unavailable", sources: [], actions: [], claims: [],
    });
  }
  ...
  // such as "How do I book it?" do not trigger guessed retrieval. A broad
  // request shape ("how much", "can I") is not proof of a resolved subject:
  // use the same dependency decision as conversation resolution.
  ...
  if (asksForAction && (!routingPlan || interpretationOutcome === "fallback")) {
    ...
    let actionSource = fallbackResult.sources.find((source) => source.connectorType === "official-action" || source.canonicalScopedProjection);
    const fallbackGoal = [...ACTION_GOALS].find((goal) => actionForGoal(actionSource, goal));
    if (fallbackGoal) actionSource = preferredSourceForGoal(fallbackResult.sources.filter(source =>
      sourceMatchesRequestedFacility(question, source)), fallbackGoal) || actionSource;
    ...
    if (Number(fallbackResult.sources[0]?.score || 0) >= 36
      && actionSource
      && fallbackAction
      && !recoveryNeedsProcessEvidence(question, fallbackResult.sources)
  ...
  if (routingPlan && ACTION_GOALS.has(routingPlan.goal)) {
    ...
    const actionSource = preferredSourceForGoal(goalSources.filter(source =>
      sourceMatchesRequestedFacility(question, source)), routingPlan.goal);
    ...
    if (Number(planned.sources[0]?.score || 0) >= 24 && hasDistinctiveCommunityEvidence(question, planned.sources)
      && !recoveryNeedsProcessEvidence(question, planned.sources)) {
  ...
  if (foodTruckRequested && foodTruckRequestDecision.eligible && options.getFoodTruckAnswer) {
    try {
      ...
      const unavailable = buildAnswerContract({
        ...
        nextStep: actions[0]?.label || "",
        actions,
        // A configured handoff proves where to look, not what is on today's
        // calendar. Keep this navigation provenance separate from live claims.
        sources: actions.length ? [{
          id: `${options.communityProfile.communityId}:${foodTruckConnector.id}:official-handoff`,
          communityId: options.communityProfile.communityId,
          title: foodTruckConnector.adapter.labels.calendarTitle,
          sourceUrl: calendarUrl, text: actions[0].label, excerpt: actions[0].label,
          connectorType: "official-action", sourceType: "official-action",
          authorityFacets: ["action"], actions, isOfficialResource: true,
        }] : [],
        status: "source-unavailable",
  ...
  if (wasteScheduleRequested && isRecurringWasteDayQuestion(question, options.now)) {
  ...
  if (wasteScheduleRequested && wasteRequestDecision.eligible && options.getWasteSchedule) {
    try {
      ...
      // The live connector proves pickup dates only. A route that happens to
      // classify “Does a holiday change this week?” as a schedule question
      // must not silently turn that date into an answer about delay status.
      // Keep the proven date and combine it with the exact owner-approved
      // holiday policy while clearly disclosing the remaining live-status gap.
      if (isWastePickupStatusRequest(question, shortcutPlan)
        && !connectorResult?.evidence?.coverage?.covered?.includes("status")) {
        return unavailableWasteScheduleAnswer(question, options.communityProfile, shortcutPlan, retainsProvenDate ? candidate : null, options);
      }
      if (decision.eligible) return candidate;
      const keepsDateForMissingFacet = retainsProvenDate
        && (decision.reasons.includes("requested-status-missing") || decision.reasons.includes("goal-not-covered"));
      return unavailableWasteScheduleAnswer(question, options.communityProfile, shortcutPlan, keepsDateForMissingFacet ? candidate : null, options);
    } catch {
      ...
      return unavailableWasteScheduleAnswer(question, options.communityProfile, shortcutPlan, null, options);
    }
  ...
  const approvedCommunityOverride = classifiedInput.classification === INPUT_CLASSIFICATIONS.UNRELATED
    && hasApprovedCommunityEvidenceMatch(question, routingPlan || shortcutPlan, options);
  if (
  ...
  if (activeIntent === "status" && poolRequestDecision.eligible && options.getPoolStatus) {
    try {
      const connectorResult = await options.getPoolStatus({ question, routingPlan: structuredActive ? routingPlan : shortcutPlan, profile: options.communityProfile });
      const poolPlan = routingPlan || shortcutPlan;
      const requestedPoolDetails = poolPlan?.requestedDetails || ["status"];
      let candidate = poolStatusAnswer(connectorResult, requestedPoolDetails, options);
      if (!candidate) throw new Error("Live pool-status evidence was unavailable or degraded.");
      if (requestedPoolDetails.includes("hours")) {
        const currentDay = localIsoDay(options.now || Date.now(), options.communityProfile?.timezone || "UTC");
        const hoursPlan = {
          ...poolPlan,
          intent: "facilities",
          goal: "hours",
          goals: ["hours"],
          requestedDetails: ["hours"],
          dateRange: poolPlan?.dateRange?.start ? poolPlan.dateRange : highConfidenceDateRange(question, options.now) || {
            kind: "explicit-date", start: currentDay, end: currentDay, label: "today",
          },
        };
        const hoursSearch = searchCommunityIndex(question, {
          index: options.index,
          indexPath: options.indexPath,
          communityId: options.communityId,
          intent: "facilities",
          interpretation: hoursPlan,
          allowPartialRequestedDetails: true,
          includeActionOnlyProjections: true,
          now: options.now,
        });
        const hourSources = hoursSearch.sources.filter((source) => source.sourceType === "facilities"
          && source.canonicalScopedProjection === true
          && sourceMatchesRequestedFacility(question, source));
        const hoursAnswer = datedFacilityHoursAnswer(question, { sources: hourSources }, {
          ...options,
          routingPlan: hoursPlan,
          statusConfirmsOpen: /\bopen\b/i.test(String(connectorResult.headline || connectorResult.summary || ""))
            && !/\bclosed\b/i.test(String(connectorResult.headline || connectorResult.summary || "")),
        });
        if (hoursAnswer?.completion?.resolvedDetails?.includes("hours")) {
          const sources = [...candidate.sources, ...hoursAnswer.sources]
            .filter((source, index, all) => all.findIndex((item) => (item.id || item.sourceUrl) === (source.id || source.sourceUrl)) === index);
          const actions = [...candidate.actions, ...hoursAnswer.actions]
            .filter((action, index, all) => all.findIndex((item) => item.url === action.url) === index);
          candidate = buildAnswerContract({
            directAnswer: candidate.directAnswer,
            keyDetails: [hoursAnswer.directAnswer],
            nextStep: candidate.nextStep || hoursAnswer.nextStep,
            actions,
            sources,
            status: "verified",
            requestedDetails: requestedPoolDetails,
            coveredDetails: ["status", "hours"],
            checkedAt: candidate.checkedAt || connectorResult.checkedAt,
            answerMode: "community-live-status-and-hours",
            claims: [...candidate.claims, ...hoursAnswer.claims],
          });
        }
      }
  ...
  if (activeIntent === "events" && eventsRequestDecision.eligible && options.getCommunityEvents) {
    try {
      const connectorResult = await options.getCommunityEvents(shortcutPlan);
      ...
      const verifiedFilterBoundary = !connectorResult.events?.length
        && connectorResult.diagnostics?.parserHealthy !== false
        && connectorResult.diagnostics?.sourceOutcome === "ok"
        && connectorResult.diagnostics?.appliedFilters?.length > 0
        && ["verified-incomplete", "source-unavailable"].includes(candidate.answerStatus)
        && candidate.claims?.some((claim) => (claim.evidenceSourceIds || []).length > 0);
      if ((decision.eligible || verifiedFilterBoundary) && actionAuthorityMatches) return candidate;
  ...
  const operationalSearch = searchCommunityIndex(question, {
    ...
    interpretation: routingPlan,
    allowPartialRequestedDetails: Boolean(options._fixedRoutingPlan),
    limit: 5,
    now: options.now,
  });
  if (isBinStorageLocationRequest(question)) {
    operationalSearch.sources = operationalSearch.sources.filter((source) => source.id !== "approved-trash-recurring-service");
  }
  if (operationalSearch.requestedDetails.includes("contact")) {
    // An approved contact for another service cannot suppress the known
    // withheld contact source merely by appearing in the search results.
    operationalSearch.sources = operationalSearch.sources.filter(source => bestContactCandidate(question, [source]));
  }
  // “What does this fee cover?” is a purpose/structure question. A payment
  // processing fee or a billing cadence shares the same words, but cannot
  // answer it. Require an approved claim for the exact fee structure before
  // any neighboring billing fact reaches the composer.
  if (isWaterFeeStructureQuestion(question)
    && !hasApprovedWaterFeeStructureFact(operationalSearch.sources, question)) {
    const pendingFaq = (operationalSearch.withheldSources || []).find((source) =>
      /^sterling-ranch-faqs-/i.test(source.id || "")
      && /\/faq(?:\?|$)/i.test(source.sourceUrl || "")
    );
    if (pendingFaq) {
      return withheldSourceAnswer(pendingFaq, activeIntent, routingPlan?.requestedDetails || ["information"], routingPlan, question);
    }
    return { ...genericEvidenceBoundary(question, {}, operationalSearch.index || options.index, options.communityProfile), communityIntent: activeIntent };
  }
  ...
  if (withheldControls) {
    const staleDatedHours = datedFacilityHoursAnswer(question, { sources: [withheld] }, {
      ...options,
      routingPlan: sourceAnswerPlan || shortcutPlan,
    });
  ...
  const datedFacilitySource = (sourceAnswerPlan || shortcutPlan)?.dateRange?.start
    && (sourceAnswerPlan || shortcutPlan)?.requestedDetails?.includes("hours")
    ? operationalSearch.sources.find((source) => source.sourceType === "facilities"
      && sourceMatchesRequestedFacility(question, source)
      && source.canonicalScopedProjection === true)
    : null;
  const facilitySource = operationalFacilitySource(question, operationalSearch.sources) || datedFacilitySource;
  if (facilitySource && Number(facilitySource.score || 0) >= 24) {
    const facilityResult = { ...operationalSearch, sources: [facilitySource] };
    const datedHours = datedFacilityHoursAnswer(question, facilityResult, {
      ...options,
      routingPlan: sourceAnswerPlan || shortcutPlan,
    });
    if (datedHours) {
  ...
  const originalBindingRequest = originalQuestionNeedsRule(question);
  const instruction = instructionalRequestShape(question);
  const asksReviewedNarrative = isReportedFindingsRequest(question)
    || instruction.check || instruction.calculation || instruction.scheduling;
  const operationalSubjects = [...operationalSearch.sources, ...(operationalSearch.withheldSources || [])]
    .filter(source => source.canonicalScopedProjection && hasOriginalOperationalSubject(question, source)
      // Only independently reviewed subject metadata or the original
      // process/report request can establish this new operational priority.
      // A generic overlap (water, fee, landscaping, parking) does not remove
      // the mature governing-rule path for ownership or consequences.
      && (source.reviewedSubjectMatch === true || asksReviewedNarrative
        || (options._fixedRoutingPlan && !originalBindingRequest
          && coveredDetails(routingPlan?.requestedDetails || operationalSearch.requestedDetails,
            [source], source.actions || [], question).length > 0)));
  const liveObservationRequest = !originalBindingRequest
    && (deterministicRequestedDetails(question).includes('status') || sourceAnswerPlan?.requestedDetails?.includes('status'));
  if (liveObservationRequest) {
    // Eligible live connectors had their opportunity above. A static source
    // or a status-page navigation label cannot observe a private device (or
    // prove any other present state) when no live result was obtained.
    const resource = operationalSubjects[0];
    const actions = resource?.sourceUrl
      ? [{ label: `Open ${resource.title}`, url: resource.sourceUrl, actionType: 'information' }] : [];
    return { ...buildAnswerContract({
      directAnswer: 'I couldn’t find a current official answer for that.',
      nextStep: actions[0]?.label || '', actions,
      sources: resource ? [{ ...sourceForDisplay(resource), text: '', excerpt: '', facts: [], actions: [] }] : [],
      status: 'could-not-verify', requestedDetails: sourceAnswerPlan?.requestedDetails || operationalSearch.requestedDetails,
      coveredDetails: [], answerMode: 'community-live-evidence-unavailable',
    }), communityIntent: activeIntent };
  }
  if (!originalBindingRequest && !liveObservationRequest && operationalSubjects.length) {
    const approvedOperational = await composeApprovedOperationalProjection(question, {
      ...operationalSearch,
      sources: operationalSearch.sources.filter(source => operationalSubjects.includes(source)),
    }, { ...options, routingPlan: sourceAnswerPlan });
    if (approvedOperational) return { ...approvedOperational, communityIntent: activeIntent };
  }
  const shouldConsultRules = options.answerRulesQuestion
    && !liveObservationRequest
    && (originalBindingRequest || !operationalSubjects.length
      || recoveryNeedsProcessEvidence(question, operationalSubjects))
  ...
  if (shouldConsultRules) {
    ...
    // A supported governing boundary is useful even when unrelated approved
    // operational sources exist. Queue state cannot change what the rulebook
    // does or does not establish about the requested permission.
    if (requested.every(detail => ["permission", "specification"].includes(detail))
      && safeRulesAnswerHasSupportedPart(question, rulesAnswer)) {
      return { ...rulesAnswer, answer: cleanSafeRulesBoundaryText(rulesAnswer.answer),
        actions: actionsForDisplayedRuleEvidence(rulesAnswer), communityIntent: intent };
    }
    ...
    if (withheld && rulesAnswer?.confidence?.canAnswer !== true
      && !relevantWithheld
      && !operationalSearch.sources.some(source => source.canonicalFactualProjection)) {
      const supportedRulePart = partialGoverningRuleAnswer(question, rulesAnswer, requested, routingPlan);
      if (supportedRulePart) return { ...supportedRulePart, communityIntent: intent };
      if (safeRulesAnswerHasSupportedPart(question, rulesAnswer)) {
        return {
    ...
    // Preserve an explicit permission clause even if the separate application
    // step is unsupported. That partial answer does not depend on which
    // unrelated page happens to be in the withheld-source queue.
    if (rulesAnswer?.confidence?.canAnswer !== true) {
    ...
    if (rulesAnswer?.inputClassification === "unclear" && rulesAnswer?.confidence?.canAnswer !== true) {
      const communityEvidence = searchCommunityIndex(question, { index: options.index, indexPath: options.indexPath, communityId: options.communityId, intent: activeIntent, interpretation: routingPlan, limit: 5, now: options.now });
    ...
    if (rulesAnswer?.confidence?.canAnswer
      ...
      let community = searchCommunityIndex(question, { index: options.index, indexPath: options.indexPath, communityId: options.communityId, intent: activeIntent, interpretation: routingPlan, limit: 3, now: options.now });
      ...
      const requestsPermissionDecision = /\b(?:can|could|may)\s+(?:i|we)\b.{0,80}\b(?:add|build|construct|create|have|install|put)\b|\b(?:approval|permission)\s+(?:is\s+)?required\b|\b(?:allowed|permitted|prohibited)\b/i.test(question);
      const preferExactPdf = exactOfficialPdf && !requestsPermissionDecision && (
        rulesAdmitMissingAttribute
        || requestedObjectMissing
        || !rulesAnswerHasQuestionSpecificEvidence(question, rulesAnswer)
      );
      if (preferExactPdf || (!bindingRule && requestedObjectMissing && hasDistinctiveCommunityEvidence(question, community.sources))) {
      ...
      // Retrieval alone does not make a page evidence for this answer. Any
      // supported operational completion has already been composed above;
      // do not append uncited neighbors to the controlling answer here.
      const extraSources = [];
      ...
      const mergedSourceIds = new Set(mergedSources.map((source) => String(source.id || source.nodeId || "")).filter(Boolean));
      const mergedClaims = (rulesAnswer.claims || []).map((claim) => ({
        ...claim,
        // Rules claims map each listed evidence chunk independently. When
        // several chunks share one displayed document URL, the source list
        // intentionally keeps one copy; keep only the retained proof IDs.
        evidenceSourceIds: (claim.evidenceSourceIds || []).filter((id) => mergedSourceIds.has(String(id))),
    ...
    if (intent === "rules") {
      ...
      const requestedFenceFinish = /\b(?:fence|fencing)\b.{0,80}\b(?:stain|paint|color|colour|finish|material)\b|\b(?:stain|paint|color|colour|finish|material)\b.{0,80}\b(?:fence|fencing)\b/i.test(question);
      if ((originalBindingRequest || requestedFenceFinish) && rulesFallback?.answerMode === "source-unavailable"
        && (!routingPlan || routingPlan.intent === "rules")) {
        // When the controlling rules path cannot verify a binding requirement,
        // retain that boundary. A builder- or village-specific drawing may be
        // useful only after the governing source establishes that it applies;
        // it cannot replace the missing controlling decision by itself.
        return {
          ...rulesFallback,
          answer: cleanAnswerText(rulesFallback.answer),
          communityIntent: intent,
          authorityDecision: "rulebook-controls-binding-claim",
        };
      }
  ...
  let result = searchCommunityIndex(question, { index: options.index, indexPath: options.indexPath, communityId: options.communityId, intent: routingPlan?.intent || intent, interpretation: routingPlan, now: options.now });
  ...
  if ((!result.sources.length || Number(result.sources[0].score || 0) < 24 || !distinctiveEvidence) && rulesFallback) {
    ...
    if (isSafeRulesBoundaryAnswer(rulesFallback) && !asksApprovedProviderDirectory(question)) {
      return {
        ...
        // Unrelated withheld records and a citation do not make the
        // controlling evidence unavailable. Preserve its actual boundary.
        answerStatus: rulesFallback.answerStatus,
        communityIntent: intent,
      };
    }
...
function memoizeShadowConnector(name, connector, keyFor, stats) {
  if (typeof connector !== "function") return connector;
  const cache = new Map();
  stats[name] = { requests: 0, actualCalls: 0, cacheHits: 0 };
  return (...args) => {
    stats[name].requests += 1;
    const key = String(keyFor(...args));
    if (cache.has(key)) {
      stats[name].cacheHits += 1;
      return cache.get(key).then((value) => structuredClone(value));
    }
    stats[name].actualCalls += 1;
    const result = Promise.resolve().then(() => connector(...args));
    cache.set(key, result);
    return result.then((value) => structuredClone(value));
  };
}

function shadowDateKey(plan = {}, question = "") {
  const dateRange = plan && typeof plan === "object" ? plan.dateRange : null;
  const relativeDate = String(question || "").match(/\b(?:today|tomorrow|tonight|this week|next week)\b/i)?.[0]?.toLowerCase() || "";
  return JSON.stringify({ dateRange: dateRange || null, relativeDate });
}

function needFirstRuntimeOptions(query, options, connectorReuse, requestContractMode) {
  const originalDateMarkers = [...new Set(String(query).toLowerCase()
    .match(/\b(?:today|tomorrow|tonight|this week|next week|\d{4}-\d{2}-\d{2})\b/g) || [])];
  const runtimeOptions = {
    ...options,
    requestContractMode,
    rulesOptions: {
      ...(options.rulesOptions || {}),
      // The candidate needs sentence-level proof from an early deterministic
      // rules return. Keep that extra metadata inside this test-only path so
      // it cannot change the resident-facing completion flow.
      needFirstEvidenceContract: true,
    },
    getPoolStatus: memoizeShadowConnector("pool-status", options.getPoolStatus,
      () => options.communityId || options.communityProfile?.communityId || "community", connectorReuse),
    getFoodTruckAnswer: memoizeShadowConnector("food-truck", options.getFoodTruckAnswer,
      (plan, question) => originalDateMarkers.length <= 1
        ? JSON.stringify({ originalDateMarkers })
        : shadowDateKey(typeof plan === "object" ? plan : {}, question || plan), connectorReuse),
    getWasteSchedule: memoizeShadowConnector("waste-schedule", options.getWasteSchedule,
      (request = {}) => `${/\bgarbage|trash\b/i.test(request.question || "") ? "garbage" : "recycling"}:${shadowDateKey(request.routingPlan, request.question)}`, connectorReuse),
    getCommunityEvents: memoizeShadowConnector("community-events", options.getCommunityEvents,
      (plan) => JSON.stringify({
        date: shadowDateKey(typeof plan === "object" ? plan : {}, typeof plan === "string" ? plan : query),
        filters: typeof plan === "object" ? plan.filters || {} : {},
      }), connectorReuse),
  };
  delete runtimeOptions.answerResidentNeed;
  delete runtimeOptions.needRouterBackend;
  delete runtimeOptions.requestContractPlan;
  delete runtimeOptions.planResidentNeeds;
  delete runtimeOptions.rewriteNeedFirstAnswer;
  return runtimeOptions;
}

function plannedContractCoversFallback(fallback, planned) {
  if (!planned || !Array.isArray(planned.needs) || !planned.needs.length || planned.needs.length > 4) return false;
  if (planned.needs.length < fallback.needs.length || (planned.complete === true && fallback.complete !== true)) return false;
  if (planned.needs.some((need) => !need?.id || !need.request || !need.routeRequest || !need.task
    || !Array.isArray(need.requestedDetails) || !need.requestedDetails.length)) return false;
  const plannedText = planned.needs.map((need) => `${need.text || ""} ${need.request || ""}`);
  return fallback.needs.every((need) => {
    const required = words(need.text || need.request);
    return plannedText.some((text) => {
      const available = new Set(words(text));
      const overlap = required.filter((word) => available.has(word)).length;
      return overlap >= Math.min(2, Math.max(1, required.length));
    });
  });
}

function rewrittenCandidatePreservesContract(candidate, rewritten) {
  if (!rewritten || typeof rewritten !== "object" || !String(rewritten.answer || "").trim()) return false;
  if (rewritten.completion?.outcome !== candidate.completion?.outcome) return false;
  if ((rewritten.completion?.needs || []).length !== (candidate.completion?.needs || []).length) return false;
  const candidateSources = new Set((candidate.sources || []).map((source) => String(source.id || source.nodeId || source.sourceUrl || "")));
  return (rewritten.sources || []).every((source) => candidateSources.has(String(source.id || source.nodeId || source.sourceUrl || "")));
}

function isSimpleResidentRequest(query, contract) {
  const text = String(query || "").trim();
  const wordCount = (text.match(/[A-Za-z0-9]+(?:['’-][A-Za-z0-9]+)*/g) || []).length;
  if (!contract?.complete || contract.needs?.length !== 1 || wordCount > 8) return false;
  // These words usually make the answer depend on a condition, comparison,
  // sequence, or second requested detail. The need-by-need path is safer for
  // those questions even when the older answer carries a verified label.
  return !/[;,]|\b(?:versus|vs\.?|instead|before|after|during|if|while|without|except|both|each|per)\b/i.test(text);
}

function supportedNeedCount(assessment = {}) {
  return (assessment?.needs || []).filter((need) => need.status === "supported").length;
}

function chooseAuditedAnswer(query, contract, baseline, baselineAssessment, candidate, candidateAssessment) {
  if (baselineAssessment?.outcome === "handled-boundary") {
    return { answer: baseline, assessment: baselineAssessment, baselinePreserved: true, reason: "baseline-safely-handles-boundary" };
  }

  const baselineSupportedNeeds = supportedNeedCount(baselineAssessment);
  const candidateSupportedNeeds = supportedNeedCount(candidateAssessment);
  const simpleRequest = isSimpleResidentRequest(query, contract);
  const baselineWasPreviouslyEligible = simpleRequest
    && baselineAssessment?.outcome === "complete";

  // Preserve the richer established response for short, single-purpose requests
  // only after the focused candidate has also been produced and audited.
  if (baselineWasPreviouslyEligible) {
    return {
      answer: baseline,
      assessment: baselineAssessment,
      baselinePreserved: true,
      reason: "baseline-fully-covers-simple-request",
    };
  }

  // A focused draft normally wins for qualified or multi-part questions. If it
  // drops a need that the established answer demonstrably covers, keep the
  // stronger answer instead of knowingly returning a regression.
  const candidateComplete = ["complete", "handled-boundary"].includes(candidateAssessment?.outcome);
  const baselineComplete = ["complete", "handled-boundary"].includes(baselineAssessment?.outcome);
  if (!candidateComplete && (baselineComplete || baselineSupportedNeeds > candidateSupportedNeeds)) {
    return {
      answer: baseline,
      assessment: baselineAssessment,
      baselinePreserved: true,
      reason: baselineComplete
        ? "baseline-rescues-incomplete-focused-draft"
        : "baseline-covers-more-needs-than-focused-draft",
    };
  }
  ...
  return {
    ...
    reason: candidateComplete
      ? "focused-draft-covers-qualified-request"
      : "focused-draft-is-best-available",
  };
}

function validateNeedFirstMode(options) {
  if (options.isTest !== true && options.needFirstResidentRelease !== true) {
    throw new Error("Need-first routing requires test mode or the server-owned resident release flag.");
  }
  if (options.planCommunitySearch !== false || options.synthesizeCommunityAnswer !== false) {
    throw new Error("Need-first routing requires local model stages to be disabled.");
  }
  if (options.answerRulesQuestion && (options.rulesOptions?.searchMode !== "legacy" || options.rulesOptions?.llmMode !== "off")) {
    throw new Error("Need-first routing requires the local deterministic rules path.");
  }
  if (options.answerResidentNeed && options.needRouterBackend) throw new Error("Choose one per-need answer backend.");
  const currentLocalBackend = options.needRouterBackend === "current-local";
  if (!currentLocalBackend && typeof options.answerResidentNeed !== "function") {
    throw new Error("Need-first routing requires an explicit per-need answer function.");
  }
  return currentLocalBackend;
}

async function answerCommunityQuestion(query, options = {}) {
  if (["shadow-route", "need-first-candidate", "need-audited-candidate"].includes(options.requestContractMode)) {
    const candidateMode = ["need-first-candidate", "need-audited-candidate"].includes(options.requestContractMode);
    const auditedMode = options.requestContractMode === "need-audited-candidate";
    const currentLocalBackend = validateNeedFirstMode(options);
    const connectorReuse = {};
    const currentOptions = needFirstRuntimeOptions(query, options, connectorReuse, candidateMode ? undefined : "shadow");
    const answerResidentNeed = options.answerResidentNeed || (async (need) => {
      const needRouteText = need.routeRequest || need.request || need.text;
      const fixedIntent = need.goal === "status"
        ? "status"
        : (/\bcaregiver pass\b/i.test(needRouteText)
        || /\b(?:watch(?:es|ing)?|care(?:s|d|ing)? for|look(?:s|ed|ing)? after)\b.{0,50}\b(?:kids?|children|child)\b/i.test(needRouteText))
        ? "facilities"
        : /\bchase drain\b/i.test(needRouteText)
        ? "forms"
        : need.goal === "application"
          || (need.requestedDetails?.includes("action")
            && /\b(?:apply|application|form|paperwork|submittal|submission)\b/i.test(needRouteText))
        ? "forms"
        : isInternetServiceSetupRequest(needRouteText)
        ? "services"
        : /\b(?:pool|clubhouse|court|facility|amenity|pavilion|shelter)\b/i.test(needRouteText)
        ? "facilities"
        : /\b(?:trash|garbage|recycl(?:e|ing)|waste|bins?|cans?|carts?|containers?|water|sewer|utility|utilities|billing|bill)\b/i.test(needRouteText)
        ? "services"
        : classifyCommunityIntent(needRouteText);
      // A resident need can use a task-shaped label such as "hours" or
      // "eligibility". The shared interpretation contract intentionally has
      // a smaller goal vocabulary, so translate those labels before
      // validation. Otherwise validation rejects the whole fixed plan and a
      // bare weekday can be misread as a specific upcoming date.
      const fixedGoal = GOALS.has(need.goal) ? need.goal : "information";
      const deterministicNeedPlan = fallbackStructuredInterpretation(need.routeRequest || need.request, fixedIntent, { now: options.now });
      const needOptions = {
        ...currentOptions,
        ...
        _fixedRoutingPlan: {
          ...
          goal: fixedGoal,
          goals: [fixedGoal],
          // The route request carries resolved subject context for follow-on
          // clauses (“can I use a check?” -> caregiver pass payment). Keep that
          // context in retrieval while the evidence focus still guides prose.
          subject: need.requestedDetails?.includes("price")
            && /\b(?:establish(?:ing|ment)?|new\s+(?:sod|turf|lawn)|reimburs\w*)\b/i.test(`${need.routeRequest || ""} ${need.evidenceFocus || ""}`)
            ? `${need.routeRequest || ""} ${need.evidenceFocus || need.request || ""}`.trim()
            : need.routeRequest || need.evidenceFocus || need.request,
          requestedDetails: need.requestedDetails,
          searchQueries: [need.routeRequest || need.request],
          dateRange: need.dateRange || deterministicNeedPlan?.dateRange,
          filters: { ...(deterministicNeedPlan?.filters || {}), ...(need.filters || {}) },
          scope: "community",
          needsClarification: false,
          clarificationQuestion: "",
        },
      };
      return answerCommunityQuestion(need.routeRequest || need.request, needOptions);
    });
    if (candidateMode) {
      const fallbackContract = buildResidentRequestContract(query, options.requestContractPlan || null, options.requestContext);
      let plannedContract = null;
      if (typeof options.planResidentNeeds === "function") {
        try {
          plannedContract = await options.planResidentNeeds(query, {
            requestContext: options.requestContext,
            now: options.now,
            communityProfile: options.communityProfile,
          });
        } catch {
          plannedContract = null;
        }
      }
      const contract = plannedContractCoversFallback(fallbackContract, plannedContract)
        ? plannedContract
        : fallbackContract;
      const startedAt = process.hrtime.bigint();
      let baseline = null;
      let baselineAssessment = null;
      if (auditedMode && isSimpleResidentRequest(query, contract)) {
        baseline = await answerCommunityQuestion(query, {
          ...currentOptions,
          requestContractMode: undefined,
        });
        baselineAssessment = assessResidentNeeds(contract, baseline);
      }
      const needsBoundaryPass = hasPromptInjectionSignals(query);
      if (needsBoundaryPass) {
        const boundary = await answerCommunityQuestion(query, {
          ...currentOptions,
          requestContractMode: undefined,
          requestContext: undefined,
        });
        if (["out-of-scope", "safety-rejected"].includes(boundary.answerStatus)) {
          const assessment = assessResidentNeeds(contract, boundary);
          const elapsedMs = Number(process.hrtime.bigint() - startedAt) / 1e6;
          return {
            ...boundary,
            answerMode: "need-first-candidate-boundary",
            completion: { outcome: "handled-boundary", needs: assessment.needs },
            _requestContract: {
              ...contract,
              assessment,
              candidate: {
      ...
      const deterministicCandidate = await runNeedFirstShadow(contract, answerResidentNeed, {
        maxNeeds: options.maxResidentNeeds,
        answerMode: "need-first-candidate",
      });
      const candidate = deterministicCandidate;
      const candidateAssessment = assessResidentNeeds(contract, candidate);
      const candidateComplete = ["complete", "handled-boundary"].includes(candidateAssessment.outcome);
      if (auditedMode && !baseline && !candidateComplete) {
        baseline = await answerCommunityQuestion(query, {
          ...currentOptions,
          requestContractMode: undefined,
        });
      ...
      if (auditedMode && baseline && baselineAssessment?.outcome !== 'complete') {
        // Reuse only retrieval provenance for identical evidence. The older
        // answer must still prove its own rendered claims and requested details.
        baseline = { ...baseline, sources: (baseline.sources || []).map(source => {
          const matching = (candidate.sources || []).find(focused => focused.id && focused.id === source.id
            && focused.sourceUrl === source.sourceUrl && focused.text === source.text
            && focused.contentHash === source.contentHash);
          return matching?.retrievedForNeedIds?.length ? { ...source,
            retrievedForNeedIds: [...new Set([...(source.retrievedForNeedIds || []), ...matching.retrievedForNeedIds])],
          } : source;
        }) };
        baselineAssessment = assessResidentNeeds(contract, baseline);
      }
      const selected = auditedMode
        ? chooseAuditedAnswer(query, contract, baseline, baselineAssessment, candidate, candidateAssessment)
        : { answer: candidate, assessment: candidateAssessment, baselinePreserved: false, reason: "need-first-candidate" };
      // Select and audit facts first. Writing an earlier draft can be wasted
      // (or discarded) when the audited selection retains the richer baseline.
      const { withoutUnnecessaryHandoff, displayedVoiceIssues } = require("./resident-writing-contract");
      const selectedAnswer = withoutUnnecessaryHandoff(selected.answer, contract, selected.assessment);
      let writtenAnswer = selectedAnswer;
      let writerAttempted = false;
      const writerDiagnostics = [];
      if (selected.assessment.outcome === "complete" && typeof options.rewriteNeedFirstAnswer === "function") {
        writerAttempted = true;
        const writerCandidate = { ...selectedAnswer, completion: { ...selectedAnswer.completion, needs: selected.assessment.needs } };
        try {
          const rewritten = await options.rewriteNeedFirstAnswer({ question: query, contract, candidate: writerCandidate,
            onDiagnostic: event => writerDiagnostics.push(event),
          });
          if (rewrittenCandidatePreservesContract(writerCandidate, rewritten)
            && assessResidentNeeds(contract, rewritten).outcome === selected.assessment.outcome
            && !displayedVoiceIssues(rewritten).length) writtenAnswer = rewritten;
        } catch {
          // Keep the audited factual answer when the provider or writer fails.
          writerDiagnostics.push({ stage: "resident-writing", accepted: false, reason: "writer-error" });
        }
      }
      const writerAccepted = writtenAnswer !== selectedAnswer;
      const voiceIssues = displayedVoiceIssues(writtenAnswer);
      const compositionEvents = writerDiagnostics.filter(event => event.stage === 'composition');
      const rejectedWriting = writerDiagnostics.filter(event => event.accepted === false).at(-1);
      const elapsedMs = Number(process.hrtime.bigint() - startedAt) / 1e6;
      return {
        ...writtenAnswer,
        ...(selected.assessment.outcome !== 'complete' && writtenAnswer.confidence?.canAnswer === true ? {
          confidence: { ...writtenAnswer.confidence, canAnswer: false, reason: 'final-need-audit-incomplete' },
        } : {}),
        qualityChecks: { ...writtenAnswer.qualityChecks, issues: [...new Set([...(writtenAnswer.qualityChecks?.issues || []), ...voiceIssues])] },
        residentWriting: {
          eligible: selected.assessment.outcome === 'complete',
          attempted: writerAttempted, accepted: writerAccepted, voiceChecked: true, voiceIssues,
          providerCalled: compositionEvents.some(event => event.cacheHit !== true),
          cacheHit: compositionEvents.some(event => event.cacheHit === true),
          providerObservedAt: compositionEvents.some(event => event.cacheHit !== true) ? new Date().toISOString()
            : compositionEvents.find(event => event.providerObservedAt)?.providerObservedAt || null,
          skipReason: writerAttempted ? '' : selected.assessment.outcome !== 'complete' ? 'incomplete-evidence' : 'writer-unavailable',
          fallbackReason: writerAttempted && !writerAccepted ? rejectedWriting?.reason || 'final-writing-contract-rejected' : '',
          usedTextDigest: writerAccepted ? require('./community-critical-capabilities').displayedAnswerDigest(writtenAnswer) : null,
        },
        _requestContract: {
          ...contract,
          assessment: writerAccepted ? assessResidentNeeds(contract, writtenAnswer) : selected.assessment,
          candidate: {
            coordinatorRuns: 1,
            baselineRuns: baseline ? 1 : 0,
            boundaryRuns: 0,
            needRuns: contract.complete ? contract.needs.length : 0,
            elapsedMs,
            connectorReuse,
            planningMethod: contract.planning?.method || "deterministic",
            planningModel: contract.planning?.model || "",
            writerAttempted,
            writerAccepted,
            writerDiagnostics,
            baselinePreserved: selected.baselinePreserved,
            reason: selected.reason,
            comparedDrafts: baseline ? 2 : 1,
            baselineAssessment: baselineAssessment || undefined,
            focusedAssessment: candidateAssessment,
          },
        },
      };
    }
    const currentAnswer = await answerCommunityQuestion(query, currentOptions);
    const shadowRoute = await runNeedFirstShadow(currentAnswer._requestContract, answerResidentNeed, {
      maxNeeds: options.maxResidentNeeds,
    });
    return {
      ...currentAnswer,
      _requestContract: { ...currentAnswer._requestContract, shadowRoute: { ...shadowRoute, connectorReuse } },
    };
  }
  const state = {};
  ...
  const requestContract = options.requestContractMode === "shadow"
    ? buildResidentRequestContract(query, state.plan || completedAnswer?.routingPlan, options.requestContext)
    : null;
  ...
  const attachRequestContract = (result) => requestContract ? {
    ...result,
    _requestContract: { ...requestContract, assessment: assessResidentNeeds(requestContract, result) },
  } : result;
  if (!state.plan && !state.shadowPlan) return attachRequestContract(answer);
  if (state.plan && !answer.routingPlan) {
    return attachRequestContract({ ...answer, ...interpretationFields(state.plan, state.outcome, state.mode), ...(state.connectorDiagnostics && !answer._connectorDiagnostics ? { _connectorDiagnostics: state.connectorDiagnostics } : {}) });
  }
  return attachRequestContract({
    ...answer,
