...
function officialResourceProjectionAnswer(sources = []) {
  return sources
    .map((source) => String(source.excerpt || "").trim())
    .filter(Boolean)
    .join(" ");
}
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (/\b(?:atlas coffee wifi|atlas wifi)\b/i.test(question)) {
    const sources = [officialResourceSource({
      id: "cab-website",
      title: "Official Sterling Ranch CAB website",
      url: CAB_SITE_URL,
      excerpt: "Official CAB website for current resident information.",
    })];
    return {
      answer: unsupportedResourceAbsenceAnswer(sources),
  ...
  if (/\bi need to submit (?:something )?to the drc\b|\bhow do i (?:submit|apply).{0,30}\bdrc\b/i.test(question)) {
    const sources = OFFICIAL_DESIGN_REVIEW_RESOURCES.slice(0, 2).map(officialResourceSource);
    return {
      answer: officialResourceProjectionAnswer(sources),
      answerMode: "official-resource",
      answerVerdict: "informational",
      inputClassification: INPUT_CLASSIFICATIONS.RULES_QUESTION,
