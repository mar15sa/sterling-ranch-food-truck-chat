...
function questionSpecificSourceAnswer(query, sources = []) {
  ...
  if (isPlantPermissionQuestion(query) && !isPlantListQuestion(query)) {
    const row = namedPlantRow(query, sources);
    if (row) {
      const article = /^[aeiou]/i.test(row.classification) ? "an" : "a";
      return structuredHelpfulAnswer(
        `Yes, for the plant choice itself: the selected preapproved list includes ${row.commonName} and classifies it as ${article} ${row.classification}.`,
        [],
        "Open the linked official section if you need the complete wording."
      );
    }
    return structuredHelpfulAnswer(
      "I could not verify whether the requested plant is preapproved from the selected official source.",
      [],
      "Open the linked official plant list to confirm the current choice before planting."
    );
  }
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const rewrittenAnswer = rewriteDecision.eligible
    ...
    : null;
  const answer = rewrittenAnswer || (llmMode === "off"
    ? derived.answer
    : naturalizeGroundedDraft(derived.answer));
  ...
  return {
    answer,
    directAnswer: String(answer || "").split(/\n\s*\n/)[0].trim(),
