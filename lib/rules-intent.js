...
function negativeCatalogMembershipClaims(value = "") {
  const resource = `[^.!?\\n]{0,110}\\b${LIST_RESOURCE_NOUN}s?\\b`;
  const entityFirst = new RegExp(
    `^(.{1,120}?)\\s+(?:is|are|was|were)(?:n't| not)\\s+` +
      `(?:(?:explicitly\\s+)?(?:listed|included|named|shown|found)\\s+)?` +
      `(?:on|in|among)\\s+(${resource})$`,
    "i"
  );
  ...
  const excludedFrom = new RegExp(
    `^(.{1,120}?)\\s+(?:is|are|was|were)\\s+(?:absent|excluded|omitted)\\s+from\\s+(${resource})$`,
    "i"
  );
  ...
  for (const sentence of String(value || "").split(/[.!?]+(?:\s+|$)|[\r\n]+/).map((part) => part.trim()).filter(Boolean)) {
    ...
    if (resourceMatch) claims.push({ entity: normalizedMembershipEntity(resourceMatch[2]), sentence });
  }
...
function unsupportedResourceAbsenceIssues(answer = "", sources = []) {
  ...
  const issues = [];
  const membershipClaims = negativeCatalogMembershipClaims(answer);
