...
function reviewedFactCoversDetail(fact = {}, detail) {
  ...
  if (detail === 'eligibility') {
    return fact.facet === 'eligibility' || (fact.facet === 'reservation-policy'
      && /\b(?:eligib(?:le|ility)|qualif(?:y|ies|ied|ication|ications)|restricted to|available (?:only )?to|open (?:only )?to)\b/i.test(text));
  }
...
function renderedCoveredDetails(requestedDetails, coveredDetails, directAnswer, keyDetails, actions, sources, claims) {
  ...
  return declared.filter((detail) => {
    ...
    if (detail === "specification") return /#[0-9]{2,}\b|\b(?:\d+(?:[ -]\d+\/\d+)?|one|two|three|four|five|six|seven|eight|nine|ten|twelve)\s*(?:feet|foot|inches|inch|ft\.?|in\.?)\b|\b(?:color|colour|paint|stain|finish|material|height|size|dimension|setback|distance)\b.{0,180}\b(?:approved|required|requires|must|match|prohibited|not allowed|no more than|at least|feet|foot|inches|#[0-9]{2,})\b/i.test(text);
