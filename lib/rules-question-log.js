...
function requiredSchema(names = propertyNames()) {
  return {
    ...
    [names.ownerVerdict]: {
      select: { options: ["Impressive", "Acceptable", "Needs work"].map((name) => ({ name })) },
    },
...
function mapNotionQuestion(page, names = propertyNames()) {
  ...
  return {
    ...
    ownerVerdict: properties[names.ownerVerdict]?.select?.name
...
async function queryQuestionLogs(options = {}, fetchImpl = globalThis.fetch) {
  ...
  } else if (["impressive", "acceptable"].includes(options.ownerReview)) {
    const verdict = options.ownerReview === "impressive" ? "Impressive" : "Acceptable";
    filters.push({ property: names.ownerVerdict, select: { equals: verdict } });
  }
  if (options.search) {
...
async function setQuestionOwnerVerdict(pageId, verdict, fetchImpl = globalThis.fetch) {
  ...
  if (!["", "Impressive", "Acceptable", "Needs work"].includes(normalizedVerdict)) {
  ...
  return { id: normalizedPageId, ownerVerdict: normalizedVerdict, needsWork: normalizedVerdict === "Needs work" };
}
