...
function hoursScopes(text = '') {
  ...
  const day = '(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|Mon|Tue|Wed|Thu|Fri|Sat|Sun|Weekdays|Weekends|Daily|Th|M|F)';
  ...
  for (const match of text.matchAll(pattern)) {
    ...
    if (/^(?:m\s*[-–]\s*f|mon(?:day)?\s*(?:[-–]|to|through)\s*fri(?:day)?|weekdays)$/.test(label)) label = 'weekday';
      ...
      .replace(/\b(?:thu|th)\b/g, 'thursday').replace(/\b(?:fri|f)\b/g, 'friday').replace(/\bsat\b/g, 'saturday').replace(/\bsun\b/g, 'sunday')
      .replace(/\bm\b/g, 'monday');
