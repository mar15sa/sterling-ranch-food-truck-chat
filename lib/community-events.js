...
function calendarEnvelope(adapter, { events, sourceUrl, checkedAt, staleAfter, degradation, range, filters, dailyEvidence = [] }) {
  if (!adapter) return null;
  const evidenceId = `${adapter.adapterId}:calendar`;
  const sources = [
    { id: "calendar", sourceUrl, checkedAt, staleAfter, controllingSourceRole: "operational" },
    ...dailyEvidence.map((evidence) => ({
      id: `calendar-${evidence.date}`,
...
async function getCommunityEvents(request, options = {}) {
  ...
  const dailyResults = results.map((result, index) => {
    const evidence = result.evidenceEnvelope?.evidence?.[0];
    return {
      date: dates[index],
      sourceUrl: result.sourceUrl,
      checkedAt: evidence?.checkedAt || result.checkedAt,
      staleAfter: evidence?.staleAfter,
      sourceOutcome: result.diagnostics?.sourceOutcome || "unavailable",
      parserHealthy: result.diagnostics?.parserHealthy !== false,
      degradation: result.evidenceEnvelope?.degradation || { state: result.diagnostics?.sourceOutcome === "ok" ? "healthy" : "unavailable" },
      coverage: result.evidenceEnvelope?.coverage || null,
    };
  });
  const checkedAt = dailyResults.map((result) => result.checkedAt).filter(Boolean).sort()[0] || now.toISOString();
  const staleAfter = dailyResults.map((result) => result.staleAfter).filter(Boolean).sort()[0];
  const requiredCoverage = ["event-date", "date"];
  const rangeHealthy = dailyResults.every((result) => result.sourceOutcome === "ok"
    && result.parserHealthy
    && result.degradation.state === "healthy"
    && (!adapter || requiredCoverage.every((facet) => result.coverage?.covered?.includes(facet))));
  const degradationReasons = [...new Set(dailyResults
    .filter((result) => result.degradation.state !== "healthy" || result.sourceOutcome !== "ok")
    .map((result) => result.degradation.reason)
    .filter(Boolean))];
  const degradation = rangeHealthy
    ? { state: "healthy" }
    : { state: "degraded", reason: degradationReasons.join("; ") || "One or more requested calendar days were not refreshed successfully." };
  ...
  return {
    ...
    evidenceEnvelope: calendarEnvelope(adapter, {
      ...
      range,
      filters,
      dailyEvidence: dailyResults.filter((result) => result.sourceUrl && result.checkedAt && result.staleAfter),
    }),
    diagnostics: {
      sourceOutcome: rangeHealthy ? "ok" : "partial",
      ...
      dailyResults,
      ...(rangeHealthy ? {} : { degradationReason: degradation.reason }),
    },
  };
}
