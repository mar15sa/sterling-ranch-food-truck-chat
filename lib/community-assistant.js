...
function approvedClaimsStayWithinProjection(claims, sources) {
  return claims.every((claim) => {
    ...
    const evidence = cited.map((source) => [
      ...
      ...(source.actions || []).flatMap((action) => [action.label, action.url, action.context]),
    ].filter(Boolean).join(" ")).join(" ").toLowerCase();
    const materialWords = words(claim.text);
    if (!materialWords.length) return false;
    const supported = materialWords.filter((word) => evidence.includes(word)).length;
    return supported / materialWords.length >= 0.6;
  });
}
