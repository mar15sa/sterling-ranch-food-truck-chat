...
const AMENITY_RENTALS_URL = `${CAB_SITE_URL}/257/Amenity-Rentals`;
...
function isAmenityReservationQuery(query) {
  const text = String(query || "");
  return (
    /\b(book|booking|reserve|reservation|rent|rental)\b/i.test(text) &&
    /\b(park|parks|shelter|pavilion|facility|facilities|amenity|amenities|clubhouse|great hall|exhibit hall|pool)\b/i.test(
      text
    )
  );
}
...
function isChickenQuery(query) {
  return /\bchicken(?:s|a)?\b/i.test(String(query || ""));
}
...
function specialSourcesForQuestion(index, query) {
  if (isAmenityReservationQuery(query)) {
    return sourcesByTitle(
      ...
      query,
      [
        /^Sec\. 17-197\. - Specific facility rental rules/i,
        /^Sec\. 17-188\. - Reservation process/i,
        /^Sec\. 17-189\. - Limitations/i,
      ],
      3
    );
  }
...
function amenityRentalResourceSourcesForQuestion(query) {
  if (!isAmenityReservationQuery(query)) return [];
...
function assessAnswerConfidence(query, results, sources) {
  ...
  if (
    (isAmenityReservationQuery(query) &&
      sources.slice(0, 3).some((source) => /17-188|reservation process/i.test(source.title || ""))) ||
    ...
    (isStreetParkingQuery(query) && sources.slice(0, 3).some((source) => /1-37|vehicles; parking/i.test(source.title || ""))) ||
...
function flagAndPoliticalSignOverviewAnswer(query, sources = []) {
  ...
  const isPoliticalSign = /political sign/i.test(query);
  return structuredHelpfulAnswer(
    isPoliticalSign
...
function simpleTopicOverviewAnswer(query, sources = []) {
  const text = combinedSourceText(sources);
  ...
  if (
    ...
    return structuredHelpfulAnswer(
      asksAboutPark
        ? "To reserve a CAB park space, start with the CAB Facility Rental Application. Park shelters and benches can be rented; playgrounds and grassy areas are not included."
        : "To reserve a CAB facility, start with the CAB Facility Rental Application.",
      [
  ...
  if (/\b(?:vegetable garden|raised (?:vegetable )?beds?)\b/i.test(query) && /Gardens; vegetable.*DRC approval is required/i.test(text)) {
...
function needsReadableTopicAnswer(query) {
  return (
    ...
    /\b(trampolines?|rv|rvs|recreational vehicle|motor home|camper|trailer|basketball|dog run|clubhouse|pool rules?|landscape lighting|commercial vehicle|vegetable garden|raised (?:vegetable )?beds?)\b/i.test(
      String(query || "")
    )
  );
}
...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "") {
  if (isLandscapeScreenQuery(query)) {
  ...
  if (/\b(rv|rvs|recreational vehicle|motor home|camper|trailer)\b/i.test(query)) {
    const answer = rvParkingOverviewAnswer(query, sources);
    if (answer) return { available: true, answer, sources, strategy: "structured" };
  }
  ...
  const topicAnswer = simpleTopicOverviewAnswer(query, sources);
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const question = cleanText(input.normalized);
  const indexPath = options.indexPath || DEFAULT_INDEX_PATH;
  ...
  if (input.classification === INPUT_CLASSIFICATIONS.UNRELATED) {
    return {
      answer:
  ...
  if (input.classification === INPUT_CLASSIFICATIONS.UNCLEAR) {
    return {
      answer:
  ...
  const specialSources = specialSourcesForQuestion(index, question);
  ...
  const needsStructuredOverview = needsReadableTopicAnswer(question);
  const needsSourceDerivedFacts = summaryHasVolatileFacts(plainAnswer) || needsStructuredOverview;
  const derived = needsSourceDerivedFacts
    ? sourceDerivedAnswerParts(question, answerSources, plainAnswer)
  ...
  if (needsSourceDerivedFacts && !derived.available) {
    return {
      ...
      inputClassification: input.classification,
      sources: sourcesWithOfficialResources(question, derived.sources),
      sourceStatus: status,
    };
  }

  // AI is a presentation layer, never the source of truth. In selective mode,
  // only generic extractive answers and genuinely compound, multi-source
  // questions reach it. Safety and confidence failures have already returned.
  ...
  const rewriteDecision = selectiveRewriteDecision({
    ...
    confidence,
    inputClassification: input.classification,
    answerStrategy: derived.strategy,
  });
  ...
  const rewrittenAnswer = rewriteDecision.eligible
    ...
    : null;
  const displaySources = sourcesWithOfficialResources(question, derived.sources);
