...
export function matchesLens(place, places, lens, trails) {
  if (lens === 'all' || lens === 'future') return true;
  if (lens === 'walks') return trails.routes.some(r => r.nearbyPlaceIds.includes(place.id)) || features(place, places).items.some(f => f.key === 'walk');
