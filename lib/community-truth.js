...
function buildFactLedger(index = {}, options = {}) {
  ...
  for (const source of index.sources || []) {
    for (const fact of source.facts || []) {
      ...
      const reviewStatus = prior?.reviewStatus || (requirePriorReview ? 'candidate'
        : fact.reviewStatus || source.reviewStatus || (trusted || versionWasTrusted || source.reviewedAt ? "approved" : "candidate"));
      const entry = {
        id,
        ...
        reviewStatus,
        reviewedAt: prior?.reviewedAt || (requirePriorReview ? '' : fact.reviewedAt || source.reviewedAt || ""),
        reviewedBy: prior?.reviewedBy || (requirePriorReview ? '' : fact.reviewedBy || source.reviewedBy || ""),
        reviewNote: prior?.reviewNote || (requirePriorReview ? '' : fact.reviewNote || source.reviewNote || ""),
      };
