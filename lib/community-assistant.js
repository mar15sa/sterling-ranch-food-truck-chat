...
function missingDetailsFromQuality(requested, issues = []) {
  return requested.filter((detail) => {
...
function renderPartialGap(answer, completion) {
  ...
  if (completion?.outcome !== "verified-partial" || !completion.missingDetails.length
...
function ensureAnswerCompletion(question, answer) {
  ...
  const missingDetails = missingDetailsFromQuality(requested, issues);
