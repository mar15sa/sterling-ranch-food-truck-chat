...
const { answerCoverageIssues, normalizeResidentQuestion } = require("./rules-intent");
...
function isLandscapeOverviewQuery(query) {
  ...
  return (
    ...
    !/\b(screen|screens|shed|fence|fences|ornament|statue|tree replacement|dead tree|watering|irrigation schedule)\b/i.test(
      text
    )
  );
}
...
function specialSourcesForQuestion(index, query) {
  ...
  if (/\b(?:approved plants?|plant list|trees? can i plant|kind of plants?)\b/i.test(query)) {
    return sourcesByTitle(index, query, [/^Sec\. 5-131\. - Preapproved plant list/i], 5);
  }
  ...
  if (/\b(?:fence stain|stain color)\b/i.test(query)) {
  ...
  if (/\b(?:specific )?paint color\b/i.test(query)) {
...
function compoundQuestionSources(query, results = [], specialSources = [], limit = 5) {
  if (!/\b(and|also|plus|both|as well as)\b/i.test(String(query || ""))) return [];
...
function sourcesWithOfficialResources(query, answerSources) {
  const sources = [
    ...
    ...designReviewResourceSourcesForQuestion(query, answerSources),
  ];
  const seen = new Set();
  return sources.filter((source) => {
    const key =
...
function structuredHelpfulAnswer(shortAnswer, findings, nextStep = "") {
  ...
  if (usefulFindings.length) {
    lines.push("", "What I found:", ...usefulFindings.map((finding) => `- ${finding}`));
  }
  if (nextStep) {
...
function conciseFactContext(fact, rawText = "") {
  ...
  if (raw && Number.isInteger(fact.sourceOffset)) {
    ...
    return `${start > 0 ? "..." : ""}${cleanText(raw.slice(start, end))}${end < raw.length ? "..." : ""}`;
  }
  const context = cleanText(fact.context || "");
...
function readableSourcePassage(text = "") {
  return cleanText(text).replace(
    /\b([A-Za-z]+(?:-[A-Za-z]+)?)\s+\(\d+\)\s+(calendar\s+)?(days?|hours?|minutes?|months?|years?)/gi,
    "$1 $2$3"
  );
}
...
function fenceAndShedOverviewAnswer(query, sources = [], index) {
  ...
  return {
    ...
    answer: structuredHelpfulAnswer(
      ...
      [
        "Fence: the allowed style depends on location. Perimeter fencing is the three-rail concrete style in Earthen and cannot be changed; interior lot lines may use the approved stained-wood style or matching concrete. New construction and additional gates require DRC approval.",
...
function simpleTopicOverviewAnswer(query, sources = [], residentQuestion = query) {
  ...
  if (/\b(?:approved landscapers?|list of approved landscapers?)\b/i.test(query) && /Registration with the CAB is required/i.test(text)) {
    return structuredHelpfulAnswer(
      "The rulebook requires the individual who designs or supervises residential landscaping or irrigation work to be registered with the CAB, but the indexed rules do not include a current roster of names.",
      ...
      "Use the official Design Review Documents page below or contact the DRC to verify the current registration of a landscaper before hiring them."
    );
  }
  ...
  if (/\b(?:approved plants?|plant list|trees? can i plant|kind of plants?)\b/i.test(query) && /preapproved plant list/i.test(`${sources[0]?.title || ""} ${text}`)) {
    return structuredHelpfulAnswer(
  ...
  if (/\b(?:fence stain|stain color)\b/i.test(query) && /stained in the approved color to match the concrete perimeter fence/i.test(text)) {
  ...
  if (
    ...
    return structuredHelpfulAnswer(
      asksAboutPark
        ? "To reserve a CAB park space, start with the Facilities Rental Application and Agreement. Park shelters and benches can be rented; playgrounds and grassy areas are not included."
...
function residentFeeOverviewAnswer(sources = []) {
  ...
  return [
    ...
    "",
    "What I found: Typical fixed monthly charges",
    `- Water service base: ${formatMoney(amounts.waterBase)}`,
    `- Sewer base: ${formatMoney(amounts.sewerBase)}`,
    `- Stormwater: ${formatMoney(amounts.stormwater)}`,
    `- Streetlight: ${formatMoney(amounts.streetlight)}`,
    `- Trash: ${formatMoney(amounts.trash)}`,
    "",
    "What I found: Charges that depend on usage",
    `- Indoor water: starts at ${formatMoney(amounts.indoorWater)} per 1,000 gallons, with higher rates at higher usage tiers`,
    `- Sewer usage: ${formatMoney(amounts.sewerUsage)} per 1,000 gallons of indoor water use`,
    "",
    "What I found: Charges that apply only to some homes",
    `- Shared-driveway maintenance: ${formatMoney(amounts.sharedDriveway)} per month`,
    `- Alley-load maintenance: ${formatMoney(amounts.alleyLoad)} per month`,
    "",
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const preferSpecialSources = Boolean(specialSources.length) && (
    ...
    /\b(?:chicken wire|pickle ?ball|sport court|rain(?:water)?(?: harvesting)? barrels?|artificial turf|synthetic turf|turf|air conditioner|ac unit|hvac|mini split|gazebo|pergola|fireworks?|approved plants?|plant list|tree lawn|fence stain|stain color|utility trailer|privacy film|tint film|window tint|long[-\s]?term rental|approved landscaper|own landscaping|redo backyard|concrete patio|extend concrete|rear landscaping|side yard|quantum fiber|internet provider|water usage|front yard tree|catio|easement)\b/i.test(routingQuery) ||
  ...
  if (/\b(?:specific )?paint color\b/i.test(routingQuery) && answerSources.some((source) => /21-22|Painting, exterior/i.test(source.title || ""))) {
  ...
  if (!earlyStructuredAnswer && /\b(?:fence stain|stain color)\b/i.test(routingQuery) && answerSources.some((source) => /^Sec\. 21-23\./i.test(source.title || ""))) {
  ...
  if (earlyStructuredAnswer && answerSources.length) {
    const displaySources = sourcesWithOfficialResources(routingQuery, answerSources);
  ...
  const derived = compoundDerived || (needsSourceDerivedFacts
    ? sourceDerivedAnswerParts(routingQuery, answerSources, plainAnswer, question)
  ...
  if (needsSourceDerivedFacts && !derived.available) {
    return {
      ...
      inputClassification: effectiveInputClassification,
      sources: sourcesWithOfficialResources(routingQuery, derived.sources),
      sourceStatus: status,
    };
  }
  ...
  const rewrittenAnswer = rewriteDecision.eligible
    ...
    : null;
  const displaySources = sourcesWithOfficialResources(routingQuery, derived.sources);
