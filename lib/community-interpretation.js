...
const DETAILS = new Set(["price", "action", "date", "hours", "contact", "permission", "specification", "examples", "status"]);
...
const DETAIL_PRIORITY = ["price", "action", "date", "hours", "contact", "permission", "specification", "examples", "status"];
...
function deterministicRequestedDetails(question = "") {
  ...
  if (/\b(?:allowed|permitted|prohibited|permission|approval|need to)\b/.test(text)
    || (/\b(?:can|may) (?:i|we)\b/.test(text) && !/\b(?:where|how|when|what)\s+(?:can|may) (?:i|we)\b/.test(text))) details.push("permission");
  if (/\b(?:color|colour|paint|stain|material|height|high|tall|size|dimension|setback|distance)\b/.test(text)
    || /\b(?:what|which)\s+finish\b|\bfinish\s+(?:type|requirement)\b/.test(text)) details.push("specification");
