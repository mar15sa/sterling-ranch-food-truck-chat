...
function answerCoverageIssues(question, answer = "", sources = []) {
  ...
  if (facets.includes("permission") && !/\b(?:yes|no|allowed|approved|preapproved|not allowed|prohibited|required|requires|may|must|need to|does not (?:say|state|specify)|evaluated on an individual basis)\b|(?:^|\n)Turn off\b/i.test(text)) issues.push("direct-permission-answer-missing");
