...
export function readNavigation(search='') {
  ...
  return {view,place,group:Math.floor(number(q.get('group'),0,0,10)),photo:q.get('photo')==='1',source:q.get('source')==='1',opened:q.get('closed')!=='1',
    ...
    walk:{area:['all','Providence','Ascent','Prospect'].includes(q.get('walkArea'))?q.get('walkArea'):'all',minutes:[15,30].includes(Number(q.get('minutes')))?Number(q.get('minutes')):0,route:id(q.get('route'))},
    future:{mode:q.get('futureMap')==='villages'?'villages':'projects',area:q.get('futureArea')||'all',selected:id(q.get('plan')),zoom:number(q.get('futureZoom'),1,1,3)},returnMap:q.get('return')==='1'?{
      opener:id(q.get('returnOpener')),view:q.get('returnView')==='walks'?'walks':'neighborhood',walk:{area:['all','Providence','Ascent','Prospect'].includes(q.get('returnWalkArea'))?q.get('returnWalkArea'):'all',minutes:[15,30].includes(Number(q.get('returnMinutes')))?Number(q.get('returnMinutes')):0,route:id(q.get('returnRoute'))},
...
export function navigationQuery(s) {
  ...
  if(s.walk?.area&&s.walk.area!=='all')q.set('walkArea',s.walk.area);
  if(s.walk?.minutes)q.set('minutes',s.walk.minutes);
  if(s.walk?.route)q.set('route',s.walk.route);
  if(s.future?.mode==='villages')q.set('futureMap','villages');
  ...
  if(s.returnMap){
    const r=s.returnMap;q.set('return','1');if(r.opener)q.set('returnOpener',r.opener);
