...
const { isInternetServiceSetupRequest } = require("./community-interpretation");
...
const RULE_TOPIC_PATTERN = new RegExp(
  `\\b(?:${[
    ...
    "architectural|catio|colors?|decks?|fences?|flags?|garage doors?|greenhouses?|hallowe+en|hallowen|holidays?|christmas|landscap\\w*|lights?|lighting|mailboxes?|ornaments?|paints?|panels?|patios?|pools?|roofs?|screens?|sheds?|signs?|solar|spa|trampolines?|trees?|yards?",
    ...
    "airbnb|amenities|amenity|clubhouse|colorado parks(?: and wildlife)?|cpw|facilities|facility|fire pit|home automation|homeseer|irrigation|lumiere|parks?|pavilion|quiet hours|reservations?|shelters?|short[-\\s]?term rentals?|sprinklers?|steward|vacation rentals?|vrbo|watering",
  ].join("|")})\\b`,
  "i"
);
...
const UNRELATED_TOPIC_PATTERN = new RegExp(
  [
    ...
    "homework|poem|write (?:an|my) essay|write (?:a|me a) story|solve (?:this|my) math",
...
function isConversation(query) {
  ...
  return (
    /^\s*(?:good\s+(?:afternoon|evening|morning)|hello|hey|hi|howdy|how are you)[!.?\s]*$/i.test(text) ||
    /^\s*(?:bye|goodbye|thanks|thank you|who are you|what can you do)[!.?\s]*$/i.test(text)
  );
}

function isIncompleteResidentStatement(query) {
  const text = normalizeInput(query);
  const words = text.match(/[a-z0-9']+/gi) || [];
  if (!text || words.length > 6 || /\?/.test(text)) return false;
  if (!/^(?:i|we)\s+(?:have|own|live in|am|are)\b/i.test(text)) return false;
  return !/\b(?:apply|book|build|can|contact|could|find|get|help|install|looking|may|need|open|pay|remove|renew|replace|reserve|should|show|submit|tell|trying|want|wondering|would)\b/i.test(text);
}
...
function classifyRulesInput(query) {
  ...
  if (isIncompleteResidentStatement(text)) {
    return { classification: INPUT_CLASSIFICATIONS.UNCLEAR, normalized: text, reason: "incomplete-statement" };
  }

  if (isInternetServiceSetupRequest(text) || RULE_TOPIC_PATTERN.test(text) || RULE_REQUEST_PATTERN.test(text) || SHORT_TERM_RENTAL_REQUEST_PATTERN.test(text) || hasCommunityTopicSignal(text)) {
