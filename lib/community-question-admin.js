...
function isSameOriginRequest(req) {
  ...
  try {
    ...
    const expectedProtocol = String(req.headers["x-forwarded-proto"] || (req.socket?.encrypted ? "https" : "http")).split(",")[0].trim();
    const expectedHost = String(req.headers["x-forwarded-host"] || req.headers.host || "").split(",")[0].trim();
    return source.protocol === `${expectedProtocol}:` && source.host === expectedHost;
  } catch {
    return false;
  }
}
