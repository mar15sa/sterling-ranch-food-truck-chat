...
function actionSupportsGoal(action = {}, goal = "") {
  if (!goal || !ACTION_GOALS.has(goal)) return true;
  const evidence = `${action.label || ""} ${action.url || ""} ${(action.keywords || []).join(" ")} ${action.actionType || ""} ${action.context || ""} ${action.evidence?.label || ""} ${action.evidence?.context || ""}`.toLowerCase();
  const signals = {
