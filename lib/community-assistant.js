...
async function sourcedAnswer(question, result, options = {}) {
  ...
  if (result.requestedDetails.includes("contact")) {
    ...
    const contactEvidence = sources.map((source) => `${source.title || ""} ${source.text || source.excerpt || ""}`).join(" ");
    const missingOrganization = requestedOrganizations.find((organization) => !new RegExp(`\\b${organization}\\b`, "i").test(contactEvidence));
    if (missingOrganization) {
      const boundary = buildAnswerContract({
        directAnswer: `I could not verify an official ${missingOrganization} phone number from the connected official sources.`,
