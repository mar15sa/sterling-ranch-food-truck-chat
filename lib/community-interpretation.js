...
function instructionalRequestShape(question = "") {
  ...
  return { check, calculation, separateStatus };
}
...
function deterministicRequestedDetails(question = "") {
  ...
  const action = instruction.check || resourceViewRequest || /\b(?:how do|where (?:do|can)|book|reserve|rent|submit|register|sign up|form|pay|payment|settle|settling|log in|login|download)\b/.test(text)
  ...
  const schedulingProcess = /\b(?:how|where)\s+(?:do|can|should)\s+(?:i|we)\s+schedule\b|\bhow to schedule\b/.test(text);
  if (/\b(?:when|date|day|today|tomorrow|weekend|upcoming)\b/.test(text)
    || (!schedulingProcess && !instruction.check && /\bschedule\b/.test(text))) details.push("date");
  ...
  if (/\b(?:permission|approval|need to)\b/.test(text)
    ...
    || (!resourceViewRequest && !specificationModal && !operationalChoiceModal && /\b(?:can|may) (?:i|we)\b/.test(text) && !/\b(?:where|how|when|what)\s+(?:can|may) (?:i|we)\b/.test(text))) details.push("permission");
  if (/\b(?:color|colour|stain|material|height|high|tall|size|dimension|setback|distance)\b/.test(text)
  ...
  if (instruction.separateStatus || (!instruction.check && (/\b(?:status|currently|right now)\b/.test(text)
  ...
  if (/\b(?:eligib(?:le|ility)|qualif(?:y|ies|ied|ication|ications))\b/.test(text)
    || /\bwho\s+(?:can|may|is allowed to)\s+(?:use|receive|get|obtain|apply|participate|join|buy|purchase)\b/.test(text)) details.push('eligibility');
...
function normalizeInterpretation(plan = {}, question = "", options = {}) {
  ...
  const processOnly = instruction.check && !instruction.separateStatus
    && !explicitDetails.includes('date') && !explicitDetails.includes('hours')
    && !explicitDetails.includes('permission')
  ...
  return {
    ...
    subject,
    requestedDetails,
    dateRange: processOnly || methodOnly ? null : validatedModelDateRange(plan.dateRange, question, options.now) || fallbackDateRange(question, intent, options.now),
