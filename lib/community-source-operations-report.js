...
function buildSourceOperationsReport({ index = {}, reviewRecords = [], previousReport = {}, now = Date.now(), expirySoonDays = 1, revalidation = {} } = {}) {
  ...
  return {
    ...
    residentSafety: {
      ...
      approvedEvidenceNeedsAttention: Boolean(revalidation.status && revalidation.status !== "passed")
        ...
        || expiryCounts(sources, now, soonMs).dueSoon > 0
        || expiryCounts(facts, now, soonMs).dueSoon > 0,
    },
  };
}
