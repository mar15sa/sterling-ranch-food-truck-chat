...
function friendlySourceSentence(sentence = "") {
  ...
  // Grammar only: subjects, conditions and values are supplied by the source.
  // These transformations also serve the provider-off/unavailable path.
  value = value.replace(/\bprior to\b/gi, "before")
    .replace(/\bsubsequent to\b/gi, "after")
    .replace(/\bbetween the hours of\b/gi, "between");
  const discretion = value.match(/^At the (?:sole )?discretion of (.+?), (.+?) may be (.+?)\.?$/i);
  if (discretion) {
    return `${capitalize(discretion[1])} decides whether ${discretion[2]} can be ${discretion[3].replace(/\bdue to\b/i, "because of").replace(/\.$/, "")}.`;
  }
  const switched = value.match(/^Turned (off|on) (.+?) for (.+?)\.?$/i);
  if (switched) return completeSentence(`Turn ${switched[1]} ${switched[3].replace(/\.$/, "")} ${switched[2]}`);
  const appropriate = value.match(/^(.+?) and seasonally appropriate\.?$/i);
  if (appropriate) return `It must be appropriate for the ${lowerFirst(appropriate[1]).replace(/^the /i, "")} and the season.`;
  value = value.replace(/^Permitted\s+/i, "This is allowed ")
    .replace(/^Required to\s+/i, "You need to ")
    .replace(/^Prohibited\s+(?!under\b)/i, "This is prohibited ");
  ...
  if (noSubjectProhibition) {
    const permittedAction = noSubjectProhibition[2].match(/^permitted to (.+?)\.?$/i);
    if (permittedAction && !noSubjectProhibition[3]) {
      return `No ${lowerFirst(noSubjectProhibition[1])} can ${permittedAction[1].replace(/\.$/, "")}.`;
    }
...
function residentVoiceIssues(answer = "") {
  ...
  if (/(?:^|[.!?]\s+)(?:Must be|Permitted|Required to|Turned (?:off|on)|Kept|Stored|Located|Installed|Removed)\b/m.test(value)
    || /(?:^|\n)\s*[^.!?\n]+\band seasonally appropriate\.?\s*(?:$|\n)/im.test(value)) {
    issues.push("sentence-fragment");
  }
  if (/\b(?:prior to|subsequent to|at the (?:sole )?discretion of|between the hours of)\b/i.test(value)) {
    issues.push("formal-rulebook-language");
  }
  ...
  if (/\bcan't be permitted to\b/i.test(value)) issues.push("passive-prohibition-language");
