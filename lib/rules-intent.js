...
function answerCoverageIssues(question, answer = "", sources = []) {
  ...
  if (facets.includes("permission") && !/\b(?:yes|no|allowed|approved|not allowed|prohibited|required|requires|may|must|does not (?:say|state|specify)|evaluated on an individual basis)\b/i.test(text)) issues.push("direct-permission-answer-missing");
