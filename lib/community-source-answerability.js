const { factIsAnswerable, lifecycleFor, resolveFactLedger } = require('./community-truth');
...
function canonicalProjectionEntries(source = {}, index = {}) {
  ...
  const factEntries = (source.facts || []).filter((fact) => approved(fact.approvalClaim)).map((fact) => ({
    ...
    subjectKey: fact.subjectKey || source.title || '',
    reviewedSubjectKey: fact.subjectKey || '',
...
function sourceReviewState(index = {}, now = Date.now()) {
  ...
  const entriesFor = source => {
    ...
    // Exact approval records identity and scope, not perpetual freshness.
    // Both canonical and ledger projections depend on the current parent
    // source; a longer-lived claim cannot renew that source implicitly.
    if (lifecycleFor(source, now) !== 'current') return [];
    if (source.staleAfter) {
      const staleAt = new Date(source.staleAfter).getTime();
      if (!Number.isFinite(staleAt) || staleAt < Number(now)) return [];
    }
    const canonicalEntries = canonicalProjectionEntries(source, index);
    // A scoped decision approves the content version, not its perpetual
    // freshness. Canonical entries are reconstructed rather than read from
    // the fact ledger, so they need the source's own verified expiry gate.
    // Withhold the whole projection (including actions and any ledger copy)
    // until exact-source revalidation provides a valid future expiry.
    const expiresAt = Date.parse(source.staleAfter || '');
    if (canonicalEntries.length && (!Number.isFinite(expiresAt) || expiresAt <= Number(now))) return [];
    ...
    return [
      ...
      // Ledger subject keys may be inferred from titles. Only a subject
      // explicitly attached to this exact reviewed source fact is a hard
      // entity boundary, including for the duplicate ledger projection.
      const fact = (source.facts || []).find(fact => fact.type === entry.factType
        && inferFacet(fact, source) === entry.facet && inferScopeKey(fact, source) === entry.scopeKey
        && JSON.stringify(normalizeFactValue(fact)) === JSON.stringify(entry.normalizedValue));
      return { ...entry, sourceTitle: source.title || '', reviewedSubjectKey: fact?.subjectKey || '',
        ...(fact?.applicabilityTerms?.length ? { applicabilityTerms: fact.applicabilityTerms } : {}) };
    });
  };
