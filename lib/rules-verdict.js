function deriveAnswerVerdict(answer, confidence = {}) {
  ...
  if (
    /\b(?:requires?|required|needs?|need to (?:receive|obtain|submit)|need (?:DRC|CAB|design review) (?:review|approval)|must (?:first )?(?:receive|obtain|submit)|approval (?:must be (?:obtained|received)|is required)|with (?:DRC|CAB|design review) approval|subject to approval|only (?:after|with)|evaluated on an individual basis)\b/i.test(
      lead
    )
  ) {
    return "conditional";
