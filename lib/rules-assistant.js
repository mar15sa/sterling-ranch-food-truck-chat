...
const { answerCoverageIssues, isPlantListQuestion, normalizeResidentQuestion } = require("./rules-intent");
...
function isStateParksPassQuery(query) {
  const text = String(query || "");
  return (
...
function specialSourcesForQuestion(index, query) {
  ...
  if (isStateParksPassQuery(query)) {
...
function sourcesWithOfficialResources(query, answerSources, answerText = "") {
  if (isStateParksPassQuery(query)) return answerSources;
  const sources = [
    ...answerSources,
...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "", residentQuestion = query) {
  if (isStateParksPassQuery(residentQuestion) || isStateParksPassQuery(query)) {
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const preferSpecialSources = Boolean(specialSources.length) && (
    ...
    isStateParksPassQuery(routingQuery) ||
  ...
  if (earlyStructuredAnswer && answerSources.length) {
    ...
    return {
      ...
      searchMode,
      sources: displaySources,
      ...(isStateParksPassQuery(routingQuery) ? { controllingSourceOnly: true } : {}),
      sourceStatus: status,
      qualityChecks: {
