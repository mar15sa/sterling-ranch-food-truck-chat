...
function versionHash(text, source = {}) {
  if (source.hashScheme === 'page-text-actions-v1') return sourceContentHash(text, '', (source.actions || []).map(action => ({ label: action.evidence?.label || action.label, url: action.evidence?.url || action.url, actionType: action.evidence?.actionType || action.actionType || 'information' })));
...
function projectedActionProof(sources = [], rawLinks = [], text = "", sourceUrl = "") {
  ...
  const observed = expected.map(({ sourceId, action }) => {
    ...
    return {
      ...
      matches: opensVerifiedPage ? [{ label: evidence.label, url: sourceUrl, actionType: action.actionType }] : rawLinks
        .filter((link) => actionUrlIdentity(link.url) === actionUrlIdentity(evidence.url)),
      contextMatches: opensVerifiedPage || Boolean(evidence.context && text.includes(evidence.context)),
    };
  });
  return {
...
async function observeCanonicalSource(sourceUrl, approvedSources, { fetchImpl = globalThis.fetch, extractPdfTextImpl = extractPdfText, timeoutMs = 30_000 } = {}) {
  ...
  if (scopedProjections.length && !ordinarySources.length) {
    ...
    return {
      ...
      observedHashes: [...new Set(scopedProjections.map(source => versionHash(text, source)))],
