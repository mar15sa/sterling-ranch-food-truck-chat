...
function claimSupportsDetail(detail, text = "", actions = []) {
  ...
  if (detail === "action") {
    ...
    return /^(?:(?:please|to)\s+)?(?:access|apply|book|bring|call|continue|download|email|exchange|log in|open|pay|register|reserve|select|sign in|submit|use|visit)\b/i.test(directive)
      || /\b(?:you|residents?|customers?|applicants?)\s+(?:can|may|must|should|need to)\s+(?:access|apply|book|bring|call|continue|download|email|exchange|log in|open|pay|register|reserve|select|sign in|submit|use|visit)\b/i.test(directive)
  ...
  if (detail === "specification") return /#[0-9]{2,}\b|\b(?:\d+(?:[ -]\d+\/\d+)?|one|two|three|four|five|six|seven|eight|nine|ten|twelve)\s*(?:consecutive\s+)?(?:seconds?|minutes?|hours?|days?|nights?|overnights?|weeks?|feet|foot|inches|inch|ft\.?|in\.?)\b|\b(?:color|paint|stain|finish|material|height|size|dimension|setback|distance|duration)\b|\b(?:must|required|allowed|prohibited)\b.{0,120}\b(?:location|area|place|screened|visible|inside|behind)\b|\b(?:return|keep|store|place|locate|screen)\w*\b.{0,120}\b(?:location|area|place|screened|visible|inside|behind)\b/i.test(text);
  return true;
}
...
function assessResidentNeeds(contract, answer = {}) {
  ...
  const needs = contract.needs.map((need) => {
    ...
    const needTokens = distinctiveTokens(`${need.evidenceFocus || need.request} ${dependentResidentContext} ${need.usedPriorContext ? need.subjectHint || "" : ""}`);
