...
function renderedCoveredDetails(requestedDetails, coveredDetails, directAnswer, keyDetails, actions) {
  ...
  return declared.filter((detail) => {
    ...
    if (detail === "specification") return /#[0-9]{2,}\b|\b(?:color|colour|paint|stain|finish|material|height|size|dimension|setback|distance)\b.{0,180}\b(?:approved|required|requires|must|match|prohibited|not allowed|no more than|at least|feet|foot|inches|#[0-9]{2,})\b/i.test(text);
