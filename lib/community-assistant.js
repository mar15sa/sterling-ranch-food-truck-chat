...
const { naturalizeGroundedDraft } = require("./rules-llm");
const {
...
async function answerCommunityQuestion(query, options = {}) {
  ...
  const retrievalScaffoldedAnswer = /^Short answer\s*:/im.test(String(completedAnswer.answer || ""))
    && /^What I found\s*:/im.test(String(completedAnswer.answer || ""));
  if (supportedRuleAnswer || retrievalScaffoldedAnswer) {
    const cleanRuleAnswer = naturalizeGroundedDraft(completedAnswer.answer);
    const proseParts = cleanRuleAnswer.split(/\n\s*\n/).map((part) => part.trim()).filter(Boolean);
    const cleanStructuredPart = (value) => naturalizeGroundedDraft(cleanAnswerText(value));
    ...
    const answer = supportedRuleAnswer
      ? [directAnswer, ...keyDetails, ...nextStep && !cautiousUnverifiedNextStep ? [nextStep] : []]
        .filter(Boolean)
        .join("\n\n") || cleanRuleAnswer
      : cleanRuleAnswer;
    completedAnswer = { ...completedAnswer, answer, directAnswer, keyDetails };
  }
