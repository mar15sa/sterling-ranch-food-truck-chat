  ...
  if (["shadow-route", "need-first-candidate", "need-audited-candidate"].includes(options.requestContractMode)) {
    ...
    if (candidateMode) {
      ...
      if (auditedMode) {
        ...
        const preserveBaseline = ["complete", "handled-boundary"].includes(baselineAssessment.outcome)
          || baselineSupportedNeeds > 0
          // The audit is deliberately not allowed to erase a source-grounded
          // answer merely because its own coarse detail labels fail to
          // recognize all of that answer's useful context.
          || baselineIsVerified;
        if (preserveBaseline) {
            ...
            _requestContract: {
              ...
              candidate: {
                ...
                reason: baselineAssessment.outcome === "complete"
                  ...
                  : baselineAssessment.outcome === "handled-boundary"
      ...
      if (auditedMode && !candidate.completion?.needs?.some((need) => need.status === "supported")) {
        const elapsedMs = Number(process.hrtime.bigint() - startedAt) / 1e6;
        return {
          ...baseline,
          _requestContract: {
            ...
            candidate: {
              ...
              planningModel: contract.planning?.model || "",
              baselinePreserved: true,
              reason: "fallback-did-not-add-verified-support",
              fallbackOutcome: candidate.completion?.outcome || "unassessed",
            },
          },
        };
      }
      ...
      return {
        ...
        _requestContract: {
          ...
          candidate: {
            ...
            reason: auditedMode ? "baseline-had-no-verified-support-and-fallback-added-support" : "need-first-candidate",
          },
        },
      };
    }
