...
function compoundQuestionSources(query, results = [], specialSources = [], limit = 5) {
  ...
  for (const term of terms) {
    ...
    const source = candidates.find(
      (candidate) => !selected.some((item) => item.nodeId === candidate.nodeId)
    );
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const overviewSources = isResidentFeeOverviewQuery(question)
    ...
    : [];
  const conceptSources = semanticConceptSources(question, results, 5);
  const compoundSources = compoundQuestionSources(question, results, specialSources, 5);
