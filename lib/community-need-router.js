...
function mergeCandidateSources(results) {
  ...
  for (const result of results) {
    const candidateIds = new Set(result.evidence.candidateSourceIds);
...
function composedAnswer(results, actions, outcome) {
  ...
  const lines = [];
  if (supported.length) {
    lines.push(...uniqueBy(supported.map((result) => directAnswerText(result.answer)).filter(Boolean), (text) => text));
  }
