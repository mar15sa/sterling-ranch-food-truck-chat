...
function questionSpecificSourceAnswer(query, sources = []) {
  ...
  const controllingApproval = asksApprovalOrPermission
    ? clauses.find(approvalRequired)
    : null;
  const primaryEntry = controllingApproval || clauses[0];
  const relevantExceptions = controllingApproval
    ? clauses.filter((entry) => entry.sourceIndex === controllingApproval.sourceIndex && approvalException(entry))
    : [];
