...
async function crawlCommunity(profileInput, options = {}) {
  ...
  const previousSourceUrls = new Set((previousIndex.sources || []).map(source => canonicalPageUrl(source.sourceUrl)));
  ...
  const contentOwners = new Map((previousIndex.pages || [])
    ...
    .map((page) => [page.contentFingerprint, canonicalPageUrl(page.url)]));
  let pageCount = 0;
  let documentCount = 0;
  ...
  while (queue.length && (pageCount < maxPages || documentCount < maxDocuments)) {
    ...
    try {
      ...
      const duplicateOf = text.length >= 80 && contentOwners.has(contentFingerprint)
        && contentOwners.get(contentFingerprint) !== canonicalPageUrl(finalUrl)
        ? contentOwners.get(contentFingerprint)
        : "";
      if (text.length >= 80 && !duplicateOf) {
        contentOwners.set(contentFingerprint, canonicalPageUrl(finalUrl));
      ...
      pages.push({
        ...
        chunkContentHashes: text.length >= 80 ? chunkText(text).map(hash) : [],
        etag,
        lastModified,
  ...
  for (const page of uniquePages) {
    if (page.indexed || page.duplicateOf || page.lifecycle === "retirement-pending") continue;
  ...
  for (const page of uniquePages) {
    ...
    const owner = pageOwners.get(page.contentFingerprint);
    if (owner && canonicalPageUrl(owner.url) !== canonicalPageUrl(page.url)) {
      page.indexed = false;
      page.duplicateOf = owner.canonicalUrl || owner.url;
    } else {
      pageOwners.set(page.contentFingerprint, page);
    }
  }
  const recordsByContent = new Map();
  for (const record of records) {
    const key = `${record.connectorType}:${record.contentHash}`;
  ...
  const uniqueRecords = [...recordsByContent.values()];
  ...
  const availableUrls = new Set(uniqueRecords.map(record => canonicalPageUrl(record.sourceUrl)));
  const availableChunks = new Map(uniqueRecords.map(record => [record.contentHash, record.id]));
  for (const page of uniquePages) {
    const chunks = page.chunkContentHashes || [];
    const completeChunkCoverage = chunks.length > 0 && chunks.every(contentHash => availableChunks.has(contentHash));
    if (completeChunkCoverage) {
      page.indexedSourceIds = [...new Set(chunks.map(contentHash => availableChunks.get(contentHash)))];
  ...
  const accountedUrls = new Set(uniquePages.filter(page =>
    page.indexed || (page.duplicateOf && availableUrls.has(canonicalPageUrl(page.duplicateOf)))
      || (page.chunkContentHashes?.length > 0 && page.chunkContentHashes.every(contentHash => availableChunks.has(contentHash)))
