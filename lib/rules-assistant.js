...
function shedOverviewAnswer(sources = []) {
  ...
  const subject = approvalClause.text.match(/^([^.!?]+)[.!?]\s+DRC approval is required/i)?.[1]?.trim();
  ...
  const screeningClause = sourceText.match(/Backyard utility sheds must be screened with landscape plantings[^.]+\./i)?.[0];
  const lead = subject
    ? `DRC approval is required for ${subject.toLowerCase()}.`
...
function landscapeScreenOverviewAnswer(sources = []) {
  return sourceClauseOverviewAnswer("landscape screens", sources);
}
...
function underEaveLightingOverviewAnswer(sources = []) {
  ...
  const approvalClauses = clauses.filter((entry) => /subject to DRC approval/i.test(entry.text)).slice(0, 2);
  const cutoffClause = clauses.find((entry) => /under-eave lighting must be turned off by/i.test(entry.text));
  if (!applicationClause || !systemsClause) return sourceClauseOverviewAnswer("under-eave lighting", sources);
  const lead = `Requires an application under the selected source: ${capitalizeFirstLetter(applicationClause.text)}`;
  return structuredHelpfulAnswer(
    lead,
    [systemsClause, ...approvalClauses, cutoffClause]
      .filter(Boolean)
...
function trampolineOverviewAnswer(sources = []) {
  return sourceClauseOverviewAnswer("trampoline", sources);
}
...
function wateringRestrictionOverviewAnswer(query, sources = []) {
  ...
  const lead = asksHandWatering && handWatering
    ...
    : asksRestrictedTime && asksRestrictedSeason
      ? `No. The requested time is inside the restricted window: ${restriction}`
...
function fenceHeightOverviewAnswer(sources = []) {
  ...
  const lead = `It depends on the fence type and lot source. ${cleanSectionTitle(heightSource.title)} states: ${heightClause}.`;
  return structuredHelpfulAnswer(
    lead,
    [styleClause?.text, approvalClause?.text].filter(Boolean),
    "Open the linked official section and confirm the current detail before acting."
  );
}
...
function trashStorageOverviewAnswer(query, sources = []) {
  ...
  const lead = asksTiming
    ? `The controlling source does not give a specific curb-placement or removal time, so it does not specify whether the requested timing is allowed. ${storageClause}`
...
function rvParkingOverviewAnswer(query, sources = []) {
  ...
  const lead = /\bstreet\b/i.test(query) && streetClause
    ...
    : duration && hourLimit && duration.hours > hourLimit
      ? `No. ${capitalizeFirstLetter(duration.label)} exceeds the source limit: ${capitalizeFirstLetter(temporaryClause.text)}`
      : `Yes, temporarily. ${capitalizeFirstLetter(temporaryClause.text)}`;
  return structuredHelpfulAnswer(
    lead,
    [overnightClause, garageClause, circumventionClause]
      .filter(Boolean)
      .map((entry) => capitalizeFirstLetter(entry.text)),
    "Open the linked official section and confirm the current detail before acting."
  );
...
function flagAndPoliticalSignOverviewAnswer(query, sources = []) {
  ...
  if (asksHeight && poleClause) {
    const lead = `The current cited rule does not set a numeric maximum height for a freestanding flagpole. ${capitalizeFirstLetter(poleClause.text)}`;
    ...
    return structuredHelpfulAnswer(
      lead,
      [flagSizeFinding, commercialClause?.text].filter(Boolean),
      "Open the linked official section and confirm the current detail before acting."
    );
  }
...
function simpleTopicOverviewAnswer(query, sources = [], residentQuestion = query) {
  ...
  if (
    ...
    return sourceClauseOverviewAnswer(query, sources);
  }
...
function sourceProjectionAnswer(shortAnswer, findings = [], nextStep = "") {
  const lines = ["Short answer: ".concat(shortAnswer)];
  const usefulFindings = findings.filter(Boolean).slice(0, 3);
  if (usefulFindings.length) lines.push("", "What I found:", ...usefulFindings.map((finding) => "- ".concat(finding)));
  if (nextStep) lines.push("", "Before you act: ".concat(capitalizeFirstLetter(nextStep)));
  return lines.join("\n");
}
...
function conditionalRuleSourceAnswer(query, sources = []) {
  ...
  return sourceProjectionAnswer(exception.text.replace(/^Exception\.\s*/i, ""), [controlling.text]);
}

function sourceDerivedFeeAnswer(query, sources = []) {
  const text = combinedSourceText(sources);
  const year = sources
...
function sourceDerivedEnforcementAnswer(query, sources = []) {
  const text = combinedSourceText(sources);
...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "", residentQuestion = query) {
  ...
  if (!isFeeQuery(query) && !isViolationProcessQuery(query) && !isDelinquentAccountQuery(query)) {
    const answer = sourceGroundedRuleAnswer(query, sources);
    if (answer) return { available: true, answer, sources, strategy: "official-clause" };
  }

  const currentFeeAnswer = sourceDerivedFeeAnswer(query, sources);
  if (currentFeeAnswer) return { available: true, answer: currentFeeAnswer, sources, strategy: "source-projection" };

  const enforcementAnswer = sourceDerivedEnforcementAnswer(query, sources);
  if (enforcementAnswer) return { available: true, answer: enforcementAnswer, sources, strategy: "source-projection" };
