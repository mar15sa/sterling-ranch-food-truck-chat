...
function sourcesMatching(index = {}, matcher, limit = 4) {
  ...
  const conflicts = enforceLedger ? resolveFactLedger(index.factLedger, { factAuthority: index.factAuthority }).unresolvedSensitive : [];
  return (index.sources || []).filter(matcher).filter(source => {
    if (!factIsAnswerable({ ...source, reviewStatus: source.reviewStatus || 'approved' }, now)) return false;
    if (!enforceLedger) return true;
    const entries = index.factLedger.filter(entry => entry.sourceId === source.id && entry.sourceVersion === source.contentHash);
    // These answers use source prose too, so filtering structured facts alone
    // would leak a withheld value from another sentence in the same source.
    if (!entries.length) return !index.factLedger.some(entry => entry.sourceId === source.id)
      && !(source.facts || []).length && ['approved', 'kept-current'].includes(source.reviewStatus);
    if (!entries.every(entry => factIsAnswerable(entry, now))) return false;
    return !conflicts.some(group => group.entries.some(entry => entry.sourceId === source.id && entry.sourceVersion === source.contentHash));
...
function trashHolidayAnswer(question, index, now) {
  if (!/\b(?:trash|garbage|recycling|pickup|pick up|collection)\b/i.test(question)) return null;
  if (!/\b(?:holiday|delay|affect|pickup|pick up|collection)\b/i.test(question)
  ...
  for (const source of sources) {
    ...
    return buildAnswerContract({
      directAnswer: `Yes. The official CAB schedule lists ${named} as a holiday when trash pickup is delayed by one day.`,
