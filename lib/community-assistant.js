...
function approvedOperationalProjectionAnswer(question, result, options = {}) {
  ...
  const directAnswer = preferredAction ? preferredAction.label : directDetail.text;
...
function ensureAnswerCompletion(question, answer) {
  ...
  if (!answer.actions.length && answer.sources.length) {
    // A withheld static sheet remains unavailable as evidence, while its
    // official source is still a useful, explicit handoff for the resident.
    answer = {
      ...answer,
      actions: (answer.sources || []).filter((source) => /^https?:\/\//i.test(source.sourceUrl || ""))
