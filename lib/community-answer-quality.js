...
function directResolutionIssues(question, result, answer) {
  ...
  const issues = [];
  const asksForTime = /\b(?:what|which)\s+time\b|\b(?:latest|how late|what hours?|time restriction|closing time|opening time)\b|\b(?:pool|facility|clubhouse|court|office|operating|operation)\s+(?:hours?|houres)\b|\b(?:hours?|houres)\s+(?:for|on|at|of)\b/i.test(question)
    || /\b(?:when|what time|how late)\b.{0,40}\b(?:open|opens?|close|closes?|hours?)\b/i.test(question)
    || /\b(?:open|opens?|close|closes?|hours?)\b.{0,40}\b(?:when|what time|how late)\b/i.test(question);
  const resolvesTime = /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b|\b(?:no|without) (?:specific )?(?:time|hour) restriction\b|\bdoes not (?:give|set|provide|publish|specify)\b.{0,60}\b(?:time|hour)\b|\b(?:rules?|sources?|pages?)\b.{0,35}\b(?:do not|don't|does not|doesn't)\b.{0,25}\b(?:set|provide|publish)\b.{0,20}\b(?:latest|specific|operating|closing|opening)?\s*(?:time|hours?)\b|\b(?:any time|sunset|sunrise|dusk|dawn)\b/i.test(answer);
