...
function requestFocusedSentences(claims = [], need = {}, maxSentences = 2) {
  ...
  const candidates = claims.flatMap((claim) => String(claim || "").split(/(?<=[.!?])\s+(?=[A-Z0-9])/))
    .map((sentence) => {
      ...
      // The requested time condition outranks repeated subject words in an
      // installation-season overview. Values still come from verified claims.
      if (/\b(?:all night|overnight|past midnight)\b/i.test(request)
        && /\b(?:overnight|nights?)\b|\b\d{1,2}(?::\d{2})?\s*[ap]\.?m\.?/i.test(sentence)) score += 8;
...
function residentFriendlyStorageAnswer(text = "") {
  if (!/\b(?:garage|enclosed structure|screened from view|behind the wing fence)\b/i.test(text)) return "";
  return "Store carts in an enclosed structure or garage, or screen them from view behind the wing fence.";
}

function latestClosingTime(text = "") {
  const matches = [...String(text).matchAll(/(?:-|–|until|to)\s*(\d{1,2})(?::(\d{2}))?\s*(a\.?m\.?|p\.?m\.?)\b/gi)];
  const times = matches.map((match) => {
    const hour = Number(match[1]);
    const minute = Number(match[2] || 0);
    const isPm = /^p/i.test(match[3]);
    return { hour, minute, isPm, value: ((hour % 12) + (isPm ? 12 : 0)) * 60 + minute };
  });
  if (!times.length) return "";
  const latest = times.sort((left, right) => right.value - left.value)[0];
  return `${latest.hour}${latest.minute ? `:${String(latest.minute).padStart(2, "0")}` : ""} ${latest.isPm ? "p.m." : "a.m."}`;
}

function residentFriendlyPermissionAnswer(needText = "", prose = "") {
  if (requestedDurationExceedsLimit(needText, prose)) {
    const hourLimit = prose.match(/\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+(consecutive\s+)?hours?\b/i);
    const overnightLimit = prose.match(/\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+overnights?\b/i);
    const period = prose.match(/\b(?:during|within) any ([a-z0-9-]+(?:[- ]day)? period)\b/i);
    if (hourLimit || overnightLimit) {
      const primary = hourLimit
        ? `${hourLimit[1]} ${hourLimit[2] || ""}hours`.replace(/\s+/g, " ")
        : `${overnightLimit[1]} overnight${/^one$/i.test(overnightLimit[1]) ? "" : "s"}`;
      const nightNote = hourLimit && overnightLimit
        ? ` (${overnightLimit[1]} night${/^one$/i.test(overnightLimit[1]) ? "" : "s"})`
        : "";
      const requestedSubject = needText.match(/\b(?:my|our|your|the)\s+([a-z][a-z-]*)\b.{0,50}\b(?:stay|park|remain|keep)\b/i)?.[1];
      const subject = requestedSubject ? `Your ${requestedSubject}` : "This";
      return `No. ${subject} is limited to ${primary}${nightNote}${period ? ` in any ${period[1]}` : ""}.`;
    }
  }
  const prohibitedUse = prose.match(/\bUnder no circumstance may (?:Owners?(?: or their agents)?|residents?) use Residential Units for (.+?)(?=\.(?:\s+Exception:|$)|$)/i)?.[1];
  if (prohibitedUse) {
    const plainUse = prohibitedUse
      .replace(/hotel purposes\s*\(i\.e\.,\s*rental or leasing on a day-to-day or week-to-week basis\)/i, "hotel-style stays rented by the day or week")
      .replace(/including rental or leasing on a day-to-day or week-to-week basis/i, "including stays rented by the day or week")
      .replace(/including rental on a day-to-day or week-to-week basis/i, "including stays rented by the day or week")
      .replace(/\s+or any similar temporary lodging or living quarter arrangements/i, ", or similar temporary stays")
      .replace(/,\s*,/g, ",")
      .trim();
    return `No. Homes in this community cannot be used for ${plainUse}.`;
  }
  return "";
}

function residentFriendlyOperationalSentence(text = "") {
  return String(text)
    .replace(/\b(one|two|three|four|five|six|seven|eight|nine|ten) \(\d+\) (?=business days?\b)/gi, "$1 ")
    .replace(/\bupon successful completion of this form\b/i, "after you successfully submit the form")
    .replace(/^Please contact ([^\s]+@[^\s]+) for more information\.?$/i, "Questions can go to $1.");
}

function requestedDurationExceedsLimit(request = "", evidence = "") {
  const words = { one: 1, two: 2, three: 3, four: 4, five: 5, six: 6, seven: 7, eight: 8, nine: 9, ten: 10 };
  const requested = String(request).match(/\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+(?:straight\s+|consecutive\s+)?(hours?|days?|nights?|overnights?)\b/i);
  const limit = String(evidence).match(/\b(?:maximum(?: amount of time)?|limit(?:ed)? to|up to)\b.{0,90}\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten)\s+(?:consecutive\s+)?(hours?|days?|nights?|overnights?)\b/i);
  if (!requested || !limit) return false;
  const value = (match) => /^\d+$/.test(match[1]) ? Number(match[1]) : words[match[1].toLowerCase()];
  const hours = (amount, unit) => amount * (/^days?|nights?|overnights?$/i.test(unit) ? 24 : 1);
  return hours(value(requested), requested[2]) > hours(value(limit), limit[2]);
}
...
function supportedAnswerText(result) {
  ...
  if (["status", "schedule", "hours"].includes(result.need.task)) {
    ...
    if (result.need.task === "hours"
      && result.answer.answerMode === "community-recurring-weekday-closing") {
      return direct;
    }
    if (result.need.task === "hours"
      && /\b(?:how late|closing time|what time\b.{0,30}\bclose)\b/i.test(result.need.request || "")) {
      const allAnswerText = [direct, ...(result.answer.keyDetails || []), ...(result.answer.claims || []).map((claim) => claim.text)].join(" ");
      const closingTime = latestClosingTime(allAnswerText);
      if (closingTime) {
        const subject = /\bpool\b/i.test(`${result.need.subjectContext || ""} ${result.need.routeRequest || ""}`) ? "The pool" : "It";
        const subjectWithContext = subject === "The pool" && /\bopen\b.{0,80}\bMemorial Day\b.{0,80}\bLabor Day\b/i.test(allAnswerText)
          ? "When it is open for the season, the pool"
          : subject;
        return `${subjectWithContext} closes at ${closingTime.replace(/\.$/, "")}.`;
      }
    }
    ...
    if (result.need.task === "schedule"
      && /\b(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),\s+[A-Z][a-z]+\s+\d{1,2},\s+\d{4}\b/.test(direct)) {
      return direct.replace(/[—–-]\s*the week of\s+[A-Z][a-z]+\s+\d{1,2},\s+\d{4}\.?$/i, ".");
    }
    return direct;
  }
  ...
  if (result.need.task === "specification"
    && (/\b(?:keep|place|put|store)\b.{0,50}\b(?:bins?|cans?|carts?|containers?)\b/i.test(result.need.request || "")
      || /\b(?:bins?|cans?|carts?|containers?)\b.{0,50}\b(?:kept|placed|put|stored)\b/i.test(result.need.request || ""))) {
    ...
    if (storageSentence) return residentFriendlyStorageAnswer(storageSentence) || storageSentence.trim();
  }
  ...
  if (result.need.task === "permission") {
    const residentFriendly = residentFriendlyPermissionAnswer(
      `${needText} ${result.need.routeRequest || ""}`,
      proseClaims.join(" ")
    );
    if (residentFriendly) return residentFriendly;
    const focused = requestFocusedSentences(proseClaims, result.need, 2);
    if (focused.length) {
      const prose = joinReadableDetails(focused);
      const lead = requestedDurationExceedsLimit(needText, prose)
        || /\b(?:not allowed|may not|must not|prohibited|forbidden|disallowed)\b/i.test(prose)
        || /\bno\b.{0,80}\b(?:is|are|be)\s+allowed\b/i.test(prose)
        ? "No."
        : "";
  ...
  if (result.need.task === "information"
    ...
    const requestsEveryNamedGroup = /\b(?:each|every|all)\s+(?:village|neighbou?rhood|area|location|facility)s?\b/i.test(needText);
    const comparison = /\b(?:versus|while|whereas)\b/i.test(needText)
      || (/\bpublic road\b/i.test(needText) && /\bshared drive\b/i.test(needText));
    // When the resident explicitly asks for every named group, keeping only
    // the single highest-scoring sentence silently turns a complete approved
    // list into a partial answer. Preserve all equally relevant sentences.
    const focused = requestFocusedSentences(proseClaims, result.need, requestsEveryNamedGroup ? 8 : comparison ? 2 : 1);
  ...
  if (result.need.requestedDetails?.includes("contact")
    ...
    if (contactClaim) return residentFriendlyOperationalSentence(focusedClaimText(contactClaim, "contact"));
  }
  ...
  if (result.need.task === "action") {
    ...
    const verifiedActions = (result.evidence.relevantActionIndexes || [])
      .map((index) => result.answer.actions?.[index])
      .filter((action) => String(action?.label || "").trim());
    const asksToSubmit = /\b(?:submit|apply|application)\b/i.test(`${result.need.evidenceFocus || ""} ${result.need.request || ""}`);
    const verifiedAction = (asksToSubmit
      ? verifiedActions.find((action) => /\b(?:submit|apply|application)\b/i.test(action.label || ""))
      : null) || verifiedActions[0];
    ...
    if (result.evidence.supportedDetails?.includes("action") && verifiedAction
      && requestedActionMatches(verifiedAction, result.need)) {
      return `Use “${String(verifiedAction.label).trim()}” below to continue.`;
    }
  ...
  if (result.need.task === "specification") {
    const asksForTiming = /\b(?:how long|how far (?:ahead|in advance)|how many\s+(?:days?|weeks?|months?)|when\b.{0,50}\b(?:hear back|response|reply|reach out|contact|get back)|expect to wait)\b/i
      .test(`${result.need.evidenceFocus || ""} ${result.need.request || ""}`);
    if (asksForTiming) {
      ...
      if (timingClaims.length) return joinReadableDetails(timingClaims.slice(0, 2).map(residentFriendlyOperationalSentence));
    }
...
function requestedActionMatches(action = {}, need = {}) {
  const label = `${action.label || ""} ${action.context || ""} ${action.evidence?.label || ""} ${action.evidence?.context || ""}`.toLowerCase().replace(/one[- ]sheet/g, "application form");
  const request = String(need.evidenceFocus || need.request || "").toLowerCase();
  return [
    [/\bmenu\b/i, /\bmenu\b/i],
    [/\b(?:apply|application|form)\b/i, /\b(?:apply|application|form|one[- ]sheet)\b/i],
    ...
    [/\b(?:set\s*up|setup|start|activate|activation|connect|get\s+(?:online|connected))\b/i, /\b(?:set\s*up|setup|start|activate|activation|connect)\b/i],
...
function actionRelevanceScore(action = {}, need = {}) {
  ...
  return (overlap * 2)
    ...
    + (/\bsubmit\b/i.test(request) && /\bsubmit\b/i.test(label) ? 6 : 0)
...
function composedAnswer(results, actions, outcome) {
  ...
  if (unresolved.length) {
    ...
    if (stillUnresolved.length === 1) {
      const unresolvedText = stillUnresolved[0].need.text || stillUnresolved[0].need.request;
      if (outcome === "ambiguous" && /^(?:community\s+)?(?:services?|utilities?)[?.!]*$/i.test(String(unresolvedText).trim())) {
        lines.push("I need one more detail before I can check this accurately: which community service do you mean?");
      } else {
        const lead = outcome === "ambiguous" ? "I need one more detail before I can check this accurately" : "I couldn’t verify this part yet";
        lines.push(`${lead}: ${unresolvedText}.`);
      }
  ...
  const bestAction = actions.find((action) => results.some((result) => requestedActionMatches(action, result.need))) || actions[0];
  const actionAddsRequestedValue = bestAction && results.some((result) => requestedActionMatches(bestAction, result.need));
  const actionAddsProactiveValue = bestAction && results.some((result) => proactivePermissionActionMatches(bestAction, result.need));
  const alreadyNamesAnAction = actions.some((action) => renderedSupport.includes(String(action.label || "").toLowerCase()));
  if (actions.length && (unresolved.length || actionAddsRequestedValue || actionAddsProactiveValue)
    && !alreadyNamesAnAction) {
    lines.push(`Next step: ${bestAction.label}.`);
  }
...
async function runNeedFirstShadow(contract, answerNeed, {
  ...
  // The existing page consumes this compatibility field. Derive it from the
  // final proof, never from a writer's self-assessment or fluent wording.
  const proofSourceIds = new Set(candidateSources.map(sourceId));
  const canAnswer = preliminaryOutcome === "complete" && outcome === "complete" && finalAssessment.outcome === "complete"
    && finalAssessment.needs.length > 0 && claims.length > 0
    && claims.every(claim => claim.verified === true && claim.evidenceSourceIds?.length > 0
      && claim.evidenceSourceIds.every(id => proofSourceIds.has(id)));
  return {
    isTest: true,
    answerMode,
    answerStatus: answerStatusForOutcome(outcome, finalResults),
    confidence: { canAnswer, confidence: canAnswer ? "high" : "low",
      reason: canAnswer ? "verified-need-evidence-complete" : "incomplete-need-evidence" },
    answer,
    directAnswer,
    keyDetails,
