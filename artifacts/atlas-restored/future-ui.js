...
export function createFutureMap({places,roots,point,details,onChange}) {
  ...
  let area='all', selected=null, zoom=1, focus=null, selectedDetail=null;const changed=()=>onChange?.();
  ...
  function chooseArea(name){area=name==='all'||areas.some(a=>a.name===name)?name:'all';selected=null;selectedDetail=null;zoom=1;focus=area==='all'?null:areaFocus(area);render();changed();}
  function select(id){const p=futureRoot(id,places);if(!p)return;selected=p.id;selectedDetail=id;area=p.village;zoom=1;focus=futureFocus(p,roots,point);render();const child=document.getElementById('project-detail-'+id);if(child){child.closest('details').open=true;child.setAttribute('tabindex','-1');child.focus({preventScroll:true});}else $('#future-note').focus({preventScroll:true});landscape.scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});changed();}
  panel.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.futureArea)chooseArea(b.dataset.futureArea);if(b.dataset.futureProject)select(b.dataset.futureProject);if('futureReset'in b.dataset)chooseArea('all');if(b.dataset.futureZoom){zoom=Math.max(1,Math.min(3,zoom+Number(b.dataset.futureZoom)));applyCamera();changed();}});
  $('#future-area').addEventListener('change',e=>chooseArea(e.target.value));addEventListener('resize',applyCamera);
  render();return {render,select,state:()=>({area,selected:selectedDetail||selected,zoom}),restore(s={}){area=s.area==='all'||areas.some(a=>a.name===s.area)?s.area:'all';const p=futureRoot(s.selected,places);selected=p?.id||null;selectedDetail=p?s.selected:null;if(p)area=p.village;zoom=Number.isFinite(s.zoom)?Math.max(1,Math.min(3,s.zoom)):1;focus=p?futureFocus(p,roots,point):area==='all'?null:areaFocus(area);render();const child=document.getElementById('project-detail-'+selectedDetail);if(child)child.closest('details').open=true;}};
}
