...
function datedFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    if (stale) {
      // Dated facility times are operational facts. Once their evidence is
      // stale, do not repeat the old schedule as though it remains current.
      // Keep only a minimal citation and direct the resident to the official
      // page while the source-refresh workflow reconfirms it.
...
function hasAuthoritativeSpecification(answer) {
  ...
  const rendered = /#[0-9]{2,}\b|\b(?:color|colour|paint|stain|finish|material|height|size|dimension|setback|distance)\b.{0,180}\b(?:approved|required|requires|must|match|prohibited|not allowed|no more than|at least|feet|foot|inches|#[0-9]{2,})\b/i.test(text);
  const admittedMissing = /\b(?:does not|doesn't|could not|couldn't)\b.{0,100}\b(?:provide|publish|expose|include|list|name|specify|state)\b.{0,80}\b(?:color|colour|paint|stain|finish|material|height|size|dimension|setback|distance|product)\b/i.test(text);
  return answer?.confidence?.canAnswer === true && explicitlyScoped && rendered && !admittedMissing;
}
...
function renderPartialGap(answer, completion) {
  ...
  if (!["verified-partial", "missing-evidence"].includes(completion?.outcome) || !completion.missingDetails.length
    || /\b(?:could not|couldn't|cannot|can't)\b.{0,100}\b(?:verify|confirm)\b/i.test(current)) return current;
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (routingPlan && ACTION_GOALS.has(routingPlan.goal)) {
    ...
    const actionSource = goalSources.find((source) => source.connectorType === "official-action"
