...
function isPromptInjectionQuery(query) {
  const text = String(query || "");
  return (
    /\b(ignore|disregard|forget|override|bypass)\b.{0,60}\b(instructions?|rules?|rulebook|prompt|system|developer|previous|above)\b/i.test(text) ||
    /\b(system|developer)\s+prompt\b/i.test(text) ||
    /\b(reveal|show|print|repeat|expose)\b.{0,50}\b(prompt|instructions?|secret|secrets|api key|token|password)\b/i.test(text) ||
    /\b(for every|all|each|every)\s+(answer|response|reply)\b.{0,80}\b(begin|start|say|reply|respond|prefix|include)\b/i.test(text) ||
    /\b(reply|respond|answer)\b.{0,40}\b(exactly|only|with)\b.{0,80}\b(every|all|future|response|answer)\b/i.test(text) ||
    /\bpretend\s+(?:that\s+)?you\s+are\b/i.test(text) ||
    /\bdo\s+not\s+(?:use|follow|cite)\b.{0,40}\b(rulebook|rules?|sources?|instructions?)\b/i.test(text)
  );
}

function isGreetingOnlyQuery(query) {
  return /^\s*(hi|hello|hey|howdy|good\s+(morning|afternoon|evening))[!.?\s]*$/i.test(
    String(query || "")
  );
}
  ...
  return sources
    ...
    .map((source) => {
      ...
      if (source.isSourceDerivedExcerpt && excerpt) {
        return `- ${title}: ${shortenText(excerpt, 6500)}`;
      }
...
function currentFactExcerpt(query, source) {
  const structuredSummary = String(source.summaryText || "");
  VOLATILE_FACT_PATTERN.lastIndex = 0;
  if (structuredSummary && VOLATILE_FACT_PATTERN.test(structuredSummary)) {
    return shortenText(structuredSummary, 6500);
  }
...
function stableLeadFromAnswer(answer) {
  ...
  const sentences = firstParagraph
    ...
    .filter(Boolean)
    .flatMap((sentence) => {
      if (!summaryHasVolatileFacts(`Short answer: ${sentence}`)) return [sentence];
      VOLATILE_FACT_PATTERN.lastIndex = 0;
      const firstVolatile = VOLATILE_FACT_PATTERN.exec(sentence);
      if (firstVolatile && firstVolatile.index > 0) {
        const stablePrefix = sentence
          .slice(0, firstVolatile.index)
          .replace(/,\s+(?:with|for|of|up to|no more than)\b[^,]*$/i, "")
          .replace(/\b(?:an aggregate limit of|a maximum of|at least|no more than|up to)\s*$/i, "")
          .trim();
        if (
          stablePrefix.length >= 18 &&
          !summaryHasVolatileFacts(`Short answer: ${stablePrefix}`)
        ) {
          return [stablePrefix.endsWith(".") ? stablePrefix : `${stablePrefix}.`];
        }
      }
      const stableClauses = [];
      for (const [index, clause] of sentence.split(/,\s+|;\s+/).entries()) {
        if (
          !summaryHasVolatileFacts(`Short answer: ${clause}`) &&
          (index === 0 || /^(?:and|but)\b|^[A-Z]/.test(clause))
        ) {
          stableClauses.push(clause);
        }
      }
      const prefix = stableClauses.join(", ").trim();
      return prefix.length >= 18 ? [prefix.endsWith(".") ? prefix : `${prefix}.`] : [];
    });
...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "") {
  const factSources = sources.flatMap((source) => {
    ...
    return excerpt ? [{ ...source, excerpt, isSourceDerivedExcerpt: true }] : [];
  }).filter((source, index, all) => {
    const key = `${source.title || ""}\n${source.excerpt || ""}`;
    return all.findIndex((candidate) => `${candidate.title || ""}\n${candidate.excerpt || ""}` === key) === index;
  });
  const hasCurrentFacts = factSources.some((source) => {
    const excerpt = String(source.excerpt || "");
    VOLATILE_FACT_PATTERN.lastIndex = 0;
    return VOLATILE_FACT_PATTERN.test(excerpt);
  });
  ...
  return {
    ...
    answer: helpfulAnswer(
      stableLead
        ? `${stableLead} The current official source supplies the changing dates, amounts, and limits shown below.`
        : "The current official source contains the controlling dates, amounts, and limits shown below.",
      factSources,
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (isPromptInjectionQuery(question)) {
    return {
      answer:
  ...
  if (isGreetingOnlyQuery(question)) {
    return {
      answer: "Hi! Ask me a question about Sterling Ranch community rules, and I'll look for the official source.",
  ...
  const needsSourceDerivedFacts = summaryHasVolatileFacts(plainAnswer);
  ...
  const rewrittenAnswer = await rewriteAnswerWithLLM(
    question,
    derived.answer,
    derived.sources
  );
  ...
  return {
    answer: rewrittenAnswer || derived.answer,
