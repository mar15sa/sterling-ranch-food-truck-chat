...
function reconcileCommunityIndex(trusted, candidate, options = {}) {
  ...
  const factLedger = buildFactLedger({ ...trusted, sources: refreshedTrustedSources }, {
    ...
    trustedVersions,
    // Once a ledger exists, a newly interpreted claim needs its own review.
    // An unchanged page hash does not approve a parser's changed claim scope.
    requirePriorReview: Array.isArray(trusted.factLedger),
  });
