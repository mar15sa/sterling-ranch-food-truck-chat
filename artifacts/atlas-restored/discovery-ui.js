...
export function createDiscovery({ places, roots, trails, models, located, selected, select, showRoute, setView, routeSvg, focusVillage, focusState }) {
  const $ = s => document.querySelector(s), byId = new Map(places.map(p=>[p.id,p]));
  let lens = 'all', village = 'all', unfolded = false;
  const lenses = [['all','All places','layers'],['play','Play & parks','play'],['everyday','Everyday stops','coffee'],['walks','Walks','walk'],['future','Coming soon','future']];
  ...
  function chooseLens(next) { unfolded=false;if(next==='future'){lens='all';renderSheets();setView('future');return;}lens=next;setView('neighborhood');render(); }
  function chooseVillage(next) { village=next; unfolded=false;focusVillage(next);setView('neighborhood');render();$('#landscape').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'}); }
  ...
  function renderSheets() {
    ...
    stage.classList.toggle('unfolded',unfolded);$('#landscape').inert=unfolded;$('#destination-strip').hidden=unfolded;$('#map-discovery').hidden=unfolded||lens==='all'&&(village==='all'||focusState()?.kind==='village');
    ...
    if(!unfolded)return;
    sheets.innerHTML=unfoldedScene({places,trails,models,routeSvg});
  }
  function render() {
  ...
  toolbar.addEventListener('click',e=>{const b=e.target.closest('button');if(b?.id==='unfold-map'){unfolded=!unfolded;render();if(unfolded)stage.scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'});}});
  ...
  return { render, collapse(){unfolded=false;renderSheets();},reset(){lens='all';village='all';unfolded=false;render();}, reveal(id) { unfolded=false;const p=byId.get(id);if(p&&!rootsInView().some(x=>x.id===p.id)){lens='all';village='all';}if(lens==='walks'||lens==='future')lens='all'; }, state:()=>({lens,village,unfolded}) };
}
