...
function isOfficialCommunitySource(source = {}) {
  return source.isOfficialResource === true || /^https:\/\//i.test(source.sourceUrl || "");
}
...
function configuredCommunityResource(index = {}, profile = {}, candidates = []) {
  const current = candidates.find((source) => source?.sourceUrl && source.lifecycle !== "stale" && source.lifecycle !== "withheld");
