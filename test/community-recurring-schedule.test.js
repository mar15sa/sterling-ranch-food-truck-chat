...
test("expired current evidence can support a past date only when captured on or after that date", () => {
  ...
  assert.equal(capturedProjectionCoversPastDate(source, "2026-09-07", "2026-09-11T12:00:00Z", "America/Denver", true), true);
  assert.equal(capturedProjectionCoversPastDate(source, "2026-09-10", "2026-09-11T12:00:00Z", "America/Denver", true), false);
  assert.equal(capturedProjectionCoversPastDate({ ...source, canonicalScopedProjection: false }, "2026-09-07", "2026-09-11T12:00:00Z", "America/Denver", true), false);
  assert.equal(capturedProjectionCoversPastDate(source, "2026-09-07", "2026-09-11T12:00:00Z", "America/Denver", false), false);
});
