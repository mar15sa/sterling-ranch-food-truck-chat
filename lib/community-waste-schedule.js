...
async function getWasteSchedule(options = {}) {
  ...
  try {
    ...
    if (!selectedArea || !hasPublicReference(selectedArea)) {
    ...
    const reference = selectedArea.officialReferenceLocation;
    const cacheKey = `${adapter.adapterId}:${selectedArea.label}`;
    ...
    const datedServiceAreas = serviceAreas.map((area) => area.label === selectedArea.label ? { ...area, date: pickup.day } : area);
    const checkedAt = new Date().toISOString();
    const sourceUrl = adapter.endpoints.find((endpoint) => endpoint.id === "pickup-calendar")?.url || calendarEndpoint;
    const evidence = emitEvidenceEnvelope(adapter, {
      observedAt: checkedAt,
      ...
      sources: [{ id: "live-calendar", sourceUrl, controllingSourceRole: "operational", checkedAt }],
      claims: [
        { id: "service-area-date", facet: "date", text: pickup.day, controllingEvidenceId: `${adapter.adapterId}:live-calendar`, controllingSourceRole: "operational" },
      ],
