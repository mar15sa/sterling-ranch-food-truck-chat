...
function sourceMatchesRequestedFacility(question, source = {}) {
  ...
  if (source.canonicalFactualProjection === true && source.reviewedSubjectMatch === true
    && hasOriginalOperationalSubject(withoutFacilityNouns, source, { requireAll: true })) return true;
...
function originalQuestionNeedsRule(question = "") {
  ...
  return details.includes('permission') || normativeSpecification || generalResidentFees
...
function hasOriginalOperationalSubject(question, source = {}, { requireAll = false } = {}) {
  ...
  const evidence = normalize(`${source.title || ''} ${(source.facts || []).filter(fact => fact.approvalClaim)
    .map(fact => `${fact.context || fact.value || ''} ${fact.subjectKey || ''}`).join(' ')}`);
  return requireAll
    ? subjectTerms.every(term => tokens(evidence).includes(term))
    : subjectTerms.some(term => evidence.includes(term));
}
...
function coveredDetails(requested, sources, actions, question = "") {
  ...
  return requested.filter((detail) => {
    ...
    if (detail === "price") return facts.some((fact) => fact.type === "money"
      || (fact.facet === 'fee' && /\b(?:free|no (?:additional )?(?:charge|cost))\b/i.test(fact.context || fact.value || '')));
...
function approvedOperationalProjectionAnswer(question, result, options = {}) {
  ...
  const requestedLeadFact = options.preferredLeadFact || (instructionalRequestShape(question).check
  ...
  const detailCandidates = (requestedLeadFact && options.excludePreferredLeadFromDetails
    ...
    : details).sort((left, right) => Number(right.explicitlyScoped) - Number(left.explicitlyScoped));
  let groupedDetails = [];
