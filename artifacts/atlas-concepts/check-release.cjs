...
(async()=>{
 ...
 const unchanged=[];for(const file of paths){const before=execFileSync('git',['show',baseline+':'+file]);const current=fs.readFileSync(file);const normal=b=>file.endsWith('.png')?b:Buffer.from(b.toString('utf8').replace(/\r\n/g,'\n'));assert.deepEqual(normal(current),normal(before),file+' changed');unchanged.push({file,sha256:crypto.createHash('sha256').update(normal(current)).digest('hex')});}
