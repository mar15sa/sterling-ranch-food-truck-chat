...
const { availableSectionContext, contextIdentity, sectionVersionKey } = require("./rules-section-context");
const {
  composePlainEnglishFallback,
  getRulesLlmMode,
...
const {
  ...
  claimModalities,
  dateTimePhrases,
  explicitDimensionBindings,
  focusedDimensionDetails,
  numericTokens,
  numberWordsToDigits,
  unsupportedQuestionLocations,
...
const SYNONYMS = {
  ...
  halloween: ["holiday", "seasonal", "decorative", "lighting"],
...
const SEMANTIC_CONCEPTS = [
  ...
  {
    name: "holiday-displays",
    boost: 175,
    groupsCompoundTerms: true,
    queryPatterns: [
      /\b(holiday|hallowe+en|hallowen|christmas|thanksgiving|hanukkah|easter|valentine(?:'s)?)\b/i,
      /\b(decor|decorat(?:e|ed|ing|ions?)|decoratons?|displays?|inflatables?)\b/i,
    ],
    sourcePatterns: [
      /\bHoliday displays\b/i,
      /\bPermitted\s+\d+\s+days prior to a holiday\b/i,
    ],
  },
  {
    ...
    boost: 150,
    queryPatterns: [
      /\b(holiday|halloween|christmas|seasonal|summer|june|july|october|january)\b/i,
      /\b(lights?|lighting|decorat(?:e|ed|ing|ions?))\b/i,
    ],
    sourcePatterns: [
...
function municodeVersionMetadata(job = {}) {
  return {
    latestJobId: normalizeJobId(job),
...
function sameMunicodeVersion(source = {}, observed = {}) {
  return Boolean(observed.latestJobId)
    && String(source.latestJobId || "") === String(observed.latestJobId)
    && String(source.codifiedThrough || "") === String(observed.codifiedThrough || "")
    && String(source.onlineUpdateDate || "") === String(observed.onlineUpdateDate || "")
    && String(source.onlinePostDate || "") === String(observed.onlinePostDate || "");
}
...
async function createRulesIndex(options = {}) {
  ...
  const job = await (options.fetchLatestJob || fetchLatestJob)();
  const observedVersion = municodeVersionMetadata(job);
  const jobId = observedVersion.latestJobId;
  if (!jobId) throw new Error("The official online source did not return a latest job ID.");

  const existing = options.existingIndex || await loadRulesIndex(indexPath);
  if (existing?.documents?.length && sameMunicodeVersion(existing.source, observedVersion)) {
    const reused = {
      ...existing,
      source: {
        ...existing.source,
        ...
        lastVersionCheckedAt: checkedAt,
        lastRefreshMode: "municode-version-unchanged",
      },
    };
    await writeIndex(reused, indexPath);
    return reused;
  }

  const toc = await (options.fetchToc || fetchToc)();
  ...
  const sectionDocs = await (options.fetchAllSectionDocs || fetchAllSectionDocs)(contentNodes, jobId);
  ...
  const index = {
    ...
    source: {
      ...
      lastFetchedAt: checkedAt,
      lastVersionCheckedAt: checkedAt,
      lastFullFetchedAt: checkedAt,
      lastRefreshMode: "municode-version-changed",
...
const PROJECT_TERM_SYNONYMS = Object.freeze({
  ...
  halloween: ["holiday", "seasonal decorative"],
...
function namedProjectEvidenceGuard(question, sources = []) {
  ...
  if (isPorchPatioLightingQuery(question) || isHolidayDisplayQuery(question) || isResidentSharingFixtureQuery(question)) {
...
function isLandscapeScreenQuery(query) {
  ...
  return !/\b(?:trash|garbage|recycl(?:e|ing)|bins?|containers?|windows?|film|tint|fenc(?:e|es|ing))\b/i.test(text) &&
    /\b(?:privacy|landscape)\s+screens?\b|\bscreens?\s+(?:privacy|landscape)\b|\bprivacy\s+screening\b/i.test(text);
}
...
function isTrashStorageQuery(query) {
  return /\b(trash|garbage|recycling|bins?|containers?)\b/i.test(String(query || "")) &&
    /\b(store|stored|storage|keep|leave|left|overnight|night|curb|pickup|collection|bring|put out|outside|screens?|screened|screening|enclosures?)\b/i.test(String(query || ""));
}
...
function isSeasonalLightingQuery(query) {
  const text = String(query || "");
  const mentionsSeasonalLighting = /\b(lights?|lighting)\b/i.test(text)
    && /\b(holiday|hallowe+en|hallowen|seasonal|decorative|christmas|summer|june|july|october|january|pride|red white blue|timeframe|season|window)\b/i.test(text);
  if (!mentionsSeasonalLighting) return false;
  const mentionsPermanentSystem = /\b(permanent|hardwired|under[-\s]?eaves?|soffits?|roofline|gemstone|jellyfish)\b/i.test(text);
  const asksForTiming = /\b(when|dates?|timeframe|season|window|june|july|october|january|turn(?:ed)? on|energize|stay|remain|installed|remove|year(?:-round)?)\b/i.test(text);
  return !mentionsPermanentSystem || asksForTiming;
}

function isHolidayDisplayQuery(query) {
  const text = String(query || "");
  const mentionsHoliday = /\b(holiday|hallowe+en|hallowen|christmas|xmas|thanksgiving|hanukkah|easter|valentine(?:'s)?)\b/i.test(text);
  const mentionsDisplay = /\b(decor|decorat(?:e|ed|ing|ions?)|decoratons?|displays?|inflatables?|sound effects?|generators?)\b/i.test(text);
  return mentionsHoliday && mentionsDisplay && !/\b(lights?|lighting)\b/i.test(text);
}

function isLandscapeApplicationQuery(query) {
  const text = String(query || "");
  return /\b(?:landscap(?:e|ing)|yard|irrigation)\b/i.test(text) &&
    /\b(?:application|apply|form|forms|packet|paperwork|submit|submittal)\b/i.test(text) &&
    !/\b(?:fee|fees|cost|price|how much)\b/i.test(text);
}

function isLightingRelatedQuery(query) {
  const text = String(query || "");
  return /\b(light|lights|lighting|fixture|fixtures|string lights?|gemstone|jellyfish|under[-\s]?eaves?|soffits?|eave\/rake|eave|rake|hardwired|roofline)\b/i.test(text);
}
...
function isOutdoorDecorativeObjectQuery(query) {
  return !isHolidayDisplayQuery(query) && /\b(ornament|ornaments|yard art|(?:lawn )?decorations?|decorative object|decorative objects|garden statue|statue|statues)\b/i.test(query);
}
...
function applyIntentBoosts(document, query) {
  ...
  if (isHolidayDisplayQuery(query)) {
    if (/\(47\)\s*Holiday displays\b/i.test(combined)) score += 280;
    if (/Permitted\s+\d+\s+days prior to a holiday/i.test(combined)) score += 180;
    if (/Install and energize seasonal decorative lighting/i.test(combined)) score -= 160;
  }
...
function searchRulesIndex(index, query, limit = 5) {
  ...
  for (const document of scoredDocuments) {
    const sectionKey = sectionVersionKey(document);
    ...
    if (document.score > existing.bestChunkScore) {
      // The winning text must retain its own identity and review metadata.
      // Only ranking statistics may accumulate across same-version chunks.
      bySection.set(sectionKey, {
        ...document,
        bestChunkScore: document.score,
        matchStats: existing.matchStats,
        supportScore: existing.supportScore,
        score: document.score + existing.supportScore,
      });
      continue;
    }
...
function uniqueSources(results) {
  ...
  for (const result of results) {
    const sectionKey = sectionVersionKey(result);
...
function holidayDisplaySources(index, query) {
  const documents = (index.documents || [])
    .filter((document) => documentEligibleForQuery(document, query))
    .filter((document) => /^Sec\. 21-22\. - General community standards/i.test(document.title || ""));
  const primaryIndex = documents.findIndex((document) => /\(47\)\s*Holiday displays\b/i.test(document.text || ""));
  if (primaryIndex < 0) return [];
  const primary = documents[primaryIndex];
  const continuation = documents.find((document) =>
    document.nodeId === primary.nodeId &&
    /removal dates of holiday displays may be extended/i.test(document.text || "")
  );
  const source = sourceFromDocument(primary, query, 205);
  source.text = cleanText([primary.text, continuation?.text].filter(Boolean).join("\n"));
  source.excerpt = makeExcerpt(source.text, expandQueryTerms(query), 700);
  return [source];
}
...
function specialSourcesForQuestion(index, query) {
  ...
  if (isResidentSharingFixtureQuery(query)) {
    return sourcesByTitleAndText(
      index,
      query,
      /^Sec\. 21-22\. - General community standards/i,
      [/any contemplated improvement not\s+listed here/i],
      1
    );
  }
  ...
  if (/\bsolar\b/i.test(query)) {
    return sourcesByTitle(
      index,
      query,
      [/^Sec\. 21-22.*\(b\)\(89\).*Solar energy devices and systems/i],
      1
    );
  }
  ...
  if (isPlantListQuestion(query) || isPlantPermissionQuestion(query)) {
  ...
  if (isHolidayDisplayQuery(query)) {
    return holidayDisplaySources(index, query);
  }
...
function sentenceStartingWithMatch(text = "", pattern) {
  const flattened = cleanText(text).replace(/\n/g, " ");
  const match = flattened.match(pattern);
  if (!match || match.index === undefined) return "";
  return flattened.slice(match.index).split(/(?<=[.!?])\s+/)[0].trim();
}
...
function fullTextForSource(index, source) {
  ...
  return availableSectionContext(source, index?.documents || []).text;
}
...
function specificExcerptForQuestionSource(query, source, fullText = source.text || "") {
  ...
  if ((isPlantListQuestion(query) || isPlantPermissionQuestion(query)) && /5-131|preapproved plant list/i.test(title)) {
    const introduction = sentenceStartingWithMatch(text, /The following preapproved plant list organizes acceptable/i);
    const lowerWater = sentenceStartingWithMatch(text, /In an effort to promote sustainable water management/i);
    const additionalSpecies = sentenceStartingWithMatch(text, /While this list represents preapproved and recommended plant material/i);
    const genericPlantTerms = new Set([
      "general", "plant", "plants", "planting", "plantings", "tree", "trees", "shrub", "shrubs",
      "bush", "bushes", "flower", "flowers", "grass", "grasses", "perennial", "perennials", "vine", "vines", "like",
      "grow", "have", "allowed", "approved", "acceptable", "permitted", "along", "beside", "near",
      "fence", "fences", "fencing", "line", "wall", "yard", "backyard", "front", "rear", "side",
    ]);
    const requestedTerms = tokenize(query)
      .map((term) => term.endsWith("berries") ? `${term.slice(0, -3)}y` : term)
      .filter((term) => term.length >= 4 && !genericPlantTerms.has(term));
    // Preserve table rows while finding the requested plant. Some merged OCR
    // chunks flatten table boundaries, which previously let one match pull
    // unrelated plants and measurements into answer synthesis.
    const flattenedPlantText = cleanText(text);
    const plantRows = [
      ...flattenedPlantText.matchAll(
        /\b([A-Z][A-Z0-9&/'".\-]*(?:\s+[A-Z][A-Z0-9&/'".\-]*){1,14})\s+(\d+(?:\.\d+)?['"]\s*[xX]\s*\d+(?:\.\d+)?['"])\s+(Evergreen|Deciduous|Ornamental|Shrub|Perennial|Grass|Vine|Rose)\b/g
      ),
    ];
    const matchingRow = plantRows.find((match) =>
      requestedTerms.some((term) => textContainsTerm(match[0], term))
    );
    ...
    const matchingLine = lines.findIndex((line) =>
      requestedTerms.some((term) => textContainsTerm(line, term))
    );
    let namedPlantEvidence = matchingRow?.[0] || "";
    if (!namedPlantEvidence && matchingLine >= 0) {
      const measurementLine = /^\d+(?:\.\d+)?['"]\s*x\s*\d/i;
      const typeLine = /^(?:Evergreen|Deciduous|Ornamental|Shrub|Perennial|Grass|Vine|Rose)$/i;
      const headerLine = /^(?:Botanical|Common|Ht x Spd|Type|Bird Friendly|\(\d+\))$/i;
      let start = matchingLine;
      while (
        start > 0 &&
        matchingLine - start < 4 &&
        !measurementLine.test(lines[start - 1]) &&
        !typeLine.test(lines[start - 1]) &&
        !headerLine.test(lines[start - 1])
      ) start -= 1;
      let end = matchingLine;
      let foundMeasurement = measurementLine.test(lines[end]);
      while (end + 1 < lines.length && end - matchingLine < 5) {
        end += 1;
        foundMeasurement = foundMeasurement || measurementLine.test(lines[end]);
        if (foundMeasurement && typeLine.test(lines[end])) break;
        if (headerLine.test(lines[end])) { end -= 1; break; }
      }
      namedPlantEvidence = lines.slice(start, end + 1).join(" ");
    }
    return shortenText(
      [namedPlantEvidence, introduction, lowerWater, additionalSpecies].filter(Boolean).join(" "),
      1500
    );
  }
  ...
  if (isResidentSharingFixtureQuery(query) && /21-22|general community standards/i.test(title)) {
    return shortenText(
      subsectionSnippet(text, /\(1\)\s*An alphabetical list of common improvements/i, /\(2\)\s*No additions, alteration or renovations/i) ||
        sentenceContaining(text, /any contemplated improvement not\s+listed here/i),
      900
    );
  }
...
function decorateSourcesForQuestion(query, sources, index) {
  return sources.map((source) => {
    const context = availableSectionContext(source, index?.documents || [], {
      eligible: (document) => documentEligibleForQuery(document, query) && !ownerReviewWithholdsQuestion(document, query),
    });
    const fullText = context.text;
    // Plant-list retrieval already selects the chunk containing the requested
    // row. Keep that row structure instead of merging every same-section chunk
    // before projection; merged OCR chunks can flatten table boundaries.
    const projectionText = (isPlantListQuestion(query) || isPlantPermissionQuestion(query))
      ? source.text || fullText
      : fullText;
    const specificExcerpt = specificExcerptForQuestionSource(query, source, projectionText);
    ...
    const useQuestionSpecificClause = Boolean(specificExcerpt) && (
      ...
      isResidentSharingFixtureQuery(query) ||
      isPlantListQuestion(query) ||
      isPlantPermissionQuestion(query) ||
    ...
    return {
      ...source,
      text: fullText,
      evidenceContext: { ...context, text: undefined },
      excerpt,
...
function designReviewResourceSourcesForQuestion(query, answerSources = [], answerText = "") {
  if (
    ((isAmenityReservationQuery(query) || semanticConceptMatchesQuery("rental-cancellations", query)) &&
      !isLandscapeOverviewQuery(query) &&
      !isLandscapeApplicationQuery(query)) ||
    isMovableOutdoorBelongingsQuestion(query)
  ) {
    return [];
  }
  ...
  const questionNeedsDesignReviewResource =
    ...
    isLandscapeOverviewQuery(questionText) ||
    isLandscapeApplicationQuery(questionText) ||
    /\b(?:submit|send|complete)\b.{0,100}\b(?:DRC|design review|application|plan|packet)\b/i.test(answerText) ||
    /\badditional species will be considered by\b.{0,60}\bDRC\b/i.test(answerText) ||
  ...
  if (/\bfenc(?:e|es|ing)\b/i.test(questionText) && !isPlantPermissionQuestion(questionText)) {
    add("standard-3-rail-fencing");
  }
  ...
  if (/\b(landscap\w*|turf|irrigation|garden|planting)\b/i.test(questionText) || isPlantPermissionQuestion(questionText)) {
    add("landscape-submittal");
  }
  if (
...
function isFacilityProviderEligibilityQuery(query = "") {
  return /\b(?:contractor|vendor|caterer)\b/i.test(query)
    && /\b(?:event|facility|clubhouse|overlook|great hall|pavilion|shelter)\b/i.test(query)
    && /\b(?:can|may|allowed|eligible|permitted|outside|non[\s-]*resi(?:dent|dnet))\b/i.test(query);
}

function sourceSupportsFacilityProviderEligibility(source = {}) {
  const evidence = `${source.title || ""} ${source.excerpt || ""} ${source.text || ""}`;
  return /\b(?:contractor|vendor|caterer)\b/i.test(evidence)
    && /\b(?:event|clubhouse|overlook|great hall|pavilion|shelter)\b/i.test(evidence)
    && /\b(?:resident|nonresident|non-resident|outside)\b/i.test(evidence);
}
...
function assessAnswerConfidence(query, results, sources) {
  ...
  if (
    isFacilityProviderEligibilityQuery(query)
    && !sources.slice(0, 3).some(sourceSupportsFacilityProviderEligibility)
  ) {
    return {
      canAnswer: false,
      confidence: "low",
      reason: "no-single-source-support",
      withholdSources: true,
    };
  }
  ...
  if (
    ...
    (
      ...
      ((isPlantListQuestion(query) || isPlantPermissionQuestion(query)) && /preapproved plant list/i.test(`${questionSpecificSources[0]?.title || ""} ${questionSpecificText}`))
    )
  ) {
  ...
  if (
    isHolidayDisplayQuery(query) &&
    sources.slice(0, 3).some((source) => /\bHoliday displays\b/i.test(`${source.text || ""} ${source.excerpt || ""}`))
  ) {
    return { canAnswer: true, confidence: "high", reason: "current-holiday-display-rule" };
  }
...
function rulesEvidenceSourceId(source = {}) {
  return String(source.id || source.nodeId || "").trim();
}

function normalizedEvidenceToken(value = "") {
  const token = String(value).toLowerCase();
  const numericWord = numberWordsToDigits(token);
  if (numericWord.length === 1 && /^[a-z]+(?:-[a-z]+)?$/.test(token)) return numericWord[0];
  if (/^(?:turn|turns|turned|turning)$/.test(token)) return "turn";
  if (/^(?:install|installs|installed|installing|installation|installations)$/.test(token)) return "install";
  if (/^(?:submit|submits|submitted|submitting|submission|submissions)$/.test(token)) return "submit";
  if (/^(?:require|requires|required|requiring)$/.test(token)) return "require";
  if (/^(?:approve|approves|approved|approving|approval|approvals)$/.test(token)) return "approve";
  if (/^(?:light|lights|lighting)$/.test(token)) return "light";
  if (/^(?:shed|sheds)$/.test(token)) return "shed";
  if (/^(?:backyard|yard|yards)$/.test(token)) return "yard";
  if (/^(?:foot|feet|(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)-foot)$/.test(token)) return "foot";
  if (/^(?:grade|ground|level)$/.test(token)) return "grade";
  if (/^(?:enough|room|square|footage)$/.test(token)) return "capacity";
  return token.endsWith("s") && token.length > 4 ? token.slice(0, -1) : token;
}

const GENERIC_RULE_EVIDENCE_TERMS = new Set([
  "allow", "allowed", "approve", "cab", "drc", "has", "measured", "must", "need", "not", "permit", "permitted", "prohibit", "prohibited", "require", "rule", "shall", "up", "will", "you",
]);
...
function evidenceTerms(value = "") {
  const expanded = String(value || "")
    ...
    .replace(/\b(zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety)-foot\b/gi, "$1 foot");
  return new Set(tokenize(expanded).map(normalizedEvidenceToken).filter((term) =>
    !GENERIC_RULE_EVIDENCE_TERMS.has(term)
  ));
}

function evidencePassageForClaim(claimText, sourceText) {
  const claimTerms = evidenceTerms(claimText);
  const claimNumbers = [...new Set([...numericTokens(claimText), ...numberWordsToDigits(claimText)])];
  const sentences = String(sourceText).replace(/\s+/g, " ")
    .split(/(?<=[.!?])\s+/)
    .map((sentence) => sentence.trim())
    .filter(Boolean);
  if (!claimTerms.size || !sentences.length) return String(sourceText);
  const scored = sentences.map((sentence) => ({
    sentence,
    score: [...evidenceTerms(sentence)].filter((term) => claimTerms.has(term)).length,
  }));
  const best = Math.max(...scored.map((item) => item.score), 0);
  const bestText = scored.filter((item) => item.score === best).slice(0, 9).map((item) => item.sentence).join(" ");
  const bestTextNumbers = new Set([...numericTokens(bestText), ...numberWordsToDigits(bestText)]);
  const bestTextCoversNumbers = claimNumbers.every((number) => bestTextNumbers.has(number));
  // Prefer the directly matching sentence when it already carries the same
  // permission or requirement. Pulling in a neighboring exception or ban can
  // otherwise reverse the meaning of an independently supported claim.
  if (bestText && bestTextCoversNumbers && modalityIsSupported(claimText, bestText)) return bestText;
  const selected = new Set();
  scored.forEach((item, index) => {
    if (item.score !== best || selected.size >= 9) return;
    for (const nearby of [index - 1, index, index + 1]) {
      if (nearby >= 0 && nearby < sentences.length) selected.add(nearby);
    }
    // Numbered rule lists often put the obligation in a parent heading such as
    // "Garden boxes shall:" and the actual condition several items later. Keep
    // that heading with a matching list item so proof does not lose the subject
    // or turn a real requirement into an unsupported fragment.
    if (!modalityIsSupported(claimText, item.sentence)) {
      for (let headingIndex = index - 1; headingIndex >= Math.max(0, index - 12); headingIndex -= 1) {
        const heading = sentences[headingIndex];
        if (!/\bshall:\s*(?:\d+\.)?$/i.test(heading)) continue;
        const headingTerms = evidenceTerms(heading);
        const headingOverlap = [...headingTerms].filter((term) => claimTerms.has(term)).length;
        if (headingOverlap >= Math.min(2, headingTerms.size)) selected.add(headingIndex);
        break;
      }
    }
  });
  // A single resident sentence may summarize several rows from the same
  // official schedule. Add the strongest matching sentence for every claimed
  // number so the proof passage covers the whole sentence without weakening
  // the later exact-number and topic checks.
  for (const number of claimNumbers) {
    const matching = scored
      .map((item, index) => ({ ...item, index }))
      .filter((item) => new Set([...numericTokens(item.sentence), ...numberWordsToDigits(item.sentence)]).has(number))
      .sort((left, right) => right.score - left.score || left.index - right.index)[0];
    if (matching && matching.score > 0) selected.add(matching.index);
  }
  return [...selected].sort((left, right) => left - right).map((index) => sentences[index]).join(" ") || String(sourceText);
}

function comparableRuleClaim(text = "") {
  return String(text)
    .replace(/^Short answer\s*:\s*/i, "")
    .replace(/^The cited official rule requires a specific step\s*:\s*/i, "")
    .replace(/^\d{4}\s+CAB service fees\s*:\s*/i, "")
    .replace(/\s+/g, " ")
    .trim();
}

function modalityIsSupported(claimText, sourceText) {
  const claim = claimModalities(claimText);
  const source = claimModalities(sourceText);
  if (claim.prohibition && !source.prohibition) return false;
  if (claim.permission && !source.permission
    && !/\b(?:install|energize|operate|use)\b.{0,120}\bapproved\b/i.test(sourceText)) return false;
  if (claim.approvalRequired && !source.approvalRequired) return false;
  if (claim.approvalNotRequired && !source.approvalNotRequired) return false;
  if (claim.required && !source.required
    && !(source.prohibition && /\b(?:outside|away from|clear of)\b/i.test(claimText))) return false;
  if (claim.optional && !source.optional) return false;
  return true;
}

function sourceSupportsRenderedRuleClaim(claimText, source = {}, residentQuestion = "") {
  const sourceText = [source.title, source.text || source.excerpt].filter(Boolean).join(" ");
  if (!sourceText.trim()) return false;
  const comparableClaim = comparableRuleClaim(claimText);
  if (
    isMovableOutdoorBelongingsQuestion(residentQuestion) &&
    /^Sec\. 1-38\. - Household items/i.test(source.title || "") &&
    /\b(?:furniture|items? of personal property|belongings)\b/i.test(comparableClaim) &&
    /\broadway\b/i.test(comparableClaim) &&
    /\bwalkway\b/i.test(comparableClaim) &&
    /\bOwner's Lot\b/i.test(sourceText) &&
    /\broadway, walkway\b/i.test(sourceText)
  ) return true;
  const normalizedClaim = comparableClaim.toLowerCase();
  const normalizedSource = sourceText.replace(/\s+/g, " ").trim().toLowerCase();
  if (normalizedSource.includes(normalizedClaim)) return true;
  // Exact, deterministic grammar equivalence preserves a source clause's
  // evidence binding after cleanup (including numbered rulebook fragments).
  // Do not use a general similarity threshold to grant this equivalence.
  const displayedClauses = sourceText.replace(/\s+/g, " ")
    .split(/(?<=[.;!?])\s+(?=[A-Z\d])/)
    .map(clause => plainLanguageSourcePart(clause.replace(/^\d+\.\s*/, "").replace(/;$/, "")).toLowerCase());
  if (displayedClauses.includes(normalizedClaim)) return true;
  const supportingPassage = evidencePassageForClaim(comparableClaim, sourceText);
  if (!modalityIsSupported(comparableClaim, supportingPassage)) return false;
  const sourceNumbers = new Set([...numericTokens(supportingPassage), ...numberWordsToDigits(supportingPassage)]);
  if ([...new Set([...numericTokens(comparableClaim), ...numberWordsToDigits(comparableClaim)])].some((value) => !sourceNumbers.has(value))) return false;
  const sourceDates = new Set(dateTimePhrases(supportingPassage));
  if (dateTimePhrases(comparableClaim).some((value) => !sourceDates.has(value))) return false;
  const claimTerms = evidenceTerms(comparableClaim);
  const sourceTerms = evidenceTerms(supportingPassage);
  if (!claimTerms.size) {
    const questionTerms = evidenceTerms(residentQuestion);
    const fullSourceTerms = evidenceTerms(sourceText);
    const questionOverlap = [...questionTerms].filter((term) => fullSourceTerms.has(term)).length;
    return questionOverlap >= Math.min(2, questionTerms.size) && questionTerms.size > 0;
  }
  const overlap = [...claimTerms].filter((term) => sourceTerms.has(term)).length;
  return overlap >= Math.min(2, claimTerms.size) && overlap / claimTerms.size >= 0.6;
}

function renderedRuleClaimSentences(directAnswer = "", keyDetails = [], answer = "", allowAnswerFallback = false) {
  const fallbackAnswer = directAnswer || (allowAnswerFallback
    ? String(answer).match(/(?:^|\n)Short answer\s*:\s*([^\n]+)/i)?.[1] || ""
    : "");
  return [...new Set([fallbackAnswer, ...keyDetails]
    .flatMap((value) => String(value || "").split(/(?<=[.!?])\s+(?=[A-Z])/))
    .map((value) => value.trim())
    .filter((value) => value && !/^(?:[A-Z]\.|(?:And\s+)?\d+\.)$/i.test(value)))];
}

function isScopedSourceAbsenceClaim(text = "") {
  return /^The selected (?:controlling )?source does not\b.{0,180}\b(?:give|set|state|specify|provide|publish|list|name|identify|show|contain|cover|establish)\b/i.test(String(text).trim());
}

function scopedSourceAbsenceIsSupported(text = "", source = {}) {
  if (!isScopedSourceAbsenceClaim(text)) return false;
  const sourceText = [source.title, source.text, source.excerpt].filter(Boolean).join(" ");
  // A scoped negative statement is proof about the inspected source version,
  // not a claim that no rule exists anywhere. Currently the only approved
  // absence facet is an exact clock time, which is invalidated if one appears.
  if (/\b(?:time|hour)\b/i.test(text)) {
    return !/\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b/i.test(sourceText);
  }
  return false;
}

function buildRulesEvidenceClaims({ directAnswer = "", keyDetails = [], answer = "", sources = [], actions = [], residentQuestion = "", allowAnswerFallback = false } = {}) {
  const claims = renderedRuleClaimSentences(directAnswer, keyDetails, answer, allowAnswerFallback).map((text) => {
    const scopedAbsence = isScopedSourceAbsenceClaim(text);
    const evidenceSourceIds = (scopedAbsence ? sources.slice(0, 1) : sources)
      .filter((source) => sourceSupportsRenderedRuleClaim(text, source, residentQuestion))
      .map(rulesEvidenceSourceId)
      .filter(Boolean);
    const scopedEvidenceSourceIds = scopedAbsence && !evidenceSourceIds.length
      ? sources.slice(0, 1).filter((source) => scopedSourceAbsenceIsSupported(text, source)).map(rulesEvidenceSourceId).filter(Boolean)
      : evidenceSourceIds;
    return scopedEvidenceSourceIds.length ? {
      text,
      evidenceSourceIds: [...new Set(scopedEvidenceSourceIds)],
      verified: true,
      ...(scopedAbsence ? { kind: "source-scope-boundary" } : {}),
    } : null;
  }).filter(Boolean);

  const rendered = String(answer || "").toLowerCase();
  for (const action of actions) {
    const actionUrl = String(action.url || "").replace(/\/$/, "").toLowerCase();
    const matchingSources = sources.filter((source) => actionUrl
      && String(source.sourceUrl || "").replace(/\/$/, "").toLowerCase() === actionUrl);
    for (const source of matchingSources) {
      const title = String(source.title || "").trim();
      if (!title || !rendered.includes(title.toLowerCase())) continue;
      claims.push({ text: title, evidenceSourceIds: [rulesEvidenceSourceId(source)], verified: true });
    }
  }
  return claims.filter((claim) => claim.evidenceSourceIds.every(Boolean));
}
...
function buildPlainAnswer(query, results, index, sourcesForAnswer = []) {
  ...
  if (
    ...
    const approval = combinedSourceText(topSources).replace(/\s+/g, " ")
      .match(/Approval must be obtained from the DRC prior to any construction or landscaping\./i)?.[0];
    if (approval) {
      return helpfulAnswer(
...
function sourceFactWindowScore(query, source, excerpt) {
  ...
  if (isHolidayDisplayQuery(query) && /\bHoliday displays\b|\bdays prior to a holiday\b/i.test(excerpt)) score += 180;
...
function dynamicFactPassages(query, source) {
  ...
  if (isHolidayDisplayQuery(query)) {
    patterns.unshift(/\(47\)\s*Holiday displays/i);
    patterns.push(/Permitted\s+\d+\s+days prior to a holiday/i);
  }
  if (isPorchPatioLightingQuery(query)) {
...
function holidayDisplayOverviewAnswer(sources = []) {
  const text = combinedSourceText(sources).replace(/\s+/g, " ");
  const complete = (value) => /[.!?]$/.test(String(value || "").trim())
    ? String(value).trim()
    : `${String(value || "").trim()}.`;
  const timingClause = text.match(/Permitted\s+\d+\s+days prior to a holiday and must be removed within\s+\d+\s+days after the holiday/i)?.[0];
  if (!timingClause) return "";
  const appropriateClause = text.match(/Holiday and seasonally appropriate/i)?.[0];
  const soundClause = text.match(/Turned off between the hours of[\s\S]{0,300}?such as music, generators, inflation systems, or similar items/i)?.[0];
  const extensionClause = text.match(/At the sole discretion of the CAB, removal dates of holiday displays may be extended due to extreme weather conditions or other factors/i)?.[0];
  return structuredHelpfulAnswer(
    capitalizeFirstLetter(complete(timingClause)),
    [appropriateClause, soundClause, extensionClause]
      .filter(Boolean)
      .map((clause) => capitalizeFirstLetter(complete(clause))),
...
function sourceGroundedRuleAnswer(query, sources = [], index = null) {
  ...
  const unmatchedProject = projectRequest && !isResidentSharingFixtureQuery(query)
...
function isSourceEvidenceBoundaryAnswer(answer = "") {
  return /^(?:Short answer:\s*)?(?:I don't have enough rulebook evidence to answer that confidently\.|I could not verify whether|The cited source does not confirm whether|The official passage I found mentions|The selected (?:official passages|rules|official passage).*(?:do not|does not|but))/i.test(
    String(answer || "")
  );
}
...
const PLANT_QUERY_FILLER_TERMS = new Set([
  "general", "plant", "plants", "planting", "plantings", "tree", "trees", "shrub", "shrubs",
  "bush", "bushes", "flower", "flowers", "grass", "grasses", "perennial", "perennials", "vine", "vines", "like",
  "grow", "have", "allowed", "approved", "acceptable", "permitted", "along", "beside", "near",
  "fence", "fences", "fencing", "line", "wall", "yard", "backyard", "front", "rear", "side",
  "zone", "zones", "easement", "easements", "rule", "rules", "restriction", "restrictions", "requirement", "requirements",
]);

function requestedPlantTerms(query = "") {
  return tokenize(query)
    .map((term) => term.endsWith("berries") ? `${term.slice(0, -3)}y` : term)
    .filter((term) => term.length >= 4 && !PLANT_QUERY_FILLER_TERMS.has(term));
}

function namedPlantRow(query, sources = []) {
  const requestedTerms = requestedPlantTerms(query);
  if (!requestedTerms.length) return null;
  const measurementLine = /^\d+(?:\.\d+)?['"]\s*x\s*\d/i;
  const typeLine = /^(Evergreen|Deciduous|Ornamental|Shrub|Perennial|Grass|Vine|Rose)$/i;
  const rowBoundary = /^(?:Botanical|Common|Ht x Spd|Type|Bird Friendly|\(\d+\)|(?:Low|Moderate|High) water need.*)$/i;

  for (const source of sources) {
    const lines = String(source.fullText || source.text || "")
      .split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean);
    for (let measurementIndex = 0; measurementIndex < lines.length; measurementIndex += 1) {
      if (!measurementLine.test(lines[measurementIndex])) continue;
      const classification = lines.slice(measurementIndex + 1, measurementIndex + 3).find((line) => typeLine.test(line));
      if (!classification) continue;
      let start = measurementIndex - 1;
      while (start > 0 && measurementIndex - start < 9 && !typeLine.test(lines[start - 1]) && !rowBoundary.test(lines[start - 1])) {
        start -= 1;
      }
      const nameLines = lines.slice(start, measurementIndex).filter((line) => line !== "✓");
      if (!nameLines.length) continue;

      // The source table keeps the botanical name in the first line (or first
      // two OCR-split lines), followed by an optional quoted cultivar and then
      // the complete common name. Preserve that structure instead of guessing
      // the common-name boundary from whichever query word happened to match.
      let commonStart = 1;
      const firstBotanicalParts = nameLines[0].split(/\s+/).filter((part) => !/^(?:TM|®)$/i.test(part));
      if (firstBotanicalParts.length < 2 && nameLines[1]) commonStart = 2;
      while (commonStart < nameLines.length && /^['"].*['"](?:\s+(?:TM|®))?$/i.test(nameLines[commonStart])) {
        commonStart += 1;
      }
      const botanicalName = nameLines.slice(0, commonStart).join(" ");
      const commonName = nameLines.slice(commonStart).join(" ");
      if (!commonName) continue;
      const searchableName = `${botanicalName} ${commonName}`;
      if (!requestedTerms.some((term) => textContainsTerm(searchableName, term))) continue;
      return {
        botanicalName,
        commonName: commonName.toLowerCase().replace(/\b[a-z]/g, (letter) => letter.toUpperCase()),
        classification: classification.toLowerCase(),
        evidence: [...nameLines, lines[measurementIndex], classification].join(" "),
      };
    }
  }
  return null;
...
function questionSpecificSourceAnswer(query, sources = []) {
  ...
  const asksApprovalOrPermission = /\b(?:approval|approve|permission|permit(?:ted)?|allowed|can i|may i|do i need)\b/i.test(query);
  const asksExplicitSpecification = /\b(?:what|which)\b.{0,45}\b(?:color|colour|paint|stain|finish|material|height|size|dimension|setback|distance)\b/i.test(query);
  const approvalRequired = (entry) => /\b(?:drc|design review)\s+approval\s+is\s+required\b/i.test(entry.text);
  const approvalException = (entry) => /\b(?:drc|design review)\s+approval\s+is\s+not\s+required\b/i.test(entry.text);
  const approvalLimitation = (entry) => /\b(?:pre-?approved|acceptable|limit(?:ed|s|ing)?)\b/i.test(entry.text);
  if (isPlantPermissionQuestion(query) && !isPlantListQuestion(query) && requestedPlantTerms(query).length) {
    const row = namedPlantRow(query, sources);
    const approvalStatus = combinedSourceText(sources).match(/\bpreapproved\b/i)?.[0]?.toLowerCase();
    if (row && approvalStatus) {
      const locationBoundary = unsupportedQuestionLocations(query, sources).length
        ? [completeSentence(["The selected source", "does not confirm", "the requested location"].join(" "))]
        : [];
      return structuredHelpfulAnswer(
        completeSentence([row.commonName, "is", "a", approvalStatus, row.classification].join(" ")),
        locationBoundary,
        "Open the linked official section if you need the complete wording."
      );
    }
    const additionalSpeciesProcess = sentenceContaining(
      combinedSourceText(sources),
      /\badditional species will be considered\b/i
    );
    return structuredHelpfulAnswer(
      "The cited source does not confirm whether the requested plant is included.",
      additionalSpeciesProcess ? [additionalSpeciesProcess] : [],
      "Open the linked official section and confirm the current detail before acting."
    );
  }
  const controllingApproval = (asksApprovalOrPermission || asksExplicitSpecification)
    ? clauses.find(approvalRequired)
    : null;
  const primaryEntry = controllingApproval || clauses[0];
  const sameSourceClauses = controllingApproval
    ? clauses.filter((entry) => entry.sourceIndex === controllingApproval.sourceIndex)
    : clauses;
  const relevantExceptions = sameSourceClauses
    .filter(approvalException)
    .sort((left, right) => Math.abs(left.order - primaryEntry.order) - Math.abs(right.order - primaryEntry.order))
    .slice(0, 1);
  const relevantLimitations = sameSourceClauses.filter(approvalLimitation);
  const relevantRequirements = sameSourceClauses
    .filter((entry) => entry !== primaryEntry && !approvalException(entry) && /\b(?:must|shall|required)\b/i.test(entry.text))
    .slice(0, 1);
  const supportingClauses = sameSourceClauses.filter(
    (entry) => !approvalException(entry) || relevantExceptions.includes(entry)
  );
  let primary = primaryEntry.text;
  let findings = [
    ...relevantExceptions,
    ...relevantLimitations,
    ...relevantRequirements,
    ...supportingClauses,
    ...clauses.filter((entry) => entry.sourceIndex !== primaryEntry.sourceIndex),
  ]
  ...
  const lead = isPlantListQuestion(query) || isPlantPermissionQuestion(query)
...
function seasonalLightingOverviewAnswer(query = "", sources = []) {
  ...
  const permanentSystemQuestion = /\b(?:permanent|hardwired|soffit|eave|rake|gemstone|jellyfish)\b/i.test(query)
    && /\b(?:stay|remain|installed|remove|year)\b/i.test(query);
  if (permanentSystemQuestion && removalClause) {
    const permanentParts = removalClause.text.match(/(?:all\s+)?(hardwired[^,.]*lighting).*?must be returned to (the settings allowed in[^.]+)\./i);
    const nonHolidaySettings = permanentParts?.[2]
      .replace(/the settings allowed in/i, "the non-holiday settings in")
      .replace(/\s+above$/i, "");
    const permanentLead = permanentParts
      ? `Yes, ${permanentParts[1].toLowerCase()} can stay installed year-round, but outside the approved seasonal periods it must be returned to ${nonHolidaySettings}.`
      : `Yes. ${capitalizeFirstLetter(removalClause.text)}`;
    return structuredHelpfulAnswer(
      permanentLead,
      [
        "Temporary string lighting and installation clips must be removed outside those seasonal periods.",
        cutoffClause?.text,
      ].filter(Boolean).map((item) => capitalizeFirstLetter(item)),
      "Open the linked official section if you need the complete wording."
    );
  }
  const halloweenQuestion = /\bhalloween\b/i.test(query);
  if (halloweenQuestion) {
    const halloweenPeriod = periodClause.text.match(/from\s+(October\s+\d{1,2})\s+(?:through|to)\s+(January\s+\d{1,2})/i);
    const halloweenLead = halloweenPeriod
      ? `For Halloween lights, the approved seasonal period starts ${halloweenPeriod[1]} and runs through ${halloweenPeriod[2]}.`
      : capitalizeFirstLetter(periodClause.text);
    return structuredHelpfulAnswer(
      halloweenLead,
      [
        removalClause?.text,
      ...
      /\bdecorat(?:e|ed|ing|ions?)\b/i.test(query) && !/\b(?:lights?|lighting)\b/i.test(query)
...
function evidenceFacetOverviewAnswer(query, sources = [], facetMatchers = []) {
  const clauses = sourceClauseCandidates(query, sources, { questionSpecificOnly: false });
  const selected = [];
  for (const matcher of facetMatchers) {
    const clause = clauses.find((entry) => matcher.test(entry.text)
      && !selected.some((current) => current.normalized === entry.normalized));
    if (clause) selected.push(clause);
  }
  if (!selected.length) return sourceClauseOverviewAnswer(query, sources);
  const remaining = selected.slice(1);
  const groupSize = Math.max(1, Math.ceil(remaining.length / 3));
  const groupedFindings = [];
  for (let index = 0; index < remaining.length; index += groupSize) {
    groupedFindings.push(remaining.slice(index, index + groupSize)
      .map((entry) => capitalizeFirstLetter(entry.text))
      .join(" "));
  }
  const lead = capitalizeFirstLetter(selected[0].text);
  return structuredHelpfulAnswer(
    lead,
    groupedFindings,
    "Open the linked official section if you need the complete wording."
...
function landscapeOverviewAnswer(sources = []) {
  const clauses = sourceClauseCandidates("landscape yard requirements", sources, { questionSpecificOnly: false });
  const sourceText = sources.map((source) => source.text || source.excerpt || "").join(" ").replace(/\s+/g, " ");
  const plan = clauses.find((entry) => /Landscape and irrigation plans[\s\S]{0,180}must be submitted[\s\S]{0,180}(?:review and approval|DRC)/i.test(entry.text));
  const frontTree = sourceText.match(/One deciduous tree \(minimum two-inch caliper\)\./i)?.[0];
  const frontSecondTree = sourceText.match(/One evergreen tree \(minimum six feet in height\) or one ornamental tree \(minimum 1½-inch caliper\)\./i)?.[0];
  const frontCoverage = sourceText.match(/80 percent live coverage\./i)?.[0];
  const backTrees = sourceText.match(/Backyard landscaping also must include two trees as specified below:/i)?.[0];
  const backDeciduous = sourceText.match(/One deciduous tree \(minimum 1½-inch caliper\)\./i)?.[0];
  const backEvergreen = sourceText.match(/One evergreen tree \(minimum six to eight feet in height\)\./i)?.[0];
  const backCoverage = sourceText.match(/backyard must contain a minimum of 30 percent live plant material \(groundcovers, turf, grasses, perennials, shrubs, trees, etc\.\)\./i)?.[0];
  const irrigation = clauses.find((entry) => /automatic, underground irrigation system is required/i.test(entry.text));
  const care = sourceText.match(/(?:All\s+)?Landscaping is to be kept healthy[^.]*\./i)?.[0]?.trim() || "";
  const reviewLabel = [
    plan?.text.match(/\bDRC\b/i)?.[0] || "",
    plan?.text.match(/\breview\b/i)?.[0] || "",
  ].filter(Boolean).join(" ");
  if (!plan) return sourceClauseOverviewAnswer("landscape yard requirements", sources);
  return structuredHelpfulAnswer(
    `${reviewLabel}: ${capitalizeFirstLetter(plan.text)}`,
    [
      frontTree && frontSecondTree && frontCoverage
        ? `Front yard design and installation: ${frontTree} ${frontSecondTree} ${frontCoverage}`
        : "",
      backTrees && backDeciduous && backEvergreen && backCoverage
        ? "Backyard landscaping: At least two trees are required: one deciduous tree (minimum 1½-inch caliper) and one evergreen tree (minimum six to eight feet high). The yard must include at least 30% live plant material."
        : "",
      irrigation || care
        ? `Irrigation and upkeep: ${[
          irrigation ? "An automatic, underground irrigation system is required for all parts of the yard that require water." : "",
          care ? "Landscaping is to be kept healthy." : "",
        ].filter(Boolean).join(" ")}`
        : "",
    ],
    "Open the linked official section if you need the complete wording."
...
function parksOpenSpaceOverviewAnswer(sources = []) {
  const text = combinedSourceText(sources).replace(/\s+/g, " ");
  const sentence = (pattern) => text.match(pattern)?.[0]?.trim() || "";
  const hours = sentence(/Parks and Open Space are open from \d{1,2}:\d{2}\s*a\.m\. to \d{1,2}:\d{2}\s*p\.m\. daily\./i);
  const dogs = sentence(/Dog owners must leash and have physical control of their dog at all times\./i);
  const vehicles = sentence(/Motorized vehicles are prohibited in parks, trails, and open space\./i);
  const glass = sentence(/Glass containers, littering, dumping and misuse of public property are prohibited\./i);
  const permittedFires = sentence(/Fires are only permitted within charcoal burning grills provided at park shelters, or within liquid-fueled or gas-fueled grills\/stoves/i);
  const openSpaceFires = sentence(/Fires in Open Space areas are strictly prohibited\./i);
  const swimming = sentence(/Swimming or the use of watercraft and\/or floatation devices is not permitted on CAB ponds and open water\./i);
  const fishing = sentence(/Fishing is permitted in CAB ponds[^.]+\./i);
  const permit = sentence(/A CAB fishing permit shall be required[^.]+\./i);
  const camping = sentence(/Camping in any CAB Park, Open Space, tract or public area is prohibited except for during designated community events/i);
  if (!hours) return sourceClauseOverviewAnswer("parks trails open space rules", sources);
  return structuredHelpfulAnswer(
    hours,
    [
      dogs ? `Dogs: ${[dogs, vehicles].filter(Boolean).join(" ")}` : vehicles,
      [glass, permittedFires, openSpaceFires, swimming].filter(Boolean).length ? `Use and safety: ${[glass, permittedFires && `${permittedFires}.`, openSpaceFires, swimming].filter(Boolean).join(" ")}` : "",
      [fishing, permit, camping].filter(Boolean).length ? `Fishing and camping: ${[fishing, permit, camping && `${camping}.`].filter(Boolean).join(" ")}` : "",
    ],
    "Open the linked official section if you need the complete wording."
  );
}

function solarDesignReviewOverviewAnswer(sources = []) {
  const text = combinedSourceText(sources).replace(/\s+/g, " ");
  const approval = text.match(/DRC approval is required for any solar installation or system on the exterior of the home or on the Lot\./i)?.[0];
  if (!approval) return sourceClauseOverviewAnswer("solar energy devices and systems", sources);
  return structuredHelpfulAnswer(
    capitalizeFirstLetter(approval),
    [],
    "Open the linked official section and confirm the current detail before acting."
...
function landscapeScreenOverviewAnswer(sources = []) {
  ...
  const attachment = text.match(/Multiple screens may not be attached[^.]+\./i)?.[0];
  ...
  const material = text.match(/Materials must be constructed[^.]+\.(?:\s*If other materials[^.]+\.)?(?:\s*Vinyl is not permitted\.)?/i)?.[0];
  const finish = text.match(/Landscape screens may be either painted or stained\.[^.]+\.[^.]+\.(?:\s*Wood grain is allowed to show through\.)?/i)?.[0];
  if (!approval) return sourceClauseOverviewAnswer("landscape screens", sources);
  const lead = approval;
  const size = [height, width].filter(Boolean).join(" ");
  const placement = [attachment, freestanding, locationAndCount]
    .filter(Boolean)
    .join(" ")
    .replace(/are not allowed in easements/i, "must remain outside easements");
  const materials = [transparency, material, finish]
    .filter(Boolean)
    .join(" ");
  return structuredHelpfulAnswer(
    capitalizeFirstLetter(lead),
    [size, placement, materials],
    "Open the linked official section if you need the complete wording."
  );
}

function underEaveLightingOverviewAnswer(query = "", sources = []) {
  ...
  const asksHolidayOnlyApplication = /\b(?:application|apply|submit|approval)\b/i.test(query) &&
    /\b(?:holiday|seasonal)\b/i.test(query) &&
    /\b(?:only|just)\b/i.test(query);
  const lead = asksHolidayOnlyApplication
    ? `Yes—${capitalizeFirstLetter(approvalClause.text)} ${capitalizeFirstLetter(systemsClause.text)}`
    : `${capitalizeFirstLetter(systemsClause.text)} ${capitalizeFirstLetter(approvalClause.text)}`;
  return structuredHelpfulAnswer(
    lead,
...
function vegetableGardenOverviewAnswer(sources = []) {
  ...
  const residentPlacementClause = placementClause.replace(/rear or side yards\./i, "rear or side yard.");
  const boxSetbackClause = boxesClause && setbackClause
    ? `${boxesClause.replace(/shall:$/i, "must")} ${setbackClause.toLowerCase()}`
    : setbackClause;
  return structuredHelpfulAnswer(
    capitalizeFirstLetter(approvalClause),
    [residentPlacementClause, boxSetbackClause]
      .filter(Boolean)
...
function simpleTopicOverviewAnswer(query, sources = [], residentQuestion = query) {
  const text = combinedSourceText(sources);
  const inlineText = text.replace(/\s+/g, " ");

  if (
    isMovableOutdoorBelongingsQuestion(residentQuestion) &&
    /Furniture, electrical cords, bicycles, barbecues, toys, or other items of personal property/i.test(inlineText)
  ) {
    const placement = inlineText.match(/No furniture, electrical cords, bicycles, barbecues, toys, or other items of personal property[^.]+Owner's Lot[.;]/i)?.[0];
    const access = inlineText.match(/All roadways and walkways must be clear for emergency traffic\./i)?.[0];
    if (placement) {
      return structuredHelpfulAnswer(
        capitalizeFirstLetter(/[.!?]$/.test(placement) ? placement : `${placement.replace(/[;:]$/, "")}.`),
...
function residentFeeOverviewAnswer(sources = []) {
  ...
  return [
    ...
    "",
    "What I found:",
    `- Typical utility fixed monthly charges are water ${formatMoney(amounts.waterBase)}, sewer ${formatMoney(amounts.sewerBase)}, and stormwater ${formatMoney(amounts.stormwater)}.`,
    `- Other typical fixed monthly charges are streetlight ${formatMoney(amounts.streetlight)} and trash ${formatMoney(amounts.trash)}.`,
...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "", residentQuestion = query) {
  if (isHolidayDisplayQuery(residentQuestion) || isHolidayDisplayQuery(query)) {
    const answer = holidayDisplayOverviewAnswer(sources);
    if (answer) return { available: true, answer, sources, strategy: "structured" };
  }

  if (isPlantListQuestion(query) || isPlantPermissionQuestion(query) || isOutdoorDecorativeObjectQuery(query)) {
    const answer = focusedTopicAnswer(query, sources, structuredHelpfulAnswer);
    if (answer) return { available: true, answer, sources, strategy: "structured" };
  }

  if (
    isStateParksPassQuestion(residentQuestion) ||
    isStateParksPassQuestion(query) ||
    isMovableOutdoorBelongingsQuestion(residentQuestion) ||
    isMovableOutdoorBelongingsQuestion(query)
  ) {
  ...
  if (/\bsolar\b/i.test(query)) {
    const answer = solarDesignReviewOverviewAnswer(sources);
    if (answer) return { available: true, answer, sources, strategy: "structured" };
  }

  if (isSeasonalLightingQuery(query)) {
    const answer = seasonalLightingOverviewAnswer(query, sources);
    if (answer) return { available: true, answer, sources, strategy: "structured" };
  }

  if (isUnderEaveLightingQuery(query)) {
    const answer = underEaveLightingOverviewAnswer(residentQuestion, sources);
    if (answer) return { available: true, answer, sources, strategy: "structured" };
  }
  ...
  if (isHotTubQuery(query)) {
    ...
    if (approvalClause) {
      const approvalLead = approvalClause.text
        .split(/(?<=[.!?])\s+/)
        .find((sentence) => /DRC approval is required/i.test(sentence)) || approvalClause.text;
      return {
        available: true,
        answer: structuredHelpfulAnswer(capitalizeFirstLetter(approvalLead), [placementClause?.text].filter(Boolean), "Open the linked official section and confirm the current detail before acting."),
        sources,
        strategy: "official-clause",
      };
    }
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const publicSportCourtOperations = /\b(?:pickle ?ball|sport court)\b/i.test(question)
    && (/\b(?:community|neighborhood|public)\b/i.test(question)
      || /\b(?:play|hours|open play|reserve|reservation)\b/i.test(question))
    && !isPrivateSportCourtQuery(question);
  if (publicSportCourtOperations) {
    return {
      answer: unclearAnswer([]),
      answerMode: "source-evidence-boundary",
      answerVerdict: "unverified",
      inputClassification: input.classification,
      confidence: { canAnswer: false, confidence: "high", reason: "no-single-source-support" },
      reviewNeeded: false,
      sources: [],
      actions: [],
      claims: [],
  ...
  if (searchMode === "ai-hybrid") {
    const publicSportCourtQuestion = /\b(?:pickle ?ball|sport court)\b/i.test(question)
      && !isPrivateSportCourtQuery(question);
    if (input.classification === INPUT_CLASSIFICATIONS.RULES_QUESTION
      && !isAmenityReservationQuery(question)
      && !publicSportCourtQuestion) {
      const groundedAnswer = await answerRulesQuestion(question, {
        ...options,
        searchMode: "legacy",
        llmMode: Object.prototype.hasOwnProperty.call(options, "llmMode") ? options.llmMode : "off",
      });
      const completeGroundedAnswer = groundedAnswer.confidence?.canAnswer === true
      ...
      const definitiveEmptyBoundary = groundedAnswer.answerMode === "source-evidence-boundary"
        && groundedAnswer.confidence?.canAnswer === false
        && groundedAnswer.confidence?.reason === "no-single-source-support"
        && !(groundedAnswer.sources || []).length
        && !(groundedAnswer.actions || []).length;
      if (completeGroundedAnswer || definitiveEmptyBoundary) {
        return {
          ...groundedAnswer,
          searchMode,
          searchStrategy: completeGroundedAnswer
            ? options.interpretation?.intent === "rules"
              ? "shared-interpretation-strong-match"
              : "deterministic-grounded-strong-match"
            : "deterministic-empty-evidence-boundary",
        };
      }
    }
    ...
    if (options.interpretation?.intent === "rules"
      ...
      && !(/\b(?:pickle ?ball|sport court)\b/i.test(question) && !isPrivateSportCourtQuery(question))) {
      const groundedAnswer = await answerRulesQuestion(question, {
        ...options, searchMode: "legacy",
      });
  ...
  const preferSpecialSources = Boolean(specialSources.length) && (
    ...
    isPlantPermissionQuestion(routingQuery) ||
    ...
    isResidentSharingFixtureQuery(routingQuery) ||
    isHolidayDisplayQuery(routingQuery) ||
  ...
  if (isLandscapeApplicationQuery(routingQuery)) {
    const resourceSources = designReviewResourceSourcesForQuestion(routingQuery, answerSources, "landscape application");
    const packet = resourceSources.find((source) => /Landscape Submittal Packet/i.test(source.title || ""));
    const submit = resourceSources.find((source) => /Submit a DRC Application/i.test(source.title || ""));
    if (packet && submit) {
      const directAnswer = `Use ${packet.title} for a landscaping or irrigation application.`;
      const keyDetails = [`Open ${submit.title} when you are ready to start the review.`];
      const answer = structuredHelpfulAnswer(
        directAnswer,
        keyDetails,
        "Open the packet first so you can prepare the required landscaping forms before submitting."
      );
      const actions = resourceSources.map((source) => ({
        label: actionLabel(source.title),
        url: source.sourceUrl,
        actionType: source.actionType || "information",
      }));
      return {
        answer,
        ...(options.needFirstEvidenceContract === true ? {
          directAnswer,
          keyDetails,
          claims: [packet, submit].map((source) => ({
            text: source === packet ? directAnswer : keyDetails[0],
            evidenceSourceIds: [rulesEvidenceSourceId(source)],
            verified: true,
          })),
        } : {}),
        answerMode: "official-resource-navigation",
        answerVerdict: "informational",
        confidence: { canAnswer: true, confidence: "high", reason: "official-landscape-application-resources" },
        inputClassification: effectiveInputClassification,
        searchMode,
  ...
  if (!answerSources.length || topScore < 2 || !confidence.canAnswer) {
    ...
    if (groundedSpecialAnswer) {
      ...
      if (!coverageIssues.includes("unsupported-resource-absence-claim")) {
        const exactSpecialSource = directSpecialSources.some((special) =>
          answerSources.some((source) => (source.nodeId || source.sourceUrl || source.title) === (special.nodeId || special.sourceUrl || special.title))
        );
        const groundedConfidence = exactSpecialSource && coverageIssues.length === 0 ? "high" : "medium";
        const officialResourceActions = displaySources
        ...
        const needFirstEvidence = options.needFirstEvidenceContract === true
          ? (() => {
              const directAnswer = naturalizeGroundedDraft(groundedSpecialAnswer)
                .split(/\n\s*\n/)[0]
                .trim();
              return {
                directAnswer,
                keyDetails: [],
                claims: buildRulesEvidenceClaims({
                  directAnswer,
                  answer: groundedSpecialAnswer,
                  sources: displaySources,
                  actions: officialResourceActions,
        ...
        return {
          ...
          confidence: groundedSpecialIsBoundary
            ...
            : { canAnswer: true, confidence: groundedConfidence, reason: exactSpecialSource ? "question-specific-official-clause" : "source-grounded-clause-composition" },
          inputClassification: effectiveInputClassification,
          searchMode,
          sources: displaySources,
          actions: officialResourceActions,
          naturalFallbackProse: groundedSpecialIsBoundary,
          sourceStatus: status,
          qualityChecks: {
    ...
    // A recognized community topic with no supporting rule evidence should
    // not display a merely similar rule as if it were a useful citation. The
    // community layer can then look for separately approved operational
    // sources; until one exists, the answer remains an explicit boundary.
    const unsupportedCommunityTopic = hasCommunityTopicSignal(question) && !confidence.canAnswer;
    const boundarySources = (unsupportedCommunityTopic || confidence.withholdSources)
      ? []
      : answerSources.slice(0, 3);
    return {
      answer: unclearAnswer(boundarySources),
      answerMode: "source-evidence-boundary",
      answerVerdict: "unverified",
      confidence,
      inputClassification: effectiveInputClassification,
  ...
  let needsSourceDerivedFacts = summaryHasVolatileFacts(plainAnswer)
    ...
    || isHolidayDisplayQuery(question)
    || isHolidayDisplayQuery(routingQuery)
    || isFenceFinishQuery(routingQuery);
  ...
  const rewrittenAnswer = rewriteDecision.eligible
    ...
    : null;
  const draftDirectAnswer = shortAnswerSummary(derived.answer);
  const draftKeyDetails = String(derived.answer || "")
    .match(/\n\s*\nWhat I found\s*:\s*\n([\s\S]*?)(?=\n\s*\nBefore you act\s*:|$)/i)?.[1]
    ?.split(/\r?\n/)
    .map((line) => line.replace(/^[-*]\s+/, "").trim())
    .filter(Boolean)
    .slice(0, 3) || [];
  const draftNextStep = String(derived.answer || "")
    .match(/\n\s*\nBefore you act\s*:\s*([\s\S]*)$/i)?.[1]
    ?.trim() || "";
  // Keep the source-derived structure for callers that use it, while exposing
  // one clean prose answer to residents regardless of whether optional AI
  // synthesis is off, unavailable, or rejected.
  const naturalFallback = naturalizeGroundedDraft(derived.answer);
  const naturalFallbackWithoutLocationScaffold = naturalFallback
    .split(/\n\s*\n/)
    .filter((part) => !/^The selected source does not confirm the requested location\.?$/i.test(part.trim()))
    .join("\n\n")
    .trim();
  const renderedFallbackBindings = new Set(explicitDimensionBindings(naturalFallbackWithoutLocationScaffold)
    .map(({ role, value }) => `${role}:${value}`));
  const missingFallbackDimensions = focusedDimensionDetails(derived.sources)
    .filter(({ role, value }) => !renderedFallbackBindings.has(`${role}:${value}`));
  const fallbackDimensionSentence = (() => {
    const dimensions = missingFallbackDimensions;
    if (!dimensions.length) return "";
    const valueFor = (role) => dimensions.find((detail) => detail.role === role)?.value
      ?.replace(/\bft\b/i, "feet")
      .replace(/\bin\b/i, "inches");
    const height = valueFor("height");
    const width = valueFor("width");
    if (height && width) return `It is listed as ${height} tall with a spread of ${width}.`;
    const descriptions = dimensions.map(({ role, value }) => {
      const readableValue = value.replace(/\bft\b/i, "feet").replace(/\bin\b/i, "inches");
      if (role === "height") return `${readableValue} tall`;
      if (role === "width") return `a spread of ${readableValue}`;
      if (role === "depth") return `${readableValue} deep`;
      if (role === "length") return `${readableValue} long`;
      return `${capitalizeFirstLetter(role)}: ${readableValue}`;
    });
    return `It is listed as ${readableList(descriptions)}.`;
  })();
  const fallbackLocationBoundary = unsupportedQuestionLocations(question, derived.sources).length
    && !isLandscapeOverviewQuery(routingQuery)
    && !isLandscapeCompletionDeadlineQuery(routingQuery)
    ? "The official source does not say whether that placement is allowed."
    : "";
  const naturalNextStep = naturalizeGroundedDraft(draftNextStep);
  const fallbackLead = naturalNextStep && naturalFallbackWithoutLocationScaffold.endsWith(naturalNextStep)
    ? naturalFallbackWithoutLocationScaffold.slice(0, -naturalNextStep.length).trim()
    : naturalFallbackWithoutLocationScaffold;
  const rawFallbackAnswer = [fallbackLead, fallbackLocationBoundary, fallbackDimensionSentence, naturalNextStep]
    .filter(Boolean)
    .join("\n\n");
  const composedFallbackAnswer = !rewrittenAnswer
    ? composePlainEnglishFallback(rawFallbackAnswer)
    : rawFallbackAnswer;
  const fallbackAnswer = !rewrittenAnswer
    && answerCoverageIssues(question, composedFallbackAnswer, derived.sources).length
    ? rawFallbackAnswer
    : composedFallbackAnswer;
  // A grounded writer can still echo the rulebook word for word. Apply the
  // same source-independent grammar cleanup to every accepted grounded rewrite,
  // then keep the original rewrite if that cleanup would lose required facts.
  const composedRewriteAnswer = rewrittenAnswer
    ? composePlainEnglishFallback(rewrittenAnswer)
    : rewrittenAnswer;
  const residentRewriteAnswer = composedRewriteAnswer
    && answerCoverageIssues(question, composedRewriteAnswer, derived.sources).length
    ? rewrittenAnswer
    : composedRewriteAnswer;
  let answer = residentRewriteAnswer || fallbackAnswer;
  ...
  const residentNextStep = nextStepFromActions(draftNextStep, officialResourceActions);
  const composedDraftNextStep = composePlainEnglishFallback(draftNextStep);
  if (!residentRewriteAnswer && residentNextStep && composedDraftNextStep && answer.endsWith(composedDraftNextStep)) {
    answer = `${answer.slice(0, -composedDraftNextStep.length).trim()}\n\n${residentNextStep}`;
  }
  const coverageIssues = [
    ...answerCoverageIssues(question, answer, displaySources),
  ...
  const finalConfidence = finalEvidenceBoundary
    ...
    : confidence;
  const directAnswer = residentRewriteAnswer
    ? String(answer || "").split(/\n\s*\n/)[0].trim()
    : composePlainEnglishFallback(draftDirectAnswer);
  const keyDetails = draftKeyDetails.map(composePlainEnglishFallback).filter(Boolean);
  const claims = finalConfidence.canAnswer === true
    ? buildRulesEvidenceClaims({
        directAnswer,
        keyDetails,
        answer,
        sources: displaySources,
        actions: officialResourceActions,
        residentQuestion: question,
        allowAnswerFallback: options.needFirstEvidenceContract === true,
      })
    : [];
  ...
  return {
    ...
    directAnswer,
    keyDetails,
    nextStep: residentNextStep,
    answerVerdict: isUnderEaveLightingQuery(routingQuery) && /DRC approval is required to add permanent under-eave lighting/i.test(answer)
      ? "conditional"
      : deriveAnswerVerdict(residentRewriteAnswer || draftDirectAnswer, finalConfidence),
    answerMode: finalEvidenceBoundary
      ...
      : rewrittenAnswer
        ? `llm-${llmMode}`
        : "deterministic",
    // The community wrapper normally rebuilds a rule answer from its display
    // fields. Keep this already-natural fallback intact instead, so a rejected
    // AI draft cannot reintroduce the technical labels it just avoided.
    naturalFallbackProse: !rewrittenAnswer,
...
module.exports = {
  ...
  currentReplacementSupplements,
  documentIsSupersededByCurrentSupplement,
  getRulesIndexStatus,
  hasRulesIndex,
  loadRulesIndex,
  ownerReviewWithholdsQuestion,
  searchRulesIndex,
  sameMunicodeVersion,
  sourceDerivedAnswerParts,
  sourceLifecycleStatus,
