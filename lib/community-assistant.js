...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (
    ...
    !(
      ...
      isStateParksPassQuestion(question)
    )
  ) {
  ...
  const clearLandscapeDeadlineQuestion =
    /\b(?:back\s*yard|rear(?:\s+yard)?|front\s+yard|landscap(?:e|ing)|yards?)\b/i.test(question) &&
    /\b(?:how long|how many (?:days|weeks|months)|when|deadline|due|finish(?:ed)?|complete(?:d|tion)?)\b/i.test(question);
  if (
    routingPlan?.needsClarification &&
    classifiedInput.classification === INPUT_CLASSIFICATIONS.RULES_QUESTION &&
    (clearLandscapeDeadlineQuestion || isStateParksPassQuestion(question))
  ) {
    routingPlan = {
      ...routingPlan,
      scope: "community",
