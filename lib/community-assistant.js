...
function datedFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    if (weekday !== 2 && weekday !== 4) segment = segment.replace(/Tuesday\s*(?:&|and)\s*Thursday\s*[-:]?\s*[^.]*?(?:maintenance|cleaning)\.?/i, "");
    ...
    const directAnswer = `For ${dateLabel}, the published ${label.toLowerCase()} hours are: ${detail}`;
    ...
    return buildAnswerContract({
      ...
      nextStep: "Check the official facility page for weather or maintenance closures before you go.",
