...
test('all documented walking-route links remain connected to their places', () => {
  assert.ok(trails.routes.length >= 8, 'the expanded walking inventory is present');
  for (const original of ['prospect-loop', 'providence-west', 'overlook-link']) assert.ok(trails.routes.some(route => route.id === original));
