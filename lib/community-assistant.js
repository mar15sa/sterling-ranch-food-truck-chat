...
async function answerCommunityQuestion(query, options = {}) {
  if (options.requestContractMode === "shadow-route") {
    if (options.isTest !== true) throw new Error("Need-first shadow routing is test-only.");
    if (options.planCommunitySearch !== false || options.synthesizeCommunityAnswer !== false) {
      throw new Error("Need-first shadow routing requires local model stages to be disabled.");
    }
    if (options.answerRulesQuestion && (options.rulesOptions?.searchMode !== "legacy" || options.rulesOptions?.llmMode !== "off")) {
      throw new Error("Need-first shadow routing requires the local deterministic rules path.");
    }
    ...
    if (!currentLocalBackend && typeof options.answerResidentNeed !== "function") {
      throw new Error("Need-first shadow routing requires an explicit per-need answer function.");
    }
    ...
    delete currentOptions.answerResidentNeed;
    delete currentOptions.needRouterBackend;
    ...
    const answerResidentNeed = options.answerResidentNeed || (async (need) => {
      ...
      return answerCommunityQuestion(need.request, needOptions);
    });
