...
function mergeSupportingClaims(results) {
  const merged = new Map();
  for (const result of results) {
    if (result.evidence.status !== "supported" && !hasPartialSupport(result)) continue;
...
function selectedActions(results) {
  const selected = results.flatMap((result) => result.evidence.relevantActionIndexes
...
function composedAnswer(results, actions, outcome) {
  const supported = results.filter((result) => ["supported", "handled-boundary"].includes(result.evidence.status) || hasPartialSupport(result));
  ...
  if (unresolved.length) {
    if (unresolved.length === 1) {
      const scopedBoundary = hasPartialSupport(unresolved[0])
        && /\b(?:does not|doesn't|could not|couldn’t|cannot|can't)\b.{0,180}\b(?:set|say|state|specify|provide|publish|list|verify|confirm|cover|apply)\b/i.test(unresolved[0].answer.nextStep || "")
        ? String(unresolved[0].answer.nextStep).trim()
        : "";
      if (scopedBoundary) lines.push(scopedBoundary);
      else {
