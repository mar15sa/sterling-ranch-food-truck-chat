...
function normalizeFilters(value = {}) {
  return Object.fromEntries(FILTER_KEYS.map((key) => [key, clean(value?.[key], 100)]));
}
...
function normalizeInterpretation(plan = {}, question = "", options = {}) {
  ...
  const goals = [...new Set([primaryGoal, ...(Array.isArray(plan.goals) ? plan.goals : [])].filter((goal) => GOALS.has(goal)))].slice(0, 4);
  ...
  let requestedDetails = [...new Set([...modelDetails, ...deterministicRequestedDetails(question)])].slice(0, 8);
  ...
  return {
    ...
    filters: normalizeFilters(plan.filters),
    searchQueries,
    scope,
    needsClarification,
    clarificationQuestion,
