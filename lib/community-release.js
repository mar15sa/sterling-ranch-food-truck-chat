...
const { containsEmbeddedInstructions, canonicalPageUrl, pageContentIdentity } = require("./community-ingest");
const {
  SENSITIVE_FACETS,
...
function validateCommunityCandidate(trusted, candidate, profile, options = {}) {
  ...
  if (candidate.inventory) {
    ...
    const inventoryKey = url => { try { return canonicalPageUrl(url); } catch { return String(url || ''); } };
    const excludedUrls = new Set([...exclusions, ...(candidate.inventory.dispositions || []).filter(row => row.status === 'excluded')]
      .map(row => inventoryKey(row.url)));
    for (const source of candidate.sources || []) {
      // A link-only action or live connector can intentionally point outside
      // static crawling. Static page/PDF content cannot be both used and excluded.
      if (!['civicplus-pages', 'official-pdf'].includes(source.connectorType) || isDynamicSource(source)) continue;
      if (excludedUrls.has(inventoryKey(source.sourceUrl))) errors.push(`Static source ${source.id} is retained from an excluded inventory URL: ${source.sourceUrl}`);
    }
    ...
    for (const page of candidate.pages || []) {
      ...
      const identity = pageContentIdentity(page);
      const owner = fingerprints.get(identity);
      if (owner && !page.duplicateOf) errors.push(`Duplicate page content was indexed for ${owner} and ${page.url}.`);
      fingerprints.set(identity, owner || page.url);
    }
