...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (shouldConsultRules) {
    ...
    if (rulesAnswer?.confidence?.canAnswer
      ...
      const supportingSources = bindingRule ? community.sources.map(sourceForDisplay) : [];
      ...
      const completedRulesAnswer = {
        ...
        ...(bindingRule ? {
          ...
          // per-facet completion model. Process evidence remains available
          // for callers that can label it as supporting, never controlling.
