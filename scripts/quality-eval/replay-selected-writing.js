...
function writerRequest(input,packet,model='claude-sonnet-5'){
 if(!['claude-haiku-4-5','claude-sonnet-5'].includes(model))throw Error('Unsupported writer comparison model');
 ...
 payload.evidence=modelEvidence(packet.sources);payload.actions=modelActions(packet.actions);body.messages[0].content=JSON.stringify(payload);body.tools[0].input_schema=compositionSchema(packet);
 body.model=model;if(model==='claude-haiku-4-5')body.temperature=0;return body;
}
async function main(){
 const [priorArg,outArg,model='claude-sonnet-5',accountingArg]=process.argv.slice(2);if(!priorArg||!outArg||!process.env.ANTHROPIC_API_KEY)throw Error('Require capture, new output and existing credential');
 if(!['claude-haiku-4-5','claude-sonnet-5'].includes(model)||(model==='claude-haiku-4-5')!==Boolean(accountingArg))throw Error('Haiku comparison requires preceding Sonnet writer accounting');
 ...
 const jobs=m.runs.filter(r=>r.model==='claude-haiku-4-5').map(r=>{const record=read(r.id+'.json'),input=read(r.caseId+'-input.json');
  ...
  return {...r,input,packet:applied.packet,body:writerRequest(input,applied.packet,model)};
 });
 ...
 let priorReserved=m.priorReservedUpperUsd+m.reservedUpperUsd;
 if(accountingArg){const accounting=JSON.parse(fs.readFileSync(path.join(path.resolve(accountingArg),'manifest.json'),'utf8'));
  if(accounting.status!=='captured'||accounting.mode!=='selected-evidence-writer-replay'||accounting.writerModel&&accounting.writerModel!=='claude-sonnet-5'||path.resolve(accounting.priorCapture)!==prior||accounting.priorReservedUpperUsd!==priorReserved)throw Error('Wrong writer accounting predecessor');
  priorReserved+=accounting.reservedUpperUsd;
 }
 const capUsd=5-priorReserved,plannedUpper=jobs.reduce((s,j)=>s+upperCost(j.body),0);
 ...
 const manifest={status:'running',isTest:true,mode:'selected-evidence-writer-replay',startedAt:new Date().toISOString(),codeRevision:cp.execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),priorCapture:prior,
  writerModel:model,accountingCapture:accountingArg?path.resolve(accountingArg):null,priorReservedUpperUsd:priorReserved,capUsd,plannedUpperUsd:plannedUpper,runs,limitations:['Historical source clock; no current-live claims.','Eight candidate writer replays versus four historical matched original writer samples, not a new randomized control.','Selection plus writing only; interpretation/retrieval/checking/hosting excluded.','Known development cases; not blind human quality calibration.']};
