...
function fingerprint(index = {}) {
  return hash((index.sources || []).map((source) => `${source.id}:${source.contentHash}`).sort().join("\n"));
}
...
function sourceReviewItem(kind, source, current, candidateFingerprint, releaseFingerprint) {
  ...
  return {
    ...
    facet: "source",
    scopeKey: "page",
    currentValue: approved.excerpt || approved.text?.slice(0, 500) || (removed ? "Approved page" : "Not currently approved"),
    proposedValue: removed ? "Page missing from candidate" : proposed.excerpt || proposed.text?.slice(0, 500) || "Updated page",
    ...
    supportingText: proposed.excerpt || approved.excerpt || "",
    candidateFingerprint,
    releaseFingerprint,
    createdAt: new Date().toISOString(),
  };
...
function compileReviewedCandidate(trusted = {}, candidate = {}, profile = {}, decisions = []) {
  ...
  const baseLedger = buildFactLedger({ ...candidate, sources }, { previousLedger: trusted.factLedger || [], trustedVersions });
...
async function syncReviewItems(items = [], fetchImpl = globalThis.fetch) {
  ...
  for (let index = 0; index < pending.length; index += 3) {
    ...
    if (index + 3 < pending.length) await new Promise((resolve) => setTimeout(resolve, 400));
  }
