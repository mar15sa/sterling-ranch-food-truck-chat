...
function hasDistinctiveCommunityEvidence(question, sources = []) {
  ...
  return sources.some((source) => {
    ...
    const approvedActionEvidence = (source.actions || [])
      ...
      .join(" ");
    const evidence = `${source.title || ""} ${source.text || ""} ${approvedSubjects} ${approvedActionEvidence}`.toLowerCase();
...
function relevantActions(question, sources, limit = 3, routingPlan = null) {
  ...
  const actions = sources.flatMap((source) => {
    ...
    return sourceActions.map((action) => ({
      ...
      sourceEvidence: `${source.title || ""} ${source.text || ""} ${source.excerpt || ""} ${action.evidence?.label || ""} ${action.evidence?.context || ""}`.toLowerCase(),
