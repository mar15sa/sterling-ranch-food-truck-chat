...
function normalizeFoodTruckQuestion(question = "") {
  let text = String(question).normalize("NFKC").replace(/\s+/g, " ").trim();
  text = text.replace(/[\u{1F6FB}\u{1F69A}\u{1F69B}]/gu, "truck");
  text = text.replace(/\b([Ff]ood\s*trucks?)\s+(?:tmr|tmrw|tom|tomorow|tommorow)\b/g, "$1 tomorrow");
  if (/^food\s*trucks?\s*[?.!]*$/i.test(text)) text = text.replace(/\s*[?.!]*$/, " today");
  return text;
}
function isFoodTruckQuestion(question = "") { const text = normalizeFoodTruckQuestion(question).toLowerCase(); return !isFoodTruckBusinessQuestion(text) && /\bfood\s*trucks?\b.{0,60}\b(?:today|tomorrow|tonight|menu|price|cost|here|coming|scheduled|schedule|calendar|which|who|when|date|day)\b|\b(?:today|tomorrow|tonight|which|who|when)\b.{0,60}\bfood\s*trucks?\b|\btruck\b.{0,45}\b(?:today|tomorrow|tonight|menu|price|cost|here|coming|scheduled)\b|\b(?:menu|price|cost|what(?:'s| is) on)\b.{0,45}\b(?:their|truck)\b/i.test(text); }
function isFoodTruckRequest(interpretation = {}, question = "") { const text = `${normalizeFoodTruckQuestion(question)} ${interpretation.subject || ""} ${(interpretation.searchQueries || []).join(" ")}`.toLowerCase(); return /\bfood\s*trucks?\b/.test(text) && !isFoodTruckBusinessQuestion(text) && (interpretation.intent === "events" || ["schedule", "status", "cost", "information"].some((goal) => (interpretation.goals || [interpretation.goal]).includes(goal))); }
...
module.exports = { foodTruckAnswer, isFoodTruckQuestion, isFoodTruckRequest, normalizeFoodTruckQuestion };
