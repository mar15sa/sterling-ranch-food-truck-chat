...
function scoreSource(source, queryTokens, intent, details, coreTokens = [], question = "", factAuthority = {}) {
  ...
  const missingObjects = requestedObjects.filter(term => !title.includes(term) && !body.includes(term));
  if (missingObjects.length) {
    ...
    const coarseVenueWords = new Set(['clubhouse', 'facility', 'center', 'centre']);
    const requestedSubject = scopeTerms(question).filter(term => !coarseVenueWords.has(term));
    ...
    if (!missingObjects.every(term => coarseVenueWords.has(term)) || !exactReviewedIdentity) return 0;
  }
...
function searchCommunityIndex(question, options = {}) {
  ...
  const entriesBySource = new Map(eligibleSources.map(source => [source.id,
    filterProcess(reviewState.entriesFor(source), source)
  ...
  const subjectCandidates = eligibleSources.filter(source => {
    ...
    return entriesSupportAnyRequestedDetail(entries, details)
      && (exactReviewedSubject || scoreSource(approvedClaimProjection(source, entries, details), queryTokens, intent, details, coreTokens, question, index.factAuthority || {}) > 0);
  });
