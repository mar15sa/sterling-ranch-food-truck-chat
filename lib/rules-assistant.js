...
function evidencePassageForClaim(claimText, sourceText) {
  ...
  return scored.filter((item) => item.score === best).slice(0, 3).map((item) => item.sentence).join(" ") || String(sourceText);
}
...
function modalityIsSupported(claimText, sourceText) {
  ...
  if (claim.permission && !source.permission) return false;
...
function sourceSupportsRenderedRuleClaim(claimText, source = {}, residentQuestion = "") {
  ...
  const normalizedClaim = String(claimText).replace(/\s+/g, " ").trim().toLowerCase();
  const normalizedSource = sourceText.replace(/\s+/g, " ").trim().toLowerCase();
  const supportingPassage = evidencePassageForClaim(claimText, sourceText);
  if (!modalityIsSupported(claimText, supportingPassage)) return false;
  const sourceNumbers = new Set([...numericTokens(supportingPassage), ...numberWordsToDigits(supportingPassage)]);
  if ([...new Set([...numericTokens(claimText), ...numberWordsToDigits(claimText)])].some((value) => !sourceNumbers.has(value))) return false;
  const sourceDates = new Set(dateTimePhrases(supportingPassage));
  if (dateTimePhrases(claimText).some((value) => !sourceDates.has(value))) return false;
...
function renderedRuleClaimSentences(directAnswer = "", keyDetails = []) {
  return [...new Set([directAnswer, ...keyDetails]
...
function buildRulesEvidenceClaims({ directAnswer = "", keyDetails = [], answer = "", sources = [], actions = [], residentQuestion = "" } = {}) {
  const claims = renderedRuleClaimSentences(directAnswer, keyDetails).map((text) => {
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const claims = finalConfidence.canAnswer === true
    ? buildRulesEvidenceClaims({ directAnswer, keyDetails, answer, sources: displaySources, actions: officialResourceActions, residentQuestion: question })
    : [];
