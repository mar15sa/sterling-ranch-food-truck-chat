...
function questionTerms(question = "") {
  const ignored = new Set(["what", "which", "where", "when", "with", "from", "that", "this", "about", "community", "official", "current", "sterling", "ranch", "hours", "phone", "email", "address", "account", "number"]);
  const terms = (String(question).toLowerCase().match(/[a-z0-9]{4,}/g) || [])
    .filter((term) => !ignored.has(term));
  // Residents often insert or omit a space inside a compound topic name.
  // Retain the individual words and add adjacent compounds so a handoff can
  // match the same subject without a topic-specific alias table.
  const compounds = terms.slice(0, -1).map((term, index) => `${term}${terms[index + 1]}`);
  return [...new Set([...terms, ...compounds])];
}

function isGenericDiscoverySource(source = {}) {
  const title = String(source.title || "");
  const url = String(source.sourceUrl || "");
  try {
    return /official (?:community )?website/i.test(title) && new URL(url).pathname.replace(/\/$/, "") === "";
  } catch {
    return false;
  }
}

function handoffIsQuestionSpecific(question, result = {}) {
  if (result.confidence?.reason === "named-project-not-supported-by-cited-evidence"
    && /selected official rules do not name .+ specifically/i.test(String(result.answer || ""))
    && (result.sources || []).some((source) => /library\.municode\.com/i.test(source.sourceUrl || ""))) return true;
  const terms = questionTerms(question);
  if (!terms.length) return true;
  const handoffs = [
    ...(result.sources || []),
    ...(result.actions || []).map((action) => ({ title: action.label, sourceUrl: action.url })),
  ];
  if (!handoffs.length) return true;
  return handoffs.some((handoff) => {
    if (isGenericDiscoverySource(handoff)) return true;
    const evidence = `${handoff.title || ""} ${handoff.text || ""} ${handoff.excerpt || ""}`.toLowerCase();
    return terms.some((term) => new RegExp(`\\b${term}\\b`, "i").test(evidence));
  });
}

function hasVerifiedClaimProjection(result = {}) {
  const exactMode = /^community-approved-operational(?:-|$)/.test(String(result.answerMode || ""));
  const claims = result.claims || [];
  const sourceIds = new Set((result.sources || []).flatMap((source) => [source.id, source.nodeId, source.sourceUrl]).filter(Boolean));
  return exactMode
    && claims.length > 0
    && claims.every((claim) => claim.verified === true
...
function residentEffortAssessment(question, result) {
  ...
  const gaps = [];
  const incomplete = ['missing-evidence', 'conflict', 'verified-partial', 'ambiguous'].includes(result?.completion?.outcome)
    || ['source-unavailable', 'could-not-verify', 'verified-incomplete', 'conflicting-sources'].includes(result?.answerStatus);
  if (incomplete) gaps.push('answer-incomplete');
  ...
  if (/\b(?:pay|payment)\b/i.test(question)
    ...
    needs(/https:\/\/srcab\.utilityhawk\.us\/?(?![\w/-])/i, "payment-link-missing");
    needs(/Pay Online/i, "payment-step-missing");
  }
  if (/\b(?:book|reserve)\b.{0,40}\b(?:park|shelter|clubhouse|overlook|great hall|pavilion)\b|\b(?:park|shelter|clubhouse|overlook|great hall|pavilion)\b.{0,40}\b(?:book|reserve)\b|\b(?:how|where)\b.{0,20}\brent\b.{0,40}\b(?:park|shelter|clubhouse|overlook|great hall|pavilion)\b/i.test(question)) {
    if (/\b(?:how much|price|cost|fee)\b/i.test(question)) needs(/\$\d+/i, "rental-price-missing");
  ...
  if (/\b(?:submit|send|file)\b.{0,35}\b(?:DRC|design review|architectural)\b|\b(?:DRC|design review)\b.{0,35}\b(?:submit|application|apply)\b/i.test(question)) {
    const exactApplicationHandoff = /\/198\/Architectural-Community-Standards/i.test(actionText)
      && /\/201\/Design-Review-Documents/i.test(actionText);
    if (!exactApplicationHandoff) {
  ...
  if (/\b(?:today|now|currently|right now|next allowed)\b.{0,35}\b(?:water|holiday|christmas|seasonal|lights?)\b|\b(?:water|holiday|christmas|seasonal|lights?)\b.{0,35}\b(?:today|now|currently|right now|next allowed)\b/i.test(question)) {
  ...
  // Completion is a floor, not an extra research step on top of a known gap.
  const remainingSteps = Math.max(gaps.filter(gap => gap !== 'answer-incomplete').length + vagueHandoff, incomplete ? 1 : 0);
...
function directResolutionIssues(question, result, answer) {
  ...
  const issues = [];
  const asksForTime = /\b(?:what|which)\s+time\b|\b(?:latest|how late|what hours?|time restriction)\b|\b(?:pool|facility|clubhouse|court|office|operating|operation)\s+hours?\b|\bhours?\s+(?:for|on|at|of)\b/i.test(question);
  const resolvesTime = /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b|\b(?:no|without) (?:specific )?(?:time|hour) restriction\b|\bdoes not (?:give|set|publish|specify)\b.{0,60}\b(?:time|hour)\b|\b(?:rules?|sources?)\b.{0,25}\b(?:do not|don't)\b.{0,20}\b(?:set|publish)\b.{0,20}\b(?:latest|specific|operating)?\s*(?:time|hours?)\b|\b(?:any time|sunset|sunrise|dusk|dawn)\b/i.test(answer);
  if (asksForTime && !resolvesTime) issues.push("requested-time-missing");

  const saysSourcesDoNotAnswer = /\b(?:sources?|rules?|guidelines?)\b.{0,40}\b(?:do not|don't|does not|doesn't)\b.{0,30}\b(?:cover|contain|say|show|address|include|mention|list|publish)\b/i.test(directAnswer)
    || /\b(?:not covered|could not find|couldn't find|unable to verify)\b/i.test(directAnswer);
  const handsResearchBack = /\b(?:check|contact|reach out|ask)\b.{0,80}\b(?:CAB|Sterling Ranch|Community Authority|capital plans?|community meeting|approval documentation|General Inquiries|feedback)\b/i.test(answer);
...
function presentationArtifactIssues(answer = "") {
  const value = String(answer || "");
  const issues = [];
  const rawTableHeader = /\bBotanical\s+Common\s+Ht\s*x\s*Spd\s+Type(?:\s+Bird Friendly)?\b/i;
  const typedMeasurementRows = value.match(
    /\b[A-Z][A-Z0-9&/'".\-]*(?:\s+[A-Z][A-Z0-9&/'".\-]*){1,12}\s+\d+(?:\.\d+)?['"]\s*x\s*\d+(?:\.\d+)?['"]\s+(?:Evergreen|Deciduous|Ornamental|Shrub|Perennial|Grass|Vine|Rose)\b/g
  ) || [];
  if (rawTableHeader.test(value) || typedMeasurementRows.length >= 2) {
    issues.push("raw-catalog-dump");
  }
  const hasShortAnswerLabel = /^Short answer\s*:/im.test(value);
  const hasRetrievalFindingsLabel = /^What I found\s*:/im.test(value);
  if (/\bAllowed choices from the selected source\s*:/i.test(value)
    || (hasShortAnswerLabel && hasRetrievalFindingsLabel)) {
    issues.push("retrieval-scaffolding");
  }
  return issues;
}
...
function scoreCommunityAnswer(question, result = {}, options = {}) {
  ...
  if (expectation?.expectedCompletion && result.completion?.outcome !== expectation.expectedCompletion) {
    return { score: 2, contentScore: 2, rating: 'Weak', issues: ['completion-outcome-mismatch'], residentEffort };
  }
  if (residentEffort.gaps.includes('answer-incomplete')) issues.push('answer-incomplete');
  ...
  issues.push(...presentationArtifactIssues(answer));
  issues.push(...residentVoiceIssues(answer));
  if (answer.length > 2600) issues.push("too-long");
  const noClaimBoundary = result.confidence?.canAnswer === false && !(result.claims || []).length;
  if (noClaimBoundary && !handoffIsQuestionSpecific(question, result)) issues.push("irrelevant-handoff-source");
  const verifiedClaimProjection = hasVerifiedClaimProjection(result);
  if (result.answerMode?.startsWith("community-approved-operational") && !verifiedClaimProjection) issues.push("claim-provenance-mismatch");
  ...
  const severe = uniqueIssues.some((issue) => ["unhelpful-fallback", "raw-source-text", "raw-catalog-dump", "retrieval-scaffolding", "source-meta-language", "sentence-fragment", "rulebook-measurement-language", "source-author-voice", "passive-prohibition-language", "duplicated-claim", "required-details-missing", "requested-time-missing", "question-unresolved", "planned-project-answer-missing", "expected-answer-missed", "required-refusal-missing", "answer-mode-mismatch", "input-classification-mismatch", "confidence-reason-mismatch", "unexpected-sources", "action-link-missing", "official-source-missing", "resident-effort-high", "question-form-mismatch", "irrelevant-handoff-source", "claim-provenance-mismatch"].includes(issue));
  const illustrativeEvidenceBoundary = result.answerMode === "source-evidence-boundary"
    && /mentions? the requested project only as an example in a different rule/i.test(answer);
  // Keep content safety/usefulness distinct from whether the resident's task
  // was resolved. A correct evidence limitation is not a content regression,
  // but its owner-facing rating must still disclose the unresolved request.
  const contentIssues = uniqueIssues.filter(issue => issue !== 'answer-incomplete');
  const contentScore = severe || contentIssues.includes('completion-outcome-mismatch') ? 2 : contentIssues.length ? 3 : verifiedClaimProjection ? 5 : illustrativeEvidenceBoundary ? 4 : result.answerMode === "community-rules-boundary" ? 5 : /^Short answer:/i.test(answer) && /What I found:/i.test(answer) ? 5 : 4;
  const value = uniqueIssues.includes('answer-incomplete') ? Math.min(3, contentScore) : contentScore;
  return {
    score: value,
    contentScore,
...
module.exports = { residentEffortAssessment, scoreCommunityAnswer, handoffIsQuestionSpecific, hasVerifiedClaimProjection };
