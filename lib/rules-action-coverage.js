...
function actionLinkIssues(answer = "", sources = []) {
  const linkedText = sources
    ...
    .map((source) => {
      const actionText = (source.actions || [])
        .filter((action) => /^https?:\/\//i.test(action?.url || "") && action.reviewStatus === "approved")
        .map((action) => `${action.label || ""} ${action.url || ""} ${action.actionType || ""} ${action.context || ""}`)
        .join(" ");
      return [
        source.title || "",
        source.sourceUrl || "",
