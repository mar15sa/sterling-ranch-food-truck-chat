...
test('Release 2 exact reviewed evidence remains intact in later source releases', () => {
  ...
  const releaseTwoIds = new Set([...strictWaterBillingImport, ...strictOwnerApprovedImport].map(([id]) => id));
  ...
  assert.ok([...sourcesByEvidence.values()]
    .filter(sources => sources.some(source => strictOwnerApprovedIds.has(source.id)))
