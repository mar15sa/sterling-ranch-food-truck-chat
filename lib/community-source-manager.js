...
function communitySourceStatus(index = getBundledIndex(), now = Date.now(), { includeStaleSources = false, includeApprovedEvidenceCheckTime = false, refreshError } = {}) {
  ...
  const staleSourceGroups = [...staleSources.reduce((groups, source) => {
    const key = source.sourceUrl || source.id;
    const group = groups.get(key) || { sourceUrl: source.sourceUrl || "", records: [] };
  ...
  return {
    ...
    staleOfficialPageCount: staleSourceGroups.length,
    ...
    })), staleSourceGroups: staleSourceGroups.map(({ sourceUrl, records }) => ({
      sourceUrl,
      recordCount: records.length,
      records: records.map(({ id, contentHash, checkedAt, staleAfter }) => ({ id, contentHash, checkedAt, staleAfter })),
    })) } : {}),
