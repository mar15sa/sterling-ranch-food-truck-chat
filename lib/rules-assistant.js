...
function seasonalLightingOverviewAnswer(sources = []) {
  ...
  if (!clauses.length) return "";
  return structuredHelpfulAnswer(
    capitalizeFirstLetter(clauses[0].text),
    clauses.slice(1, 4).map((entry) => capitalizeFirstLetter(entry.text)),
    "Open the linked official section if you need the complete wording."
  );
}
...
function shedOverviewAnswer(sources = []) {
  ...
  return structuredHelpfulAnswer(
    ...
    clauses.filter((entry) => entry !== approvalClause).slice(0, 3).map((entry) => capitalizeFirstLetter(entry.text)),
    "Open the linked official section if you need the complete wording."
  );
}
