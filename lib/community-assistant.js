...
const { addDays, correctNavigationSubject, deterministicRequestedDetails, highConfidenceDateRange, isInternetServiceSetupRequest, isOfficialInformationPageRequest, isReportedFindingsRequest, isWaterUsageAccessRequest, instructionalRequestShape, normalizeInterpretation, resolveInterpretationMode } = require("./community-interpretation");
...
function hasDistinctiveCommunityEvidence(question, sources = []) {
  ...
  return sources.some((source) => {
    ...
    const evidence = `${source.title || ""} ${source.text || ""} ${approvedSubjects}`.toLowerCase();
...
function isVehicleParkingRuleQuestion(question = "") {
  const text = String(question);
  const regulatedVehicle = /\b(?:rvs?|recreational vehicles?|motor ?homes?|campers?|camper trailers?|travel trailers?|utility trailers?|boat trailers?|mobile homes?)\b/i;
  const parkingScope = /\b(?:park|parking|leave|left|keep|store|street|roadway|driveway|garage|overnight|hours?|days?)\b/i;
  return regulatedVehicle.test(text) && parkingScope.test(text) && !isStateParksPassQuestion(text);
}
...
function actionGoalStrength(action, goal) {
  ...
  return directVerbs[goal]?.test(`${action.label || ''} ${action.evidence?.label || ''}`) ? 2 : 1;
}
...
function originalQuestionNeedsRule(question = "") {
  ...
  return (details.includes('permission') && !isParkingPriceRequest(question)) || normativeSpecification || generalResidentFees
    ...
    || isVehicleParkingRuleQuestion(question)
...
function actionForGoal(source = {}, goal = "") {
  return (source?.actions || []).filter(action => actionSupportsGoal(action, goal))
...
function unavailableWasteScheduleAnswer(question, profile = {}, plan = {}, candidate = null, options = {}) {
  ...
  const holidayLead = holidayContext
    ? String(holidayContext.fact.value || "").trim()
    : "";
  ...
  return {
    ...buildAnswerContract({
      directAnswer,
      keyDetails: [
        ...(hasVerifiedDate && holidayLead
          ? [candidate.directAnswer, (candidate.keyDetails || []).join("; ")].filter(Boolean)
          : hasVerifiedDate ? candidate.keyDetails : []),
      ],
      nextStep: actions[0]?.label || "",
      actions,
      sources,
...
function datedFacilityHoursAnswer(question, result, options = {}) {
  // A resident asking what *normally* happens on a named weekday is asking for
  // the published recurring schedule, not for the next calendar occurrence of
  // that weekday. The interpreter may attach a retrieval date, so preserve the
  // wording distinction here before applying date-specific season boundaries.
  const recurringWeekdayClosing = recurringWeekdayClosingAnswer(question, result);
  if (recurringWeekdayClosing) return recurringWeekdayClosing;
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (isOfficialInformationPageRequest(question, routingPlan)) {
    ...
    const resource = informationSearch.sources.filter((source) => source.sourceUrl
      && source.lifecycle === "current"
      && source.canonicalScopedProjection === true
      && hasDistinctiveCommunityEvidence(resourceTerms.join(" "), [source]))
      .sort((left, right) => {
    ...
    const answerResidentNeed = options.answerResidentNeed || (async (need) => {
      ...
      const fixedIntent = need.goal === "status"
        ...
        : need.requestedDetails?.includes("action")
          && /\b(?:application|form|paperwork|submittal|submission)\b/i.test(needRouteText)
        ? "forms"
