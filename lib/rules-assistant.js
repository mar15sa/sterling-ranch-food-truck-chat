...
function isSeasonalLightingQuery(query) {
  const text = String(query || "");
  const halloweenDecorating = /\bhalloween\b/i.test(text)
    && /\b(lights?|lighting|decorat(?:e|ed|ing|ions?))\b/i.test(text);
  const mentionsSeasonalLighting = halloweenDecorating || (/\b(lights?|lighting)\b/i.test(text)
    && /\b(holiday|seasonal|decorative|christmas|summer|june|july|october|january|pride|red white blue|timeframe|season|window)\b/i.test(text));
...
function isLightingRelatedQuery(query) {
  const text = String(query || "");
  return /\b(light|lights|lighting|fixture|fixtures|holiday|seasonal|string lights?|gemstone|jellyfish|under[-\s]?eaves?|soffits?|eave\/rake|eave|rake|hardwired|permanent|roofline|trim)\b/i.test(text)
    || (/\bhalloween\b/i.test(text) && /\bdecorat(?:e|ed|ing|ions?)\b/i.test(text));
}
