...
async function getSterlingRanchWasteSchedule(options = {}) {
  ...
  const service = /\b(?:trash|garbage)\b/i.test(requestText) && !/\brecycl(?:e|ing)\b/i.test(requestText)
    ? "garbage"
    : "recycling";
  ...
  try {
    ...
    Object.entries({
      nomerge: "1",
      hide: "reminder_only",
      // Keep a named holiday in view when it is the resident's explicit
      // question, including the day immediately before the current date.
      after: addDays(requestedDate && requestedDate < today ? requestedDate : today, -1),
      before: addDays(requestedDate && requestedDate > today ? requestedDate : today, 45),
      locale: "en",
    ...
    const pickup = events
      .filter((event) => event.day >= (requestedDate || today) && eventNames(event).includes(service))
      .sort((a, b) => a.day.localeCompare(b.day))[0];
    if (!pickup?.day) throw new Error(`Official calendar did not return an upcoming ${service} date`);

    return {
      today,
      service,
      date: pickup.day,
      range: requestedDate ? { start: requestedDate, end: requestedDate } : null,
      timing: scheduleTimingLabel(pickup.day, today),
      anchorDate: pickup.day,
      villageDates: villageDatesForAnchor(pickup.day, events),
      holidayNote: holidayMessage(events, pickup.day),
