...
function specificExcerptForQuestionSource(query, source, fullText = source.text || "") {
  ...
  if ((isPlantListQuestion(query) || isPlantPermissionQuestion(query)) && /5-131|preapproved plant list/i.test(title)) {
    ...
    const matchingLine = lines.findIndex((line) => requestedTerms.some((term) => textContainsTerm(line, term)));
    const namedPlantEvidence = matchingLine >= 0
      ? lines.slice(Math.max(0, matchingLine - 2), matchingLine + 3).join(" ")
      : "";
    return shortenText(
