...
function normalizeFactValue(fact = {}) {
  if (fact.type === 'money') {
    const amount = require('./community-money').moneyAmount(fact.value);
...
function inferScopeKey(fact = {}, source = {}) {
  ...
  if (fact.type === 'limit') {
    ...
    const unit = '(?:feet|foot|ft\\.?|inches?|days?|hours?|guests?|vehicles?|ornaments?|trees?|percent|%)';
    const quantity = `\\d+(?:\\.\\d+)?\\s+${unit}`;
    const ranges = [...context.matchAll(new RegExp(`\\b(${quantity})\\s*(?:to|[-–])\\s*(${quantity})(?=\\s|[.,;:]|$)`, 'gi'))];
    const value = String(fact.value || '').trim().toLowerCase();
    if (ranges.length === 1) {
      const range = ranges[0];
      const normalizedUnit = quantity => quantity.replace(/^\d+(?:\.\d+)?\s+/, '').toLowerCase().replace(/\.$/, '')
        .replace(/^(?:feet|foot)$/, 'ft').replace(/^inches$/, 'inch').replace(/s$/, '');
      const lower = range[1].toLowerCase() === value;
      const upper = range[2].toLowerCase() === value;
      if (lower !== upper && normalizedUnit(range[1]) === normalizedUnit(range[2])) {
        const purpose = context.slice(0, range.index) + ' quantity range ' + context.slice(range.index + range[0].length);
        return `range-${lower ? 'lower' : 'upper'}-${slug(purpose)}`;
      }
    }
  }
  if (fact.type === 'date') {
...
function buildFactLedger(index = {}, options = {}) {
  ...
  for (const source of index.sources || []) {
    for (const fact of source.facts || []) {
      ...
      const requirePriorReview = options.requirePriorReview === true;
      // A trusted page or source-level review is not a claim decision. Only an
      // exact prior ledger entry or decision-bearing fact can carry approval.
      // Because the ledger id includes sourceVersion, a changed page cannot
      // inherit an older claim decision.
      const factDecisionIsExact = Boolean(
        fact.reviewDecisionId
        && fact.sourceVersion === sourceVersion
        && fact.reviewedAt
        && fact.reviewedBy
      );
      // Older snapshots could contain an "approved" label without the
      // reviewer, decision id, and timestamp that make approval real. Do not
      // preserve that misleading label when rebuilding the ledger.
      const priorReviewStatus = prior
        && APPROVED_REVIEW_STATUSES.has(prior.reviewStatus)
        && !factApprovalIsExplicit(prior)
        ? "candidate"
        : prior?.reviewStatus;
      const reviewStatus = priorReviewStatus || (!requirePriorReview && factDecisionIsExact
      ...
      const entry = {
        ...
        reviewStatus,
        reviewedAt: prior?.reviewedAt || (factDecisionIsExact ? fact.reviewedAt : ""),
        reviewedBy: prior?.reviewedBy || (factDecisionIsExact ? fact.reviewedBy : ""),
        reviewNote: prior?.reviewNote || (factDecisionIsExact ? fact.reviewNote || "" : ""),
        reviewDecisionId: prior?.reviewDecisionId || (factDecisionIsExact ? fact.reviewDecisionId : ""),
        approvalClaim: fact.approvalClaim || "",
        ...(fact.applicabilityTerms?.length ? { applicabilityTerms: fact.applicabilityTerms } : {}),
      };
...
function factIsAnswerable(entry = {}, now = Date.now()) {
  ...
  if (!factApprovalIsExplicit(entry)) return false;
  const staleAt = dateValue(entry.staleAfter);
  return !staleAt || staleAt >= now;
}

function factApprovalIsExplicit(entry = {}) {
  return Boolean(
...
function factLedgerStatus(index = {}, now = Date.now()) {
  ...
  return {
    ...
    approvedFactCount: ledger.filter(factApprovalIsExplicit).length,
    candidateFactCount: ledger.filter((entry) => entry.reviewStatus === "candidate"
      || APPROVED_REVIEW_STATUSES.has(entry.reviewStatus) && !factApprovalIsExplicit(entry)).length,
    staleFactCount: ledger.filter((entry) => factApprovalIsExplicit(entry) && !factIsAnswerable(entry, now)).length,
...
function applyReviewDecisions(ledger = [], decisions = []) {
  ...
  for (const decision of decisions) {
    const group = byFact.get(decision.factId) || [];
    group.push(decision);
    byFact.set(decision.factId, group);
  }
  const updated = ledger.map((entry) => {
    const history = byFact.get(entry.id) || [];
    const matching = history.filter(decision => reviewDecisionMatches(decision, entry));
    for (const decision of history.filter(decision => !reviewDecisionMatches(decision, entry))) stale.push({ decision, entry });
    if (!matching.length) return entry;
    const decision = latestReviewDecision(matching);
    if (!decision) return { ...entry, reviewStatus: 'candidate', lifecycle: 'uncertain', reviewNote: 'Decision ordering requires review.' };
    ...
    const base = {
      ...
      reviewDecisionId: decision.id || decision.reviewId || hash(`${entry.id}:${reviewedAt}`).slice(0, 24),
    };
    let resolved = base;
    if (decision.decision === "approve-proposed" || decision.decision === "mark-current-superseded") {
      resolved = { ...base, reviewStatus: "approved", lifecycle: lifecycleFor({ ...base, reviewStatus: "approved", lifecycle: "current" }) };
