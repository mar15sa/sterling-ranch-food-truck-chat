...
function poolSummary(mapping, season, date) {
  const inSeason = withinConfiguredPoolSeason(date, season);
  if (mapping.state === "closed" && inSeason === false && season?.startLabel && season?.endLabel) {
    return `The official CAB status is ${mapping.label}: the pool is closed for the season. Its regular season runs from ${season.startLabel} through ${season.endLabel}.`;
  }
  return `The official CAB status is ${mapping.label}: ${mapping.residentMeaning}`;
}

async function getCommunityPoolStatus(options = {}) {
  const { profile, fetchImpl = global.fetch, now = () => new Date() } = options;
  if (typeof fetchImpl !== "function") throw new Error("Fetch is unavailable.");
  const { adapter, sourceUrl, mappings, markerClass, season } = configuredPoolStatus(profile);
  ...
  return {
    ...
    summary: poolSummary(parsed.mapping, season, checkedAt),
...
module.exports = { configuredPoolStatus, getCommunityPoolStatus, parseConfiguredPoolStatus, withinConfiguredPoolSeason };
