...
const { buildFactLedger, factApprovalIsExplicit, factIsAnswerable, factLedgerStatus, resolveFactLedger } = require("./community-truth");
const { buildReviewItems, syncReviewItems } = require("./community-source-review");
const { createReviewSyncState } = require('./community-review-sync-state');
const { isDynamicSource, isFreshnessTrackedSource, isOwnerObservation } = require("./community-source-identity");
const { applyDomainOutageGrace, observeCanonicalSource, revalidateApprovedEvidence, sourceEvidenceIdentity } = require("./community-approved-revalidation");
...
let exactRevalidationPromise = null;
...
const reviewSyncState = createReviewSyncState();
...
function communitySourceStatus(index = getBundledIndex(), now = Date.now(), { includeStaleSources = false, includeApprovedEvidenceCheckTime = false, refreshError } = {}) {
  const freshnessTrackedSources = index.sources.filter(isFreshnessTrackedSource);
  const staleSources = freshnessTrackedSources.filter((source) => source.staleAfter && new Date(source.staleAfter).getTime() < now);
  const approvedCheckTimes = freshnessTrackedSources
  ...
  const expiredOwnerObservationCount = index.sources.filter((source) => isOwnerObservation(source)
    && source.staleAfter && new Date(source.staleAfter).getTime() < now).length;
  const conflictGroups = includeApprovedEvidenceCheckTime
    ? resolveFactLedger(index.factLedger || [], { factAuthority: index.factAuthority }).unresolvedSensitive : [];
  const inventoryBacklog = Number(index.inventory?.pendingCount || 0);
  const expiredApprovedSourceCount = staleSources.length;
  const expiredApprovedFactCount = (index.factLedger || []).filter((entry) => {
    const source = sourcesById.get(entry.sourceId);
    return (!source || isFreshnessTrackedSource(source))
      && factApprovalIsExplicit(entry) && !factIsAnswerable(entry, now);
  }).length;
  ...
  return {
    ...
    // Exact evidence timestamps are owner diagnostics. Keep them out of the
    // public health response, where a one-record aggregate could disclose the
    // timestamp of that private source record.
    ...(includeApprovedEvidenceCheckTime ? {
      approvedEvidenceLastCheckedAt: approvedCheckTimes.length ? new Date(Math.max(...approvedCheckTimes)).toISOString() : null,
      approvedConflictGroupCount: conflictGroups.filter(group => group.entries.some(entry => entry.reviewStatus === 'approved')).length,
      candidateConflictGroupCount: conflictGroups.filter(group => !group.entries.some(entry => entry.reviewStatus === 'approved')).length,
    } : {}),
    ...
    exactRevalidating: Boolean(exactRevalidationPromise),
...
async function revalidateDueApprovedEvidence(options = {}) {
  ...
  const result = await revalidateApprovedEvidence(trusted, {
    ...
    refreshAheadMs: options.refreshAheadMs,
  ...
  if (!options.index || options.replaceCurrent === true) {
    currentIndex = !options.index && getBundledIndex() !== trusted
      ? mergeApprovedRenewal(getBundledIndex(), trusted, result.temporaryIndex)
      : result.temporaryIndex;
  }
  return result;
}

function mergeApprovedRenewal(current, baseline, renewed) {
  const baselineById = new Map(baseline.sources.map(source => [source.id, source]));
  const renewedById = new Map(renewed.sources.map(source => [source.id, source]));
  const sources = current.sources.map(source => {
    const before = baselineById.get(source.id), after = renewedById.get(source.id);
    if (!before || !after || sourceEvidenceIdentity(current, source) !== sourceEvidenceIdentity(baseline, before)
      || Date.parse(after.checkedAt) < Date.parse(source.checkedAt)
      || !(Date.parse(after.staleAfter) > Date.parse(source.staleAfter))) return source;
    return { ...source, checkedAt: after.checkedAt, staleAfter: after.staleAfter,
      ...(after.documentFingerprint ? { documentFingerprint: after.documentFingerprint, hashScheme: after.hashScheme } : {}) };
  });
  return { ...current, sources, factLedger: refreshApprovedFactLedger(current.factLedger, sources) };
}

function refreshApprovedEvidence(options = {}) {
  if (exactRevalidationPromise) return exactRevalidationPromise;
  // One cadence plus a small scheduling margin: never deliberately wait until
  // approved evidence expires. This lock is independent of discovery/review.
  exactRevalidationPromise = revalidateDueApprovedEvidence({
    refreshAheadMs: REFRESH_INTERVAL_MS + 15 * 60_000, ...options,
  }).finally(() => { exactRevalidationPromise = null; });
  return exactRevalidationPromise;
}
...
function refreshApprovedSource(source, candidateSources) {
  ...
  return refreshed && Date.parse(refreshed.checkedAt) > Date.parse(source.checkedAt)
    && Date.parse(refreshed.staleAfter) >= Date.parse(source.staleAfter)
    ? { ...source, checkedAt: refreshed.checkedAt, staleAfter: refreshed.staleAfter } : source;
}
...
async function refreshCommunitySources(options = {}) {
  // Even when discovery or durable review synchronization is still occupied,
  // the next scheduled tick must be able to renew approved evidence.
  if (options.exactRevalidation !== false) await refreshApprovedEvidence(options);
  if (refreshPromise) return refreshPromise;
  reviewSyncState.beginObservation();
  refreshPromise = (async () => {
    ...
    .then(async (index) => {
      ...
      reviewSyncState.observe(index, reviewItems);
      reviewSyncState.beginSync();
      try {
        const syncResult = await syncReviewItems(reviewItems);
        reviewSyncState.finishSync(syncResult);
        lastReviewSyncError = "";
      } catch (error) {
        reviewSyncState.failSync(error);
    ...
    .catch((error) => {
      ...
      reviewSyncState.failObservation(error);
      throw error;
    })
    .finally(() => { refreshPromise = null; reviewSyncState.endObservation(); });
  return refreshPromise;
}

function getCommunityIndex(options = {}) {
  const index = getBundledIndex();
  if (options.refreshIfStale !== false && communitySourceStatus(index).stale) {
...
function getSourceReviewSnapshot() { return reviewSyncState.snapshot(); }

module.exports = { communitySourceStatus, getCommunityIndex, getCommunityProfile, getSourceReviewSnapshot, isFreshnessTrackedSource, mergeApprovedRenewal, reconcileCommunityIndex, refreshApprovedEvidence, refreshCommunitySources, revalidateDueApprovedEvidence, scheduleCommunityRefresh };
