...
import {REQUIRED_RESIDENT_JOURNEYS} from './check.mjs';
...
test('future village UI cannot reuse project-only screenshot evidence',()=>withFixture(({root,review})=>{
 ...
 writeFileSync(path.join(root,'eval/visual-review.json'),JSON.stringify(review));
 assert.equal(evaluate({root}).ok,false);
 review.villagePlanChecks={passed:true,states:['village-plan-desktop','village-plan-mobile','village-plan-selected'].map(name=>{
