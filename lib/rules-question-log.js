...
function buildQuestionLogEntry(question, answer = {}, options = {}) {
  ...
  const quality = qualityAssessmentForLog(question, answer, options);
  return {
...
async function setQuestionNeedsWork(pageId, needsWork, fetchImpl = globalThis.fetch) {
  ...
  await notionRequest(
    pagePath,
    {
      ...
      body: JSON.stringify({
        properties: {
          [names.needsWork]: { checkbox: Boolean(needsWork) },
        },
      }),
    },
    fetchImpl
  );
  return { id: normalizedPageId, needsWork: Boolean(needsWork) };
}
