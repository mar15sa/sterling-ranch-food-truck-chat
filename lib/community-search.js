...
function normalizedRoutingPlan(plan = {}, question = "", options = {}) {
  ...
  const goal = compoundGoal || modelGoal;
  ...
  const normalized = normalizeInterpretation({
    ...
    intent,
    goal,
    goals: [goal, ...(Array.isArray(plan.goals) ? plan.goals : [])],
  }, question, options);
  return normalized;
}
...
function searchCommunityIndex(question, options = {}) {
  ...
  const ledgerEntriesBySource = new Map();
  ...
  if (enforceLedger) {
    for (const entry of index.factLedger) {
      ...
      const entries = ledgerEntriesBySource.get(key) || [];
      entries.push(entry);
      ledgerEntriesBySource.set(key, entries);
      if (!factIsAnswerable(entry)) continue;
  ...
  const requestedFactFacet = requestedFacet(question, details, intent);
  const ranked = index.sources
    ...
    .map((source) => {
      ...
      const sourceLedger = ledgerEntriesBySource.get(`${source.id}:${source.contentHash}`) || [];
      const relevantLedger = requestedFactFacet ? sourceLedger.filter((entry) => entry.facet === requestedFactFacet) : [];
      ...
      return {
        ...source,
        facts,
        withheldByFreshness: enforceLedger && relevantLedger.length > 0 && !relevantLedger.some((entry) => factIsAnswerable(entry)),
