...
function normalizeInterpretation(plan = {}, question = "", options = {}) {
  ...
  // Contact changes both retrieval and the type of evidence returned. Accept
  // it only when the resident explicitly asks for a person, phone, email, or
  // call; a model-inferred contact preference must not displace the rule or
  // service the resident actually requested.
  if (!explicitDetails.includes("contact")) goalSet.delete("contact");
  ...
  const detailSet = new Set([...modelDetails, ...explicitDetails]);
  if (!explicitDetails.includes("contact")) detailSet.delete("contact");
  const consequenceQuestion = /\b(?:what happens|what if|do not|don't|cannot|can't|fail to|late|past due|delinquent|penalty|consequence|disconnect|shut ?off|collection)\b/i.test(question);
  if (goals[0] === "information" && consequenceQuestion) detailSet.delete("action");
  let requestedDetails = DETAIL_PRIORITY.filter((detail) => detailSet.has(detail));
  if (goals[0] === "payment" && requestedDetails.includes("action")) {
    requestedDetails = requestedDetails.filter((detail) => detail !== "price");
  }
