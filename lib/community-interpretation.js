...
const DETAILS = new Set(["price", "action", "date", "hours", "contact", "permission", "specification", "methods", "examples", "status"]);
...
const DETAIL_PRIORITY = ["price", "action", "date", "hours", "contact", "permission", "specification", "methods", "examples", "status"];
...
function deterministicRequestedDetails(question = "") {
  ...
  const action = resourceViewRequest || /\b(?:how do|where (?:do|can)|book|reserve|rent|submit|register|sign up|form|pay|payment|settle|settling|log in|login|download)\b/.test(text)
  ...
  if (/\b(?:when|date|day|today|tomorrow|weekend|upcoming)\b/.test(text)
    || (!schedulingProcess && /\bschedule\b/.test(text))) details.push("date");
  ...
  if (/\b(?:payment |delivery |renewal |service )?methods?\b|\bways? (?:can|may|do) (?:i|we)\b/.test(text)) details.push("methods");
