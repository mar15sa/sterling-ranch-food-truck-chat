...
function focusedTopicAnswer(query, sources, formatAnswer) {
  ...
  if (/\b(?:pickle ?ball|sport court)\b/i.test(query)) {
    ...
    // Public-court hours, reservations, fees, and open-play rules belong to
    // the current CAB facility page. Keep the narrower rulebook assistant from
    // presenting the private-home court section as public-facility guidance;
    // the unified assistant will replace this boundary with the facility page.
    return formatAnswer(
      "The rulebook source I found covers private courts at homes; it does not control operations at Sterling Ranch’s public pickleball courts.",
      ["Public-court hours, reservations, fees, and open-play instructions are maintained on the CAB’s Pickleball Courts page."],
      "Use the current CAB Pickleball Courts page for public-court information. If you mean building a private court, DRC approval is required and it may not be lighted."
    );
  }
  ...
  if (isPlantListQuestion(query) && /preapproved plant list/i.test(`${sources[0]?.title || ""} ${text}`)) {
    ...
    return formatAnswer(
      "The selected official source organizes acceptable tree, shrub, grass and perennial species by relative water need in a preapproved plant list.",
      findings,
