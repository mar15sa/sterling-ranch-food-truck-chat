...
function approvedOperationalSubmissionAnswer(question, routingPlan, options = {}) {
  ...
  // A project-specific permission request must continue to the governing
  // rules path. This route is only for a generic submission declaration that
  // matches the approved operational source itself.
  if (requested.includes("permission")
    && !/\b(?:drc|design review|architectural)\b/i.test(question)) return null;
...
function partialGoverningRuleAnswer(question, rulesAnswer, requested = [], routingPlan = null) {
  ...
  const partialAction = displayedSources[0]?.sourceUrl
    ...
    : null;
  const unverifiedActionStep = "I could not verify the application or submission step from current owner-approved source claims.";
