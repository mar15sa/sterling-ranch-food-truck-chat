...
export function visitCard(place, { parent = null, compact = false } = {}) {
  ...
  return `<section class="visit-card${compact ? ' visit-card-compact' : ''}" aria-label="Visit information for ${escapeHtml(place.name || 'this place')}">${statusBadge}${location}${access}${reservation}${hours}${freshness}${arrival}${highlightsHtml}${practicalHtml}${primary.length ? `<div class="visit-primary-actions">${primary.join('')}</div>` : ''}${unknownHtml}${sourcesHtml}</section>`;
}
