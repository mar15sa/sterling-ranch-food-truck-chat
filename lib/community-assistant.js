...
function withheldSourceAnswer(source, intent) {
  const unavailable = buildAnswerContract({
    directAnswer: "I could not safely confirm the current value from approved, up-to-date CAB information.",
...
function exactOperationalFacilitySource(question, sources = []) {
  ...
  if (asksForPoolHours(normalized)) {
    return sources.find((source) =>
      ["facilities", "services"].includes(source.sourceType)
      && /\bOverlook Outdoor Pool\b/i.test(source.title || "")
    ) || null;
  }
...
function denverCalendar(now = new Date()) {
...
function nthWeekdayOfMonth(year, monthIndex, weekday, occurrence) {
  ...
  return 1 + ((weekday - first.getUTCDay() + 7) % 7) + (occurrence - 1) * 7;
}
...
function poolHoursAnswer(source, question, now = new Date()) {
  const text = String(source.text || "").replace(/\s+/g, " ").trim();
  const hasControllingSchedule = /open Memorial Day weekend through Labor Day/i.test(text)
    && /Monday-Friday:\s*5:00 am\s*-\s*9:00 am:\s*Lap Swim/i.test(text)
    && /9:00 am\s*-\s*8:45 pm:\s*Open Swim/i.test(text)
    && /Saturday:\s*7:00 am\s*-\s*8:45 pm/i.test(text)
    && /Sunday:\s*7:00 am\s*-\s*8:45 pm/i.test(text);
  if (!asksForPoolHours(question) || !/\bOverlook Outdoor Pool\b/i.test(source.title || "") || !hasControllingSchedule) return null;

  const officialSource = sourceForDisplay(source);
  const asksLaborDay = /\blabor day\b/i.test(question);
  const calendar = denverCalendar(now);
  ...
  const seasonStart = 500 + memorialDay;
  const seasonEnd = 900 + laborDay;
  const inSeason = dateNumber >= seasonStart && dateNumber <= seasonEnd;

  let directAnswer;
  let keyDetails;
  let status = "verified";
  if (asksLaborDay) {
    directAnswer = "The CAB says the outdoor pool season runs through Labor Day. Its published Monday–Friday schedule lists lap swim from 5:00 am–9:00 am and open swim from 9:00 am–8:45 pm.";
    keyDetails = ["The page does not label a separate holiday schedule, so check the live pool status before going."];
    status = "verified-incomplete";
  } else if (!inSeason) {
    directAnswer = "The CAB’s published outdoor-pool season ended on Labor Day, so its page does not provide a closing time for today.";
    keyDetails = ["During pool season, the published closing time is 8:45 p.m."];
    status = "verified-incomplete";
  } else if (["Saturday", "Sunday"].includes(calendar.weekday)) {
    directAnswer = `The CAB’s published closing time for the outdoor pool today (${calendar.weekday}) is 8:45 p.m.`;
    keyDetails = ["Weekend pool hours begin at 7 a.m.; lifeguards are listed from 9 a.m.–8:45 p.m."];
  } else {
    directAnswer = `The CAB’s published closing time for the outdoor pool today (${calendar.weekday}) is 8:45 p.m.`;
    keyDetails = ["Open swim is listed from 9 a.m.–8:45 p.m.; lap swim is listed from 5–9 a.m."];
  }

  const action = { label: "Open official pool hours", url: source.sourceUrl, actionType: "information" };
  const nextStep = status === "verified-incomplete"
    ? "Open the official pool page and check its live status before you go."
    : "Check the live pool status before you go in case of weather, maintenance, or capacity closures.";
  ...
  }), _connectorDiagnostics: {
    sourceOutcome: "ineligible",
    appliedFilters: [],
    beforeFilterCount: null,
    afterFilterCount: null,
    shortcutRejections: [{ connector: "pool-status", reasons: ["goal-not-supported"] }],
  } };
}
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (asksForAction && (!routingPlan || interpretationOutcome === "fallback")) {
    ...
    const fallbackGoal = [...ACTION_GOALS].find((goal) =>
      (actionSource?.actions || []).some((action) => actionSupportsGoal(action, goal))
    );
    const actionLabelTerms = String(actionSource?.actions?.[0]?.label || "").toLowerCase()
    ...
    if (Number(fallbackResult.sources[0]?.score || 0) >= 36
      ...
      const preferredAction = actionSource.actions?.[0];
      if (fallbackGoal === "payment"
        && /utilityhawk/i.test(`${preferredAction?.label || ""} ${preferredAction?.url || ""}`)
        && /utilityhawk/i.test(`${contentSource?.title || ""} ${contentSource?.text || ""}`)) {
        const paymentAnswer = buildAnswerContract({
          directAnswer: "Pay your Sterling Ranch water bill through the UtilityHawk payment portal.",
          keyDetails: ["The approved CAB FAQ identifies UtilityHawk as the single sign-on path to the payment portal."],
          nextStep: "Open UtilityHawk below and sign in to continue to payment.",
          actions: [preferredAction],
          sources: fallbackResult.sources,
          status: "verified",
          requestedDetails: ["action"],
          coveredDetails: ["action"],
          checkedAt: contentSource?.checkedAt,
          answerMode: "community-source-action",
        });
        ...
        const paymentDecision = shortcutEligibility("official-action", {
          question,
        ...
        if (paymentDecision.eligible) return paymentCandidate;
        recordShortcutRejection(options, "official-action", paymentDecision);
      }
  ...
  const recyclingRequestDecision = shortcutEligibility("recycling", { question, plan: shortcutPlan });
  if (recyclingScheduleRequested && recyclingRequestDecision.eligible && options.getWasteSchedule) {
    try {
      const connectorResult = await options.getWasteSchedule();
      const candidate = { ...liveRecyclingScheduleAnswer(question, connectorResult), communityIntent: "services" };
      const decision = shortcutEligibility("recycling", { question, plan: shortcutPlan, candidate, connectorResult });
      if (decision.eligible) return candidate;
      recordShortcutRejection(options, "recycling", decision);
    } catch {
  ...
  } else if (recyclingScheduleRequested && !recyclingRequestDecision.eligible) recordShortcutRejection(options, "recycling", recyclingRequestDecision);
  ...
  if (activeIntent === "status" && poolRequestDecision.eligible && options.getPoolStatus) {
    try {
      ...
      const candidate = poolStatusAnswer(connectorResult);
  ...
  if (operationalSource && Number(operationalSource.score || 0) >= 24) {
    const structuredFacilityAnswer = poolHoursAnswer(operationalSource, question, options.now) || pickleballFacilityAnswer(operationalSource);
    const facilityCandidate = {
      ...(structuredFacilityAnswer || await sourcedAnswer(question, { ...operationalSearch, sources: [operationalSource] }, options)),
      communityIntent: intent,
  ...
  if (shouldConsultRules) {
    ...
    try {
      ...
      if (withheldControls) return withheldSourceAnswer(withheld, intent);
      throw error;
    }
    ...
    if (withheldControls) {
      ...
      if (needsBlockedOperationalDetail
        || rulesAnswer?.confidence?.canAnswer !== true
        || !independentlySourced) {
        return withheldSourceAnswer(withheld, intent);
      }
    }
...
module.exports = { answerCommunityQuestion, bestContactContext, cleanAnswerText, composeProactiveAnswer, conciseRecurringSchedule, contactDirectAnswer, directlyAnswersQuestionForm, eventsAnswer, extractiveAnswer, liveRecyclingScheduleAnswer, poolHoursAnswer, poolStatusAnswer, relevantActions, sourcedAnswer, unanchoredRecurringScheduleAnswer, usefulSentences };
