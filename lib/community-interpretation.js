...
function deterministicRequestedDetails(question = "") {
  ...
  if (/\b(?:hours?|time)\b/.test(text) || (!yesNoOpenState && /\b(?:open|closed)\b/.test(text))) details.push("hours");
  ...
  if (serviceStatus || yesNoOpenState || instruction.separateStatus || (!instruction.check && (/\b(?:status|currently|right now)\b/.test(text)
