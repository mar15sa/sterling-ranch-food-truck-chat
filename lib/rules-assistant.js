...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "") {
  if (isLandscapeScreenQuery(query)) {
    ...
    if (answer) return { available: true, answer, sources };
  }
  ...
  if (currentFeeAnswer) return { available: true, answer: currentFeeAnswer, sources };

  const enforcementAnswer = currentEnforcementOverviewAnswer(query, sources);
  if (enforcementAnswer) return { available: true, answer: enforcementAnswer, sources };

  const topicAnswer = simpleTopicOverviewAnswer(query, sources);
  if (topicAnswer) return { available: true, answer: topicAnswer, sources };
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const derived = needsSourceDerivedFacts
    ...
    : { available: true, answer: plainAnswer, sources: answerSources };
  ...
  // Deterministic answers have already been assembled from and checked against
  // the controlling sources. Avoid adding latency and cost by rewriting them
  // unless the owner explicitly enables the optional presentation layer.
  const rewrittenAnswer = process.env.RULES_ENABLE_LLM_REWRITE === "true"
    ? await rewriteAnswerWithLLM(question, derived.answer, derived.sources)
    : null;
