...
function holidayFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    if (stale || source.canonicalScopedProjection !== true) continue;
    ...
    const regularSchedule = regularScheduleForDate(text, holidayDate);
...
function datedFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    const holidayHours = explicitHolidayHours(text, holidayName);
    const sourceForAnswer = sourceForDisplay(source);
    if (holidayHours) {
  ...
  if (!answer.actions.length && answer.sources.length) {
    ...
    answer = {
      ...
      actions: (answer.sources || [])
        .filter((source) => source.isOfficialResource === true && /^https?:\/\//i.test(source.sourceUrl || ""))
