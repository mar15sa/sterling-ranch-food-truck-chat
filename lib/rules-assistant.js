...
function underEaveLightingOverviewAnswer(sources = []) {
  return sourceClauseOverviewAnswer("under-eave lighting", sources);
}
...
function shortTermRentalOverviewAnswer(sources = []) {
  return sourceClauseOverviewAnswer("short-term rental", sources);
}

function wateringRestrictionOverviewAnswer(query, sources = []) {
  return sourceClauseOverviewAnswer(query || "watering restriction", sources);
}

function fenceHeightOverviewAnswer(sources = []) {
  return sourceClauseOverviewAnswer("fence height", sources);
}

function trashStorageOverviewAnswer(query, sources = []) {
  return sourceClauseOverviewAnswer(query || "trash recycling storage", sources);
}

function rvParkingOverviewAnswer(query, sources = []) {
  return sourceClauseOverviewAnswer(query || "recreational vehicle parking", sources);
}
