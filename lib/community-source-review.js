...
async function validateSchema(dataSourceId, fetchImpl = globalThis.fetch) {
  const config = reviewConfig();
  const key = `${dataSourceId}|${config.titleProperty}`;
  if (!schemaPromises.has(key)) {
    const promise = notionRequest(`/data_sources/${encodeURIComponent(dataSourceId)}`, {}, fetchImpl)
...
async function createReviewRecord(record, fetchImpl = globalThis.fetch) {
  ...
  const titleProperty = await validateSchema(dataSourceId, fetchImpl);
...
async function listReviewRecords(filters = {}, fetchImpl = globalThis.fetch) {
  const dataSourceId = await resolveDataSourceId(fetchImpl);
  await validateSchema(dataSourceId, fetchImpl);
  const records = [];
