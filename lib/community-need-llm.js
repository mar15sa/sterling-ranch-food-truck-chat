...
const DEFAULT_MODEL = process.env.COMMUNITY_NEED_INTERPRETER_MODEL
  ...
  || "claude-haiku-4-5";
const MAX_NEEDS = 4;
const REQUESTED_DETAILS = new Set([...DETAILS, "information", "reimbursement"]);
...
function normalizePlannedContract(question, requestContext, parsed, fallback, model) {
  ...
  for (const raw of parsed.needs) {
    ...
    const requestedDetails = [...new Set((Array.isArray(raw?.requestedDetails) ? raw.requestedDetails : [])
      .map(String).filter((detail) => REQUESTED_DETAILS.has(detail)))].slice(0, 6);
    if (!requestedDetails.length) return null;
    const request = clean(raw.request);
    const routeRequest = clean(raw.routeRequest, 300);
  ...
  try {
    const response = await fetchImpl(API_URL, {
      ...
      body: JSON.stringify({
        ...
        tools: [NEEDS_TOOL],
        tool_choice: { type: "tool", name: NEEDS_TOOL.name },
    ...
    const contract = normalizePlannedContract(question, options.requestContext, parsed, fallback, model);
    if (!contract) {
      metrics.rejected += 1;
...
function writerSources(candidate) {
  ...
  const sources = (candidate.sources || []).map((source) => {
    ...
    const claimText = claims.filter((claim) => (claim.evidenceSourceIds || []).includes(id)).map((claim) => claim.text);
    const sourceActions = actions.filter((action) => action.sourceId === id || action.url === source.sourceUrl);
    return {
      ...source,
      id,
      text: [source.text, source.excerpt, ...claimText].filter(Boolean).join("\n"),
...
async function rewriteNeedFirstCandidate({ question, contract, candidate }, options = {}) {
  ...
  if (!candidate || !contract || candidate.completion?.outcome !== "complete"
    || !Array.isArray(candidate.sources) || !candidate.sources.length) return null;
  ...
  const shape = verifyStructuredDraft(draft, sources, { question, routingPlan });
  if (!shape.valid) return null;
  const sentences = draftSentences(shape.draft);
  const sentenceClaims = sentences.map((text) => ({ text, evidenceSourceIds: evidenceForClaim(text, sources), verified: true }));
  if (sentenceClaims.some((claim) => !claim.evidenceSourceIds.length)) return null;
  ...
  const answer = [shape.draft.directAnswer, ...(shape.draft.keyDetails || []), shape.draft.nextStep]
    .filter(Boolean).join("\n\n");
  const protectedAllowed = new Set(protectedValues(sources.map((source) => source.text).join("\n")));
  if (protectedValues(answer).some((value) => !protectedAllowed.has(value))) return null;
  const assessment = assessResidentNeeds(contract, {
    ...candidate,
    answer,
    directAnswer: shape.draft.directAnswer,
    keyDetails: shape.draft.keyDetails,
    nextStep: shape.draft.nextStep,
    claims,
  });
  if (!sameNeedCoverage(candidate.completion?.needs || [], assessment.needs)) return null;
  return {
    ...candidate,
    ...
    aiComposition: { model, accepted: true },
  };
}
