...
5. Calculate the registry fingerprint with the exported `baselineRegistryFingerprint` helper in `check.mjs` and put it in the review field of the same name. It hashes `JSON.stringify` of the parsed registry, rather than whitespace in the raw file.
