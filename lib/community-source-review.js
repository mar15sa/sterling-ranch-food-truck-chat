...
const { createReviewCache } = require('./community-review-cache');
...
function approvedLinkProjectionStillExists(source = {}, candidates = []) {
  const facts = source.facts || [];
  const actions = source.actions || [];
  if (facts.length || !actions.length
    || !actions.every((action) => APPROVED_REVIEW_STATUSES.has(action.reviewStatus))) return false;
  const currentActions = candidates.flatMap((candidate) => candidate.actions || []);
  return actions.every((approved) => currentActions.some((current) =>
    normalizeUrl(current.url) === normalizeUrl(approved.url)
      && String(current.label || "").trim() === String(approved.evidence?.label || approved.label || "").trim()
  ));
}
...
let connectionContexts = new WeakMap();
let titlePropertyName = "";

function connectionContext(fetchImpl = globalThis.fetch, config = reviewConfig()) {
  if (typeof fetchImpl !== 'function') throw new Error('The source-review Notion connection is not configured.');
  let contexts = connectionContexts.get(fetchImpl);
  if (!contexts) { contexts = new Map(); connectionContexts.set(fetchImpl, contexts); }
  // Credentials are never stored in a public cache key or status response.
  const key = hash(JSON.stringify([config.token, config.databaseId, config.dataSourceId, config.titleProperty]));
  if (!contexts.has(key)) {
    if (contexts.size >= 16) {
      const idle = [...contexts].find(([, context]) => !context.cache.isLoading() && !context.syncPending && !context.writesPending);
      if (!idle) throw new Error('The source-review connection cache is busy; try again shortly.');
      contexts.delete(idle[0]);
    }
    contexts.set(key, { config, dataSourcePromise: null, schemaPromises: new Map(), cache: createReviewCache(), syncTail: Promise.resolve(), syncPending: 0, writesPending: 0 });
  }
...
function sourceMap(index = {}) {
  ...
  for (const source of index.sources || []) {
    if (!source.id || result.has(source.id)) {
      const error = new Error(`Source review requires unique source identifiers; reconcile ${source.id || 'the missing identifier'} before compiling a batch.`);
      error.code = 'AMBIGUOUS_SOURCE_ID';
      throw error;
    }
...
function buildReviewItems(trusted = {}, candidate = {}, profile = {}) {
  ...
  for (const [id, source] of before) {
    ...
    const samePageCandidates = (candidate.sources || []).filter((candidateSource) =>
      normalizeUrl(candidateSource.sourceUrl) === normalizeUrl(source.sourceUrl));
    // Link-only reviewed projections use a stable resident-facing ID while a
    // fresh crawl uses extraction IDs. Do not tell the owner the link vanished
    // when the exact reviewed label and destination are still on the page.
    // Exact page-version revalidation remains a separate required gate.
    if (approvedLinkProjectionStillExists(source, samePageCandidates)) continue;
    items.push(sourceReviewItem("source-removal", null, source, candidateFingerprint, releaseFingerprint));
  }
...
function decisionForItem(item = {}, decisions = []) {
  return latestReviewDecision(decisions.filter((decision) =>
...
async function notionRequest(path, options = {}, fetchImpl = globalThis.fetch, config = reviewConfig()) {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      lastError = error;
      // Page creation has no idempotency key. A lost response may have created
      // the record; do not retry it and create a duplicate decision/review.
      if (path === '/pages' && options.method === 'POST') throw error;
      if (/HTTP 4\d\d/.test(error.message || "") && !/429/.test(error.message || "")) throw error;
    }
    if (path === '/pages' && options.method === 'POST') throw lastError;
    await new Promise((resolve) => setTimeout(resolve, 250 * (attempt + 1)));
  }
  throw lastError || new Error("Notion source review request failed.");
}

async function resolveDataSourceId(fetchImpl = globalThis.fetch, context = connectionContext(fetchImpl)) {
  ...
  if (!context.dataSourcePromise) {
    context.dataSourcePromise = notionRequest(`/databases/${encodeURIComponent(config.databaseId)}`, {}, fetchImpl, config).then((database) => {
      ...
      return sources[0].id;
    }).catch((error) => { context.dataSourcePromise = null; throw error; });
  }
  return context.dataSourcePromise;
}

function sourceReviewSetupError(problems = []) {
  const error = new Error(
    `The private source-review database needs one-time setup before it can be used: ${problems.join("; ")}. `
    + "Set COMMUNITY_SOURCE_REVIEW_NOTION_TITLE_PROPERTY and add the listed properties in Notion. Regular dashboard use will not change the database schema."
  );
  error.code = "SOURCE_REVIEW_SCHEMA_SETUP_REQUIRED";
  return error;
}

function validateSchemaProperties(source = {}, config = reviewConfig()) {
  const properties = source?.properties || {};
  const problems = [];
  if (!config.titleProperty) {
    problems.push("COMMUNITY_SOURCE_REVIEW_NOTION_TITLE_PROPERTY is not configured");
  } else if (!properties[config.titleProperty]) {
    problems.push(`the configured title property “${config.titleProperty}” is missing`);
  } else if (properties[config.titleProperty].type !== "title") {
    problems.push(`the configured title property “${config.titleProperty}” must be a title field`);
  }

  for (const [name, expected] of Object.entries(requiredSchema())) {
    const actual = properties[name];
    const expectedType = Object.keys(expected)[0];
    if (!actual) {
      problems.push(`the required “${name}” property is missing`);
      continue;
    }
    if (actual.type !== expectedType) {
      problems.push(`the required “${name}” property must be a ${expectedType} field`);
      continue;
    }
    if (expectedType === "select") {
      const expectedOptions = new Set((expected.select?.options || []).map((option) => option.name));
      const actualOptions = new Set((actual.select?.options || []).map((option) => option.name));
      const missingOptions = [...expectedOptions].filter((option) => !actualOptions.has(option));
      if (missingOptions.length) problems.push(`the “${name}” select field is missing: ${missingOptions.join(", ")}`);
    }
  }
  if (problems.length) throw sourceReviewSetupError(problems);
  return config.titleProperty;
}

// This is deliberately validation only. Schema changes are a separate,
// owner-initiated Notion setup task and may never happen during a dashboard
// read, background sync, or review decision.
async function validateSchema(dataSourceId, fetchImpl = globalThis.fetch, context = connectionContext(fetchImpl)) {
  const { config, schemaPromises } = context;
  const key = `${dataSourceId}|${config.titleProperty}`;
  if (!schemaPromises.has(key)) {
    const promise = notionRequest(`/data_sources/${encodeURIComponent(dataSourceId)}`, {}, fetchImpl, config)
      .then((source) => validateSchemaProperties(source, config))
      .catch((error) => {
  ...
  titlePropertyName = await schemaPromises.get(key);
  return titlePropertyName;
}

function notionProperties(record = {}, titleProperty = titlePropertyName) {
  if (!titleProperty) throw sourceReviewSetupError(["the source-review schema has not been validated"]);
  const title = record.recordType === "decision" ? `${record.decision}: ${record.reviewId}` : `${record.topic}: ${record.kind}`;
  return {
    [titleProperty]: { title: richText(title.slice(0, 500)) },
...
async function createReviewRecord(record, fetchImpl = globalThis.fetch, context = connectionContext(fetchImpl)) {
  context.writesPending += 1;
  try {
    const dataSourceId = await resolveDataSourceId(fetchImpl, context);
    const titleProperty = await validateSchema(dataSourceId, fetchImpl, context);
    context.cache.invalidate();
    return await notionRequest("/pages", {
      method: "POST",
      body: JSON.stringify({ parent: { type: "data_source_id", data_source_id: dataSourceId }, properties: notionProperties(record, titleProperty) }),
    }, fetchImpl, context.config);
  } finally {
    // Also invalidate after an ambiguous failure: the write may have succeeded.
    context.cache.invalidate();
    context.writesPending -= 1;
  }
}
...
async function listReviewRecords(filters = {}, fetchImpl = globalThis.fetch, context = connectionContext(fetchImpl)) {
  const dataSourceId = await resolveDataSourceId(fetchImpl, context);
  await validateSchema(dataSourceId, fetchImpl, context);
  const records = [];
  ...
  do {
    ...
    if (!Array.isArray(response.results) || typeof response.has_more !== 'boolean' || (response.has_more && !response.next_cursor)) throw new Error('Notion returned an incomplete review inventory.');
...
// Operational reporting reads existing records only. Unlike review writes, it
// does not need a title field or schema validation, and it never mutates the
// private database.
async function listReviewRecordsReadOnly(filters = {}, fetchImpl = globalThis.fetch) {
  const context = connectionContext(fetchImpl);
  const dataSourceId = await resolveDataSourceId(fetchImpl, context);
  const records = [];
  const seenCursors = new Set();
  let cursor = "";
...
// Dashboard snapshots return immediately, including on a cold process. A
// complete inventory is published atomically, never page-by-page as false zero.
function getReviewRecordsSnapshot(options = {}, fetchImpl = globalThis.fetch) {
  try {
    const context = connectionContext(fetchImpl);
    return context.cache.snapshot(() => listReviewRecords({}, fetchImpl, context), options);
  } catch (error) {
    return { records: [], loaded: false, loading: false, stale: true, checkedAt: '', lastAttemptAt: '', error: error.message };
  }
}

async function loadReviewRecordsCached(options = {}, fetchImpl = globalThis.fetch) {
  const context = connectionContext(fetchImpl);
  return context.cache.load(() => listReviewRecords({}, fetchImpl, context), options);
}
...
async function syncReviewItems(items = [], fetchImpl = globalThis.fetch) {
  ...
  const context = connectionContext(fetchImpl);
  // Same-process refreshes cannot race each other into duplicate creation.
  // A failed batch settles every outstanding write before the next sync starts.
  context.syncPending += 1;
  const run = context.syncTail.catch(() => {}).then(() => syncReviewItemsLocked(items, fetchImpl, context))
    .finally(() => { context.syncPending -= 1; });
  context.syncTail = run;
  return run;
}

async function syncReviewItemsLocked(items, fetchImpl, context) {
  const records = await context.cache.load(() => listReviewRecords({}, fetchImpl, context));
  const existing = records.filter(record => record.recordType === 'review-item');
  ...
  for (let index = 0; index < pending.length; index += 3) {
    ...
    const results = await Promise.allSettled(batch.map((item) => createReviewRecord(item, fetchImpl, context)));
...
function resetSourceReviewCachesForTest() {
  connectionContexts = new WeakMap();
  titlePropertyName = "";
}
...
module.exports = {
  ...
  createReviewRecord,
  fingerprint,
  getReviewRecordsSnapshot,
  listReviewRecords,
  listReviewRecordsReadOnly,
  loadReviewRecordsCached,
  notionProperties,
  recordFromNotion,
  reviewCoverage,
  reviewConfig,
