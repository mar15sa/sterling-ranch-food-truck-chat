...
function immutableCrawlBaseline(index = {}) {
  // The reviewed index is caller-owned evidence. Crawling can retain and
  // disambiguate its records, so work against a complete copy rather than
  // allowing candidate cleanup to rewrite trusted identities or facts.
...
function disambiguateSourceIds(records = []) {
  ...
  for (const [originalId, group] of groups) {
    ...
    for (const source of group) {
      // The same source ID can legitimately arrive from two distinct records
      // during a refresh (for example, a retained page record and a configured
      // action). Only an exact ID + URL + version match above is a duplicate.
      // Keep every differing record and give it an identity that cannot collide
      // when the URL is the same but the version differs.
      const identity = `${new URL(source.sourceUrl).href}\n${source.contentHash}`;
      const sourceId = `${originalId}-${hash(identity).slice(0, 10)}`;
      source.id = sourceId;
...
async function extractPdfBufferText(data) {
...
async function extractPdfText(url, options = {}) {
  ...
  return extractPdfBufferText(data);
}
...
function documentWidgetLinks(html, baseUrl) {
  const links = new Map();
  // CivicPlus document widgets may omit anchor tags until browser hydration.
  // Read their selected IDs as discovery evidence, never as approved actions.
  for (const match of String(html).matchAll(/\bdata-sections\s*=\s*(["'])([\s\S]*?)\1/gi)) {
    let sections;
    try { sections = JSON.parse(decodeHtml(match[2])); } catch { continue; }
    if (!Array.isArray(sections)) continue;
    for (const section of sections) {
      if (typeof section?.DocumentIDs !== "string") continue;
      for (const value of section.DocumentIDs.split(",")) {
        const id = value.trim();
        if (!/^[1-9]\d{0,9}$/.test(id)) continue;
        const url = new URL(`/DocumentCenter/View/${id}`, baseUrl).href;
        links.set(url, { label: `Official document ${id}`, url, context: "Selected in an official document widget", discoveryOnly: true });
      }
    }
  }
  return [...links.values()];
...
function extractActions(links = []) {
  return links
    .filter((link) => !link.navigationOnly && !link.discoveryOnly)
...
function extractFacts(text = "") {
  ...
  const patterns = [
    ...
    { type: "phone", regex: /(?<![\w+])(?:\+?1[-.\s]*)?(?:\((?=\d{3}\)))?[2-9]\d{2}\)?[-.\s]+[2-9]\d{2}[-.\s]+\d{4}\b/g },
    { type: "email", regex: /\b[A-Z0-9._%+&'-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/gi },
    { type: "time", regex: /(?<!\w)(?<!\d:)(?:0?[1-9]|1[0-2])(?::[0-5]\d(?::[0-5]\d)?)?\s*(?:a\.?m\.?|p\.?m\.?)\b/gi },
  ...
  for (const pattern of patterns) {
    for (const match of text.matchAll(pattern.regex)) {
      if (pattern.type === 'phone') {
        // PDF whitespace may wrap punctuation, but bare table columns are not contacts.
        const number = match[0].replace(/^(?:\+?1[-.\s]*)/, '');
        const formatted = /^(?:\([2-9]\d{2}\)\s*[2-9]\d{2}\s*[-.]\s*\d{4}|[2-9]\d{2}\s*[-.]\s*[2-9]\d{2}\s*[-.]\s*\d{4})$/.test(number);
        ...
        const contactLabel = /\b(?:phone|telephone|tel|call|fax|text|at)\s*(?:number\s*)?[:#]?\s*$/i.test(prefix)
          || /^\s*(?:phone|fax)(?=\b|\d)/i.test(suffix);
        const isolatedMixedFormat = /^[2-9]\d{2}\s+[2-9]\d{2}\s*[-.]\s*\d{4}$/.test(number)
          && !/\d[\s,-]*$/.test(prefix) && !/^[\s,-]*\d/.test(suffix);
        if (!formatted && !contactLabel && !isolatedMixedFormat) continue;
      }
...
function pageSources(index = {}, url = "", page = {}) {
  const canonical = canonicalPageUrl(url);
  const indexedIds = new Set(page.indexedSourceIds || []);
  return (index.sources || []).filter((source) => {
    // Configured actions are routing pointers. They are re-created from the
    // current profile below, so a page refresh must not carry an older action
    // version forward just because it points to the same official URL.
    if (source.connectorType === "official-action") return false;
    if (indexedIds.has(source.id)) return true;
...
function sourceActionIdentity(actions = []) {
...
function sourceContentHash(text, documentFingerprint = '', actions = []) {
  const actionIdentity = sourceActionIdentity(actions);
...
function documentTitle(url, label, previousTitle = '') {
  const generic = value => !value || /^(?:(?:previously discovered|pending|configured) official source|official document(?: \d+)?|official sitemap page)$/i.test(value.trim());
  if (!generic(label)) return label;
  const segment = new URL(url).pathname.split('/').filter(Boolean).at(-1) || '';
  if (!/^\d+$/.test(segment)) {
    try { return decodeURIComponent(segment).replace(/[-_]+/g, ' ').trim(); } catch { /* Use preserved metadata below. */ }
  }
  return !generic(previousTitle) ? previousTitle : `Official document ${segment}`;
}
...
async function crawlCommunity(profileInput, options = {}) {
  ...
  const previousIndex = immutableCrawlBaseline(options.previousIndex);
  ...
  const now = typeof suppliedNow === "string" ? Date.parse(suppliedNow) : Number(suppliedNow);
  // `previousIndex` is the reviewed snapshot supplied by source management.
  // Keep that evidence on its own queue: a discovery backlog must never decide
  // which resident-facing pages are allowed to be rechecked.
  const approvedEvidence = new Map();
  for (const source of previousIndex.sources || []) {
    if (isDynamicSource(source) || /-connector-/.test(String(source.id || "")) || source.connectorType === "official-action") continue;
    const url = canonicalPageUrl(source.sourceUrl);
    const prior = approvedEvidence.get(url);
    // A page with several chunks is due when any approved chunk is due.
    if (!prior || new Date(source.staleAfter || 0).getTime() < new Date(prior.staleAfter || 0).getTime()) approvedEvidence.set(url, source);
  }
  const expiredApprovedUrls = new Set([...approvedEvidence.entries()]
    .filter(([, source]) => source.staleAfter && new Date(source.staleAfter).getTime() < now)
    .map(([url]) => url));
  // Keep a small, bounded share of each ordinary incremental crawl for the
  // reviewed evidence set. Urgent expired pages can still preempt this share;
  // this reservation prevents the next set from being starved by discovery.
  const approvedRefreshReserve = options.incrementalRefresh === true
    ? Math.max(1, Math.min(Number(options.approvedRefreshReserve || Math.ceil(maxPages / 4)), maxPages))
    : 0;
  const routineApprovedUrls = [...approvedEvidence.entries()]
    .filter(([url]) => !expiredApprovedUrls.has(url))
    .sort(([, left], [, right]) => new Date(left.checkedAt || 0).getTime() - new Date(right.checkedAt || 0).getTime())
    .slice(0, approvedRefreshReserve)
    .map(([url]) => url);
  const pendingFirst = previousInventory.pendingUrls || [];
  const initialCandidates = [
    ...[...expiredApprovedUrls].map((url) => ({
      // Retain the approved page's title so unchanged content keeps the same
      // source identity as well as the same URL and hash.
      label: previousPages.get(canonicalPageUrl(url))?.title || "Expired approved source",
      url,
      urgentRefresh: true,
    })),
    ...routineApprovedUrls.map((url) => ({
      label: previousPages.get(canonicalPageUrl(url))?.title || "Approved source refresh",
      url,
      approvedRefresh: true,
    })),
  ...
  function queueOrder(candidate) {
    ...
    // Expired approved evidence is an urgent resident-trust repair. It must
    // sort ahead of inventory-only work, even when the latter is much larger
    // than this run's page budget.
    return (candidate.urgentRefresh ? 10_000 : 0)
      + (candidate.approvedRefresh ? 500 : 0)
      + (candidate.pending || missingContent ? 30 : 0)
      + crawlPriority(candidate) * 10 + age;
  }
  ...
  while (queue.length && (pageCount < maxPages || documentCount < maxDocuments)) {
    ...
    try {
      ...
      if (documentUrl) {
        ...
        title = documentTitle(canonical, candidate.label, previousPage?.title);
        ...
        if (!hasReadableDocumentText(text)) {
          const error = new Error('PDF has no readable text layer; OCR or manual document review is required.');
          error.code = 'UNREADABLE_DOCUMENT_TEXT';
          throw error;
        }
      } else {
        ...
        if (fetched.notModified && previousPage) {
          const reused = pageSources(previousIndex, previousPage.canonicalUrl || canonical, previousPage).map((source) => ({
            ...source,
            checkedAt,
        ...
        directoryRows = require('./community-staff-directory').staffDirectoryRows(html, finalUrl);
        directoryRows ||= require('./community-faq-records').faqRows(html, finalUrl);
        if (directoryRows) {
          directoryRows = directoryRows.map(row => ({ ...row, text: stripEmbeddedInstructions(row.text) }));
          text = directoryRows.map(row => row.text).join(' ');
        }
        links = linksFromHtml(contentHtml(html), finalUrl);
        discoveryLinks = [...linksFromHtml(html, finalUrl), ...documentWidgetLinks(html, finalUrl)];
      }
      const pageActions = extractActions(links);
      const actionIdentity = sourceActionIdentity(pageActions);
      const textFingerprint = normalizedContentFingerprint(text);
      const contentFingerprint = actionIdentity ? hash(`${textFingerprint}\nactions:${actionIdentity}`) : textFingerprint;
      const chunks = directoryRows ? directoryRows.map(row => row.text) : chunkText(text);
      ...
      if (text.length >= 80 && !duplicateOf) {
        ...
        const pageSourceType = sourceTypeFor(title, finalUrl, text);
        chunks.forEach((chunk, chunkIndex) => {
          const row = directoryRows?.[chunkIndex];
          const sourceType = row?.kind === 'staff' ? 'services' : row ? sourceTypeFor(row.title, finalUrl, chunk) : pageSourceType;
          const actions = row?.html ? extractActions(linksFromHtml(row.html, finalUrl)) : pageActions;
          const sourceId = `${profile.communityId}-${slug(title)}-${chunkIndex + 1}`;
          const contentHash = sourceContentHash(chunk, documentFingerprint, actions);
          ...
          const source = {
            ...
            title: row ? row.kind === 'staff' ? `Staff directory: ${row.title}` : row.title : title,
            sourceUrl: finalUrl,
            sourceType,
          ...
          source.subjectKey = directoryRows ? directoryRows[chunkIndex].subjectKey : inferSubjectKey(source);
      ...
      pages.push({
        ...
        indexedSourceUrl: canonicalPageUrl(finalUrl),
        title,
        contentFingerprint,
        ...(documentFingerprint ? { documentFingerprint } : {}),
        contentHash: sourceContentHash(text, documentFingerprint, pageActions),
        chunkContentHashes: text.length >= 80 ? chunks.map((chunk, i) => sourceContentHash(chunk, documentFingerprint,
          directoryRows?.[i]?.html ? extractActions(linksFromHtml(directoryRows[i].html, finalUrl)) : pageActions)) : [],
        etag,
        lastModified,
      ...
      if (documentUrl && error?.code === 'UNREADABLE_DOCUMENT_TEXT') {
        // A valid, reachable image-only PDF is an inventory classification,
        // not a website availability failure. It remains visible for OCR or
        // manual review but cannot contribute source text, facts, or actions.
        eligibleUrls.delete(canonical);
        exclusions.set(canonical, { url: canonical, reason: 'no-readable-text-layer-manual-review-required' });
        continue;
      }
      ...
      if (missingStatus && previousPage) {
        ...
        records.push(...pageSources(previousIndex, previousPage.canonicalUrl || canonical, previousPage));
        continue;
      }
  ...
  for (const connector of profile.connectors.filter((item) => item.id !== websiteConnector.id)) {
    ...
    const source = {
      ...
      text,
      excerpt: text,
      // A configured destination is not itself an approved resident action.
      // Some public catalogs expose only navigation, so their profile can
      // require a separately reviewed action record before it is shown.
      actions: connector.adapter?.requiresApprovedActionEvidence
        ? []
        : [{ id: `${connector.id}-open`, label: `Open official ${sourceType}`, url: connector.baseUrl, actionType: "information" }],
      ...(connector.adapter?.requiresApprovedActionEvidence ? { actionEvidenceRequired: true } : {}),
      facts: [],
      contentHash: sourceContentHash(text, '', connector.adapter?.requiresApprovedActionEvidence
        ? []
        : [{ label: `Open official ${sourceType}`, url: connector.baseUrl, actionType: 'information' }]),
      checkedAt,
  ...
  for (const action of profile.actions || []) {
    ...
    const source = {
      ...
      facts: [],
      contentHash: sourceContentHash(text, '', [{ label: action.label, url: action.url, actionType: action.actionType || 'information', keywords: action.keywords || [] }]),
      checkedAt,
  ...
  // Preserve the fetched representation as an evidenced alias when a normal
  // official page redirects into a route excluded from independent discovery.
  // Only complete retained chunks establish coverage; metadata alone cannot.
  for (const page of [...uniquePages]) {
    const target = page.indexedSourceUrl;
    if (!target || isDocumentUrl(page.url) || target === canonicalPageUrl(page.url)
      || uniquePages.some(other => canonicalPageUrl(other.url) === target)
      || !page.chunkContentHashes?.length || !pageChunkRecords(page).every(Boolean)) continue;
    uniquePages.push({ ...page, url: target, canonicalUrl: target, indexed: false, duplicateOf: canonicalPageUrl(page.url) });
    eligibleUrls.add(target);
  }
