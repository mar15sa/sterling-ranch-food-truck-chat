...
function scopedExpectedActionIdentity(sourceUrl, sources = []) {
  ...
  // FAQ records share page-wide navigation links even though each record is a
  // separate answer. They cannot establish a record-level action boundary.
  // Exclude only actions copied onto every record; a link attached to a
  // particular FAQ answer remains part of that answer's fingerprint.
  ...
  const localActions = sources.flatMap((source) => (source.actions || []).filter((action) => {
    const identity = actionIdentity([action]);
    return actionUrlIdentity(action.url) !== actionUrlIdentity(sourceUrl) && counts.get(identity) !== sources.length;
  }));
...
function faqGlobalActionIdentities(sources = []) {
  const counts = new Map();
  for (const source of sources) {
    for (const action of source.actions || []) {
      const identity = actionIdentity([action]);
  ...
  return new Set([...counts].filter(([, count]) => count === sources.length).map(([identity]) => identity));
}
...
async function observeCanonicalSource(sourceUrl, approvedSources, { fetchImpl = globalThis.fetch, extractPdfTextImpl = extractPdfText, timeoutMs = 30_000 } = {}) {
  ...
  const observedActionIdentity = isFaqUrl(sourceUrl)
    ? actionIdentity(observedActions.filter((action) => actionUrlIdentity(action.url) !== actionUrlIdentity(sourceUrl)
      && !faqGlobalActionIdentities(approvedSources).has(actionIdentity([action]))))
...
module.exports = { actionDigest, actionIdentity, actionUrlIdentity, hasApprovedProjection, isApprovedSource, isRevalidatableApprovedSource, observeCanonicalSource, renewExactApprovedEvidence, revalidateApprovedEvidence, selectRevalidationTargetUrls, sourceHash, stableObservedActions };
