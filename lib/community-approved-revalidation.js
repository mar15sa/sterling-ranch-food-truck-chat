...
function renewExactApprovedEvidence(index = {}, { sourceUrl, observedHashes = [], checkedAt, staleAfter } = {}) {
  ...
  const renewedSources = (index.sources || []).filter((source) => source.sourceUrl === sourceUrl
    && isApprovedSource(source) && hashes.has(source.contentHash));
...
module.exports = { actionDigest, actionIdentity, isApprovedSource, observeCanonicalSource, renewExactApprovedEvidence, revalidateApprovedEvidence, selectRevalidationTargetUrls, sourceHash };
