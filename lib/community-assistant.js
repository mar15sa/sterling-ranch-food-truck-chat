...
async function answerCommunityQuestion(query, options = {}) {
  ...
  const answer = completedAnswer.answerMode === "source-evidence-boundary"
    && completedAnswer.answerStatus === "could-not-verify"
    && (completedAnswer.actions || []).some((action) => /sterlingranchcab\.com\/.+/i.test(action.url || ""))
...
module.exports = { answerCommunityQuestion, bestContactContext, cleanAnswerText, composeApprovedOperationalProjection, conciseRecurringSchedule, contactDirectAnswer, datedFacilityHoursAnswer, directlyAnswersQuestionForm, eventsAnswer, extractiveAnswer, liveRecyclingScheduleAnswer, poolStatusAnswer, relevantActions, sourcedAnswer, unanchoredRecurringScheduleAnswer, usefulSentences };
