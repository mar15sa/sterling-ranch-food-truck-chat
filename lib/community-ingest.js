...
async function crawlCommunity(profileInput, options = {}) {
  ...
  function queueOrder(candidate) {
    ...
    return (candidate.pending ? 30 : 0) + crawlPriority(candidate) * 10 + age;
  }
    ...
    try {
      ...
      if (documentUrl) {
        ...
        const fetched = await fetchPublicPage(new URL(canonical), {
          ...fetchOptions,
          etag: previousPage?.etag,
          lastModified: previousPage?.lastModified,
        });
      ...
      if (/returned (?:403|404|410)\b|too large to index safely|redirected outside the official website/i.test(message)) {
        eligibleUrls.delete(canonical);
        exclusions.set(canonical, {
          url: canonical,
          reason: /403/.test(message) ? "restricted-official-route"
            : /too large/i.test(message) ? "oversized-document-safety-limit"
  ...
  const pendingUrls = [...eligibleUrls].filter((url) => !crawled.has(url) && !previousPages.has(url)).sort();
