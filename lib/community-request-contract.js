...
const IMPERATIVE_REQUEST = /^(?:check|explain|find|give|help|list|look up|open|show|tell|write)\b/i;
...
function routingRequestForClause(text = "", priorClauses = []) {
  ...
  const subject = priorClauseSubject(priorClauses[priorClauses.length - 1]);
  ...
  return contentTokens.length ? text : `${text} for ${subject}`;
}
...
function buildResidentRequestContract(question = "", routingPlan = null, requestContext = {}) {
  ...
  const needs = clauses.map((text, index) => {
    ...
    if (/^open\b.*\b(?:application|document|form|link|page|website)\b/i.test(evidenceFocus)) {
  ...
  return {
    ...
    complete: needs.length > 0 && needs.every((need) => need.request
      ...
      && (LEADING_QUESTION.test(need.text) || IMPERATIVE_REQUEST.test(need.text)
        || (usedPriorContext && need.request === resolvedQuestion))),
  };
}
