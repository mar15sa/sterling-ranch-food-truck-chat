...
const [inputArg,cacheArg,outArg]=process.argv.slice(2);if(!inputArg||!cacheArg||!outArg)throw Error('Require prior integration directory, community vector cache and new output directory');
...
const jobs=cases.flatMap(c=>[1,2].flatMap(repetition=>['keyword','semantic'].map(method=>({caseId:c.id,repetition,method,order:crypto.randomBytes(8).toString('hex')})))).sort((a,b)=>a.order.localeCompare(b.order));
fs.mkdirSync(out,{recursive:true});fs.mkdirSync(path.join(out,'code'));
for(const file of ['flow-evidence.js','community-semantic.js','community-semantic-ranker.mjs','compare-combined-retrieval.mjs'])fs.copyFileSync('scripts/quality-eval/'+file,path.join(out,'code',file));
...
const manifest={status:'initializing',isTest:true,startedAt:new Date().toISOString(),codeRevision:cp.execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),
 sourceSnapshotHash:flow.hash(communityIndex),rulesSnapshotHash:flow.hash(rulesIndex),casesHash:flow.hash(cases),communityId:communityIndex.communityId,design:jobs,completed:[],paidApiCalls:0,newSubscriptions:0,
 limitations:['Five known authored plans replayed; not unseen, planning or final-answer quality evidence.','Current rules loaded with supplements, but no new live rules-source attestation.','Only community ranking changes. Existing rule keyword search, source expansion and budgets are retained.','Local timing is not production cost or whole-answer latency.']};
