...
function buildPlainAnswer(query, results, index, sourcesForAnswer = []) {
  ...
  if (
    ...
    const approvalClause = sourceClauseCandidates("fencing approval", topSources, { questionSpecificOnly: false })
      .find((entry) => /approval must be obtained from the DRC prior to any construction or landscaping/i.test(entry.text));
    if (approvalClause) {
      return helpfulAnswer(
        capitalizeFirstLetter(approvalClause.text),
        topSources,
...
function seasonalLightingOverviewAnswer(query = "", sources = []) {
  ...
  if (permanentSystemQuestion && removalClause) {
    return structuredHelpfulAnswer(
      capitalizeFirstLetter(removalClause.text),
      [
        periodClause.text,
        cutoffClause?.text,
...
function underEaveLightingOverviewAnswer(query = "", sources = []) {
  ...
  const generalApprovalClause = clauses.find((entry) => /DRC approval is required to modify or add exterior lighting/i.test(entry.text));
  ...
  const lead = asksHolidayOnlyApplication
    ? `${capitalizeFirstLetter((generalApprovalClause || approvalClause).text)} ${capitalizeFirstLetter(systemsClause.text)}`
...
function simpleTopicOverviewAnswer(query, sources = [], residentQuestion = query) {
  ...
  if (
    ...
    if (placement) {
      return structuredHelpfulAnswer(
        capitalizeFirstLetter(placement),
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (isLandscapeApplicationQuery(routingQuery)) {
    ...
    if (packet && submit) {
      const packetTitle = decodeURIComponent(new URL(packet.sourceUrl).pathname.split("/").filter(Boolean).at(-1) || packet.title)
        .replace(/[-_]+/g, " ").trim();
      const directAnswer = `Open ${packetTitle}.`;
      const keyDetails = [`Open ${submit.title}.`];
      const answer = structuredHelpfulAnswer(
        directAnswer,
        keyDetails,
        actionLabel(packetTitle)
      );
  ...
  return {
    ...
    answerVerdict: isUnderEaveLightingQuery(routingQuery) && /\bDRC approval\b/i.test(answer)
      ? "conditional"
