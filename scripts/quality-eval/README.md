...
The comparison now rejects absent, failed, expired, or identity-mismatched community revalidation before any provider call. It saves the actual community/rules/profile snapshots for later auditing; successful community refresh does not independently establish rules freshness. Composer action IDs are constrained to the supplied action inventory. These are offline-checked preparations, not evidence of improved model performance.

`compare-flow-retrieval.mjs OUT_DIR VECTOR_CACHE COMPLETED_FLOW_DIR` replays one captured interpretation per diagnostic case through keyword and local semantic-plus-keyword rules ranking in the same approved evidence packet. Both saved source attestations must still pass. It reuses exact vectors with model, document and window-identity checks and does not call an AI service. Community action retrieval is unchanged, so this rules-only variant cannot by itself fix a missing or irrelevant process link.
