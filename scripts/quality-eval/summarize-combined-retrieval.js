...
function readCapture(directory){
 ...
 const methods=m.methods||['keyword','semantic'];assert.ok(methods.length===2||methods.length===4);assert.equal(new Set(methods).size,methods.length);assert.ok(methods.every(x=>['keyword','semantic','combined-hybrid','combined-semantic','purpose-routed','purpose-routed-hybrid','prepared-hybrid'].includes(x)));
