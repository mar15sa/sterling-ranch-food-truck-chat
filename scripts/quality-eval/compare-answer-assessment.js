...
async function compare({outDir,fixtures=require('./assessment-fixtures'),models=['claude-haiku-4-5','claude-sonnet-5'],repetitions=2,
  capUsd=1,revision='v1',kind='assessment',apiKey=process.env.ANTHROPIC_API_KEY,fetchImpl=fetch}){
  ...
  if(!['assessment','acceptance'].includes(kind)||!fixtures.length||!models.length)throw new Error('Invalid comparison kind or empty cases/models');
  const compact=require('./compact-acceptance-candidate');
  const makeRequest=kind==='acceptance'?compact.acceptanceRequest:assessmentRequest;
  const validate=kind==='acceptance'?compact.acceptanceIssues:assessmentIssues;
  const disposition=kind==='acceptance'?compact.acceptanceDisposition:assessedDisposition;
  const toolName=kind==='acceptance'?'check_answer_acceptance':'assess_resident_answer';
  ...
  const requestContract=makeRequest(fixtures[0],models[0],{revision});
  const manifest={isTest:true,status:'running',startedAt:new Date().toISOString(),capUsd,models,repetitions,revision,kind,
  ...
  for(const [i,{row,model,rep}] of jobs.entries()){
    ...
    try{
      const response=await observed('https://api.anthropic.com/v1/messages',{method:'POST',headers:{'content-type':'application/json','anthropic-version':'2023-06-01','x-api-key':apiKey},
        body:JSON.stringify(makeRequest(row,model,{revision})),signal:AbortSignal.timeout(15000)});
      const data=await response.json();result.httpStatus=response.status;result.stopReason=data.stop_reason;
      result.assessment=(data.content||[]).find(b=>b.type==='tool_use'&&b.name===toolName)?.input||null;
      result.rawContent=data.content||[];
      result.issues=validate(result.assessment,row.response.sources.map(s=>s.id));
      ...
      result.disposition=result.issues.length?{status:'unassessed',issues:result.issues}:disposition(result.assessment,row.response.sources.map(s=>s.id));
