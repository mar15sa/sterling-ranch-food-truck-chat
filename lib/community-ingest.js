  ...
  for (const pattern of patterns) {
    for (const match of text.matchAll(pattern.regex)) {
      ...
      const numeric = pattern.type === 'money'
        ? Number(value.match(/^\$(\d[\d,]*(?:\.\d+)?)/)?.[1]?.replace(/,/g, ''))
...
function hasReadableDocumentText(value = '') {
  ...
  const content = String(value).replace(/--\s*\d+\s+of\s+\d+\s*--/gi, '').trim();
...
async function crawlCommunity(profileInput, options = {}) {
  ...
  const availableChunks = new Map(readableRecords.map(record => [record.contentHash, record.id]));
  for (const page of uniquePages) {
