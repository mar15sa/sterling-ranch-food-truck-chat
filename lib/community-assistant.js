...
function hasSafeWithheldHandoffEvidence(question, sources = [], routingPlan = null) {
  ...
  if (plan.intent !== "facilities" && !facilityAccessRequest) return hasQuestionSpecificHandoffEvidence(question, sources);
  if (!goals.some((goal) => ["booking", "cost", "permission", "account-access"].includes(goal)) && !facilityAccessRequest) return false;
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  const relevantWithheld = (operationalSearch.withheldSources || []).find((source) =>
