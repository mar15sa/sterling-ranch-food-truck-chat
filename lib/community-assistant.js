...
function isOperationalFacilityQuestion(question = "") {
...
function operationalFacilitySource(question, sources = []) {
  if (!isOperationalFacilityQuestion(question)) return null;
  ...
  if (!liveDateIsProven) {
    return buildAnswerContract({
      directAnswer: requestedArea
        ? `I could not verify a current ${service} pickup date for ${requestedArea.label}.`
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  const operationalSearch = searchCommunityIndex(question, {
    ...
    indexPath: options.indexPath,
    communityId: options.communityId,
    intent: isOperationalFacilityQuestion(question) ? "facilities" : activeIntent,
    interpretation: routingPlan,
    limit: 5,
    now: options.now,
  });
