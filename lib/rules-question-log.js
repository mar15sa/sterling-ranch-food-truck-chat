...
const { assessCommunityAnswerQuality } = require("./community-quality-rubric");
...
function propertyNames() {
  return {
    ...
    needsWork: env("RULES_QUESTION_NOTION_NEEDS_WORK_PROPERTY", "Needs work"),
    ownerVerdict: env("RULES_QUESTION_NOTION_OWNER_VERDICT_PROPERTY", "Owner verdict"),
    ownerNotes: env("RULES_QUESTION_NOTION_OWNER_NOTES_PROPERTY", "Owner notes"),
    ratingCandidate: env("RULES_QUESTION_NOTION_RATING_CANDIDATE_PROPERTY", "Rating candidate"),
    ratingCandidateScore: env("RULES_QUESTION_NOTION_RATING_CANDIDATE_SCORE_PROPERTY", "Rating candidate score"),
    ratingCandidateDetails: env("RULES_QUESTION_NOTION_RATING_CANDIDATE_DETAILS_PROPERTY", "Rating candidate details"),
  };
}
...
function qualityAssessmentForLog(question, answer = {}, options = {}) {
  const diagnostic = scoreCommunityAnswer(question, answer);
  const assessment = options.qualityAssessment;
  const allowedRatings = new Set(["Excellent", "Good", "Mixed", "Weak", "Poor"]);
  const allowedEffortRatings = new Set(["Resolved", "Some work remains", "High resident effort"]);
  const calibrated = assessment?.calibrated === true
    && String(assessment.version || "").trim()
    && allowedRatings.has(assessment.rating)
    && Number.isInteger(assessment.score)
    && assessment.score >= 1
    && assessment.score <= 5
    && allowedEffortRatings.has(assessment.residentEffort)
    && Number.isInteger(assessment.residentEffortScore)
    && assessment.residentEffortScore >= 1
    && assessment.residentEffortScore <= 5;

  if (!calibrated) {
    const candidateValid = assessment
      && String(assessment.version || "").trim()
      && allowedRatings.has(assessment.rating)
      && Number.isInteger(assessment.score)
      && assessment.score >= 1
    ...
    return {
      rating: "Not rated",
      score: 0,
      issues: [...new Set([...(diagnostic.issues || []), "automatic-rating-calibration-required"])],
      residentEffort: { rating: "Not rated", score: 0 },
      candidate: candidateValid ? {
        version: String(assessment.version),
        rating: assessment.rating,
        score: assessment.score,
        dimensionTotal: Number(assessment.dimensionTotal) || 0,
        dimensions: Object.fromEntries(Object.entries(assessment.dimensions || {}).map(
          ([name, dimension]) => [name, Number(dimension?.value) || 0]
        )),
        hardFailures: Array.isArray(assessment.hardFailures) ? assessment.hardFailures.map(String) : [],
      } : null,
    };
  }
  ...
  return {
    rating: assessment.rating,
    score: assessment.score,
    issues: [...new Set(Array.isArray(assessment.issues) ? assessment.issues.map(String) : [])],
    residentEffort: {
      rating: assessment.residentEffort,
      score: assessment.residentEffortScore,
    },
    candidate: null,
  };
}
...
function buildQuestionLogEntry(question, answer = {}, options = {}) {
  ...
  // The legacy scorer remains useful for diagnostics, but the owner audit
  // showed that its positive labels do not agree with human judgments.
  // Publish a grade only after a versioned assessment has been calibrated.
  const qualityAssessment = options.qualityAssessment || assessCommunityAnswerQuality(question, answer);
  const quality = qualityAssessmentForLog(question, answer, { ...options, qualityAssessment });
  return {
    ...
    ratingCandidate: quality.candidate?.rating || "Not rated",
    ratingCandidateScore: quality.candidate?.score || 0,
    ratingCandidateDetails: quality.candidate ? JSON.stringify(quality.candidate) : "",
  };
}
...
function notionProperties(entry, names = propertyNames()) {
  return {
    ...
    [names.needsWork]: { checkbox: Boolean(entry.needsWork) },
    [names.ownerVerdict]: entry.ownerVerdict ? { select: { name: entry.ownerVerdict } } : { select: null },
    [names.ownerNotes]: { rich_text: richText(entry.ownerNotes) },
    [names.ratingCandidate]: { select: { name: entry.ratingCandidate || "Not rated" } },
    [names.ratingCandidateScore]: { number: Number(entry.ratingCandidateScore) || 0 },
    [names.ratingCandidateDetails]: { rich_text: richText(entry.ratingCandidateDetails) },
  };
}
...
function requiredSchema(names = propertyNames()) {
  return {
    ...
    [names.needsWork]: { checkbox: {} },
    [names.ownerVerdict]: {
      select: { options: ["Impressive", "Okay", "Needs work"].map((name) => ({ name })) },
    },
    [names.ownerNotes]: { rich_text: {} },
    [names.ratingCandidate]: {
      select: { options: ["Excellent", "Good", "Mixed", "Weak", "Poor", "Not rated"].map((name) => ({ name })) },
    },
    [names.ratingCandidateScore]: { number: { format: "number" } },
    [names.ratingCandidateDetails]: { rich_text: {} },
  };
}
...
function mapNotionQuestion(page, names = propertyNames()) {
  ...
  return {
    ...
    needsWork: Boolean(properties[names.needsWork]?.checkbox),
    ownerVerdict: properties[names.ownerVerdict]?.select?.name === "Acceptable"
      ? "Okay"
      : properties[names.ownerVerdict]?.select?.name
        || (properties[names.needsWork]?.checkbox ? "Needs work" : ""),
    ownerNotes: plainTextProperty(properties[names.ownerNotes]),
    ratingCandidate: properties[names.ratingCandidate]?.select?.name || "Not rated",
    ratingCandidateScore: Number(properties[names.ratingCandidateScore]?.number || 0),
    ratingCandidateDetails: plainTextProperty(properties[names.ratingCandidateDetails]),
  };
}
...
async function queryQuestionLogs(options = {}, fetchImpl = globalThis.fetch) {
  ...
  if (options.ownerReview === "needs-work") {
    filters.push({ property: names.needsWork, checkbox: { equals: true } });
  } else if (["impressive", "okay", "acceptable"].includes(options.ownerReview)) {
    if (options.ownerReview === "impressive") {
      filters.push({ property: names.ownerVerdict, select: { equals: "Impressive" } });
    } else {
      filters.push({
        or: ["Okay", "Acceptable"].map((verdict) => ({
          property: names.ownerVerdict,
          select: { equals: verdict },
        })),
      });
    }
  }
...
// Monitoring must include old unresolved marks and must never migrate the schema.
async function listOwnerMarkedQuestions(fetchImpl = globalThis.fetch) {
  if (!env("RULES_QUESTION_NOTION_TOKEN", env("NOTION_API_KEY"))) {
    throw new Error("The owner-question monitor is not configured.");
  }
  const dataSourceId = await resolveNotionDataSourceId(fetchImpl);
  if (!dataSourceId) throw new Error("The owner-question monitor is not configured.");
  const names = propertyNames();
  const items = new Map();
  const cursors = new Set();
  let cursor;
  do {
    const result = await notionRequest(`/data_sources/${encodeURIComponent(dataSourceId)}/query`, {
      method: "POST",
      body: JSON.stringify({
        page_size: 100,
        ...(cursor ? { start_cursor: cursor } : {}),
        filter: { and: [
          { property: names.needsWork, checkbox: { equals: true } },
          { property: names.testing, checkbox: { equals: false } },
        ] },
        sorts: [{ property: names.askedAt, direction: "ascending" }],
      }),
    }, fetchImpl);
    if (!Array.isArray(result?.results)) throw new Error("Owner-question monitor received an incomplete response.");
    for (const page of result.results) {
      const item = mapNotionQuestion(page, names);
      if (item.id && item.needsWork && !item.isTest) items.set(item.id, item);
    }
    cursor = result.has_more ? result.next_cursor : null;
    if (result.has_more && (!cursor || cursors.has(cursor))) {
      throw new Error("Owner-question monitor could not establish complete pagination.");
    }
...
async function setQuestionNeedsWork(pageId, needsWork, fetchImpl = globalThis.fetch) {
  return setQuestionOwnerVerdict(pageId, needsWork ? "Needs work" : "", fetchImpl);
}

async function setQuestionOwnerVerdict(pageId, verdict, fetchImpl = globalThis.fetch) {
  return setQuestionOwnerReview(pageId, { ownerVerdict: verdict }, fetchImpl);
}

async function setQuestionOwnerReview(pageId, review = {}, fetchImpl = globalThis.fetch) {
  const normalizedPageId = String(pageId || "").trim();
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(normalizedPageId)) {
    throw new Error("Choose a valid question to review.");
  }
  const hasVerdict = Object.prototype.hasOwnProperty.call(review, "ownerVerdict");
  const hasNotes = Object.prototype.hasOwnProperty.call(review, "ownerNotes");
  if (!hasVerdict && !hasNotes) throw new Error("Choose a rating or add a note.");
  const submittedVerdict = hasVerdict ? String(review.ownerVerdict || "").trim() : "";
  const normalizedVerdict = submittedVerdict === "Acceptable" ? "Okay" : submittedVerdict;
  if (hasVerdict && !["", "Impressive", "Okay", "Needs work"].includes(normalizedVerdict)) {
    throw new Error("Choose a valid owner verdict.");
  }
  if (hasNotes && typeof review.ownerNotes !== "string") {
    throw new Error("Add a valid owner note.");
  }
  const normalizedNotes = hasNotes ? cleanAnswerForLog(review.ownerNotes).slice(0, MAX_NOTION_RICH_TEXT_CHARS) : "";
  const dataSourceId = await resolveNotionDataSourceId(fetchImpl);
  if (!dataSourceId) throw new Error("The Notion question log is not configured.");
  await ensureQuestionLogSchema(dataSourceId, fetchImpl);
  const names = propertyNames();
  const pagePath = `/pages/${encodeURIComponent(normalizedPageId)}`;
  const page = await notionRequest(pagePath, {}, fetchImpl);
  const normalizedParentId = String(page?.parent?.data_source_id || "").replaceAll("-", "").toLowerCase();
  const normalizedSourceId = String(dataSourceId).replaceAll("-", "").toLowerCase();
  if (!normalizedParentId || normalizedParentId !== normalizedSourceId) {
    throw new Error("Choose a valid question to review.");
  }
  await notionRequest(
    pagePath,
    {
      ...
      body: JSON.stringify({
        properties: {
          ...(hasVerdict ? {
            [names.needsWork]: { checkbox: normalizedVerdict === "Needs work" },
            [names.ownerVerdict]: normalizedVerdict ? { select: { name: normalizedVerdict } } : { select: null },
          } : {}),
          ...(hasNotes ? { [names.ownerNotes]: { rich_text: richText(normalizedNotes) } } : {}),
        },
      }),
    },
    fetchImpl
  ...
  return {
    id: normalizedPageId,
    ...(hasVerdict ? { ownerVerdict: normalizedVerdict, needsWork: normalizedVerdict === "Needs work" } : {}),
    ...(hasNotes ? { ownerNotes: normalizedNotes } : {}),
  };
}
