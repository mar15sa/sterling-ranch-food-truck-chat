...
function inferScopeKey(fact = {}, source = {}) {
  ...
  if (/\bphone\b|\bcall\b/.test(text) || fact.type === "phone") scopes.push("phone");
  if (/\bemail\b/.test(text) || fact.type === "email") scopes.push("email");
...
function inferFacet(fact = {}, source = {}) {
  ...
  if (fact.type === "time" && /\b(?:hours?|open|close|dusk|weekday|weekend)\b/.test(text)) return "facility-hours";
...
function buildFactLedger(index = {}, options = {}) {
  ...
  for (const source of index.sources || []) {
    for (const fact of source.facts || []) {
      const subjectKey = inferSubjectKey(source, fact);
      ...
      const reviewStatus = fact.reviewStatus || source.reviewStatus || (trusted || versionWasTrusted || source.reviewedAt ? "approved" : "candidate");
      const entry = {
        id,
        ...
        reviewStatus,
        reviewedAt: fact.reviewedAt || source.reviewedAt || "",
        reviewedBy: fact.reviewedBy || source.reviewedBy || "",
        reviewNote: fact.reviewNote || source.reviewNote || "",
      };
