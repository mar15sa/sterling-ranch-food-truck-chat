...
async function observeCanonicalSource(sourceUrl, approvedSources, {
  ...
  if (scopedProjections.length && !ordinarySources.length) {
    ...
    return {
      ...
      observedHashes: [...new Set(scopedProjections.map(source => versionHash(text, source, observedActions, observedDocumentFingerprint)))],
  ...
  if (scopedProjections.length) {
    ...
    const projectionHashes = scopedProjections
      .map((source) => versionHash(text, source, observedActions, observedDocumentFingerprint))
