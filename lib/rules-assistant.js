...
function facilityRentalOverviewAnswer(query, sources = []) {
  ...
  const reservationLead = asksPark
    ? `Open the linked official resource for ${parkResource?.title || "the current facility details"}. ${pricingLead}`
    : reservationClause
      ? `${capitalizeFirstLetter(reservationClause)} ${pricingLead}`
      : pricingLead;
  const nextStep = asksPark
    ? `${effectiveDate ? `These published amounts are labeled effective ${effectiveDate}. ` : ""}Open the linked official resource for ${parkResource?.title || "the current facility details"}.`
...
function rvParkingOverviewAnswer(query, sources = []) {
  ...
  const lead = /\bstreet\b/i.test(query) && streetClause
    ...
    : duration && hourLimit && duration.hours > hourLimit
      ? `No. ${capitalizeFirstLetter(temporaryClause.text)}`
