...
const RULE_TOPIC_PATTERN = new RegExp(
  `\\b(?:${[
    ...
    "cars?|campers?|commercial vehicle|driveways?|motor homes?|parking|rv|street|trailers?|vehicles?",
    "assessments?|deadlines?|delinquent|enforcement|fees?|fines?|rates?|trash|utilities|utility|violations?|water",
    "amenities|amenity|clubhouse|facilities|facility|fire pit|home automation|homeseer|lumiere|parks?|pavilion|quiet hours|reservations?|shelters?|steward|watering",
  ].join("|")})\\b`,
  "i"
);
...
function hasPromptInjectionSignals(query) {
  ...
  const disclosureAttack =
    /\b(?:display|expose|print|repeat|reveal|show|tell me)\b.{0,65}\b(?:api keys?|developer|hidden|instructions?|password|prompt|secret|system|token)\b/i.test(
      text
    ) ||
    /(?:display|expose|print|repeat|reveal|show)(?:all|me|the|your)*(?:apikeys?|developer|hidden|instructions|password|prompt|secret|system|token)/.test(
      compact
    );
