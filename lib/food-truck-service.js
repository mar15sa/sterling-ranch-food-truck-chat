...
function createFoodTruckService(options = {}) {
  ...
  async function getAnswerForDate(question, targetDate) {
    ...
    // The official calendar establishes which trucks are scheduled. Menu and
    // profile lookups add useful detail, but must not erase that verified
    // schedule when one vendor site is unavailable.
    ...
    const menuFailures = [];
    const menus = menuResults.map((result, index) => {
      const listing = uniqueListingInputs[index];
      const displayName = displayTruckName(listing.name);
      if (result.status === "fulfilled") return result.value;
      menuFailures.push({ truck: displayName, component: "menu-profile" });
      return { ...listing, name: displayName, menu: { links: [], items: [] } };
    });
    const menu = menus[0]?.menu || { links: [], items: [] };
    const menuEnrichment = {
      status: menuFailures.length ? "degraded" : "complete",
      failures: menuFailures,
    };
    const data = buildLookupAnswer({
      question,
      targetDate,
      truck: menus.length ? formatTruckList(menus.map((listing) => listing.name)) : displayTruckName(truck),
      calendar,
      menu,
      menuEnrichment,
      truckListings: menus,
