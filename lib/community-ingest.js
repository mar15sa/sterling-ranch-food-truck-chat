...
async function crawlCommunity(profileInput, options = {}) {
  ...
  function queueOrder(candidate) {
    ...
    const missingContent = previous?.indexed && !pageSources(previousIndex, previous.canonicalUrl || canonical).length;
  ...
  for (const page of uniquePages) {
    if (page.indexed && !availableUrls.has(canonicalPageUrl(page.canonicalUrl || page.url))) {
      page.indexed = false;
