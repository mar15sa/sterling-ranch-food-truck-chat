...
function projectedActionProof(sources = [], rawLinks = [], text = "", sourceUrl = "") {
  ...
  const observed = expected.map(({ sourceId, action }) => {
    const opensVerifiedPage = actionUrlIdentity(action.url) === actionUrlIdentity(sourceUrl);
    return {
      sourceId,
      action,
      ...
      matches: opensVerifiedPage ? [{ label: action.label, url: sourceUrl, actionType: action.actionType }] : rawLinks
        .filter((link) => actionUrlIdentity(link.url) === actionUrlIdentity(action.url)),
      contextMatches: opensVerifiedPage || Boolean(action.context && text.includes(action.context)),
    };
  });
  return {
...
async function observeCanonicalSource(sourceUrl, approvedSources, { fetchImpl = globalThis.fetch, extractPdfTextImpl = extractPdfText, timeoutMs = 30_000, fetchAttempts = 2, retryDelayMs = 250, sleepImpl } = {}) {
  ...
  if (scopedProjections.length && !ordinarySources.length) {
    ...
    return {
      ...
      observedHashes: [sourceHash(text)],
  ...
  if (scopedProjections.length) {
    ...
    const fullPageHash = sourceHash(text);
    const provesFullPageProjection = scopedProjections.some((source) => source.contentHash === fullPageHash);
    return {
      ...
      observedHashes: [...new Set([...chunkHashes, ...(provesFullPageProjection ? [fullPageHash] : [])])],
...
module.exports = { actionDigest, actionIdentity, actionUrlIdentity, civicPlusDocumentId, fetchOfficialHtmlWithRetry, hasApprovedProjection, isApprovedSource, isRevalidatableApprovedSource, isScopedExactVersionProjection, observeCanonicalSource, projectedActionProof, renewExactApprovedEvidence, revalidateApprovedEvidence, sameOfficialDocumentIdentity, selectRevalidationTargetUrls, sourceHash, stableObservedActions };
