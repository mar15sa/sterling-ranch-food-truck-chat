...
function paginateReviews(items, params) {
  ...
  const queue = String(params.get('queue') || 'all');
  const filtered = items.filter(item =>
    (queue === 'all' || (queue === 'attention' ? ['current', 'comparison'].includes(item.queueBucket) : item.queueBucket === queue))
  ...
  return {
    ...
    summary: {
      pending: filtered.filter(item => ['pending', 'escalated'].includes(item.status)
        && !['history', 'outside'].includes(item.queueBucket)).length,
