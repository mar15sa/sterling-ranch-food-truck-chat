...
const { asksForCalendarOccurrence } = require("./community-search");
...
const PROFILES = {
  ...
  events: {
    ...
    subject: /\b(?:events?|calendar|activities|class(?:es)?|meetings?|concerts?|festivals?|markets?|giveaways?)\b/i,
  },
...
function coversDetail(detail, candidate = {}, plan = {}) {
  ...
  if (detail === "hours") {
    ...
    if (plan.goal === "status" && !requestedOperatingHours && /\b(?:right now|currently|now|at the moment|live status)\b/i.test(text)) return /\b(?:open|closed|at capacity)\b/i.test(text);
  ...
  if (detail === "menu") return candidate.completion?.resolvedDetails?.includes("menu") === true
    && ((candidate.keyDetails || []).length > 0
      || (candidate.claims || []).some((claim) => ["menu", "price"].includes(claim.kind)));
...
function coversDateRange(kind, plan = {}, candidate = {}, connectorResult = {}) {
  if (!plan.dateRange?.start || !plan.dateRange?.end) return true;
  const { start, end, label } = plan.dateRange;
  if (kind !== "waste-schedule"
    && connectorResult.range?.start === start && connectorResult.range?.end === end) return true;
  const dates = [connectorResult.date, ...(connectorResult.villageDates || []).map((item) => item.date)]
    .filter(Boolean);
  if (dates.some((date) => date >= start && date <= end)) return true;
  if (connectorResult.range?.start === start && connectorResult.range?.end === end
    && connectorResult.holidayNote) return true;
...
function shortcutEligibility(kind, { question = "", plan = null, candidate = null, connectorResult = null } = {}) {
  ...
  if (profile.subject && !profile.subject.test(planText(question, effectivePlan))
    && !(kind === "events" && asksForCalendarOccurrence(question))) reasons.push("subject-not-supported");
  ...
  if (!coversDateRange(kind, effectivePlan, candidate, connectorResult || {})) reasons.push("date-range-not-covered");
