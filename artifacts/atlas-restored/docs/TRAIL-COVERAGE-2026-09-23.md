...
Status: separate local candidate at http://127.0.0.1:4197/?view=walks. Previous 4196 remains unchanged. Owner review pending; no staging or production deployment. This is not source approval for the Community Assistant.

## Current visual behavior

The full uncropped 3:2 approved neighborhood illustration appears first. Its village buttons select walking areas, not precise trailheads. That selection filters all eight guides, six access places and forty recorded map sections. The general place directory is hidden on Walks and restored on other applicable tabs.

Eight guide cards use three new approximate village illustrations, each cropped around its own highlighted route. Source paths and A/B map junctions are retained; the backdrop is generated art and does not certify exact roads, houses, trees, trail entrances or access. Every guide offers the original official map for route detail. The forty mileage-label sections keep a source-map locator, with no invented route line. Main neighborhood art, destination models and future inventory are unchanged.
