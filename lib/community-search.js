...
const REQUESTED_OBJECT_TERMS = new Set([
  ...
  "flagpole", "mailbox", "pool", "clubhouse", "pavilion", "park", "trash", "recycling", "water",
]);
...
function requestedDetails(question = "") {
  ...
  const asksForAction = /\b(?:how do|where (?:do|can)|book|reserve|rent|apply|submit|register|sign up|form|pay|payment|log in|login|download)\b/.test(text);
...
function explicitGoalFromQuestion(question = "", intent = "") {
  ...
  if (/\b(?:apply|application|submit|submission|which form|what form)\b/.test(text)) return "application";
  if (/\b(?:register|registration|sign up|enroll)\b/.test(text)) return "registration";
  if (/\b(?:allowed|prohibited|permission|need approval|need .* approval|can i|can we|may i)\b/.test(text)) return "permission";
...
function normalizedRoutingPlan(plan = {}, question = "") {
  ...
  const goal = explicitGoalFromQuestion(question, intent) || modelGoal;
