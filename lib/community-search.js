...
function scoreSource(source, queryTokens, intent, details, coreTokens = [], question = "", factAuthority = {}) {
  ...
  const body = [source.text || '', ...(source.facts || []).filter(fact => fact.approvalClaim)
...
function scopeTerms(value = '') {
  return [...new Set(String(value).toLowerCase().replace(/\bnon[-_ ]+([a-z]+)/g, 'non$1')
    ...
    .filter(term => term.length >= 3 && !SCOPE_TASK_WORDS.has(term) && !/^[a-f0-9]{8,}$/.test(term));
}

function reviewedSubjectFilter(question, sourceEntries = []) {
  // Scope keys identify the reviewed claim's subject; a page title is broader
  // and can describe several entities whose findings must remain separate.
  const termsFor = entry => scopeTerms(entry.scopeKey || entry.subjectKey || '');
  const findingYears = [...String(question).matchAll(/\b(?:in|during|for)\s+((?:19|20)\d{2})\b/gi)].map(match => match[1]);
  const matchesPeriod = entry => {
    if (!findingYears.length) return true;
    const years = String(entry.subjectKey || entry.scopeKey || '').match(/\b(?:19|20)\d{2}\b/g)
      || String(entry.scopeKey || '').match(/(?:19|20)\d{2}/g) || [];
    return !years.length || findingYears.some(year => years.includes(year));
  };
  const counts = new Map();
  for (const entries of sourceEntries) {
    const terms = new Set(entries.filter(entry => entry.factType !== 'link')
      .flatMap(termsFor));
  ...
  const allEntries = sourceEntries.flat().filter(entry => entry.factType !== 'link');
  const subjects = String(question).split(/\b(?:and|or|versus|vs)\b|[,;]/i).map(part => {
      const recognized = scopeTerms(part).filter(term => counts.has(term));
  ...
  return entries => entries.filter(entry => entry.factType === 'link'
    || (matchesPeriod(entry) && subjects.some(subject => subject.score(entry) >= subject.best - 1e-9)));
}
...
function searchCommunityIndex(question, options = {}) {
  ...
  const filterSubject = reviewedSubjectFilter(options.subjectQuestion || question, [...entriesBySource.values()]);
