...
The writer sees verified claims, not uncited facts from the surrounding page. Validation accepts equivalent clock formatting while rejecting altered values, relative windows, reversed clock windows, changed operating states, lost conditions, softened obligations, dropped measurement origins and lost resident needs. Existing source/action objects, completion and confidence remain bound to the audited result. One correction is allowed after a meaning/voice rejection, inside the same total 15-second writer deadline; provider failure does not trigger a retry. No provider change or additional service is introduced.
...
## Staging findings and corrections

Real-model staging checks at `e38cb0c` and `63460eb` accepted the Halloween and Christmas rewrites; the browser showed the new direct answer, noise rule, CAB discretion, source citation and no generic Next step box with Test mode visible. Neighboring cases exposed false rejections from schedule-keyword matching, missing action associations, coordinated yard wording, URL numbers and generic source headings being misread as names. These were repaired in the shared validation path with focused regressions, including an independent TownPay community fixture.

Manual inspection caught a height rewrite that preserved numbers but omitted the ground-level measurement origin. That is now rejected. A single correction attempt asks the same configured writer to repair specific meaning/voice failures; two invalid attempts still return the audited fallback. Tests prove the retry cap and that provider failures do not retry. All 16 final contract tests pass, including the selected-baseline integration. Lighting remains distinct from general holiday displays, and unverified holiday hours remain outside the writer.

The first full run had 1,298 passes and three failures: two old assertions required the formal wording/generic handoff that this repair intentionally removes, and the isolated health-handler test needed the new configuration dependency. Those tests were updated without weakening source or privacy checks. Focused testing against the current approved-evidence snapshot verified those corrections. The next full candidate and final exact staging checks must pass before production merge.
