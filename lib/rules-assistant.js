...
async function answerRulesQuestion(query, options = {}) {
  ...
  const rawFallbackAnswer = [fallbackLead, fallbackLocationBoundary, fallbackDimensionSentence, naturalNextStep]
    .filter(Boolean)
    .join("\n\n");
  const composedFallbackAnswer = !rewrittenAnswer && derived.strategy === "structured"
  ...
  // same source-independent grammar cleanup to accepted structured rewrites,
  // then keep the original rewrite if that cleanup would lose required facts.
  const composedRewriteAnswer = rewrittenAnswer && derived.strategy === "structured"
