...
function deterministicRequestedDetails(question = "") {
  ...
  const action = instruction.check || instruction.scheduling || resourceViewRequest || /\b(?:how do|where (?:do|can)|book|reserve|rent|submit|register|sign up|form|pay|payment|settle|settling|log in|login|download)\b/.test(text)
...
function isRegularHoursRequest(question = '', options = {}) {
  ...
  return details.includes('hours') && !details.some(detail => ['date', 'status'].includes(detail))
    && !validatedModelDateRange(options.dateRange, question, options.now)
...
function normalizeInterpretation(plan = {}, question = "", options = {}) {
  ...
  if (isRegularHoursRequest(question, { ...options, dateRange: plan.dateRange })) detailSet.delete('date');
  ...
  return {
    ...
    subject,
    requestedDetails,
    dateRange: processOnly || methodOnly || reportedFindings ? null : validatedModelDateRange(plan.dateRange, question, options.now) || fallbackDateRange(question, intent, options.now),
