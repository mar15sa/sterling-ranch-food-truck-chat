...
function isLightingRelatedQuery(query) {
  return /\b(light|lights|lighting|fixture|fixtures|holiday|seasonal|string lights?|gemstone|jellyfish|under[-\s]?eaves?|soffits?|eave\/rake|eave|rake|hardwired|permanent)\b/i.test(
    String(query || "")
  );
}
...
function isUnderEaveLightingQuery(query) {
  ...
  const mentionsUnderEaveSystem =
    /\b(gemstone|jellyfish|under[-\s]?eaves?|soffits?|eave\/rake|eave|rake|hardwired|permanent)\b/i.test(
      text
    );
...
function designReviewResourceSourcesForQuestion(query, answerSources = []) {
  ...
  if (/\b(landscape screens?|screening)\b/i.test(questionText)) add("landscape-screens");
...
function sourcesWithOfficialResources(query, answerSources) {
  ...
  return sources.filter((source) => {
    const key = source.sourceUrl || source.nodeId || source.title;
