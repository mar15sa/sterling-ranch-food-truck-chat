...
const { isResidentSharingFixtureQuery } = require('./rules-intent');

const DEFAULT_INDEX_PATH = path.join(__dirname, "..", "data", "community-index.json");
const STOP_WORDS = new Set(["a", "about", "an", "and", "are", "authority", "board", "cab", "can", "community", "do", "for", "from", "how", "i", "in", "is", "it", "job", "me", "my", "of", "on", "or", "ranch", "sterling", "the", "to", "we", "what", "where", "with"]);
const EXPANSIONS = {
  ...
  wood: ["cedar", "lumber"],
  snowplow: ["snow", "removal", "roadway"],
  plow: ["snow", "removal", "roadway"],
  road: ["roadway", "county"],
  roadway: ["road", "county"],
};
const CONCEPTS = [
...
const GENERIC_QUERY_TERMS = new Set(["apply", "application", "book", "call", "contact", "cost", "fee", "form", "help", "much", "pay", "price", "rent", "report", "reserve", "rule"]);
...
function tokens(value = "") {
  ...
  if (/\b(?:watch(?:es|ing)?|care(?:s|d|ing)? for|look(?:s|ed|ing)? after)\b.{0,40}\b(?:kids?|children|child)\b/.test(text)
    || /\b(?:kids?|children|child)\b.{0,40}\b(?:watch(?:es|ing)?|care(?:s|d|ing)? for|look(?:s|ed|ing)? after)\b/.test(text)) {
    expanded.add("caregiver");
  }
  return [...expanded];
}
...
function asksForCalendarOccurrence(question = "") {
  ...
  const asksForNamedNextOccurrence = /\bwhen\s+(?:is|are)\s+(?:the\s+)?next\s+[a-z0-9][a-z0-9'/-]*(?:\s+[a-z0-9][a-z0-9'/-]*){0,4}\s*[?.!]*$/i.test(text);
  const namesNonEventSchedule = /\b(?:trash|garbage|recycl(?:e|ing)|waste|pickup|collection|bill|billing|payment|assessment|inspection|deadline|due|pool\s+(?:opening|reopening))\b/i.test(text);
  ...
  return asksForTiming
    && (namesCalendarItem || (asksForNamedNextOccurrence && !namesNonEventSchedule))
    && !asksForGovernance;
}
...
function calendarOccurrenceTopic(question = "") {
  const ignored = new Set([
    ...
    "about", "regarding", "this", "today", "tomorrow", "tonight", "weekend",
    "what", "when", "where",
  ]);
...
function classifyCommunityIntent(question = "") {
  ...
  if (isReportedFindingsRequest(text)) return "services";
  ...
  if (asksForCalendarOccurrence(text)) return "events";
  if (isResidentSharingFixtureQuery(text)) return "rules";
  if (
  ...
  if (/\bmailbox\b/i.test(text) && /\b(?:key|keys|lost|missing|replace|replacement|builder|post office|who)\b/i.test(text)) return "services";
...
function normalizedRoutingPlan(plan = {}, question = "", options = {}) {
  ...
  const defaultDetail = {
    ...
    contact: "contact",
    cost: "price",
    schedule: isRegularHoursRequest(question, { ...options, dateRange: normalized.dateRange }) ? 'hours' : 'date',
    status: "status",
  }[normalized.goal];
...
function actionSupportsGoal(action = {}, goal = "") {
  if (!goal || !ACTION_GOALS.has(goal)) return true;
  const reviewedApplicationEvidence = goal === "application"
    ? `${action.context || ""} ${action.evidence?.label || ""} ${action.evidence?.context || ""}`
    : "";
  const evidence = `${action.label || ""} ${action.url || ""} ${(action.keywords || []).join(" ")} ${action.actionType || ""} ${reviewedApplicationEvidence}`.toLowerCase();
  const signals = {
...
function scoreSource(source, queryTokens, intent, details, coreTokens = [], question = "", factAuthority = {}) {
  ...
  const actionEvidence = intent === 'forms'
    ? (source.actions || []).map((action) => `${action.label || ''} ${action.context || ''} ${action.evidence?.label || ''} ${action.evidence?.context || ''}`).join(' ')
    : '';
  const body = `${source.text || ''} ${actionEvidence}`.toLowerCase();
  ...
  const caregiverPhrase = /\b(?:watch(?:es|ing)?|care(?:s|d|ing)? for|look(?:s|ed|ing)? after)\b.{0,40}\b(?:kids?|children|child)\b|\b(?:kids?|children|child)\b.{0,40}\b(?:watch(?:es|ing)?|care(?:s|d|ing)? for|look(?:s|ed|ing)? after)\b/i.test(question);
  const requestedObjects = [...new Set([
    ...meaningfulCore.filter((term) => REQUESTED_OBJECT_TERMS.has(term)),
    ...(caregiverPhrase ? ["caregiver"] : []),
  ])];
  const objectAppears = term => {
    const evidence = `${title} ${body}`;
    if (["snow", "snowplow"].includes(term)) return /\bsnow\b/.test(evidence);
    if (term === "leak") return /\b(?:leak|forgiveness|excessive water use)\b/.test(evidence);
    if (term === "shed") return /\b(?:sheds?|accessory buildings?)\b/.test(evidence);
    return title.includes(term) || body.includes(term);
  };
  const missingObjects = requestedObjects.filter(term => !objectAppears(term));
  if (missingObjects.length) {
    // A coarse venue noun cannot erase an explicitly reviewed membership or
    // service identity. Unknown audience/place modifiers still have to match
    // the same approved subject; substantive objects retain their hard gate.
    const coarseObjectWords = new Set(['clubhouse', 'facility', 'center', 'centre', 'court', 'park', 'water']);
    const requestedSubject = scopeTerms(question).filter(term => !coarseObjectWords.has(term));
    const exactReviewedIdentity = requestedSubject.length > 0 && (source.facts || []).some(fact => {
      if (!fact.approvalClaim || !fact.subjectKey) return false;
      const subject = scopeTerms(fact.subjectKey);
      return subject.length > 0 && requestedSubject.every(term => subject.includes(term));
    });
    // A narrow qualifier can identify an exact reviewed source even when the
    // broad noun is absent from the approved excerpt (for example, "leak"
    // identifies the water-leak adjustment page). A different service noun
    // must not do that: matching "trash" cannot excuse a missing "water" in
    // a compound utility-setup question.
    const qualifierObjects = new Set(['leak', 'caregiver', 'snow', 'snowplow', 'pickleball']);
    const matchedDistinctiveObject = requestedObjects.some(term => qualifierObjects.has(term) && objectAppears(term));
    if (!missingObjects.every(term => coarseObjectWords.has(term)) || (!exactReviewedIdentity && !matchedDistinctiveObject)) return 0;
  }
  if (meaningfulCore.length && !meaningfulCore.some((term) => objectAppears(term) || subjectContext.includes(term))) return 0;
  let lexical = 0;
  ...
  // Prefer a source that answers both sides of an explicit comparison. This
  // keeps a comprehensive setup page ahead of a neighboring page that only
  // says which services start automatically.
  if (/\bautomatically\b/i.test(question) && /\b(?:set ?up|start|activate)\b/i.test(question)
    && /\bautomatically\s+(?:be\s+)?set up\b/i.test(body)
    && /\bresponsible for setting up personal accounts\b/i.test(body)) score += 40;
  for (const object of requestedObjects) {
    if (title.includes(object)) score += 36;
    else if (objectAppears(object)) score += 14;
  }
...
function entrySupportsHours(entry = {}) {
  const text = String(entry.supportingText || entry.displayValue || '');
  return entry.facet === 'facility-hours'
    || (['information', 'specification'].includes(entry.facet)
      && (/\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b/i.test(text)
        || /\b(?:end|start|beginning) of (?:the )?(?:pickup|collection|service) day\b/i.test(text)));
}

function entriesForRequestedDetails(entries = [], details = [], question = '') {
  if (!details.length) return entries;
  return entries.filter((entry) => entry.factType === 'link' || details.some((detail) => {
    if (detail === 'price' && /\b(?:pay|cost|fee|free|price)\b[\s\S]{0,40}\bpark(?:ing)?\b|\bpark(?:ing)?\b[\s\S]{0,40}\b(?:pay|cost|fee|free|price)\b/i.test(question)) {
      return /\b(?:street )?parking\b/i.test(`${entry.supportingText || ''} ${entry.displayValue || ''} ${entry.scopeKey || ''}`)
        && /\b(?:free|no (?:additional )?(?:charge|cost)|no fee|complimentary|\$|cost|fee|price|rate)\b/i.test(`${entry.supportingText || ''} ${entry.displayValue || ''}`);
    }
    ...
    if (detail === 'contact') return entry.facet === 'contact'
      || /@|(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]\d{3}[-.\s]\d{4}/.test(String(entry.supportingText || entry.displayValue || ''));
    ...
    if (detail === 'hours') return entrySupportsHours(entry);
    ...
    if (detail === 'specification') return entry.facet === 'specification'
      || (['fee', 'information', 'reservation-policy'].includes(entry.facet)
        && /\b(?:(?:one|two|three|four|five|six|seven|eight|nine|ten|twelve|forty[- ]eight)\s*(?:\(\s*\d+(?:\.\d+)?\s*\))?|\d+(?:\.\d+)?)\s*(?:business\s+)?(?:"|(?:seconds?|minutes?|hours?|days?|weeks?|months?|years?|feet|foot|inches|inch|ft\.?|in\.?)\b)/i.test(String(entry.supportingText || entry.displayValue || '')));
...
function approvedClaimProjection(source = {}, entries = [], details = [], question = '') {
  const selectedEntries = entriesForRequestedDetails(entries, details, question);
...
function entriesSupportRequestedDetails(entries = [], details = []) {
  ...
  if (details.includes('contact') && !facets.has('contact')
    && !entries.some((entry) => /@|(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]\d{3}[-.\s]\d{4}/.test(String(entry.supportingText || entry.displayValue || '')))) return false;
  ...
  if (details.includes('specification') && !entriesForRequestedDetails(entries, ['specification']).some(entry => entry.factType !== 'link')) return false;
  if (details.includes('hours') && !entries.some(entrySupportsHours)) return false;
  if (details.includes('date') && !facets.has('event-date') && !facets.has('facility-hours')) return false;
  if (details.includes('action') && !types.has('link') && !facets.has('submission') && !facets.has('reservation-policy')
    && !entries.some((entry) => entry.facet === 'information'
      && /^(?:(?:please|to)\s+)?(?:access|apply|ask|bring|call|check|connect|contact|email|exchange|go|open|register|submit|use|visit)\b/i
        .test(String(entry.supportingText || entry.displayValue || '').trim()))) return false;
...
function entryAppliesToQuestion(entry = {}, question = '') {
  if (!entry.applicabilityTerms?.length) return true;
  const normalized = String(question).toLowerCase().replace(/[^a-z0-9]+/g, ' ');
  return entry.applicabilityTerms.some(term => normalized.includes(String(term).toLowerCase()));
}
...
const PROCESS_QUERY_WORDS = new Set('how where what which who when do does did can could should would may i we my our your their the a an and or to for of in on at with about from by is are be whether if then please need want get find out process steps instructions request schedule scheduling arrange arranging check checking verify see determine confirm online next current today tomorrow monday tuesday wednesday thursday friday saturday sunday morning afternoon evening date time available availability'.split(' '));
function processEvidenceTerms(value = '') {
  return [...new Set((String(value).toLowerCase().match(/[a-z]+/g) || [])
    .filter(term => !PROCESS_QUERY_WORDS.has(term))
    // Ordinary process vocabulary can name the same object differently in
    // its title and instruction (watering schedule / irrigation controller).
    .map(term => (term === 'watering' ? 'irrigation' : term).replace(/(?:ing|ed|s)$/, '').replace(/e$/, ''))
    .filter(term => term.length >= 3))];
}

function originalProcessEvidenceFilter(question = '') {
  const shape = instructionalRequestShape(question);
  if (!shape.check && !shape.scheduling) return entries => entries;
  // Preserve every independently requested task, but never let expanded search
  // words replace the resident's object (an inspection is not any schedule).
  const subjects = String(question).split(/\b(?:and|or)\b|[?;]/i)
    .map(processEvidenceTerms).filter(terms => terms.length >= 2);
  if (!subjects.length) return entries => entries;
  return (entries, source) => {
    const matches = entry => {
      // Each claim is checked separately. A long PDF cannot assemble a match
      // from unrelated paragraphs; legacy title/scope context may identify
      // the object, but it does not grant that claim any approval authority.
      const terms = new Set(processEvidenceTerms(`${source.title || ''} ${entry.scopeKey || ''} ${entry.supportingText || ''}`));
      return subjects.some(subject => subject.every(term => terms.has(term)));
    };
    const facts = entries.filter(entry => entry.factType !== 'link' && matches(entry));
    return entries.filter(entry => entry.factType === 'link' ? facts.length > 0 || matches(entry) : facts.includes(entry));
  };
}
...
function searchCommunityIndex(question, options = {}) {
  ...
  const filterProcess = originalProcessEvidenceFilter(options.subjectQuestion || question);
  const reimbursementRequest = /\b(?:reimburs\w*|refund\w*|pay(?:ing)? back)\b/i.test(options.subjectQuestion || question);
  const entriesBySource = new Map(eligibleSources.map(source => {
    let entries = filterProcess(reviewState.entriesFor(source), source)
      .filter(entry => entryAppliesToQuestion(entry, options.subjectQuestion || question));
    if (reimbursementRequest) {
      const subjectEvidence = `${source.title || ""} ${entries.map(entry => `${entry.reviewedSubjectKey || ""} ${entry.supportingText || entry.displayValue || ""}`).join(" ")}`;
      if (!/\b(?:reimburs\w*|refund\w*|pay(?:ing)? back)\b/i.test(subjectEvidence)) entries = [];
    }
    return [source.id, entries];
  }));
  ...
  const originalSubjectTerms = new Set(scopeTerms(options.subjectQuestion || question));
  const subjectCandidates = eligibleSources.filter(source => {
    const entries = entriesBySource.get(source.id) || [];
    // A punctuation or coarse venue-word mismatch must not remove an exact
    // reviewed subject from the competition before sibling selection runs.
    // This admits only an explicitly approved subject whose terms all occur
    // in the original request; final source scoring/eligibility still apply.
    const exactReviewedSubject = entries.some(entry => {
      if (entry.factType === 'link' || !entry.reviewedSubjectKey) return false;
      const terms = scopeTerms(entry.reviewedSubjectKey);
      return terms.length > 0 && terms.every(term => originalSubjectTerms.has(term));
    });
    return entriesSupportAnyRequestedDetail(entries, details)
      && (exactReviewedSubject || scoreSource(approvedClaimProjection(source, entries, details, question), queryTokens, intent, details, coreTokens, question, index.factAuthority || {}) > 0);
  });
  ...
  const ranked = eligibleSources.map((source) => {
      ...
      const projected = dynamic ? source : answerable
        ? approvedClaimProjection(source, entries, details, question)
...
module.exports = {
  approvedClaimProjection,
  asksForCalendarOccurrence,
  ACTION_GOALS,
  DEFAULT_INDEX_PATH,
  actionSupportsGoal,
