...
function isTrashStorageQuery(query) {
  return /\b(trash|garbage|recycling|bins?|containers?)\b/i.test(String(query || "")) &&
    /\b(store|stored|storage|leave|left|overnight|night|curb|pickup|collection|bring|put out|outside|screens?|screened|screening|enclosures?)\b/i.test(String(query || ""));
}
