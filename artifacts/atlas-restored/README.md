# Resident landing comparison — September 23, 2026

Current local preview: http://127.0.0.1:4203/. Previous 4202 preserved. No staging or production deployment.

The complete neighborhood now fits the first desktop screen. Four readable villages with place counts guide the phone view; no destination is preselected. Selecting a village reveals its places and connections. The Whole Ranch chooser is repaired and has explicit overlap/hit-target checks. Search, detailed models, inventory, routes and source information remain.

See `eval/LANDING-REVIEW.md` for scope, actual screenshots, acceptance evidence and remaining limitations. Owner review and explicit launch permission remain separate. The earlier records below are historical; the 4202 Whole Ranch phone-prompt acceptance is superseded by this repair.

# Complete resident-outing preview — September 23, 2026

Current comparison: http://127.0.0.1:4202/. Previous 4201 remains available. No staging or production deployment.

All three approved passes are implemented: sourced visits and return-time walks; three-tab/global-search/mobile navigation; dimensional destination connections and an illustrated wider plan. Full inventory, routes, geography and approved neighborhood/landmark art remain. The wider landscape is approximate and keeps the official plan one click away.

Read `eval/FULL-PLAN-REVIEW.md` for the complete requirements, real desktop/phone journeys, remaining unknowns and evidence. `eval/FULL-PLAN-ART-DIRECTION.md` records the landscape references and review. The owner has not approved this candidate or a launch.

Run `npm run preview` for normal gated startup. `npm run preview:review` is only for developing an unapproved candidate. Runtime edits invalidate review. Approved baselines must not be replaced to make a change pass.

The entries below are historical. The rejected 4200 diagram and temporary 4201 source-map restoration are superseded by this comparison.

# Wider-plan correction — September 23, 2026

Current local preview: http://127.0.0.1:4201/?view=future&futureMap=villages. The owner rejected the blank planning diagram in 4200; its earlier positive visual judgment is withdrawn for that view. Previous previews remain available. No staging or production deployment.

The wider view now displays the complete, unchanged official CAB plan directly. Five compact numbered markers correspond to readable names alongside the map (below it on phones). Streets, open spaces, active villages and the full planning extent remain visible. An explicit full-size image link supports closer reading. Planning dates, status, catalog and geographic coordinates were not changed.

See `eval/PLAN-REPAIR-REVIEW.md` for actual desktop, narrower desktop and phone comparisons. Focused checks require the visible geographic image, preserved image aspect, separate labels, 44px targets and measured nonoverlap. These checks target the reported failure; they do not replace owner visual judgment.

Earlier implementation history follows; the blank planning composition and its prior acceptance claim are superseded.

# Resident outings comparison — September 23, 2026

Current local preview: http://127.0.0.1:4200/. Previous http://127.0.0.1:4199/ is preserved. No staging or production deployment.

Three map-first tabs, global search, access/reservation labels, source-dated visit summaries, complete return-time walk filters, meaningful destination/village connections and a combined future-project directory. All original artwork, saved bounds, IDs, nested listings, detailed models and eight highlighted routes remain. The wider future-plan diagram shows the five planned areas with active-village context and the original official plan available for comparison.

107 focused checks passed. Actual desktop and 390 × 844 phone journeys and screenshot comparisons are recorded in `eval/RESIDENT-JOURNEYS-REVIEW.md` and `eval/visual-review.json`. The gate now requires evidence for eight resident journeys as well as the preserved visual/coverage gates. Unknown operating/arrival facts remain explicit. Real-device reduced motion, 200% text enlargement and physical touch checks remain follow-ups; static motion suppression, keyboard flows and phone reflow were checked. Owner review remains separate from launch approval.

Run `npm run preview` for the normal gated preview, or `npm run preview:review` only while changing/reviewing a candidate. Runtime edits invalidate the saved visual review. Keep approved baselines unchanged.

Earlier implementation history follows.

# Future villages and planning areas - September 23, 2026

Local candidate: http://127.0.0.1:4199/?view=future. Previous 4198 preserved. No staging or production deployment.

Coming Soon opens the complete CAB August 8, 2025 illustrative master plan with five directly selectable future areas: Heirloom Village, Paramount Center, Heritage Village, Promontory Village and Pinnacle Village. The existing 11 project groups / 16 future listings remain in Parks & projects. Both layers stay maps; active villages appear only for geographic context in the official planning illustration.

The developer confirms nine villages with four active; it does not name the future five on its current village page. The CAB plan supplies the five planning labels and relative locations. Paramount remains Center, not a newly asserted village name. Dates, precise boundaries and amenities are not invented. Original neighborhood artwork, walks, close-up models and place catalog remain unchanged.

See eval/PLANNED-VILLAGES-REVIEW.md for sources, maintenance and acceptance. No owner visual approval or launch approval is claimed.

# Routes directly on the neighborhood overview - September 23, 2026

Local candidate: http://127.0.0.1:4198/?view=walks. Previous 4197 preserved; no deployment.

All eight saved walking guides are highlighted directly on the full neighborhood painting on initial load. Each line and numbered marker opens its matching guide; a matching route key supports small screens. Village filters only narrow the list below and never remove routes or crop the overview. Existing detail illustrations and official-map comparisons remain intact.

Graphic registration uses three landmark references on the approved painting. These are approximate illustrated route positions, not surveyed geography. Source route coordinates remain unchanged. See eval/OVERVIEW-ROUTES-REVIEW.md for acceptance and evidence.

Earlier overview behavior below is superseded.

# Illustrated trail scenes — September 23, 2026

Local candidate: http://127.0.0.1:4197/?view=walks. Previous 4196 remains unchanged. Owner review pending; no staging or production deployment.

Walks opens with the approved full 3:2 neighborhood landscape. Village-area buttons filter walking guides, access places and mapped sections together. Only walking content appears below this tab; the general destination directory remains in other applicable tabs.

Three new village illustrations give all eight guides their own framed visual, with contrasting route highlights and A/B markers. Artwork is approximate, generated scenery, not surveyed geography: road, tree and building details may differ. Every guide retains a one-click official-map comparison with the original source geometry. Forty additional map sections retain their official map locator because a mileage label alone does not establish a complete walk. Existing source dates and access limitations remain unchanged.

Artwork files and exact prompts: `eval/TRAIL-SCENES-REVIEW.md`, `eval/PROVIDENCE-ART.md`, `eval/PROSPECT-ART.md`, `eval/ASCENT-ART.md`.

Earlier implementation history below is superseded where it describes the walking artwork.

# Route highlighting — September 23, 2026

Local preview: http://127.0.0.1:4196/?view=walks. Previous 4195 remains unchanged. No deployment.

The owner rejected repeating the landscape crop with a detached route-shape inset as unhelpful. Each walking guide now directly highlights its saved path over the matching official-map context, using the atlas’s muted green/cream palette and a contrasting copper line. Start/finish markers, directional flow and surrounding street labels make the route identifiable. This source-aligned route view is not a new 3D landscape; the main neighborhood painting is unchanged. Forty section entries highlight their saved mileage label, not an invented route. See eval/ROUTE-HIGHLIGHT-REVIEW.md.

Earlier history follows; the detached-inset treatment below is superseded.
