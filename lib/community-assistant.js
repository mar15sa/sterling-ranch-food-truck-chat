...
function unanchoredRecurringScheduleAnswer(question, result) {
  ...
  return buildAnswerContract({
    directAnswer: `I could not verify whether the requested pickup is this week or next from the published recurring schedule: ${scheduleSummary.join("; ")}.`,
...
function poolStatusAnswer(status, requested = ["status"], options = {}) {
  ...
  const poolConnector = (options.communityProfile?.connectors || []).find((connector) => connector.type === "live-status");
  ...
  if (!details.length) {
    if (!diagnostics.parserHealthy) {
      return { ...buildAnswerContract({
        ...
        actions: [{ label: result.calendarActionLabel || calendarSource.title, url: result.sourceUrl, actionType: "calendar" }],
    ...
    if (diagnostics.appliedFilters?.length && result.alternatives?.length) {
      ...
      return { ...buildAnswerContract({
        ...
        actions: [
          ...
          { label: result.calendarActionLabel || calendarSource.title, url: result.sourceUrl, actionType: "calendar" },
        ],
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (foodTruckRequested && foodTruckRequestDecision.eligible && options.getFoodTruckAnswer) {
    try {
      ...
      const unavailable = buildAnswerContract({
        ...
        actions: calendarUrl && foodTruckConnector?.adapter?.labels?.calendarAction ? [{ label: foodTruckConnector.adapter.labels.calendarAction, url: calendarUrl, actionType: "calendar" }] : [],
        status: "source-unavailable",
  ...
  if (
    ...
    const unrelated = buildAnswerContract({
      ...
      nextStep: identity?.[1] || "Ask about a community rule, service, form, facility, event, pool status, or food truck instead.",
      actions: identity?.[2] || [],
      status: "out-of-scope",
      answerMode: "conversation",
    });
