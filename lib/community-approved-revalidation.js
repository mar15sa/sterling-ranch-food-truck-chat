...
const {
  contentHtml, extractActions, extractPdfBufferText, extractPdfText, fetchOfficialDocument, isDocumentUrl, linksFromHtml,
...
const { faqRows } = require("./community-faq-records");

const DOMAIN_OUTAGE_MIN_URLS = 3;
const DOMAIN_OUTAGE_MIN_FAILURE_RATIO = 0.75;
const DOMAIN_OUTAGE_GRACE_MS = 24 * 60 * 60 * 1000;

function canonicalizeEvidence(value, volatileKeys = new Set()) {
  if (Array.isArray(value)) return value.map((item) => canonicalizeEvidence(item, volatileKeys))
    .sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
  if (!value || typeof value !== "object") return value;
  return Object.fromEntries(Object.keys(value).sort().flatMap((key) => {
    if (volatileKeys.has(key)) return [];
    return [[key, canonicalizeEvidence(value[key], volatileKeys)]];
  }));
}
...
function sourceEvidenceIdentity(index = {}, source = {}) {
  ...
  const ledger = (index.factLedger || []).filter((entry) => entry.sourceId === source.id
    ...
    .map((entry) => canonicalizeEvidence(entry, ledgerVolatileKeys))
    .sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
  return JSON.stringify({ source: canonicalizeEvidence(source, sourceVolatileKeys), ledger });
}

function unchangedBaselineSources(temporaryIndex, check, baselineIndex) {
  if (!baselineIndex || check.outcome !== "review-required" || check.reason !== "fetch-or-extraction-failed") return [];
  const baselineById = new Map((baselineIndex.sources || []).map((source) => [source.id, source]));
  const currentAtUrl = (temporaryIndex.sources || []).filter((source) => source.sourceUrl === check.sourceUrl);
  const baselineAtUrl = (baselineIndex.sources || []).filter((source) => source.sourceUrl === check.sourceUrl);
  if (!currentAtUrl.length || currentAtUrl.length !== baselineAtUrl.length || !currentAtUrl.every((source) => {
  ...
  const checkedIds = new Set((check.sources || []).map((source) => source.id));
  return currentAtUrl.filter((source) => checkedIds.has(source.id) && isFreshnessTrackedSource(source));
}
...
function applyDomainOutageGrace(temporaryIndex, checks = [], baselineIndex, {
  now = Date.now(), graceMs = DOMAIN_OUTAGE_GRACE_MS,
} = {}) {
  if (!baselineIndex || graceMs <= 0) return [];
  const byOrigin = new Map();
  for (const check of checks) {
    const origin = sourceOrigin(check.sourceUrl);
    if (!origin) continue;
    const group = byOrigin.get(origin) || { total: 0, transient: [] };
    group.total += 1;
    if (check.failureKind === "transient-official-unavailable"
      && unchangedBaselineSources(temporaryIndex, check, baselineIndex).length) group.transient.push(check);
    byOrigin.set(origin, group);
  }
  const incidentOrigins = new Set([...byOrigin].filter(([, group]) =>
    group.transient.length >= DOMAIN_OUTAGE_MIN_URLS
      && group.transient.length / group.total >= DOMAIN_OUTAGE_MIN_FAILURE_RATIO
  ).map(([origin]) => origin));
  const graced = [];
  for (const check of checks) {
    if (check.failureKind !== "transient-official-unavailable"
      || !incidentOrigins.has(sourceOrigin(check.sourceUrl))) continue;
    for (const source of unchangedBaselineSources(temporaryIndex, check, baselineIndex)) {
      const previousExpiry = Date.parse(source.staleAfter);
      const graceUntilMs = previousExpiry + graceMs;
      if (!Number.isFinite(previousExpiry) || graceUntilMs <= now) continue;
      const graceUntil = new Date(graceUntilMs).toISOString();
      source.staleAfter = graceUntil;
      for (const fact of temporaryIndex.factLedger || []) {
        if (fact.sourceId === source.id && fact.sourceVersion === source.contentHash) fact.staleAfter = graceUntil;
      }
      graced.push({ id: source.id, sourceUrl: source.sourceUrl, contentHash: source.contentHash,
        reason: check.reason, failureKind: check.failureKind, lastVerifiedAt: source.checkedAt || "",
        graceUntil, checkedAt: check.checkedAt });
    }
  }
  if (graced.length) temporaryIndex.revalidationGrace = {
    mode: "last-exact-version-during-domain-outage",
    maxGraceMs: graceMs,
    sources: graced,
  };
  return graced;
...
function versionHash(text, source = {}, observedActions = [], documentFingerprint = "") {
  if (source.hashScheme === 'page-text-actions-v1') return sourceContentHash(text, '', observedActions);
  if (source.hashScheme === 'page-text-v1') return sourceHash(text);
  ...
  if (source.documentFingerprint) return sourceContentHash(text, documentFingerprint, observedActions);
  return sourceHash(text);
}

function chunkVersionHashes(text, sources = [], observedActions = [], documentFingerprint = "") {
  return [...new Set(chunkText(text).flatMap((chunk) => sources.map((source) =>
    versionHash(chunk, source, observedActions, documentFingerprint))))];
}

function matchingProjectionHashes(text, sources = [], observedActions = [], documentFingerprint = "", structuredTexts = []) {
  const candidates = [...new Set([text, ...chunkText(text), ...structuredTexts].filter(Boolean))];
  return [...new Set(sources.flatMap((source) => candidates.some((candidate) =>
    versionHash(candidate, source, observedActions, documentFingerprint) === source.contentHash)
    ? [source.contentHash] : []))];
}

function projectionVersionHashes(sourceUrl, text, sources = [], observedActions = [], documentFingerprint = "", structuredTexts = []) {
  if (isFaqUrl(sourceUrl)) return matchingProjectionHashes(text, sources, observedActions, documentFingerprint, structuredTexts);
  return [...new Set(sources.map((source) => versionHash(text, source, observedActions, documentFingerprint)))];
}
...
function selectRevalidationTargetUrls(index = {}, now = Date.now(), { refreshAheadMs = 0 } = {}) {
  const cutoff = now + Math.max(0, Number(refreshAheadMs) || 0);
  ...
  const urls = new Set(activeSources.filter((source) => Date.parse(source.staleAfter) < cutoff).map((source) => source.sourceUrl));
  for (const fact of index.factLedger || []) {
    if (!APPROVED_REVIEW_STATUSES.has(fact.reviewStatus) || !(Date.parse(fact.staleAfter) < cutoff)) continue;
...
function renewExactApprovedEvidence(index = {}, {
  sourceUrl, observedHashes = [], checkedAt, staleAfter, documentFingerprint = "",
} = {}) {
  ...
  const renewedSources = (index.sources || []).filter((source) => source.sourceUrl === sourceUrl
    ...
    && (!source.requireExactDocumentFingerprint || Boolean(source.documentFingerprint
  ...
  for (const source of renewedSources) {
    Object.assign(source, { checkedAt, staleAfter });
    if (documentFingerprint && (!source.documentFingerprint || source.hashScheme === "page-text-v1")) Object.assign(source, {
      documentFingerprint,
      // Existing approved PDF identities were text-only. Preserve that
      // identity while recording the binary proof used by later fast checks.
      hashScheme: source.hashScheme || "page-text-v1",
    });
  }
  ...
  const requiresReview = (index.sources || []).filter((source) => source.sourceUrl === sourceUrl
    && isRevalidatableApprovedSource(source, index)
    && !renewedVersions.has(`${source.id}:${source.contentHash}`));
...
async function fetchOfficialPdfBytesWithRetry(sourceUrl, fetchOfficialDocumentImpl, options, attempts = 2) {
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try { return await fetchOfficialDocumentImpl(sourceUrl, options); } catch (error) {
      lastError = error;
      if (error?.retryable === false) break;
    }
  }
  const error = new Error(`Official PDF fetch failed after ${attempts} attempts: ${lastError?.message || "unknown error"}`);
  error.cause = lastError;
  throw error;
}

function officialHttpStatus(error) {
  let current = error;
...
function isRetryableOfficialFetchError(error) {
...
function transientOfficialFailure(error) {
  let current = error;
  for (let depth = 0; current && depth < 6; depth += 1) {
    if (isRetryableOfficialFetchError(current)) return true;
    current = current.cause;
  }
  return false;
}

async function pause(ms, sleepImpl) {
  return sleepImpl ? sleepImpl(ms) : new Promise((resolve) => setTimeout(resolve, ms));
}

async function fetchOfficialHtmlWithRetry(sourceUrl, expectedUrl, {
  fetchImpl, timeoutMs, attempts = 2, retryDelayMs = 250, sleepImpl,
} = {}) {
  let lastError;
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      if (canonicalPageUrl(response.url || sourceUrl) !== expectedUrl) {
        const error = new Error("Canonical URL changed during revalidation.");
      ...
      lastError = error;
      if (!isRetryableOfficialFetchError(error) || attempt === attempts) break;
      // Keep each retry bounded and short. A later successful response still
      // has to pass the same exact URL, text-version, and action checks.
      await pause(retryDelayMs * attempt, sleepImpl);
    }
  }
  const error = new Error(`Official page fetch failed after ${attempts} attempts: ${lastError?.message || "unknown error"}`);
  error.cause = lastError;
  throw error;
}

async function observeCanonicalSource(sourceUrl, approvedSources, {
  fetchImpl = globalThis.fetch,
  extractPdfTextImpl = extractPdfText,
  fetchOfficialDocumentImpl = fetchOfficialDocument,
  extractPdfBufferTextImpl = extractPdfBufferText,
  timeoutMs = 30_000,
  fetchAttempts = 2,
  retryDelayMs = 250,
  sleepImpl,
  ...
  let observedDocumentFingerprint = "";
  if (isDocumentUrl(sourceUrl)) {
    ...
    const approvedFingerprints = [...new Set(approvedSources.map((source) => source.documentFingerprint).filter(Boolean))];
    const exactDocumentSources = approvedSources.filter((source) => source.requireExactDocumentFingerprint);
    const requiredFingerprints = [...new Set(exactDocumentSources.map((source) => source.documentFingerprint))];
    if (exactDocumentSources.length && (requiredFingerprints.length !== 1
      || !/^[a-f0-9]{64}$/i.test(String(requiredFingerprints[0] || '')))) {
      const error = new Error('Exact document fingerprint approval is missing or conflicting.');
      error.retryable = false;
      throw error;
    }
    const requiredFingerprint = exactDocumentSources.length ? requiredFingerprints[0] : '';
    if (approvedFingerprints.length === 1 || requiredFingerprint) {
      const data = await fetchOfficialPdfBytesWithRetry(sourceUrl, fetchOfficialDocumentImpl, { fetchImpl: trackedFetch }, fetchAttempts);
      observedDocumentFingerprint = crypto.createHash("sha256").update(data).digest("hex");
      // Text extraction cannot prove that an image-only table, map or diagram
      // is unchanged. Opted-in approvals bind the whole PDF and must return
      // to review before a new binary identity can replace the reviewed one.
      if (requiredFingerprint && observedDocumentFingerprint !== requiredFingerprint) {
        return {
          observedHashes: [],
          ...
          documentProof: {
            ...
            retrievedUrl: returnedUrls.at(-1) || requests.at(-1),
            verificationMode: 'required-binary-fingerprint-changed',
            expectedFingerprint: requiredFingerprint,
            observedFingerprint: observedDocumentFingerprint,
          },
        };
      }
      const storedEvidenceIsSelfConsistent = approvedSources.every((source) => source.documentFingerprint === approvedFingerprints[0]
        && source.contentHash === versionHash(source.text, source, source.actions || [], source.documentFingerprint));
      if (observedDocumentFingerprint === approvedFingerprints[0] && storedEvidenceIsSelfConsistent) {
        ...
        return {
          observedHashes: [...new Set(approvedSources.map((source) => source.contentHash))],
          actionMismatch: false,
          actionProof: { mode: "identical-approved-document-bytes" },
          documentProof: {
            requestedUrl: sourceUrl,
            retrievedUrl: returnedUrls.at(-1) || requests.at(-1),
            verificationMode: "binary-fingerprint",
            expectedFingerprint: approvedFingerprints[0],
            observedFingerprint: observedDocumentFingerprint,
          },
        };
      }
      text = await extractPdfBufferTextImpl(data);
    } else {
      text = await extractOfficialPdfWithRetry(sourceUrl, extractPdfTextImpl, {
        fetchImpl: trackedFetch,
        onDocumentFingerprint: (value) => { observedDocumentFingerprint = value; },
      });
    }
    if (!requests.length || (canonicalPageUrl(requests.at(-1)) !== expectedUrl
      && !sameOfficialDocumentIdentity(sourceUrl, requests.at(-1)))) throw new Error("Canonical URL changed during PDF revalidation.");
    documentProof = {
      requestedUrl: sourceUrl,
      retrievedUrl: returnedUrls.at(-1) || requests.at(-1),
      ...(observedDocumentFingerprint ? {
        verificationMode: approvedFingerprints.length ? "binary-fingerprint-changed" : "full-extraction-fingerprint",
        ...(approvedFingerprints.length ? { expectedFingerprint: approvedFingerprints[0] } : {}),
        observedFingerprint: observedDocumentFingerprint,
      } : {}),
    };
  } else {
    const { response, html } = await fetchOfficialHtmlWithRetry(sourceUrl, expectedUrl, {
      fetchImpl, timeoutMs, attempts: fetchAttempts, retryDelayMs, sleepImpl,
    });
    text = stripEmbeddedInstructions(pageText(html));
    structuredTexts = (faqRows(html, response.url || sourceUrl) || [])
      .map((row) => stripEmbeddedInstructions(row.text)).filter(Boolean);
  ...
  if (scopedProjections.length && !ordinarySources.length) {
    ...
    return {
      ...
      observedHashes: projectionVersionHashes(sourceUrl, text, scopedProjections, observedActions,
        observedDocumentFingerprint, structuredTexts),
  ...
  if (scopedProjections.length) {
    const projectionProof = projectedActionProof(scopedProjections, observedLinks, text, sourceUrl);
    const chunkHashes = chunkVersionHashes(text, ordinarySources, observedActions, observedDocumentFingerprint);
    const projectionHashes = projectionVersionHashes(sourceUrl, text, scopedProjections, observedActions,
      observedDocumentFingerprint, structuredTexts)
      .filter((hash) => scopedProjections.some((source) => source.contentHash === hash));
    return {
  ...
  return {
    observedHashes: chunkVersionHashes(text, approvedSources, observedActions, observedDocumentFingerprint),
...
async function revalidateApprovedEvidence(index, { now = Date.now(), fetchObservedHashes, staleAfterMs = 86_400_000, concurrency = 2, refreshAheadMs = 0 } = {}) {
  ...
  const urls = selectRevalidationTargetUrls(temporaryIndex, now, { refreshAheadMs });
  ...
  const check = async (sourceUrl) => {
    ...
    try {
      ...
      const documentMismatch = observation?.documentMismatch || approvedSources.some((source) =>
        source.requireExactDocumentFingerprint && (!source.documentFingerprint
          || observation?.documentProof?.observedFingerprint !== source.documentFingerprint));
      ...
      if (documentMismatch || observation?.actionMismatch || !observedHashes.length || extraHashes.length || missing.length) {
        return reviewRecord(sourceUrl, approvedSources, "review-required", {
          reason: documentMismatch ? 'document-binary-fingerprint-changed-or-unproved'
            : observation?.actionMismatch ? "action-identity-changed" : !observedHashes.length ? "no-valid-proof"
      ...
      const renewal = renewExactApprovedEvidence(temporaryIndex, {
        sourceUrl,
        observedHashes,
        checkedAt,
        staleAfter,
        documentFingerprint: observation?.documentProof?.observedFingerprint || "",
      });
      ...
      const httpStatus = officialHttpStatus(error);
      const removedOrMoved = httpStatus === 404 || httpStatus === 410;
      return reviewRecord(sourceUrl, approvedSources, "review-required", {
        reason: removedOrMoved ? "source-removed-or-moved" : "fetch-or-extraction-failed",
        failureKind: removedOrMoved ? "source-removed-or-moved"
          : transientOfficialFailure(error) ? "transient-official-unavailable" : "verification-error",
        error: error.message,
...
module.exports = { actionDigest, actionIdentity, actionUrlIdentity, applyDomainOutageGrace, canonicalizeEvidence, chunkVersionHashes, civicPlusDocumentId, fetchOfficialHtmlWithRetry, fetchOfficialPdfBytesWithRetry, hasApprovedProjection, isApprovedSource, isRevalidatableApprovedSource, isScopedExactVersionProjection, observeCanonicalSource, officialHttpStatus, projectedActionProof, renewExactApprovedEvidence, revalidateApprovedEvidence, sameOfficialDocumentIdentity, selectRevalidationTargetUrls, sourceEvidenceIdentity, sourceHash, stableObservedActions, transientOfficialFailure, unchangedBaselineSources, versionHash };
