...
const GENERIC_EVIDENCE_TERMS = new Set([
  ...
  "home", "lost", "number", "official", "phone", "question", "ranch", "restore", "rule", "rules",
...
function isSafeRulesBoundaryAnswer(rulesAnswer = {}) {
  return rulesAnswer.answerMode === "source-evidence-boundary"
    && /\b(?:does not|do not|doesn't|don't)\b.{0,120}\b(?:establish|state|specify|show|provide|publish|name|identify|list|contain|cover)\b/i.test(String(rulesAnswer.answer || ""));
}
...
function genericEvidenceBoundary(question, rulesAnswer = {}, index = {}, profile = {}, candidates = []) {
  const resource = configuredCommunityResource(index, profile, candidates, question);
  ...
  const boundary = buildAnswerContract({
    directAnswer: "I could not verify an answer from approved, up-to-date community sources.",
    nextStep: resource ? "Open the official community source below for the current information." : "Check the community’s official website or contact its current resident-support team.",
    actions,
...
function liveWasteScheduleAnswer(question, schedule) {
  ...
  if (!liveDateIsProven) {
    return buildAnswerContract({
      directAnswer: "I could not verify a current pickup date from the live collection service.",
      nextStep: actions[0] ? "Use the official pickup calendar to check your address." : "Use the official collection service to check your address.",
      actions,
      status: "source-unavailable",
      requestedDetails: ["date"],
      coveredDetails: [],
      answerMode: "community-live-waste-unavailable",
    });
  }
  const service = schedule.service === "garbage" ? "trash" : "recycling";
  const requestedArea = schedule.serviceAreas.find(({ label }) =>
    new RegExp(`\\b${label.replace(/ Village$/i, "")}\\b`, "i").test(question)
  );
  ...
  if (requestedArea && !dates.length) {
    return buildAnswerContract({
      directAnswer: `I could not verify a current ${service} pickup date for ${requestedArea.label}.`,
      nextStep: actions[0] ? "Use the official pickup calendar to check your address." : "Use the official collection service to check your address.",
      actions,
      status: "source-unavailable",
      requestedDetails: ["date"],
...
function unavailableWasteScheduleAnswer(question, profile = {}, plan = {}, candidate = null) {
  ...
  const action = configuredAction || (calendarEndpoint?.url ? {
    type: "information",
    label: connector?.adapter?.labels?.sourceTitle || "Open the official pickup calendar",
    url: calendarEndpoint.url,
  } : null);
  const requested = [...new Set(plan.requestedDetails || requestedDetails(question))];
  const asksAboutStatus = requested.includes("status") || /\b(?:delay(?:ed)?|late|on\s+time|holiday)\b/i.test(question);
  const hasVerifiedDate = candidate?.answerStatus === "verified"
    && candidate?.directAnswer
    && (candidate.sources || []).length;
  const directAnswer = hasVerifiedDate
    ? `${candidate.directAnswer} The live calendar confirms that pickup date, but it does not label whether the service is delayed.`
    : asksAboutStatus
      ? "I could not check the live pickup service just now, so I cannot confirm whether pickup is delayed."
      : "I could not check the live pickup service just now, so I cannot confirm the current pickup date.";
  return {
    ...buildAnswerContract({
      directAnswer,
      keyDetails: hasVerifiedDate ? candidate.keyDetails : [],
      nextStep: action ? "Check your address in the official pickup calendar before putting out bins." : "Check with the official collection service before putting out bins.",
...
function coveredDetails(requested, sources, actions, question = "") {
  ...
  return requested.filter((detail) => {
    ...
    if (detail === "hours") return !/\bholidays?\b/i.test(question) && facts.some((fact) =>
...
function missingContactAnswer(organization, routingPlan, options = {}) {
  ...
  const boundary = buildAnswerContract({
    ...
    nextStep: contactDirectory ? "Use the selected official community contact source rather than an unrelated service number." : "Check the community’s official website for the current contact source.",
...
function approvedOperationalProjectionAnswer(question, result, options = {}) {
  ...
  const answer = buildAnswerContract({
    ...
    claims: [
      { text: directAnswer, evidenceSourceIds: directEvidenceSourceIds, approvalClaimIds: preferredAction ? actionApprovalClaimIds : (directDetail?.approvalClaimIds || []) },
...
async function approvedOperationalActionAnswer(question, routingPlan, options = {}) {
  ...
  return composeApprovedOperationalProjection(question, result, { ...options, routingPlan, preferredAction });
}
...
async function approvedOperationalInstructionAnswer(question, routingPlan, options = {}) {
  ...
  const answer = buildAnswerContract({
    ...
    nextStep: action ? `Use “${action.label}” below.` : `Open the official ${source.title} source below for the current instructions.`,
...
async function approvedOperationalContactAnswer(question, routingPlan, options = {}) {
  ...
  const contactPageAction = source.sourceUrl
    ? [{ label: `Open official ${source.title}`, url: source.sourceUrl, actionType: "contact" }]
    : [];
  const covered = requested.includes("action") ? ["contact", "action"] : ["contact"];
  const answer = buildAnswerContract({
    directAnswer: String(fact.context).trim(),
    nextStep: `Open the official ${source.title} page below for the current contact details.`,
  ...
  const renderedContact = [answer.directAnswer, ...(answer.keyDetails || []), answer.nextStep].join(" ").toLowerCase();
  ...
  if (answer.confidence?.canAnswer !== true || !renderedContact.includes(String(fact.value).toLowerCase())) return null;
  return { ...answer, authorityDecision: "exact-version-approved-claims" };
}
...
function poolStatusAnswer(status, requested = ["status"], options = {}) {
  ...
  return { ...buildAnswerContract({
    ...
    sources: [{ title: "Official CAB pool status", sourceUrl: status.sourceUrl, text: `${status.headline}. ${status.summary}`, excerpt: status.summary, authorityScore: 1, checkedAt: status.checkedAt, isOfficialResource: true, connectorType: "live-status", sourceType: "status", capabilities: ["status"], ...(evidence ? { id: evidence.evidenceId, communityId: envelope.communityId, staleAfter: evidence.staleAfter, controllingSourceRole: evidence.controllingSourceRole, authorityFacets: ["status"] } : {}) }],
    status: "verified",
    requestedDetails,
    coveredDetails,
    checkedAt: status.checkedAt,
    answerMode: "community-live-status",
  }), evidenceEnvelope: envelope, claims: envelope?.claims || [], _connectorDiagnostics: { sourceOutcome: "ok", beforeFilterCount: 1, afterFilterCount: 1, appliedFilters: [] } };
}
  ...
  if (!details.length) {
    if (!diagnostics.parserHealthy) {
      return { ...buildAnswerContract({
        ...
        actions: [{ label: result.calendarActionLabel || "Open official community calendar", url: result.sourceUrl, actionType: "calendar" }],
    ...
    if (diagnostics.appliedFilters?.length && result.alternatives?.length) {
      ...
      return { ...buildAnswerContract({
        ...
        actions: [
          ...
          { label: result.calendarActionLabel || "Open official community calendar", url: result.sourceUrl, actionType: "calendar" },
        ],
    ...
    return { ...buildAnswerContract({
      ...
      requestedDetails,
      coveredDetails,
      claims: calendarEvidence ? [{ text: `The official calendar does not list an event for ${result.range.label}.`, evidenceSourceIds: [calendarEvidence.evidenceId] }] : [],
      checkedAt: result.checkedAt,
      answerMode: "community-live-events",
    }), _connectorDiagnostics: diagnostics };
  }
...
function eventFailureActions(options = {}) {
  const connector = (options.communityProfile?.connectors || []).find((item) => item.type === "civicplus-calendar");
  const adapter = connector?.adapter;
  const endpoint = adapter?.endpoints?.find((item) => item.id === "primary") || adapter?.endpoints?.[0];
  const url = endpoint?.url || connector?.baseUrl || options.communityProfile?.website || options.index?.website;
  if (!url) return [];
  return [{ label: adapter?.labels?.openAction || "Open official community calendar", url, actionType: "calendar" }];
}
...
function partialGoverningRuleAnswer(question, rulesAnswer, requested = [], routingPlan = null) {
  ...
  const partial = buildAnswerContract({
    directAnswer,
    keyDetails,
    nextStep: "I could not verify the application or submission step from an exact owner-approved action source. Use the controlling rule below while that process is confirmed.",
    actions: displayedSources[0]?.sourceUrl
      ? [{ label: `Open ${displayedSources[0].title || "the controlling rule"}`, url: displayedSources[0].sourceUrl, actionType: "information" }]
      : [],
    sources: displayedSources,
...
function conditionalOperationalCompletion(question, rulesAnswer, routingPlan, options = {}) {
  ...
  const ruleDirectAnswer = String(rulesAnswer.answer || rulesAnswer.directAnswer || "")
  ...
  const completed = buildAnswerContract({
    ...
    keyDetails: [...(rulesAnswer.keyDetails || []), String(instructionFact.context || instructionFact.value).trim()],
...
function renderPartialGap(answer, completion, question = "") {
  ...
  if (["verified-partial", "missing-evidence"].includes(completion?.outcome)
    ...
    return current.replace(/^Short answer:\s*([^\n]+)/i, (_, groundedDirectAnswer) =>
  ...
  if (!["verified-partial", "missing-evidence"].includes(completion?.outcome) || !completion.missingDetails.length
    || /\b(?:could not|couldn't|couldn’t|cannot|can't|can’t)\b.{0,100}\b(?:verify|confirm|safely restate)\b/i.test(current)
    || /\b(?:official passages?|selected official passages?|rules?|sources?)\b.{0,100}\b(?:do not|don't|does not|doesn't)\b.{0,80}\b(?:establish|state|specify|show|provide|publish)\b/i.test(current)
...
function ensureAnswerCompletion(question, answer) {
  ...
  const renderedAnswer = renderPartialGap(answer.answer, completion, question);
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (foodTruckRequested && foodTruckRequestDecision.eligible && options.getFoodTruckAnswer) {
    try {
      ...
      const foodTruckConnector = (options.communityProfile?.connectors || []).find((connector) => connector.type === "food-truck-schedule");
      const calendarUrl = foodTruckConnector?.adapter?.endpoints?.find((endpoint) => endpoint.id === "schedule")?.url || options.communityProfile?.website;
      const unavailable = buildAnswerContract({
        directAnswer: "I could not check the live food-truck schedule just now.",
        nextStep: "Use the official calendar below before making plans.",
        actions: calendarUrl ? [{ label: foodTruckConnector?.adapter?.labels?.calendarAction || "Open official food-truck schedule", url: calendarUrl, actionType: "calendar" }] : [],
        status: "source-unavailable",
  ...
  if (wasteScheduleRequested && wasteRequestDecision.eligible && options.getWasteSchedule) {
    try {
      ...
      if (decision.eligible) return candidate;
      if (decision.reasons.includes("requested-status-missing") || decision.reasons.includes("goal-not-covered")) {
        return unavailableWasteScheduleAnswer(question, options.communityProfile, shortcutPlan, candidate);
      }
    ...
    try {
      ...
      if (structuredActive) {
        return { ...buildAnswerContract({
          ...
          nextStep: "Open the official calendar below before making plans.",
          actions: eventFailureActions(options),
          status: "source-unavailable",
          answerMode: "community-live-events",
...
async function answerCommunityQuestion(query, options = {}) {
  const state = {};
  const completedAnswer = ensureAnswerCompletion(query, await answerCommunityQuestionCore(query, { ...options, _interpretationState: state }));
...
module.exports = { answerCommunityQuestion, bestContactContext, cleanAnswerText, composeApprovedOperationalProjection, conciseRecurringSchedule, contactDirectAnswer, datedFacilityHoursAnswer, directlyAnswersQuestionForm, eventFailureActions, eventsAnswer, extractiveAnswer, genericEvidenceBoundary, liveRecyclingScheduleAnswer, poolStatusAnswer, relevantActions, sourcedAnswer, unanchoredRecurringScheduleAnswer, usefulSentences };
