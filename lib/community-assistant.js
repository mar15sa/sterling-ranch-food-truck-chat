...
function configuredCommunityResource(index = {}, profile = {}, candidates = []) {
...
function genericEvidenceBoundary(question, rulesAnswer = {}, index = {}, profile = {}, candidates = []) {
  const resource = configuredCommunityResource(index, profile, candidates);
...
function missingContactAnswer(organization, routingPlan, options = {}) {
  ...
  const contactDirectory = configuredCommunityResource(options.index, options.profile, contactCandidates);
  const boundary = buildAnswerContract({
...
function extractiveAnswer(question, result, options = {}) {
  ...
  if (result.requestedDetails.includes("contact") || options.routingPlan?.goal === "contact") {
    ...
    if (missingOrganization) return missingContactAnswer(missingOrganization, options.routingPlan, { index: result.index, profile: options.communityProfile, sources });
...
function conditionalInstructionMatchesQuestion(question, fact = {}) {
  const generic = new Set(["approval", "application", "architectural", "controlling", "design", "drc", "how", "needed", "need", "required", "review", "rule", "send", "submit", "where"]);
  const subjectTokens = tokens(question).filter((token) => token.length >= 4 && !generic.has(token));
  const evidence = `${fact.context || ""} ${fact.value || ""}`.toLowerCase();
  return subjectTokens.some((token) => evidence.includes(token));
}
...
async function approvedOperationalInstructionAnswer(question, routingPlan, options = {}) {
  ...
  const searches = [...new Set([intent, "forms", "services", "facilities"])].map((candidateIntent) =>
    searchCommunityIndex(question, {
      index: options.index,
      indexPath: options.indexPath,
      communityId: options.communityId,
  ...
  const details = (source.facts || []).filter((candidate) => candidate !== fact && candidate.type === "information")
    ...
    .slice(0, 2);
  const actionSourceIds = action?.url
...
async function approvedOperationalContactAnswer(question, routingPlan, options = {}) {
  ...
  const source = sourceForDisplay(selected.source);
  const fact = (source.facts || []).find((candidate) =>
    ["email", "phone"].includes(candidate.type) && (!requestedType || candidate.type === requestedType)
  );
  ...
  if (answer.confidence?.canAnswer !== true || !preservesRequestedContactValues({
    directAnswer: answer.directAnswer,
    keyDetails: answer.keyDetails,
    nextStep: answer.nextStep,
  }, [source], requested)) return null;
