...
const { factIsAnswerable, inferFacet, normalizeFactValue } = require("./community-truth");
...
function asksForPoolHours(question = '') {
  const text = String(question);
  if (!/\bpool\b/i.test(text)) return false;
  return /\b(?:hours?|houres|closing time|opening time)\b/i.test(text)
    || /\b(?:when|what time|how late)\b.{0,35}\b(?:open|opens?|close|closes?|shut|shuts?)\b/i.test(text)
    || /\b(?:open|opens?|close|closes?|shut|shuts?)\b.{0,35}\b(?:when|what time|how late)\b/i.test(text);
}

function classifyCommunityIntent(question = "") {
  const text = String(question).toLowerCase();
  if (asksForPoolEvents(text)) return 'events';
  if (asksForPoolHours(text)) return 'facilities';
...
function normalizedRoutingPlan(plan = {}, question = "", options = {}) {
  ...
  const poolHours = asksForPoolHours(question) && (!goal || ['status', 'information', 'schedule'].includes(goal));
  if (poolHours) { proposedIntent = 'facilities'; goal = 'schedule'; }
  ...
  const normalized = normalizeInterpretation({
    ...plan,
    ...((poolEvents || poolHours) ? {
      filters: { ...plan.filters, facility: poolEvents ? '' : 'pool', ...(poolEvents ? { location: 'pool' } : {}) },
      needsClarification: false,
      clarificationQuestion: '',
    } : {}),
    ...(poolHours ? { requestedDetails: [...new Set([...(plan.requestedDetails || []), 'hours', ...(/\b(?:today|labor day)\b/i.test(question) ? ['date'] : [])])] } : {}),
    intent,
    goal,
    goals: (poolEvents || poolHours) ? ['schedule'] : [goal, ...(Array.isArray(plan.goals) ? plan.goals : [])],
  }, question, options);
  return normalized;
}
...
function searchCommunityIndex(question, options = {}) {
  ...
  const enforceLedger = Array.isArray(index.factLedger) && index.truthStatus?.migrationMode !== "trusted-baseline";
  const reviewNow = new Date(options.now || Date.now()).getTime();
  const canUseSource = sourceReviewGate(index, reviewNow);
  if (enforceLedger) {
    for (const entry of index.factLedger) {
      const key = `${entry.sourceId}:${entry.sourceVersion}`;
      if (!factIsAnswerable(entry, reviewNow)) continue;
      const values = ledgerBySource.get(key) || new Set();
      values.add(`${entry.factType}:${JSON.stringify(entry.normalizedValue)}`);
      ledgerBySource.set(key, values);
    }
  }
  const ranked = index.sources
    ...
    .map((source) => {
      const approvedFacts = ledgerBySource.get(`${source.id}:${source.contentHash}`);
      const facts = !enforceLedger ? source.facts : (source.facts || []).filter((fact) =>
        approvedFacts?.has(`${fact.type}:${JSON.stringify(normalizeFactValue(fact))}`)
      );
      return {
        ...source,
        facts,
        withheldByFreshness: !canUseSource(source),
        score: scoreSource(source, queryTokens, intent, details, coreTokens, question, index.factAuthority || {}),
      };
    })
    .filter((source) => source.score > 0)
    .sort((a, b) => b.score - a.score || b.authorityScore - a.authorityScore);
  const withheldSources = ranked.filter((source) => source.withheldByFreshness);
