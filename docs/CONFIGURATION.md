...
- Keep `RULES_ENABLE_LLM_REWRITE=false` unless an owner deliberately chooses slower, paid rewrites. Grounded deterministic answers remain the default.
...
| Anthropic | Optional answer rewriting only | Deterministic grounded answer is returned. Disabled by default. |
