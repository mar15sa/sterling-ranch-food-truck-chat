...
## Precise pending update: simpler flow and semantic writing comparison

Targets: Decisions, How the project works, and audit follow-ups. Fetch current pages before editing. Status: completed local experiments; no selected implementation or verified-live change. Automatic approval review previously rejected the external update; confirmation remains unanswered, so preserve this exact local update.

Proposed decision text: “A simpler experimental flow uses Haiku to understand the request, then Haiku or Sonnet to write from eligible evidence. Independent review occurs afterward; outputs are explicitly unreviewed and carry no verified completion claim. In 48 diagnostic trials, both writers produced 16 drafts. Answering-only estimates were $11.32/1,000 for Haiku and $26.42 for Sonnet; sample p95 completion times were 17.114s and 11.797s. The repaired current-local control had two rewrite timeouts with unknown charges, so it has no complete price estimate for this run. No candidate meets the release criteria.”

Proposed retrieval finding: “A separate 16-call replay held interpretations and writing instructions fixed and changed only keyword versus local semantic-plus-keyword rule retrieval. All eight hybrid drafts used the specific pergola limits rather than losing that governing section. This is targeted evidence for better retrieval, not an overall quality score or proof that a paid vector database is needed. Exact form applicability, qualification of requirements, concision and independent human calibration remain unresolved. One draft leaked tool-format markup; a later validator rejects it without rewriting the historical record.”

Proposed cost/status text: “The two captures used $0.993524 in known token-priced charges plus two unknown timed-out control charges; conservative reserved maximum was $2.743461 of the announced $3 phase ceiling. No embedding API, new subscription or deployment was added. Full operating costs still need hosting, indexing, live connectors and any asynchronous grading. Older phase costs and unknowns remain recorded separately.”

Experimental diagram branch: interpretation → complete approved community catalog + keyword or semantic rules evidence → bounded writing → explicitly unreviewed local output → independent review. Preserve the existing live flow diagram. If adding this experimental diagram, update its accessible text and matching hub image with a proposed/local label. Supporting reports: `docs/COMMUNITY-SIMPLE-FLOW-COMPARISON.md`, captured source/code revision d88c7bb, semantic replay revision 49af2ed, both artifact directories, and the separate post-capture validation record. Runtime suite remains 874/874 from the preceding runtime commit; later changes are experiment-only and have focused validation.
