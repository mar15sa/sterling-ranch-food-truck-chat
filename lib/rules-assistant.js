...
async function answerRulesQuestion(query, options = {}) {
  ...
  const renderedFallbackBindings = new Set(explicitDimensionBindings(naturalFallback)
    .map(({ role, value }) => `${role}:${value}`));
  const fallbackAttributeDetails = focusedDimensionDetails(derived.sources)
    .filter(({ role, value }) => !renderedFallbackBindings.has(`${role}:${value}`))
    .map(({ role, value }) => {
      const label = role === "width" ? "Spread/width" : capitalizeFirstLetter(role);
  ...
  const fallbackLead = naturalNextStep && naturalFallback.endsWith(naturalNextStep)
    ? naturalFallback.slice(0, -naturalNextStep.length).trim()
    : naturalFallback;
  const answer = rewrittenAnswer || [fallbackLead, ...fallbackAttributeDetails, naturalNextStep]
    .filter(Boolean)
    .join("\n\n");
  ...
  return {
    ...
    keyDetails: rewrittenAnswer ? draftKeyDetails : [...draftKeyDetails, ...fallbackAttributeDetails].slice(0, 3),
