...
export function illustratedWalk(route={},source={}){
 const paths=(route.paths||[]).filter(p=>p.length>1),frame=routeFrame(route),unit=frame[2]/400;
 ...
 return '<svg class="route-sketch route-context" viewBox="'+frame.join(' ')+'" role="img" aria-label="'+esc(route.name||'Trail section')+': '+(paths.length?'highlighted route on its actual map, A marks '+(loop?'start and return':'start, B marks finish'):'highlighted mileage label on the source map')+'" preserveAspectRatio="xMidYMid meet"><title>'+esc(route.name||'Trail section')+' — CAB map alignment; approximate distances.</title><image class="route-source-context" href="'+SOURCE+'" width="'+SIZE[0]+'" height="'+SIZE[1]+'" preserveAspectRatio="none"/>'+annotations+(paths.length?'<g class="route-lines">'+lines(paths,'route-halo')+lines(paths,'route-casing')+lines(paths,'route-highlight')+lines(paths,'route-direction')+'</g>':'')+marker+badge(start,'A','route-start')+(!loop?badge(end,'B','route-finish'):'')+'</svg>';
}
