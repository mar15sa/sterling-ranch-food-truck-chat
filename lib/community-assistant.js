...
const { isWaterUsageAccessRequest, instructionalRequestShape, normalizeInterpretation, resolveInterpretationMode } = require("./community-interpretation");
...
function recoveryInstructionBoundary(question, result, options = {}) {
  ...
  return buildAnswerContract({
    directAnswer: 'I couldn’t verify recovery instructions from the approved sources.',
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (shouldConsultRules) {
    ...
    if (rulesAnswer?.confidence?.canAnswer
      ...
      const bindingRule = asksBindingRule(question, routingPlan) || hasAuthoritativePermissionDecision(rulesAnswer);
