...
The first local implementation of that correction adds an explicit `audited-legacy-candidate` mode. It audits the current grounded answer against the resident-needs contract and preserves the full answer whenever it is verified, safely handles a boundary, or contains verified support for any need. The per-need fallback is shown only when the current answer has no verified support and the fallback adds verified support. All seven public examples now remain byte-for-byte identical to their richer baseline answers, including their action links. A separate regression proves that a complete per-need answer can still replace a baseline that has no verified support. This mode is implemented and verified locally only; staging stays on `legacy` after containment.
