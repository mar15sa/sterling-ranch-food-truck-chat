function deriveAnswerVerdict(answer, confidence = {}) {
  ...
  if (
    /\b(?:requires?|required|must (?:first )?(?:receive|obtain|submit)|with (?:DRC|CAB|design review) approval|subject to approval|only (?:after|with))\b/i.test(
      lead
    )
  ) {
    return "conditional";
  ...
  if (
    /^(?:no\b|not allowed\b)|\b(?:is|are) (?:not allowed|prohibited)\b|\b(?:may not|cannot|can't)\b/i.test(
      lead
    )
  ) {
    return "prohibited";
