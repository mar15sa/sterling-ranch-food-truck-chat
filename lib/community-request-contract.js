...
function assessResidentNeeds(contract, answer = {}) {
  ...
  const needs = contract.needs.map((need) => {
    ...
    const relevantClaimEntries = verifiedClaimEntries.filter(({ claim }) => (claim.evidenceSourceIds || []).some((id) => candidateSourceIds.includes(id))
      && enoughSemanticOverlap(needTokens, claim.text));
    const relevantClaims = relevantClaimEntries.map(({ claim }) => claim);
    ...
    return {
      ...
      supportingClaimIndexes: relevantClaimEntries.map(({ index }) => index),
