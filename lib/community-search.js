...
function approvedClaimProjection(source = {}, entries = []) {
  const facts = (source.facts || []).filter(fact => entries.some(entry => factMatchesApprovedEntry(fact, source, entry)));
  const actions = (source.actions || []).filter(action => entries.some(entry => actionMatchesApprovedEntry(action, entry)));
  const snippets = entries.map(entry => String(entry.supportingText || '').trim()).filter(Boolean);
...
function searchCommunityIndex(question, options = {}) {
  ...
  const ranked = index.sources
    ...
    .map((source) => {
      ...
      const answerable = reviewState.canUseProjection(source)
        && (dynamic || entriesSupportRequestedDetails(entries, details));
      const projected = dynamic ? source : answerable
        ? approvedClaimProjection(source, entries)
