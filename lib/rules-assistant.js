...
function landscapeOverviewAnswer(sources = []) {
  ...
  const front = clauses.find((entry) => /front yard (?:shall consist|design and installation shall include)|live coverage/i.test(entry.text));
  const back = clauses.find((entry) => /backyard landscaping|backyard must contain/i.test(entry.text));
  const irrigation = clauses.find((entry) => /automatic, underground irrigation system is required/i.test(entry.text));
  const care = sourceText.match(/Landscaping is to be kept healthy[^.]*\./i)?.[0]?.trim() || "";
  const reviewLabel = [
    plan?.text.match(/\bDRC\b/i)?.[0] || "",
    plan?.text.match(/\breview\b/i)?.[0] || "",
  ].filter(Boolean).join(" ");
  const conciseYardClause = (entry) => entry
    ? `${capitalizeFirstLetter(entry.text.split(/:\s*(?:\(\d+\))?/)[0].replace(/[.;:]?\s*$/, ""))}.`
    : "";
  if (!plan) return sourceClauseOverviewAnswer("landscape yard requirements", sources);
  return structuredHelpfulAnswer(
    `${reviewLabel}: ${capitalizeFirstLetter(plan.text)}`,
    [
      front || back ? `Yard design: ${[conciseYardClause(front), conciseYardClause(back)].filter(Boolean).join(" ")}` : "",
      irrigation ? `Irrigation: ${capitalizeFirstLetter(irrigation.text)}` : "",
      care ? `Ongoing care: ${capitalizeFirstLetter(care)}` : "",
    ],
    "Open the linked official section if you need the complete wording."
  );
}
...
async function answerRulesQuestion(query, options = {}) {
  ...
  return {
    ...
    answerVerdict: deriveAnswerVerdict(residentRewriteAnswer || draftDirectAnswer, finalConfidence),
