...
async function answerCommunityQuestion(query, options = {}) {
  if (["shadow-route", "need-first-candidate"].includes(options.requestContractMode)) {
    ...
    if (candidateMode) {
      const contract = buildResidentRequestContract(query, options.requestContractPlan || null, options.requestContext);
      ...
      const candidate = await runNeedFirstShadow(contract, answerResidentNeed, {
