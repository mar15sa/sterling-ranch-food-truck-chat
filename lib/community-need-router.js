  ...
  if (["status", "schedule", "hours"].includes(result.need.task)) {
    ...
    if (result.need.task === "hours"
      ...
      if (closingTime) {
        ...
        const seasonSentence = allAnswerText.split(/(?<=[.!?])\s+(?=[A-Z0-9])/)
          .find((sentence) => /\bopen\b.{0,120}\bthrough\b/i.test(sentence))?.trim() || "";
        return `${subject} closes at ${closingTime.replace(/\.$/, "")}.${seasonSentence ? ` ${seasonSentence}` : ""}`;
      }
    }
