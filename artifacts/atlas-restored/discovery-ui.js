...
export function createDiscovery({ places, roots, trails, models, located, selected, select, showRoute, setView, routeSvg, focusVillage, focusState, point, onChange }) {
  ...
  function renderSheets() {
    ...
    if(!unfolded)return;
    sheets.innerHTML='<button class="connections-return" data-close-connections>← Back to the map</button>'+connectionScene({places,trails,models,routeSvg,roots,selected:selected(),village,focusKind:focusState()?.kind});
  }
  function render() {
  ...
  toolbar.addEventListener('click',e=>{const b=e.target.closest('button');if(b?.id==='unfold-map'){unfolded=!unfolded;render();if(unfolded)$('#unfold-sheets').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'});changed();}});
  ...
  document.addEventListener('click',e=>{
    ...
    if('closeConnections' in b.dataset){unfolded=false;render();$('#unfold-map').focus({preventScroll:true});$('#landscape').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'});changed();}
    if(b.dataset.village){const fromConnections=!!b.closest('#unfold-sheets');const fromMap=b.classList.contains('village-label');chooseVillage(b.dataset.village,true,!fromConnections);if(fromConnections){unfolded=true;render();$('#unfold-sheets').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'});$('#unfold-sheets button')?.focus({preventScroll:true});changed();}if(fromMap)toolbar.querySelector(`[data-village="${b.dataset.village}"]`)?.focus({preventScroll:true});}
