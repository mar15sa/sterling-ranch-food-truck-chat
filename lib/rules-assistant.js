...
const PROJECT_PHRASE_STOP_WORDS = new Set([
  "in", "on", "at", "for", "from", "with", "to", "near", "by", "behind", "beside", "under", "over", "within", "outside", "of", "that", "which",
]);
const PROJECT_DESCRIPTOR_WORDS = new Set([
  "new", "small", "large", "big", "my", "our", "the", "a", "an", "existing", "proposed", "permanent", "temporary", "freestanding", "attached", "detached", "outdoor", "indoor", "backyard", "frontyard", "front", "rear", "side", "residential",
]);
...
function namedProjectTerms(query = "") {
  ...
  for (const token of tokenize(match[1])) {
    if (PROJECT_PHRASE_STOP_WORDS.has(token)) break;
...
function isSourceEvidenceBoundaryAnswer(answer = "") {
  return /Short answer:\s*(?:The official passage I found mentions|The selected (?:official passages|rules|official passage).*(?:do not|does not|but))/i.test(
    String(answer || "")
  );
}
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (/^pickle\s*ball[?.!]*$/i.test(question)) {
    return {
      answer: "Are you asking about using a community pickleball court, or about building a private pickleball court on your lot? Those are covered by different official rules.",
      answerMode: "targeted-clarification",
      answerVerdict: "informational",
      inputClassification: INPUT_CLASSIFICATIONS.UNCLEAR,
      confidence: { canAnswer: false, confidence: "high", reason: "ambiguous-pickleball-question" },
      reviewNeeded: false,
      sources: [],
    };
  }
  ...
  const directSpecialSources = specialSourcesForQuestion(index, question);
  if (directSpecialSources.length) routingQuery = question;
  const sources = chapterSources(index, routingQuery, 5);
  const specialSources = directSpecialSources.length
  ...
  if (projectEvidence.applies && !projectEvidence.supported) {
    return {
      answer: structuredHelpfulAnswer(
        "I could not verify whether the named project is allowed or requires approval from current official rulebook evidence.",
        [],
        "Check the official rulebook or confirm the project with the CAB before planning or building it."
      ),
