...
const GENERIC_RULE_EVIDENCE_TERMS = new Set([
  "allow", "allowed", "approve", "cab", "drc", "must", "permit", "permitted", "prohibit", "prohibited", "require", "rule", "shall",
]);

function evidenceTerms(value = "") {
  return new Set(tokenize(value).map(normalizedEvidenceToken).filter((term) => !GENERIC_RULE_EVIDENCE_TERMS.has(term)));
}
...
function modalityIsSupported(claimText, sourceText) {
  ...
  if (claim.required && !source.required) return false;
