...
function validateCommunityProfile(profile) {
  ...
  if (profile.connectorAdapterContractVersion !== undefined) {
    if (profile.connectorAdapterContractVersion !== 1) throw new Error("Unsupported connector adapter contract version.");
    createConnectorAdapters(profile);
  }
  return profile;
}
  ...
  return claims
    ...
    .map((claim) => {
      ...
      return {
        ...
        evidenceSourceIds,
        ...((claim.approvalClaimIds || []).length ? {
          approvalClaimIds: [...new Set(claim.approvalClaimIds.map(String).map((id) => id.trim()).filter(Boolean))],
        } : {}),
...
function reviewedFactCoversDetail(fact = {}, detail) {
  const type = fact.type || fact.factType;
  const text = String(fact.context || fact.supportingText || fact.value || '');
  if (/\b(?:unknown|unconfirmed|not (?:known|available|published|provided|specified)|no (?:information|count|quantity|number)|does not (?:specify|provide|publish))\b/i.test(text)) return false;
  if (detail === 'eligibility') {
    return fact.facet === 'eligibility' || (fact.facet === 'reservation-policy'
      && /\b(?:eligib(?:le|ility)|qualif(?:y|ies|ied|ication|ications)|restricted to|available (?:only )?to|open (?:only )?to|(?:non)?residents? can reserve)\b/i.test(text));
  }
  if (detail === 'quantity') return ['quantity', 'count'].includes(fact.facet)
    && !['date', 'time', 'schedule', 'money', 'phone', 'email', 'link'].includes(type)
    && /\b(?:\d+(?:,\d{3})*|zero|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|no|unlimited)\b/i.test(text);
  if (detail === 'specification') return fact.facet === 'specification'
    || (['fee', 'information', 'reservation-policy'].includes(fact.facet)
      && /\b(?:(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|forty[- ]eight)\s*(?:\(\s*\d+(?:\.\d+)?\s*\))?|\d+(?:\.\d+)?)\s*(?:business\s+)?(?:"|(?:seconds?|minutes?|hours?|days?|nights?|weeks?|months?|years?|feet|foot|inches|inch|ft\.?|in\.?)\b)/i.test(text));
  return false;
}

function reviewedDetailIsRendered(detail, text, sources = [], claims = []) {
  const normalize = value => String(value || '').replace(/\s+/g, ' ').trim().toLowerCase();
  const rendered = normalize(text);
  return sources.some(source => (source.facts || []).some(fact => {
    if (!reviewedFactCoversDetail(fact, detail) || !fact.approvalClaim) return false;
    // Structured coverage alone is not proof. Require the selected reviewed
    // claim to carry this exact detail and actually appear in the answer.
    const quote = normalize(fact.context || fact.value);
    if (!quote || !rendered.includes(quote)) return false;
    return claims.some(claim => claim.verified
      && claim.evidenceSourceIds.includes(source.id || source.nodeId)
      && (claim.approvalClaimIds || []).includes(fact.approvalClaim)
      && normalize(claim.text).includes(quote));
  }));
}

function renderedCoveredDetails(requestedDetails, coveredDetails, directAnswer, keyDetails, actions, sources, claims) {
  ...
  return declared.filter((detail) => {
    if (['eligibility', 'quantity', 'specification'].includes(detail)
      && hasStructuredCoverage && reviewedDetailIsRendered(detail, text, sources, claims)) return true;
    if (['eligibility', 'quantity'].includes(detail)) return hasStructuredCoverage
      && reviewedDetailIsRendered(detail, text, sources, claims);
    ...
    if (detail === "action") return actions.length > 0 || /\b(?:apply|ask|check|contact|submit|book|reserve|register|pay|open|call|email|use|visit)\b/i.test(text);
    ...
    if (detail === "specification") return /#[0-9]{2,}\b|\b(?:(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|forty[- ]eight)\s*(?:\(\s*\d+(?:\.\d+)?\s*\))?|\d+(?:\.\d+)?)\s*(?:business\s+)?(?:"|(?:seconds?|minutes?|hours?|days?|nights?|weeks?|months?|years?|feet|foot|inches|inch|ft\.?|in\.?)\b)|\b(?:color|colour|paint|stain|finish|material|height|size|dimension|setback|distance|duration)\b.{0,180}\b(?:approved|required|requires|must|match|prohibited|not allowed|no more than|at least|feet|foot|inches|#[0-9]{2,})\b/i.test(text);
    return hasStructuredCoverage;
  });
}

function normalizeDetailResolutions(detailResolutions = {}, sources = []) {
  const sourceIds = new Set(sources.map((source) => source.id || source.nodeId).filter(Boolean));
  const price = detailResolutions?.price;
  if (!price || price.status !== "not-applicable") return {};
  const evidenceSourceIds = [...new Set((price.evidenceSourceIds || []).filter((id) => sourceIds.has(id)))];
  if (!evidenceSourceIds.length) return {};
  return { price: { status: "not-applicable", evidenceSourceIds } };
}

function hasResolvedUnavailablePrice(detailResolutions = {}, directAnswer = "") {
  return detailResolutions?.price?.status === "not-applicable"
    && (detailResolutions.price.evidenceSourceIds || []).length > 0
    && /\b(?:no|not available|unavailable)\b.{0,80}\b(?:pool\s+)?(?:rental|fee|price|cost)\b|\bpool\b.{0,80}\bnot available for rental\b/i.test(directAnswer);
}
...
function buildAnswerContract({
  ...
  const normalizedDetailResolutions = normalizeDetailResolutions(detailResolutions, sources);
  ...
  // Select once before measuring coverage: a fourth, hidden detail cannot
  // make the displayed answer complete. Preserve each selected claim in full.
  const conciseDetails = keyDetails.filter(Boolean).slice(0, 3).map((detail) => String(detail).replace(/\s+/g, " ").trim());
  const renderedDetails = renderedCoveredDetails(requestedDetails, coveredDetails, cleanDirect, conciseDetails, normalizedActions, sources, normalizedClaims);
  if (hasResolvedUnavailablePrice(normalizedDetailResolutions, cleanDirect) && requestedDetails.includes("price") && !renderedDetails.includes("price")) {
    renderedDetails.push("price");
  }
  const resolvedDetails = ["verified", "verified-incomplete"].includes(status)
    ? renderedDetails
  ...
  // Keep the selected detail count bounded, but preserve each complete claim.
  // A character or sentence cutoff can remove its outcome, exception or
  // condition while the full claim still carries a verified label below.
  ...
  return {
    ...
    ...(Object.keys(normalizedDetailResolutions).length ? { detailResolutions: normalizedDetailResolutions } : {}),
    conflicts,
...
module.exports = {
  ...
  detectFactConflicts,
  normalizeClaims,
  reviewedFactCoversDetail,
  reviewedDetailIsRendered,
