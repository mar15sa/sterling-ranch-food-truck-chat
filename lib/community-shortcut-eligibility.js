...
const PROACTIVE_PROFILES = {
  ...
  "community-proactive-clubhouse-access": { goals: ["booking", "account-access", "information"], subject: /\b(?:clubhouse|overlook)\b/i },
  "community-proactive-clubhouse-access-partial": { goals: ["booking", "account-access", "information"], subject: /\b(?:clubhouse|overlook)\b/i },
...
function coversDetail(detail, candidate = {}, plan = {}) {
  ...
  if (detail === "action") {
    ...
    return (candidate.actions || []).some((action) => signals[plan.goal]?.test(`${action.label || ""} ${action.url || ""}`)
      || (plan.goal === "booking" && candidate.answerMode?.startsWith("community-proactive-clubhouse-access")
