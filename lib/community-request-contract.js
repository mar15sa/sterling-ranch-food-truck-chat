...
function evidenceKindForNeed(goal = "", details = []) {
  if (["permission", "specification", "quantity", "eligibility"].some((detail) => details.includes(detail))) return "governing-rule";
  if (["date", "status", "hours"].some((detail) => details.includes(detail))) return "live-operation";
...
function buildResidentRequestContract(question = "", routingPlan = null, requestContext = {}) {
  ...
  const needs = clauses.map((text, index) => {
    const request = usedPriorContext && clauses.length === 1 ? resolvedQuestion : text;
    let explicitDetails = deterministicRequestedDetails(request);
    ...
    return {
      ...
      evidenceKind: evidenceKindForNeed(goal, requestedDetails),
      goal,
      requestedDetails,
...
function claimSupportsDetail(detail, text = "", actions = []) {
  ...
  if (detail === "specification") return /#[0-9]{2,}\b|\b(?:\d+(?:[ -]\d+\/\d+)?|one|two|three|four|five|six|seven|eight|nine|ten|twelve)\s*(?:feet|foot|inches|inch|ft\.?|in\.?)\b|\b(?:color|paint|stain|finish|material|height|size|dimension|setback|distance)\b/i.test(text);
  return true;
}
...
function assessResidentNeeds(contract, answer = {}) {
  ...
  const claims = claimsWereReported ? answer.claims.filter((claim) => claim?.verified === true) : [];
  ...
  const needs = contract.needs.map((need) => {
    // The global route subject can contain words from sibling needs (for
    // example, "food truck and menu"). Score evidence against this need's
    // own standalone request so proof for one part cannot satisfy another.
    const needTokens = distinctiveTokens(need.request);
    ...
    const relevantClaims = claims.filter((claim) => (claim.evidenceSourceIds || []).some((id) => candidateSourceIds.includes(id))
    ...
    const supportedDetails = need.requestedDetails.filter((detail) => relevantClaims.some((claim) =>
      claimSupportsDetail(detail, claim.text, detail === "action" ? relevantActions : [])
      && responseText.includes(String(claim.text))));
    ...
    return {
      ...
      supportingSourceIds: [...new Set(relevantClaims.flatMap((claim) => claim.evidenceSourceIds || []))],
      supportingClaimIndexes: relevantClaims.map((claim) => claims.indexOf(claim)), supportedDetails, missingDetails,
    };
  });
