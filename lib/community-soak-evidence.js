...
function configurationFingerprint(env = process.env) {
  const values = Object.fromEntries(Object.keys(env).filter(key => /^(RULES|COMMUNITY)_/.test(key)
    && /(?:MODE|MODEL|TIMEOUT_MS|INTERVAL_MS|AUTO_REFRESH|MAX_TOKENS|ENABLED|BUDGET|SEARCH_LIMIT)$/.test(key))
    .sort().map(key => [key, env[key]]));
  values.aiConfigured = Boolean(env.ANTHROPIC_API_KEY);
  return require('node:crypto').createHash('sha256').update(JSON.stringify(values)).digest('hex');
}
module.exports = { resumeEvidence, writeEvidence, configurationFingerprint };
