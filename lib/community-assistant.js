...
async function answerCommunityQuestion(query, options = {}) {
  if (["shadow-route", "need-first-candidate", "need-audited-candidate"].includes(options.requestContractMode)) {
    ...
    if (candidateMode) {
      ...
      if (typeof options.rewriteNeedFirstAnswer === "function") {
        try {
          const rewritten = await options.rewriteNeedFirstAnswer({
            question: query,
            contract,
            candidate: deterministicCandidate,
          });
          if (rewrittenCandidatePreservesContract(deterministicCandidate, rewritten)) candidate = rewritten;
        } catch {
          candidate = deterministicCandidate;
        }
      }
      ...
      return {
        ...
        _requestContract: {
          ...
          candidate: {
            ...
            writerAccepted: candidate !== deterministicCandidate,
