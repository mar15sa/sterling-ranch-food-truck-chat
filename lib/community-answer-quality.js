...
function directResolutionIssues(question, result, answer) {
  ...
  const resolvesTime = /\b\d{1,2}(?::\d{2})?\s*(?:a\.?m\.?|p\.?m\.?)\b|\b(?:no|without) (?:specific )?(?:time|hour) restriction\b|\b(?:rules?|sources?)\b.{0,25}\b(?:do not|don't)\b.{0,20}\b(?:set|publish)\b.{0,20}\b(?:latest|specific|operating)?\s*(?:time|hours?)\b|\b(?:any time|sunset|sunrise|dusk|dawn)\b/i.test(directAnswer);
