...
function blocks(html, tag, className) {
  ...
  // A response cut off inside a selected row must not look like a complete
  // collection of the earlier rows. Let callers retain ordinary page handling.
  return stack.some(entry => entry.selected) ? null : result;
}
...
function staffDirectoryRows(html = '', sourceUrl = '') {
  ...
  const departments = blocks(html, 'div', 'directory-table-row');
  const people = blocks(html, 'li', 'list-group-item');
  if (!departments || !people) return null;
  const groups = [
    ...departments.map(html => ({ kind:'department', html })),
    ...people.map(html => ({ kind:'person', html })),
  ];
  ...
  for (const group of groups) {
    ...
    seen.add(key);
    rows.push({ kind:'staff', subjectKey, title:link.label, text, html:group.html });
  }
  return rows.length ? rows : null;
}

module.exports = { staffDirectoryRows, htmlBlocks: blocks, htmlAttribute: attribute };
