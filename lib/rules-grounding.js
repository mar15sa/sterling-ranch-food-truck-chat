const { unsupportedResourceAbsenceIssues } = require("./rules-intent");
...
function properNounPhrases(text = "") {
  ...
  const matches = value.match(
    /\b(?:[A-Z][A-Za-z0-9&/'-]+|CAB|DRC)(?:[ \t]+(?:[A-Z][A-Za-z0-9&/'-]+|CAB|DRC))*\b/g
  ) || [];
...
function phraseSupportedByCorpus(phrase, corpus) {
  ...
  if (normalizedCorpus.includes(normalizedPhrase.replace(/['’]s\b/g, ""))) return true;
...
function claimModalities(text = "") {
  ...
  const approvalNotRequired =
    ...
    /\b(?:you|owners?|residents?|homeowners?)\s+(?:do not|don't)\s+need\b.{0,30}\bapproval\b/.test(value) ||
...
function claimPolarityIssues(answer, sources = []) {
  ...
  if (answerModes.permission && sourceModes.prohibition && !sourceModes.permission && !relevantException) {
  ...
  if (answerModes.approvalNotRequired && !sourceModes.approvalRequired && !sourceModes.approvalNotRequired) {
    issues.push("opening claim says approval is unnecessary, but the relevant cited text does not say that");
  }
...
const LOCATION_CONCEPT_PATTERNS = [
  ["utility-easement", "utility easement", /\butility easements?\b/gi],
  ["utility-line", "utility line", /\b(?:underground utilit(?:y|ies)|utility lines?)\b/gi],
  ["setback", "setback", /\bsetbacks?\b/gi],
  ["property-line", "property line", /\b(?:property|lot) lines?\b/gi],
  ["fence-line", "fence line", /\bfence lines?\b/gi],
  ["front-yard", "front yard", /\bfront (?:yards?|lawns?)\b/gi],
  ["rear-yard", "rear yard", /\b(?:rear yards?|back ?yards?)\b/gi],
  ["side-yard", "side yard", /\bside yards?\b/gi],
  ["tree-lawn", "tree lawn", /\btree lawns?\b/gi],
  ["common-area", "common area", /\bcommon areas?\b/gi],
  ["open-space", "open space", /\bopen space\b/gi],
  ["easement", "easement", /\beasements?\b/gi],
  ["sidewalk", "sidewalk", /\bsidewalks?\b/gi],
  ["driveway", "driveway", /\bdriveways?\b/gi],
  ["garage", "garage", /\bgarages?\b/gi],
  ["patio", "patio", /\bpatios?\b/gi],
  ["porch", "porch", /\bporches?\b/gi],
  ["deck", "deck", /\bdecks?\b/gi],
  ["roof", "roof", /\broofs?\b/gi],
  ["street", "street", /\bstreets?\b/gi],
  ["wall", "wall", /\bwalls?\b/gi],
  ["fence", "fence", /\bfences?\b/gi],
  ["gate", "gate", /\bgates?\b/gi],
  ["yard", "yard", /\byards?\b/gi],
];

function locationConcepts(text = "") {
  const value = String(text || "");
  const found = [];
  for (const [key, label, pattern] of LOCATION_CONCEPT_PATTERNS) {
    pattern.lastIndex = 0;
    if (!pattern.test(value)) continue;
    if (key === "easement" && found.some((item) => item.key === "utility-easement")) continue;
    if (key === "fence" && found.some((item) => item.key === "fence-line")) continue;
    if (key === "yard" && found.some((item) => /-yard$/.test(item.key))) continue;
    found.push({ key, label });
  }
  return found;
}

function focusedSourceCorpus(sources = []) {
  return sources
    .map((source) => source?.questionSpecificExcerpt && source?.excerpt
      ? source.excerpt
      : [source?.text, source?.excerpt].filter(Boolean).join(" "))
    .join(" \n ");
}

function establishedLocationKeys(text = "") {
  const value = String(text || "");
  const established = new Set();
  for (const [key, , pattern] of LOCATION_CONCEPT_PATTERNS) {
    pattern.lastIndex = 0;
    for (const match of value.matchAll(pattern)) {
      const before = value.slice(Math.max(0, match.index - 60), match.index);
      const after = value.slice(match.index + match[0].length, match.index + match[0].length + 35);
      if (
        key === "setback" ||
        /\b(?:in|on|at|along|near|by|beside|behind|within|outside|from|between|toward|towards|adjacent\s+to|next\s+to)\s+(?:all|any|the|a|an|your|their|my|our)?\s*$/i.test(before) ||
        /\b(?:located|placed|installed|kept|stored|planted|positioned)\b[^.!?]{0,45}$/i.test(before) ||
...
function unsupportedQuestionLocations(residentQuestion = "", sources = []) {
  const supportedKeys = establishedLocationKeys(focusedSourceCorpus(sources));
  return locationConcepts(residentQuestion).filter(({ key }) => !supportedKeys.has(key));
}

function locationBoundaryAcknowledged(answer = "") {
  return /\b(?:source|rule|rulebook|list|evidence|section|passage)\b.{0,90}\b(?:does not|doesn't|cannot|can't|did not|didn't)\b.{0,45}\b(?:address|approve|confirm|cover|establish|permit|say|show|specify|verify)\b|\b(?:cannot|can't|could not|couldn't)\b.{0,35}\b(?:confirm|verify)\b.{0,45}\b(?:location|placement|there)\b|\b(?:location|placement)\b.{0,35}\b(?:is|are)\s+not\s+(?:addressed|approved|confirmed|covered|established|permitted|specified|verified)\b/i.test(
    String(answer || "")
  );
}

function affirmativePlacementLocationKeys(answer = "", locationKeys = []) {
  const targetKeys = new Set(locationKeys);
  const segments = String(answer || "")
    .split(/[.;!?]|\b(?:but|however|although|though|yet)\b/gi)
    .map((segment) => segment.trim())
    .filter(Boolean);
  const claimed = new Set();

  for (const segment of segments) {
    if (locationBoundaryAcknowledged(segment)) continue;
    const segmentKeys = locationConcepts(segment)
      .map(({ key }) => key)
      .filter((key) => targetKeys.has(key));
    if (!segmentKeys.length) continue;
    const directPlacement =
      claimModalities(segment).permission ||
      /\b(?:can|may)\s+(?:be\s+)?(?:plant(?:ed)?|put|plac(?:e|ed)|install(?:ed)?|keep|kept|stor(?:e|ed)|locat(?:e|ed)|position(?:ed)?)\b/i.test(segment) ||
      /\b(?:allowed|permitted)\s+(?:in|on|at|along|near|by|beside|behind|within|outside|between|adjacent\s+to|next\s+to)\b/i.test(segment);
    if (!directPlacement) continue;
    for (const key of segmentKeys) claimed.add(key);
  }
  return [...claimed];
}

function locationScopeIssues(answer, sources = [], residentQuestion = "") {
  const sourceKeys = establishedLocationKeys(focusedSourceCorpus(sources));
  const questionKeys = new Set(locationConcepts(residentQuestion).map(({ key }) => key));
  const answerLocations = locationConcepts(answer);
  const unsupportedQuestion = [...questionKeys].filter((key) => !sourceKeys.has(key));
  const boundaryAcknowledged = locationBoundaryAcknowledged(answer);
  const questionRequestsPermission = /\b(?:can|may)\s+i\b|\b(?:allowed|permitted|permission|approval)\b/i.test(
    String(residentQuestion || "")
  );
  const unsupportedPlacementClaims = affirmativePlacementLocationKeys(answer, unsupportedQuestion);
  const issues = [];

  if (unsupportedPlacementClaims.length) {
    issues.push(`affirmative placement claim is not established by cited sections: ${unsupportedPlacementClaims.join(", ")}`);
  }
  if (unsupportedQuestion.length && questionRequestsPermission && !boundaryAcknowledged) {
    issues.push(`requested location(s) are not resolved by the cited sections: ${unsupportedQuestion.join(", ")}`);
  }
  if (unsupportedQuestion.length && claimModalities(leadClaimText(answer)).permission && !boundaryAcknowledged) {
    issues.push(`opening claim treats unsupported resident location(s) as approved: ${unsupportedQuestion.join(", ")}`);
  }

  const echoedUnsupported = answerLocations
    .map(({ key }) => key)
    .filter((key) => questionKeys.has(key) && !sourceKeys.has(key));
  if (echoedUnsupported.length && !boundaryAcknowledged) {
    issues.push(`resident-supplied location(s) not established by cited sections: ${[...new Set(echoedUnsupported)].join(", ")}`);
  }

  const inventedLocations = answerLocations
    .map(({ key }) => key)
    .filter((key) => !questionKeys.has(key) && !sourceKeys.has(key));
  if (inventedLocations.length) {
    issues.push(`location detail(s) not in the question or cited sections: ${[...new Set(inventedLocations)].join(", ")}`);
  }
  return issues;
}

function groundednessIssues(answer, sources = [], residentQuestion = "") {
  const corpus = citedCorpus(sources);
  // A proper noun copied from the resident's question is context, not a new
  // source claim. Keep it separate from the evidence corpus so dates, numbers,
  // section references, and rule claims must still come from cited sources.
  const properNounContext = `${corpus}\n${String(residentQuestion || "")}`;
  const residentProperNouns = new Set(properNounPhrases(residentQuestion));
  ...
  const missingProperNouns = properNounPhrases(answerFacts).filter(
    (phrase) => {
      if (phraseSupportedByCorpus(phrase, properNounContext)) return false;
      // The extractor can include a capitalized sentence opener (for example,
      // "For Labor Day") in the same phrase. Only discard that context word;
      // the remaining proper noun still has to appear exactly in the question.
      const withoutSentenceOpener = phrase.replace(
        /^(?:about|after|before|during|for|near|on|through|until|with)\s+/i,
        ""
      );
      return !residentProperNouns.has(withoutSentenceOpener);
    }
  );
...
function missingDraftNumberIssues(answer, draftAnswer) {
  const draftFacts = String(draftAnswer || "")
    .replace(/^(?:Short answer|What I found|Before you act)\s*:\s*/gim, "")
    .replace(/^[-*]\s+/gm, "");
...
function normalizedMeasurement(number, unit) {
  const normalizedNumber = String(number || "").replace(/\s+/g, " ").trim();
  const normalizedUnit = /^(?:'|feet|foot|ft\.?)$/i.test(unit) ? "ft"
    : /^(?:\"|inches|inch|in\.?)$/i.test(unit) ? "in"
      : String(unit || "").toLowerCase();
  return `${normalizedNumber} ${normalizedUnit}`;
}

function explicitDimensionBindings(text = "") {
  const bindings = [];
  const measurement = `(\\d+(?:\\.\\d+)?(?:\\s+\\d+\\/\\d+)?)\\s*(feet|foot|ft\\.?|'|inches|inch|in\\.?|\")`;
  const attribute = `(height|high|tall|width|wide|spread|depth|deep|length|long|setback)`;
  const add = (role, number, unit) => bindings.push({
    role: DIMENSION_ATTRIBUTE[String(role).toLowerCase()],
    value: normalizedMeasurement(number, unit),
  });
  for (const match of String(text || "").matchAll(new RegExp(`${measurement}\\s*(?:in\\s+)?${attribute}\\b`, "gi"))) {
    add(match[3], match[1], match[2]);
  }
  for (const match of String(text || "").matchAll(new RegExp(`\\b${attribute}\\b(?:\\s+(?:of|is|at|up\\s+to|no\\s+more\\s+than|maximum|minimum))?\\s*:?\\s*${measurement}`, "gi"))) {
    add(match[1], match[2], match[3]);
  }
  return bindings;
}

function citedDimensionBindings(sources = []) {
  const bindings = sources.flatMap((source) => {
    const focusedText = source?.questionSpecificExcerpt && source?.excerpt
      ? source.excerpt
      : [source?.text, source?.excerpt].filter(Boolean).join(" ");
    const sourceBindings = explicitDimensionBindings(focusedText);
    const sourceSchema = [source?.fullText, source?.text, source?.excerpt].filter(Boolean).join(" ");
    if (/\bHt\s*x\s*Spd\b/i.test(sourceSchema)) {
      const pairPattern = /(\d+(?:\.\d+)?(?:\s+\d+\/\d+)?)\s*(feet|foot|ft\.?|'|inches|inch|in\.?|")\s*x\s*(\d+(?:\.\d+)?(?:\s+\d+\/\d+)?)\s*(feet|foot|ft\.?|'|inches|inch|in\.?|")/gi;
      for (const match of focusedText.matchAll(pairPattern)) {
        sourceBindings.push(
          { role: "height", value: normalizedMeasurement(match[1], match[2]) },
          { role: "width", value: normalizedMeasurement(match[3], match[4]) }
        );
      }
    }
    return sourceBindings;
  });
  return new Map([...new Set(bindings.map(({ role }) => role))].map((role) => [
    role,
    new Set(bindings.filter((binding) => binding.role === role).map((binding) => binding.value)),
  ]));
}

function focusedDimensionDetails(sources = []) {
  const focusedSources = sources.filter((source) => source?.questionSpecificExcerpt && source?.excerpt);
  if (!focusedSources.length) return [];
  return [...citedDimensionBindings(focusedSources)]
    .filter(([, values]) => values.size === 1)
    .map(([role, values]) => ({ role, value: [...values][0] }));
}

function attributeBindingIssues(answer, sources = []) {
  const citedBindings = citedDimensionBindings(sources);
  const changed = explicitDimensionBindings(answer).filter(({ role, value }) =>
    citedBindings.has(role) && !citedBindings.get(role).has(value)
  );
  return changed.length
    ? [`attribute/value binding(s) not in cited sections: ${changed.map(({ role, value }) => `${role}=${value}`).join(", ")}`]
    : [];
}
...
function requiredPhraseIssues(answer, draftAnswer) {
  ...
  if (/\bdead trees? must be replaced\b/i.test(draftFacts)
    && !/\b(?:dead trees? must be replaced|must replace (?:a |the )?dead tree|replacement (?:of )?(?:a |the )?dead tree is required)\b/i.test(answerText)) {
    issues.push("required dead-tree replacement obligation dropped");
  }
...
function missingDraftProperNounIssues(answer, draftAnswer) {
  ...
  // Deterministic composers often put examples and responsible agencies in
  // bullets after the lead. Those details are still part of the grounded
  // answer and must survive a prose rewrite.
  const draftFacts = String(draftAnswer || "")
    .replace(/^(?:Short answer|What I found|Before you act)\s*:\s*/gim, "")
    .replace(/^[-*]\s+/gm, "");
  const answerText = normalizeFactText(answer);
...
function llmRewriteIssues(answer, draftAnswer, sources = [], residentQuestion = "") {
  const issues = [];
  if (!String(answer || "").trim()) return ["empty rewrite"];

  issues.push(...groundednessIssues(answer, sources, residentQuestion));
  issues.push(...locationScopeIssues(answer, sources, residentQuestion));
  issues.push(...attributeBindingIssues(answer, sources));
  ...
  issues.push(...unsupportedResourceAbsenceIssues(answer, sources, residentQuestion));

  const draftFacts = answerFactText(draftAnswer);
  const citedFacts = citedCorpus(sources);
  if (/does not grant permission/i.test(draftFacts)
    && /\b(?:can't|cannot|not allowed|prohibited)\b/i.test(answer)
    && !/\b(?:can't|cannot|not allowed|prohibited)\b/i.test(citedFacts)) {
    issues.push("unsupported prohibition replaced a limited-permission statement");
  }
