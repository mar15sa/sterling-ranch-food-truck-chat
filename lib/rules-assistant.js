...
function sourceClauseCandidates(query, sources = []) {
  const terms = expandQueryTerms(query);
  const clauses = [];
  for (const source of sources.filter((item) => item.questionSpecificExcerpt)) {
    const rawPieces = cleanText(source.excerpt || "")
      .replace(/\n/g, " ")
      .split(/(?<=[.!?])\s+|;\s+/)
    ...
    for (const piece of pieces) {
      ...
      clauses.push({
        text: piece,
        normalized,
        score: termScore + ruleScore + detailScore + rearLandscapeScore + requestedPlantScore + unrelatedTurfPenalty + namedTopicScore + ornamentConditionScore + courtConditionScore + crossReferencePenalty,
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const needsSourceDerivedFacts = summaryHasVolatileFacts(plainAnswer) || needsStructuredOverview || hasQuestionSpecificExcerpt;
