...
function isFoodTruckQuestion(question = "") {
  const text = String(question).toLowerCase();
  if (isFoodTruckBusinessQuestion(text)) return false;
...
function isFoodTruckRequest(interpretation = {}, question = "") {
  const text = `${question || ""} ${interpretation.subject || ""} ${(interpretation.searchQueries || []).join(" ")}`.toLowerCase();
  if (!/\bfood\s*trucks?\b/.test(text)) return false;
  if (isFoodTruckBusinessQuestion(text)) return false;
  return interpretation.intent === "events" || ["schedule", "status", "cost", "information"].some((goal) => (interpretation.goals || [interpretation.goal]).includes(goal));
}
...
function foodTruckAnswer(result = {}) {
  ...
  const menuEnrichmentDegraded = result.menuEnrichment?.status === "degraded";
  const sources = [{
  ...
  const answer = buildAnswerContract({
    ...
    actions,
    sources,
    status: menuEnrichmentDegraded ? "verified-incomplete" : "verified",
    checkedAt,
    answerMode: "community-live-food-truck",
    claims: [
  ...
  return {
    ...answer,
    menuEnrichment: result.menuEnrichment || { status: "complete", failures: [] },
    presentation: {
      kind: "food-truck",
      dateLabel: result.friendlyDate || "That date",
