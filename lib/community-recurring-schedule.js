...
function capturedProjectionCoversPastDate(source = {}, requestedDate = "", now = Date.now(), timeZone = "UTC", sourceCoversRequestedDate = false) {
  if (sourceCoversRequestedDate !== true
    || source.canonicalScopedProjection !== true
    || !/^\d{4}-\d{2}-\d{2}$/.test(requestedDate)) return false;
