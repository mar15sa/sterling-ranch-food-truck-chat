...
function requestedDuration(query) {
  ...
  const unit = match[2];
  return {
    hours: /^hour/.test(unit) ? amount : amount * 24,
...
function needsReadableTopicAnswer(query) {
  return (
    ...
    /\b(trampolines?|rv|rvs|recreational vehicle|motor home|motorhome|camper|trailer|basketball|dog run|clubhouse|pool rules?|landscape lighting|commercial vehicle|vegetable garden|raised (?:vegetable )?beds?)\b/i.test(
      String(query || "")
    )
  );
}
