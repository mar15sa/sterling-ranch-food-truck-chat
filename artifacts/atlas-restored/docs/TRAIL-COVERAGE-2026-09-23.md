...
Status: implemented as a separate local review at http://127.0.0.1:4195/?view=walks. Previous 4194 remains unchanged. Owner review pending; no staging or production deployment. This is not source approval for the Community Assistant.

## What changed

The old three-card walking selection omitted most mapped trail links. Walking art now reuses the owner-selected full landscape, with distinct Providence, Prospect and Ascent crops. A route-shape inset is deliberately separate from the painting, which is not georeferenced. Official CAB route geometry opens inside each guide. The main Walks filter includes all six documented access places, and Unfold previews one guide per village with all eight below.
