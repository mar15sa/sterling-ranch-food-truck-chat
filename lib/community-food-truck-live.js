...
function normalizedCalendarText(value = "") {
  return String(value)
    ...
    .replace(/<\/tr\s*>/gi, "\n")
    .replace(/<[^>]+>/g, " ")
    // Decode after removing markup so escaped text cannot become HTML. CivicPlus
    // editors serialize typographic punctuation as named or numeric entities.
    .replace(/&(?:#(x[\da-f]+|\d+)|([a-z]+));/gi, (entity, numeric, name) => {
      if (numeric) {
      ...
      return ({ nbsp: " ", thinsp: " ", amp: "&", quot: '"', apos: "'", lt: "<", gt: ">",
        ndash: "–", mdash: "—", lsquo: "‘", rsquo: "’", ldquo: "“", rdquo: "”" })[name.toLowerCase()] ?? entity;
    })
    .replace(/[\t\r ]+/g, " ")
    .split("\n")
    .map((line) => line.trim())
...
async function getCommunityFoodTruckSchedule(request = {}, { profile, fetchImpl = global.fetch, now = new Date(), timeoutMs = 20_000 } = {}) {
  ...
  const response = await fetchImpl(url, { headers: { accept: "text/html" }, signal: AbortSignal.timeout(timeoutMs) });
  if (!response.ok) throw new Error(`Official food-truck calendar returned ${response.status}.`);
  if (response.url && new URL(response.url).href !== url.href) throw new Error("Official food-truck calendar redirected to an unverified destination.");
