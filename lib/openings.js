...
function getOpeningsSourceStatus() {
  ...
  const sources = sourceConfig.sources.map((source) => {
    ...
    const manual = source.monitorMode === "manual";
    return {
      ...source,
      status: manual ? "manual-review" : result.status || "awaiting-first-scan",
      checkedAt: manual ? null : result.checkedAt || null,
      changedAt: result.changedAt || null,
      signalCount: Array.isArray(result.signals) ? result.signals.length : 0,
      error: manual ? null : result.error || null,
    };
  });
  ...
  return {
    ...
    manual: sources.filter((source) => source.monitorMode === "manual").length,
    automated: sources.filter((source) => source.monitorMode !== "manual").length,
...
function cleanTip(body) {
  ...
  return {
    ...
    sourceUrl: sourceUrl && !/^[a-z][a-z\d+.-]*:\/\//i.test(sourceUrl) ? `https://${sourceUrl}` : sourceUrl,
...
async function submitOpeningTip(body, fetchImpl = fetch) {
  ...
  const resendKey =
    process.env.OPENINGS_TIP_RESEND_API_KEY ||
    process.env.RESEND_API_KEY ||
    process.env.RULES_ALERT_RESEND_API_KEY ||
    "";
  const emailFrom = process.env.OPENINGS_TIP_EMAIL_FROM || process.env.RULES_ALERT_EMAIL_FROM || "";
  const emailTo = (process.env.OPENINGS_TIP_EMAIL_TO || process.env.RULES_ALERT_EMAIL_TO || "")
    .split(",")
    .map((value) => value.trim())
    .filter(Boolean);
  const detailLines = [
    `Opening tip: ${record.businessName}`,
    `Community: ${record.community || "Not provided"}`,
    `Location: ${record.location || "Not provided"}`,
    `Source: ${record.sourceUrl || "Not provided"}`,
  ...
  if (webhook) {
    const response = await fetchImpl(webhook, {
      ...
      body: JSON.stringify({
        ...record,
        subject: `Opening tip: ${record.businessName}`,
        text: detailLines.join("\n"),
      }),
      signal: AbortSignal.timeout(8000),
    });
    if (!response.ok) throw new Error(`Tip destination returned ${response.status}.`);
  } else if (resendKey && emailFrom && emailTo.length) {
    ...
    if (!response.ok) throw new Error(`Tip email returned ${response.status}.`);
  } else if (process.env.NODE_ENV === "production") {
    return {
      accepted: false,
      status: 503,
      message: "The tip inbox is temporarily unavailable. Please try again soon.",
    };
  } else {
