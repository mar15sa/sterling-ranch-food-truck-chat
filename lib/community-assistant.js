...
const { deterministicRequestedDetails, isWaterUsageAccessRequest, instructionalRequestShape, normalizeInterpretation, resolveInterpretationMode } = require("./community-interpretation");
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (shouldConsultRules) {
    ...
    if (rulesAnswer?.confidence?.canAnswer
      ...
      const bindingRule = originalQuestionNeedsRule(question);
