...
async function answerCommunityQuestion(query, options = {}) {
  if (options.requestContractMode === "shadow-route") {
    ...
    if (typeof options.answerResidentNeed !== "function") throw new Error("Need-first shadow routing requires an explicit per-need answer function.");
    ...
    const shadowRoute = await runNeedFirstShadow(currentAnswer._requestContract, options.answerResidentNeed, {
