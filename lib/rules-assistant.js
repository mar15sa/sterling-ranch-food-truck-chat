...
function buildPlainAnswer(query, results, index, sourcesForAnswer = []) {
  ...
  if (
    ...
    return helpfulAnswer(
      "Approval must be obtained from the DRC prior to any construction or landscaping.",
      topSources,
...
function seasonalLightingOverviewAnswer(query = "", sources = []) {
  ...
  if (permanentSystemQuestion && removalClause) {
    return structuredHelpfulAnswer(
      "Yes, a hardwired soffit/eave lighting system can stay installed year-round. Outside the approved seasonal periods, it must be returned to Section B's non-holiday settings.",
      [
...
function underEaveLightingOverviewAnswer(query = "", sources = []) {
  ...
  const lead = asksHolidayOnlyApplication
    ? `Yes—DRC approval is required to add permanent under-eave lighting. Holiday colors and patterns are limited to the approved seasonal periods. ${capitalizeFirstLetter(systemsClause.text)}`
...
function simpleTopicOverviewAnswer(query, sources = [], residentQuestion = query) {
  ...
  if (
    ...
    const placement = inlineText.match(/Furniture, electrical cords, bicycles, barbecues, toys, or other items of personal property[^.]+Owner's Lot[.;]/i)?.[0];
    const access = inlineText.match(/All roadways and walkways must be clear for emergency traffic\./i)?.[0];
    if (placement) {
      return structuredHelpfulAnswer(
        "Furniture and other items of personal property may not be stored, left, or parked on a roadway, walkway, or any place within the community other than your own lot.",
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (isLandscapeApplicationQuery(routingQuery)) {
    ...
    if (packet && submit) {
      const directAnswer = "Use the Landscape Submittal Packet 2026 for a landscaping or irrigation application.";
      const keyDetails = ["Open Submit a DRC Application when you are ready to start the review."];
      const answer = structuredHelpfulAnswer(
        directAnswer,
        keyDetails,
