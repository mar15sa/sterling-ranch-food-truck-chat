...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (isOfficialInformationPageRequest(question, routingPlan)) {
    ...
    if (resource) {
      ...
      const action = { label: `Open official ${resource.title}`, url: resource.sourceUrl, actionType: "information" };
      return {
        ...buildAnswerContract({
          directAnswer: resource.title,
          nextStep: action.label,
          actions: [action],
          ...
          claims: [{ text: resource.title, evidenceSourceIds: [displayedSource.id] }],
        }),
