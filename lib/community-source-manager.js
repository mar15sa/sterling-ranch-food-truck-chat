...
function communitySourceStatus(index = getBundledIndex(), now = Date.now(), { includeStaleSources = false } = {}) {
  ...
  return {
    ...
    releaseReady: expiredApprovedSourceCount === 0 && expiredApprovedFactCount === 0,
    ...
    stale: expiredApprovedSourceCount > 0,
...
async function refreshCommunitySources(options = {}) {
  ...
  refreshPromise = crawlCommunity(getCommunityProfile(), { previousIndex: getBundledIndex(), incrementalRefresh: true, ...options })
    .then(async (index) => {
