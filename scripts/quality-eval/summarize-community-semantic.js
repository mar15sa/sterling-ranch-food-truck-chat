...
const {projectionIdentity}=require('./community-semantic');
...
function analyze(directory){
 ...
 for(const [i,row] of rows.entries()){
  ...
  for(const method of methods){if(new Set(row[method].map(r=>r.id)).size!==row[method].length)throw Error('Duplicate ranked source');
   for(const r of row[method]){const d=docs.get(r.id);if(!d||r.sourceIdentity!==projectionIdentity(d)||r.text!==d.text||r.sourceUrl!==d.sourceUrl||hash(r.actions)!==hash(d.actions))throw Error('Changed returned source identity or content');}}
 }
 ...
 const report={status:'complete-development-retrieval-comparison',codeRevision:m.codeRevision,isTest:true,queries:rows.length,positiveAttempts:positive.length,negativeControlAttempts:negative.length,methods:{},repeatComparisons:[],negativeControls:[],cases:[],
