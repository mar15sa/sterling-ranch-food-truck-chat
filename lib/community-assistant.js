...
function bestContactCandidate(question, sources) {
  ...
  const contactBoilerplate = new Set([...GENERIC_PROJECT_TERMS, "who", "whom", "service", "services", "support", "team", "person", "reach"]);
  ...
  const candidates = sources.flatMap((source, sourceIndex) => (source.facts || [])
    ...
    .filter((fact) => !subjectTokens.length || subjectTokens.some(token =>
      `${source.title || ""} ${fact.context || ""}`.toLowerCase().includes(token)))
    .map((fact) => ({
      source,
      fact,
...
function missingContactAnswer(organization, routingPlan, options = {}) {
  ...
  const contactDirectory = configuredCommunityResource(options.index, options.profile,
    [...approvedResources, ...contactCandidates], options.question);
  const boundary = buildAnswerContract({
