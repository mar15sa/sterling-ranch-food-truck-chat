...
function goalCoverageIssues(plan = {}, draft = {}, sources = []) {
  ...
  const checks = {
    ...
    schedule: /\b(?:today|tomorrow|date|day|week|month|schedule|calendar|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday|January|February|March|April|May|June|July|August|September|October|November|December)\b|\b\d{1,2}(?::\d{2})?\s*[ap]\.?m\.?/i,
    status: /\b(?:open|closed|status|available|unavailable|currently)\b/i,
  };
  const issues = [];
  if (goal === "permission" && !hasPermissionStatement(answer)) issues.push("requested-goal-missing:permission");
