...
function evaluateRuleResult(check, result, options = {}) {
  ...
  if (check.expectedAnswerStatus && result.answerStatus !== check.expectedAnswerStatus) {
    issues.push(`expected answer status "${check.expectedAnswerStatus}", got "${result.answerStatus || ""}"`);
  }
  if (typeof check.expectedCanAnswer === "boolean" && result.confidence?.canAnswer !== check.expectedCanAnswer) {
    issues.push(`expected canAnswer ${check.expectedCanAnswer}, got ${result.confidence?.canAnswer}`);
  }
