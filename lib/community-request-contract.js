...
function claimSupportsDetail(detail, text = "", actions = []) {
  ...
  if (detail === "action") return actions.length > 0 || /\b(?:access|apply|bring|submit|book|continue|exchange|reserve|register|pay|open|call|email|use|visit(?:ing)?|select)\b/i.test(text);
