...
import {REQUIRED_RESIDENT_JOURNEYS,planningLayoutIssues} from './check.mjs';

const validPlanLayout=()=>({image:{visible:true,loaded:true,src:'http://test/assets/village-master-plan.jpg',x:0,y:0,width:300,height:300*2088/1296},pins:Array.from({length:5},(_,i)=>({x:30,y:30+i*70,width:44,height:44})),namedCardsOnMap:0,directoryNames:5,horizontalOverflow:false});
test('planning layout rejects missing geography, cropped source aspect and overlapping targets',()=>{
 assert.deepEqual(planningLayoutIssues(validPlanLayout()),[]);
 const blank=validPlanLayout();blank.image.loaded=false;assert.ok(planningLayoutIssues(blank).some(x=>x.includes('geographic')));
 const cropped=validPlanLayout();cropped.image.height=300;assert.ok(planningLayoutIssues(cropped).some(x=>x.includes('aspect')));
 const overlap=validPlanLayout();overlap.pins[1].y=35;assert.ok(planningLayoutIssues(overlap).includes('overlapping markers'));
 const cards=validPlanLayout();cards.namedCardsOnMap=5;assert.ok(planningLayoutIssues(cards).some(x=>x.includes('compact')));
});
...
test('complete plan rejects missing phone journeys, missing measurements and small touch targets',()=>withFixture(({root,review})=>{
 writeFileSync(path.join(root,'connections.css'),'/* connection experience */');
 review.runtimeFingerprint=runtimeFingerprint(root).sha256;
 const save=()=>writeFileSync(path.join(root,'eval/visual-review.json'),JSON.stringify(review));
 save();assert.equal(evaluate({root}).ok,false);
 const states=(device='desktop')=>REQUIRED_RESIDENT_JOURNEYS.map(name=>{const bytes=device+name,p='eval/evidence/'+device+'-'+name+'.png';writeFileSync(path.join(root,p),bytes);return {name,passed:true,actualJudgment:true,notes:'Fixture journey verified',candidate:{path:p,sha256:hash(bytes)}};});
 const m={phoneInitialMapTop:300,phoneWalkMapTop:250,phoneNeighborhoodMapTop:290,largeTextDesktop:{base:'20',size:'40px',overflow:false,animation:'none',transition:'0s'},largeTextPhone:{base:'20',size:'40px',overflow:false},reducedMotionPhone:{animation:'none'},desktopTargets:[{width:44,height:44}],phoneTargets:[{width:44,height:44}],desktopNestedReturn:{focused:'atlas-coffee'},phoneNestedReturn:{focused:'atlas-coffee'}};
 review.completePlanChecks={passed:true,desktop:states(),phone:states('phone')};
 const measure=()=>{const bytes=JSON.stringify(m);writeFileSync(path.join(root,'eval/evidence/browser.json'),bytes);review.completePlanChecks.browserEvidence={path:'eval/evidence/browser.json',sha256:hash(bytes)};save();};
 measure();assert.equal(evaluate({root}).ok,true);
 review.completePlanChecks.phone.pop();save();assert.equal(evaluate({root}).ok,false);
 review.completePlanChecks.phone=states('phone');delete review.completePlanChecks.phone[0].actualJudgment;save();assert.equal(evaluate({root}).ok,false);
 review.completePlanChecks.phone=states('phone');review.completePlanChecks.phone[0].candidate=review.completePlanChecks.desktop[0].candidate;save();assert.equal(evaluate({root}).ok,false);
 review.completePlanChecks.phone=states('phone');delete m.phoneInitialMapTop;measure();assert.equal(evaluate({root}).ok,false);
 m.phoneInitialMapTop=300;m.phoneTargets[0].height=30;measure();assert.equal(evaluate({root}).ok,false);
}));
test('resident journey evidence cannot be replaced by a screenshot-only pass',()=>withFixture(({root,review})=>{
 writeFileSync(path.join(root,'journeys.mjs'),'export const resident=true;');
 review.runtimeFingerprint=runtimeFingerprint(root).sha256;
 const save=()=>writeFileSync(path.join(root,'eval/visual-review.json'),JSON.stringify(review));save();
 assert.equal(evaluate({root}).ok,false);
 review.residentChecks={passed:true,states:REQUIRED_RESIDENT_JOURNEYS.map(name=>({name,passed:true,actualJudgment:true,notes:'Fixture task completed',candidate:review.states[0].candidate}))};save();assert.equal(evaluate({root}).ok,true);
 review.residentChecks.states[0].passed=false;save();assert.equal(evaluate({root}).ok,false);
}));
...
test('future village UI cannot reuse project-only screenshot evidence',()=>withFixture(({root,review})=>{
 writeFileSync(path.join(root,'village-plans-ui.js'),'export const villages=true;');
 review.runtimeFingerprint=runtimeFingerprint(root).sha256;
 writeFileSync(path.join(root,'eval/visual-review.json'),JSON.stringify(review));
 assert.equal(evaluate({root}).ok,false);
 review.villagePlanChecks={passed:true,layouts:['desktop','compact','phone'].map(name=>({...validPlanLayout(),name})),states:['village-plan-desktop','village-plan-mobile','village-plan-selected'].map(name=>{
  const rel='eval/evidence/'+name+'.png',bytes='fixture '+name;writeFileSync(path.join(root,rel),bytes);
  return {name,candidate:{path:rel,sha256:hash(bytes)},actualJudgment:true,passed:true,notes:'Fixture judgment'};
 })};
 writeFileSync(path.join(root,'eval/visual-review.json'),JSON.stringify(review));
 assert.equal(evaluate({root}).ok,true);
 writeFileSync(path.join(root,review.villagePlanChecks.states[0].candidate.path),'changed');
 assert.equal(evaluate({root}).ok,false);
}));
