...
function isFeeQuery(query) {
  return /\b(fee|fees|charge|charges|cost|assessment|payment|rate|rates)\b/i.test(query);
}
...
function helpfulAnswer(shortAnswer, sources, nextStep = "") {
  ...
  if (nextStep) {
    lines.push("", `Before you act: ${nextStep}`);
  }
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const answerSources = sources.length ? sources : fallbackSources;
  ...
  return {
    answer:
      (await rewriteAnswerWithLLM(question, answerSources)) ||
      buildPlainAnswer(question, results, index, answerSources),
    confidence,
