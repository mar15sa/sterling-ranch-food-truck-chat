...
const { deterministicRequestedDetails, highConfidenceDateRange, isOfficialInformationPageRequest, isReportedFindingsRequest, isWaterUsageAccessRequest, instructionalRequestShape, normalizeInterpretation, resolveInterpretationMode } = require("./community-interpretation");
...
async function answerCommunityQuestionCore(query, options = {}) {
  ...
  if (isOfficialInformationPageRequest(question, routingPlan)) {
    ...
    const subjectText = question.split(/\s+/).filter(word => !navigationTerms.has(word.toLowerCase().replace(/[^a-z]/g, ""))).join(" ");
    ...
    return buildAnswerContract({
      directAnswer: "I couldn’t confirm a current approved information page for that topic.",
