...
const { hasPermissionStatement } = require("./resident-permission-language");
...
const GENERIC_INCOMPLETE_REQUEST = /^(?:please\s+)?(?:help(?:\s+me)?|more info(?:rmation)?|details|tell me more|what about (?:that|this|it)|(?:community\s+)?(?:services?|utilities?))$/i;
...
function buildResidentRequestContract(question = "", routingPlan = null, requestContext = {}) {
  ...
  const needs = clauses.map((text, index) => {
    ...
    const independentClauseTokens = [...distinctiveTokens(text)].filter((token) => ![
      "action", "application", "apply", "correct", "date", "form", "hour", "information", "menu", "permission", "price", "regular", "report", "status",
    ].includes(token));
    ...
    if (/\bwhen\b.{0,50}\b(?:bring|return|put)\b.{0,45}\b(?:bins?|cans?|carts?|containers?)\b.{0,20}\b(?:back|in)\b/i.test(evidenceFocus)) {
      // Residents put the action before the object as often as after it:
      // “when do I bring my recycling cans back in?” is the same operational
      // deadline as “when should the cans be brought back in?”.
      explicitDetails = ["hours"];
    }
    ...
    if (explicitDetails.includes("contact")) {
      // “What email can I contact?” asks for a contact channel. The modal
      // phrase “can I” must not add a separate permission decision.
      explicitDetails = explicitDetails.filter((detail) => !["examples", "permission"].includes(detail));
    }
    ...
    if (isResidentSharingFixtureQuery(evidenceFocus)) {
      // A terse fixture name can look like a request to "book" a facility.
      // Its residential placement context is instead a permission question.
      explicitDetails = ["permission"];
    }
...
function normalizedToken(value = "") {
  ...
  if (/^(?:decor|decorat(?:e|ed|es|ing|ion|ions|ons)|displays?)$/.test(token)) return "decoration";
  ...
  if (/^(?:apply|applies|applied|applying|application|applications|document|documents|form|forms|paperwork|sheet|sheets|submittal|submittals|submission|submissions)$/.test(token)) return "application";
...
function distinctiveTokens(value = "") {
  ...
  if (/\b(?:seasonal|holiday|hallowe+en|hallowen|christmas|xmas|thanksgiving|hanukkah|easter|valentine(?:'s)?)\b/i.test(text)
    && /\b(?:lights?|lighting)\b/i.test(text)) result.add("seasonal-lighting");
  if (/\bsheds?\b|\baccessory buildings?\b/i.test(text)) result.add("accessory-building");
  if (/\b(?:wi[-\s]?fi|wireless|internet|fiber)\b/i.test(text)) result.add("internet-service");
  if (/\b(?:set\s*up|setup|start|activate|activation|connect|get\s+(?:online|connected))\b/i.test(text)) result.add("service-setup");
...
function claimSupportsDetail(detail, text = "", actions = []) {
  ...
  if (detail === "permission") return hasPermissionStatement(text);
  ...
  if (detail === "action") {
    ...
    return /^(?:(?:please|to)\s+)?(?:access|apply|ask|book|bring|call|check|connect|contact|continue|download|email|exchange|log in|open|pay|register|reserve|select|sign in|submit|use|visit)\b/i.test(directive)
      || /\b(?:you|residents?|customers?|applicants?)\s+(?:can|may|must|should|need to)\s+(?:access|apply|ask|book|bring|call|check|connect|contact|continue|download|email|exchange|log in|open|pay|register|reserve|select|sign in|submit|use|visit)\b/i.test(directive)
...
function assessResidentNeeds(contract, answer = {}) {
  ...
  const needs = contract.needs.map((need) => {
    ...
    const relevantActions = actions.filter((action) => action.sourceId && candidateSourceIds.includes(action.sourceId)
      ...
      || enoughSemanticOverlap(needTokens, `${action.label || ""} ${action.context || ""} ${action.evidence?.label || ""} ${action.evidence?.context || ""}`));
    ...
    const relevantClaimEntries = verifiedClaimEntries.filter(({ claim }) => {
      ...
      const explicitlyRoutedUnlistedImprovement = isResidentSharingFixtureQuery(need.request)
        && /(?:all improvements must be submitted|any contemplated improvement not\s+listed here)/i.test(String(claim.text || ""))
        && mappedCandidates.some((source) => (source.retrievedForNeedIds || []).includes(need.id)
          && /any contemplated improvement not\s+listed here/i.test(sourceEvidenceText(source, claims, actions)));
      return mappedCandidates.length > 0 && (mappedSubjectMatch || explicitlySupportedNeed || explicitlyRoutedVerifiedClaim
        ...
        || explicitlyRoutedUnlistedImprovement
    ...
    const relevantConflicts = conflicts.filter((conflict) => {
      ...
      if (detail === "permission" && /\b(?:all night|overnight|past midnight)\b/i.test(needText)
        && !/\b(?:overnight|nights?|24\/7)\b|\b\d{1,2}(?::\d{2})?\s*[ap]\.?m\.?|\b(?:\d+|one|two|three)\s+(?:consecutive\s+)?(?:hours?|days?)\b/i.test(claimText)) return false;
