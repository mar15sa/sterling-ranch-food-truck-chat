...
function isWaterUsageAccessRequest(question = "") {
  ...
  const namesWaterUsage = /\bwater\s+(?:usage|use|consu(?:m)?ption)\b|\b(?:usage|consu(?:m)?ption)\b.{0,24}\bwater\b/.test(text);
  const asksForOnlineAccess = /\b(?:internet|online|web|website|portal|dashboard|account|app|access|log ?in|login|sign ?in|register|registration|sign ?up|view|see|check|track|monitor|thresholds?|alerts?|notifications?|notify|notified)\b/.test(text);
...
function deterministicRequestedDetails(question = "") {
  const text = String(question).toLowerCase();
  if (isInternetServiceSetupRequest(text)) return ["action"];
...
// This recognizes only the resident's goal. It never supplies a provider,
// credential, URL, or setup instruction; those must come from current,
// approved source evidence.
function isInternetServiceSetupRequest(question = "") {
  const text = String(question).toLowerCase();
  if (/\batlas(?:\s+coffee)?\b/.test(text)) return false;
  const internetSubject = /\b(?:wi[-\s]?fi|wireless|internet|fiber)\b/.test(text);
  const setupGoal = /\b(?:set\s*up|setup|start|activate|activation|connect|get\s+(?:online|connected))\b/.test(text);
  const homeContext = /\b(?:move(?:d|s|ing)?\s+in|moving|new\s+(?:home|house)|home\s+internet|internet\s+service|closing|resident)\b/.test(text);
  return internetSubject && setupGoal && homeContext;
}
...
function isOfficialInformationPageRequest(question = "", routingPlan = null) {
  ...
  const internetSetup = isInternetServiceSetupRequest(question);
  return (asksToNavigate || internetSetup) && (internetSetup || !asksForDifferentFacet)
