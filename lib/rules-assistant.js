...
function trampolineOverviewAnswer(sources = []) {
  const clauses = sourceClauseCandidates("trampoline approval property lines plant material", sources, { questionSpecificOnly: false });
  const approval = clauses.find((entry) => /Trampolines\.\s*DRC approval is required/i.test(entry.text));
  const setback = clauses.find((entry) => /minimum of five feet from all property lines/i.test(entry.text));
  const screening = clauses.find((entry) => /Tall plant material is required between the trampoline and all property lines/i.test(entry.text));
  if (!approval) return sourceClauseOverviewAnswer("trampoline", sources);
  return structuredHelpfulAnswer(
    `Trampolines require DRC approval: ${capitalizeFirstLetter(approval.text)}`,
    [setback?.text, screening?.text].filter(Boolean),
    "Open the linked official section if you need the complete wording."
  );
}
...
function flagAndPoliticalSignOverviewAnswer(query, sources = []) {
  ...
  const politicalClause = clauses.find((entry) => /political signs promoting or opposing a candidate for office or a ballot issue/i.test(entry.text));
  const politicalSizeClause = clauses.find((entry) => /no such sign shall exceed 36 inches by 48 inches/i.test(entry.text));
  ...
  if (/\bpolitical sign/i.test(query) && politicalClause) {
    return structuredHelpfulAnswer(
      `Allowed: the cited rule covers a political sign for a candidate or ballot issue. ${capitalizeFirstLetter(politicalClause.text)}`,
      [politicalSizeClause?.text].filter(Boolean),
      "Open the linked official section if you need the complete wording."
    );
  }
  if (allowedFrontFlags) {
    return structuredHelpfulAnswer(
      capitalizeFirstLetter(allowedFrontFlags.text),
      [poleClause ? `A flagpole requires DRC approval: ${capitalizeFirstLetter(poleClause.text)}` : "", sizeClause?.text, commercialClause?.text].filter(Boolean),
      "Open the linked official section if you need the complete wording."
    );
  }
...
function petKeepingOverviewAnswer(query, sources = []) {
  ...
  if (isPoultryQuery(query)) {
    const animal = /\bchicken/i.test(query) ? "Backyard chickens are not allowed" : "You can't keep poultry";
    return structuredHelpfulAnswer(
      `${animal} under the cited rule: ${capitalizeFirstLetter(keepingClause)}`,
      [nuisanceClause].filter(Boolean),
      "Open the linked official section if you need the complete wording."
    );
  }
  return structuredHelpfulAnswer(
    `Allowed under the cited domestic-animal exception: ${capitalizeFirstLetter(keepingClause)}`,
...
function greenhouseOverviewAnswer(sources = []) {
  ...
  return structuredHelpfulAnswer(
    `Greenhouses require DRC approval: ${capitalizeFirstLetter(greenhouse)} The cited cross-reference applies the accessory-building rule.`,
    [],
    "Open the linked official section if you need the complete wording."
  );
}
...
function simpleTopicOverviewAnswer(query, sources = [], residentQuestion = query) {
  ...
  if (
    ...
    const entitlement = text.match(/Each Qualified Residence is allowed one Annual Pass per year\./i)?.[0];
    const voucher = text.match(/The Voucher must be brought to the Sterling Ranch Information Center and exchanged for an Annual Pass\./i)?.[0];
    const renewal = text.match(/Owners of Qualified Residences may submit an Application for a new Annual Pass upon expiration of the initial or immediately preceding Annual Pass\./i)?.[0];
    ...
    const lead = asksReimbursement
      ? `The selected Parks Pass Program source does not describe reimbursing a separately purchased pass. ${entitlement}`
      : asksHowToGet
        ? `To get a Colorado Parks and Wildlife pass, follow the cited voucher or application process. ${voucher || renewal}`
        : `One annual Colorado Parks and Wildlife pass is available under the cited program. ${entitlement}`;
    return structuredHelpfulAnswer(
      lead,
      [voucher, renewal].filter((item, index, all) => item && !lead.includes(item) && all.indexOf(item) === index),
      "Open the linked official section if you need the complete wording."
    );
  }
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (/\b(?:atlas coffee wifi|atlas wifi)\b/i.test(question)) {
    const requestedService = "Atlas Coffee Wi-Fi";
    const resource = officialResourceSource({
      id: "cab-website",
      title: "Official Sterling Ranch CAB website",
      url: CAB_SITE_URL,
      excerpt: `Requested topic: ${requestedService}. Official CAB website for current resident information.`,
    });
    return {
      answer: structuredHelpfulAnswer(
