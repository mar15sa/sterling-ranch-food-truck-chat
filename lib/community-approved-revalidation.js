...
async function observeCanonicalSource(sourceUrl, approvedSources, { fetchImpl = globalThis.fetch, extractPdfTextImpl = extractPdfText, timeoutMs = 30_000, fetchAttempts = 2, retryDelayMs = 250, sleepImpl } = {}) {
...
async function revalidateApprovedEvidence(index, { now = Date.now(), fetchObservedHashes, staleAfterMs = 86_400_000, concurrency = 2 } = {}) {
  ...
  const check = async (sourceUrl) => {
    ...
    try {
      ...
      return reviewRecord(sourceUrl, approvedSources, "review-required", {
        reason: "fetch-or-extraction-failed",
        failureKind: transientOfficialFailure(error) ? "transient-official-unavailable" : "verification-error",
        error: error.message,
        checkedAt,
        staleAfter,
      });
...
module.exports = { actionDigest, actionIdentity, actionUrlIdentity, applyDomainOutageGrace, canonicalizeEvidence, civicPlusDocumentId, fetchOfficialHtmlWithRetry, hasApprovedProjection, isApprovedSource, isRevalidatableApprovedSource, isScopedExactVersionProjection, observeCanonicalSource, projectedActionProof, renewExactApprovedEvidence, revalidateApprovedEvidence, sameOfficialDocumentIdentity, selectRevalidationTargetUrls, sourceEvidenceIdentity, sourceHash, stableObservedActions, transientOfficialFailure, unchangedBaselineSources, versionHash };
