...
function assessAnswerConfidence(query, results, sources) {
  ...
  if (
    ...
    (isChickenQuery(query) && sources.slice(0, 3).some((source) => /1-33|pets and livestock/i.test(source.title || ""))) ||
...
function buildPlainAnswer(query, results, index, sourcesForAnswer = []) {
  ...
  if (isChickenQuery(query) && topSources.some((source) => /1-33|pets and livestock/i.test(source.title || ""))) {
    return helpfulAnswer(
