...
function approvedWasteHolidayContext(question, index = {}) {
  ...
  const fact = (source?.facts || []).find((item) => item.id === "trash-holiday-delay"
    && item.reviewStatus === "approved"
    && item.approvalClaim === "trash-recurring-service-guidance");
...
async function answerCommunityQuestion(query, options = {}) {
  if (["shadow-route", "need-first-candidate", "need-audited-candidate"].includes(options.requestContractMode)) {
    ...
    const answerResidentNeed = options.answerResidentNeed || (async (need) => {
      ...
      const fixedIntent = (/\bcaregiver pass\b/i.test(needRouteText)
        ...
        ? "forms"
        : /\b(?:trash|garbage|recycl(?:e|ing)|waste|water|sewer|utility|utilities|billing|bill)\b/i.test(needRouteText)
        ? "services"
    ...
    if (candidateMode) {
      ...
      if (auditedMode) {
        ...
        const baselineSupportedNeeds = baselineAssessment.needs.filter((need) => need.status === "supported").length;
        ...
        const safelyHandledBoundary = baselineAssessment.outcome === "handled-boundary";
        const simpleRequest = isSimpleResidentRequest(query, contract);
        const preserveBaseline = safelyHandledBoundary || (simpleRequest
          ...
          return {
            ...
            _requestContract: {
              ...
              candidate: {
                ...
                reason: safelyHandledBoundary
                  ? "baseline-safely-handles-boundary"
                  : baselineAssessment.outcome === "complete"
                  ? "baseline-fully-covers-resident-needs"
                  : baselineSupportedNeeds > 0
                      ? "baseline-useful-partial-preserved"
                      : "verified-baseline-preserved-despite-audit-gap",
              },
            },
          };
        }
      ...
      return {
        ...
        _requestContract: {
          ...
          candidate: {
            ...
            reason: auditedMode ? "qualified-or-unanswered-request-uses-need-first" : "need-first-candidate",
          },
        },
      };
    }
