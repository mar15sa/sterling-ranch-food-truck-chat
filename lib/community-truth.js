...
function inferScopeKey(fact = {}, source = {}) {
  ...
  if (fact.type === "time") {
    ...
    if (position >= 0 && context.indexOf(value, position + value.length) < 0) {
      ...
      const dayLabels = [...prefix.matchAll(/\b(?:mon(?:day)?\s*(?:[-–]|to)\s*fri(?:day)?|weekdays?|weekends?|saturday|sunday|daily)\b/g)];
      const label = dayLabels.at(-1);
      const day = label && (/mon|weekday/.test(label[0]) ? "weekday" : /weekend/.test(label[0]) ? "weekend" : slug(label[0]));
