...
function deterministicRequestedDetails(question = "") {
  ...
  const action = /\b(?:how do|where (?:do|can)|book|reserve|rent|submit|register|sign up|form|pay|payment|settle|settling|log in|login|download)\b/.test(text)
  ...
  if (/\b(?:permission|approval|need to)\b/.test(text)
    ...
    || (!specificationModal && !operationalChoiceModal && /\b(?:can|may) (?:i|we)\b/.test(text) && !/\b(?:where|how|when|what)\s+(?:can|may) (?:i|we)\b/.test(text))) details.push("permission");
...
function normalizeInterpretation(plan = {}, question = "", options = {}) {
  ...
  if (deterministicRequestedDetails(question).includes("specification")) detailSet.add("specification");
  if (deterministicRequestedDetails(question).includes("methods")) detailSet.add("methods");
