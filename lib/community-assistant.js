...
async function answerCommunityQuestion(query, options = {}) {
  ...
  if ((asksForAction || looksLikeCompleteQuestion) && options.planCommunitySearch !== false) {
    const planner = options.planCommunitySearch || defaultPlanSearch;
    searchPlan = await planner(question, options.llmOptions || {});
    routingPlan = normalizedRoutingPlan(searchPlan, question);
  }
  ...
  if (needsExactRecurringDate(question) && options.getWasteSchedule) {
    try {
  ...
  if (withheld && Number(withheld.score || 0) >= Number(operationalSearch.sources[0]?.score || 0)) {
    const unavailable = buildAnswerContract({
      ...
      actions: [{ label: `Open ${withheld.title || "the controlling official source"}`, url: withheld.sourceUrl, actionType: "information" }],
      sources: [sourceForDisplay(withheld)],
      status: "source-unavailable",
  ...
  let result = searchCommunityIndex(question, { index: options.index, indexPath: options.indexPath, communityId: options.communityId, intent: routingPlan?.intent || intent });
