...
function normalizedRoutingPlan(plan = {}, question = "", options = {}) {
  ...
  const hasValidCompoundGoal = modelGoal && modelGoals.length > 1 && modelGoals.includes(modelGoal);
  const goal = hasValidCompoundGoal ? modelGoal : explicitGoalFromQuestion(question, proposedIntent) || modelGoal;
  const fixedIntent = {
    permission: "rules",
    payment: "services",
    booking: "facilities",
  ...
  const intent = fixedIntent || (goal === "schedule"
    ? (proposedIntent === "events" || /\b(?:events?|classes?|activities|programs?|meetings?|calendar)\b/i.test(question) ? "events" : "services")
  ...
  const details = normalized.requestedDetails;
