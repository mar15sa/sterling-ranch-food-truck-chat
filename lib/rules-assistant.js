...
function landscapeScreenOverviewAnswer(sources = []) {
  ...
  const lead = approval.replace(/^(.+?)\.\s*(DRC approval is required\.)$/i, "$1: $2");
  const size = [height?.replace(/\bWe encourage\b/i, "The rule encourages").replace(/\band, therefore, will allow\b/i, "and allows"), width]
    .filter(Boolean)
    .join(" ");
  ...
  const materials = [transparency, material?.replace(/\bVinyl is not permitted\b/i, "Vinyl is not allowed"), finish]
    .filter(Boolean)
    .join(" ");
  return structuredHelpfulAnswer(
...
async function answerRulesQuestion(query, options = {}) {
  ...
  if (!answerSources.length || topScore < 2 || !confidence.canAnswer) {
    ...
    const boundarySources = unsupportedCommunityTopic ? [] : answerSources.slice(0, 3);
    return {
