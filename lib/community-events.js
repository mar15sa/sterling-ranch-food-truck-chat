...
function legacyEventTerms(question = "") {
  const ignored = new Set(["what", "events", "event", "are", "is", "the", "next", "happening", "this", "weekend", "today", "tomorrow", "upcoming", "when", "where", "calendar", "community", "class", "classes"]);
  return (String(question).toLowerCase().match(/[a-z0-9']+/g) || []).filter((term) => term.length > 2 && !ignored.has(term));
}

function eventMatchesFilter(event, filter) {
  const isVenue = filter.field === "location" || filter.field === "facility";
  const corpus = isVenue
    ? event.location
    : `${event.title || ""} ${event.category || ""}`;
  // Audience filters historically contain alternative labels (e.g. youth kids).
  if (!isVenue && filter.field !== "category") {
    const terms = (String(filter.value).toLowerCase().match(/[a-z0-9']+/g) || []).filter((term) => term.length > 2);
    return terms.some((term) => `${corpus} ${event.location || ""}`.toLowerCase().includes(term));
  }
  const words = (value) => String(value || "").normalize("NFKC").toLowerCase().match(/[\p{L}\p{N}]+/gu) || [];
  const canonical = (word) => isVenue ? word : word === "classes" ? "class" : word === "events" ? "event" : word;
  const corpusWords = new Set(words(corpus).map(canonical));
  const functionWords = new Set(["the", "a", "an", "at", "in", "of", "for", "and"]);
  const descriptors = new Set(["class", "event"]);
  return String(filter.value || "").split(/\bor\b/i).some((alternative) => {
    let terms = words(alternative).map(canonical).filter((word) => isVenue ? word !== "the" : !functionWords.has(word));
    if (!isVenue && terms.some((word) => !descriptors.has(word))) {
      terms = terms.filter((word) => !descriptors.has(word));
    }
    return terms.length > 0 && terms.every((term) => corpusWords.has(term));
  });
}
...
function resultFromCalendarData(data, request, range, { degraded = null, reason = null, limit = 8 } = {}) {
  ...
  const envelope = calendarEnvelope(data.adapter, {
    events: filtered.length ? filtered.slice(0, limit) : alternatives,
...
async function getCommunityEventsForDay(request, options = {}) {
  ...
  if (cached && now.getTime() - new Date(cached.checkedAt).getTime() < refreshMs) {
    return resultFromCalendarData(cached, request, range, { limit: options.limit });
  }
  ...
  try {
    ...
    return resultFromCalendarData(data, request, range, { limit: options.limit });
  } catch (error) {
...
async function getCommunityEvents(request, options = {}) {
  ...
  const now = options.now instanceof Date ? options.now : new Date(options.now || Date.now());
  const range = requestRange(request, now, timeZone);
  if (range.start === range.end) return getCommunityEventsForDay(request, options);

  const dates = [];
  for (let date = range.start; date <= range.end; date = addDays(date, 1)) {
    dates.push(date);
    if (dates.length > 31) throw new Error("Official calendar requests are limited to 31 days.");
  }
  // CivicPlus list responses are day-scoped even when the request describes a
  // longer range. Fetch every requested day, then rebuild one range-level
  // evidence envelope so the assistant cannot mistake day one for the week.
  const legacyTerms = typeof request === "string" ? legacyEventTerms(request) : [];
  const dailyRequest = request && typeof request === "object"
    ? request
    : { filters: legacyTerms.length ? { category: legacyTerms.join(" ") } : {} };
  const results = await Promise.all(dates.map((date) => getCommunityEventsForDay({
    ...dailyRequest,
    dateRange: { start: date, end: date, label: range.label },
  }, options)));
  const unique = (items) => [...new Map(items.map((item) => [`${item.startDate}:${item.id || item.url}:${item.title}`, item])).values()];
  const events = unique(results.flatMap((result) => result.events || [])).sort((a, b) => a.startDate.localeCompare(b.startDate)).slice(0, options.limit ?? 8);
  const alternatives = events.length ? [] : unique(results.flatMap((result) => result.alternatives || [])).sort((a, b) => a.startDate.localeCompare(b.startDate)).slice(0, 8);
  // A range is only as current and healthy as every day it contains. Do not
  // let a successful refresh renew a retained day whose refresh failed.
  const checkedAt = results.map((result) => result.checkedAt).filter(Boolean).sort()[0] || now.toISOString();
  const staleAfter = results.map((result) => result.evidenceEnvelope?.evidence?.[0]?.staleAfter).filter(Boolean).sort()[0];
  const incompleteDays = results.filter((result) =>
    (result.diagnostics?.sourceOutcome && result.diagnostics.sourceOutcome !== "ok")
    || (result.evidenceEnvelope?.degradation?.state && result.evidenceEnvelope.degradation.state !== "healthy")
  );
  const degradation = incompleteDays.length
    ? { state: "degraded", reason: "At least one requested calendar day could not be refreshed." }
    : { state: "healthy" };
  const sourceUrl = results[0]?.sourceUrl || configuredCalendarUrl(adapter, options);
  const filters = appliedEventFilters(dailyRequest);
  return {
    events,
    alternatives,
    range,
    sourceUrl,
    calendarLabel: results[0]?.calendarLabel,
    calendarActionLabel: results[0]?.calendarActionLabel,
    checkedAt,
    evidenceEnvelope: calendarEnvelope(adapter, {
      events: events.length ? events : alternatives,
      sourceUrl,
      checkedAt,
      staleAfter,
      degradation,
    ...
    diagnostics: {
      sourceOutcome: incompleteDays.length ? "partial" : "ok",
      parserHealthy: results.every((result) => result.diagnostics?.parserHealthy !== false),
      uniqueEventMarkers: results.reduce((sum, result) => sum + Number(result.diagnostics?.uniqueEventMarkers || 0), 0),
      structuredDateMarkers: results.reduce((sum, result) => sum + Number(result.diagnostics?.structuredDateMarkers || 0), 0),
      parsedCount: results.reduce((sum, result) => sum + Number(result.diagnostics?.parsedCount || 0), 0),
      beforeFilterCount: results.reduce((sum, result) => sum + Number(result.diagnostics?.beforeFilterCount || 0), 0),
