...
const PLANNER_PROMPT = [
  ...
  "intent: one of design_review, enforcement, facility_reservation, fees, landscaping, lighting, parking, parks, pets, rental_cancellation, utilities, other",
...
function buildRetrievalQueries(question, plan) {
  ...
  const enrichedQuery = uniqueStrings(
    [plan?.normalizedQuestion, ...(plan?.searchQueries || []).slice(0, 2), plan?.entities?.join(" "), intentTerms],
    5,
    180
  ).join(" ");
  return uniqueStrings([enrichedQuery || question], 1, 700);
}

function buildRoutingQuery(question, plan) {
  return uniqueStrings(
    [question, plan?.normalizedQuestion, ...(plan?.searchQueries || []), plan?.entities?.join(" "), INTENT_SEARCH_TERMS[plan?.intent] || ""],
    8,
    300
  ).join(" ");
}
