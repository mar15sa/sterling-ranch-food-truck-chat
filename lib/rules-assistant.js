...
async function answerRulesQuestion(query, options = {}) {
  ...
  const groundedSpecialAnswer = directSpecialSources.length && !isDefensiveSprayQuery(routingQuery)
  ...
  const classifiedProjectAnswer = projectEvidence.applies && !projectEvidence.supported
    ? sourceDerivedAnswerParts(routingQuery, answerSources, "", question).answer
    : "";
