...
export function createDiscovery({ places, roots, trails, models, located, selected, select, showRoute, setView, routeSvg, focusVillage, focusState, point, onChange }) {
  ...
  const validLens = value => lenses.some(([id]) => id === value);
  ...
  function chooseVillage(next, notify=true, scroll=true) {
    ...
    if (scroll) $('#landscape').scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'});
    if (notify) changed();
  }
  ...
  function renderMarkers() {
    ...
    for (const b of document.querySelectorAll('.map-label')) {
      ...
      b.classList.toggle('selected', p.id === selected());
  ...
  function renderSheets() {
    ...
    if(!unfolded)return;
    sheets.innerHTML=unfoldedScene({places,trails,models,routeSvg,roots,located,point,village,lens,layer});
  }
  function render() {
    ...
    stage.dataset.lens=lens;stage.dataset.village=village; stage.dataset.layer=layer; syncDirectoryArea(); renderMarkers(); renderCard(byId.get(selected())); renderDiscovery(); renderDestinationStrip(); renderSheets();
    $('#place-note').hidden=unfolded;
    if(document.body.dataset.view==='neighborhood'){
      const plansOnly=unfolded&&layer==='future';if(plansOnly)$('#map-match-count').textContent=futureGroups(places).filter(p=>village==='all'||inVillage(p,village)).length+' future projects';$('.more-to-explore').hidden=plansOnly;$('#all-places').hidden=plansOnly;
  ...
  toolbar.addEventListener('click',e=>{const b=e.target.closest('button');if(b?.id==='unfold-map'){unfolded=!unfolded;render();if(unfolded)stage.scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion:reduce)').matches?'auto':'smooth'});changed();}});
