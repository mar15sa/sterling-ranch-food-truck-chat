// Scope contacts only when a complete company/phone row is available.
...
function directoryContactSubjects(sources = []) {
  ...
  for (const [url, chunks] of documents) {
    ...
    for (const match of text.matchAll(pattern)) {
      ...
      const footer = prefix.lastIndexOf('for your home.');
      if (footer >= 0) prefix = prefix.slice(footer + 'for your home.'.length).trim();
      const email = prefix.match(/([A-Z0-9._%+&'-]+@[A-Z0-9.-]+\.[A-Z]{2,})$/i)?.[1];
      if (email) prefix = prefix.slice(0, -email.length).trim();
      ...
      for (const [type, value] of [['phone', match[0]], ...(email ? [['email', email]] : [])]) {
