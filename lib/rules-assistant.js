...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "", residentQuestion = query) {
  ...
  // Volatile facts need the source-derived evidence path instead of a general
  // clause that could keep a short, non-factual display excerpt.
  const sourceAnswer = (isFeeQuery(query) || summaryHasVolatileFacts(originalAnswer))
    ? ""
