...
The replacement candidate is baseline-preserving. It builds the resident-needs contract, audits the existing grounded answer, and keeps that answer whenever it is verified, safely handles a boundary, or contains verified support for any requested need. The per-need fallback may become resident-facing only when the existing answer has no verified support and the fallback adds verified support. Audit gaps remain visible in internal diagnostics so they can drive targeted retrieval and composition work without deleting useful resident context.
