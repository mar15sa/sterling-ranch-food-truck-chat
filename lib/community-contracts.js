...
function renderedCoveredDetails(requestedDetails, coveredDetails, directAnswer, keyDetails, actions) {
  ...
  return declared.filter((detail) => {
    if (detail === "price") return /\$\s*\d|\b\d+(?:\.\d+)?\s*%|\b(?:free|no charge|zero dollars)\b/i.test(text);
...
function buildAnswerContract({
  ...
  const renderedDetails = renderedCoveredDetails(requestedDetails, coveredDetails, cleanDirect, keyDetails, normalizedActions);
