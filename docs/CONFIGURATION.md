...
`npm run check` runs the complete deterministic release gate, including historical, authored, unseen, source-safety, and comparative evaluations. `npm run community:routing:live` separately asks the real staging AI router every labeled routing case three times, bypassing the answer-planning cache and requiring one deployment revision throughout. It requires 98% goal-and-subject accuracy, 98% intent accuracy, 98% structured-detail accuracy, 98% repeat consistency, and 100% prompt-injection rejection. The daily `AI routing consistency` workflow saves that real-model gate's evidence. `npm run community:portability:live` performs a live end-to-end proof against the configured second CivicPlus community. `npm run eval:rules:unseen` remains the smaller rules-only holdout scorecard.

Keep `COMMUNITY_AUTO_PROMOTE=false` during reconciliation. Production promotion requires Marissa's approval of the verified release. Release 2 replaces the old source-publishing job with a monitoring-only workflow so checks continue independently of publishing permission. Optional `STAGING_BASE_URL` and `PRODUCTION_BASE_URL` variables override the documented defaults.
