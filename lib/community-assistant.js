...
function holidayFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    const historicalCapture = capturedProjectionCoversPastDate(source, holidayDate, suppliedNow, options.communityProfile?.timezone || "UTC");
...
function datedFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    const approvedText = source.canonicalScopedProjection === true
      ...
      : "";
    const historicalCapture = capturedProjectionCoversPastDate(source, plan.dateRange.start, suppliedNow, options.communityProfile?.timezone || "UTC");
    ...
    const season = recurringSeasonCoverage(text, plan.dateRange.start);
