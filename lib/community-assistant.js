...
function isOfficialCommunitySource(source = {}) {
  ...
  return source.isOfficialResource === true
    && (!source.controllingSourceRole || validatedRoles.has(source.controllingSourceRole));
}
...
function configuredCommunityResource(index = {}, profile = {}, candidates = []) {
  const current = candidates.find((source) => source?.sourceUrl
    && isOfficialCommunitySource(source)
    && source.lifecycle === "current"
    && (source.canonicalScopedProjection === true || source.answerableProjection === true || source.reviewStatus === "approved"));
