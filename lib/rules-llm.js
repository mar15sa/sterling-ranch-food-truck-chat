...
const { plainLanguageSourcePart, residentVoiceIssues } = require("./resident-answer-voice");
...
// Rewriting is optional presentation work. It must yield quickly to the
// complete source-built answer so a slow provider cannot make a resident wait.
const TIMEOUT_MS = Math.min(Number(process.env.RULES_LLM_TIMEOUT_MS) || 3000, 3000);
const MAX_SOURCES = 5;
const { budgetEvidenceText } = require("./evidence-prompt-budget");
...
function getRulesLlmMode(env = process.env) {
  ...
  if (env.RULES_ENABLE_LLM_REWRITE === "true") return "all";
  // When the existing provider key is present, source-grounded synthesis is
  // the normal resident experience. Operators can still explicitly set off.
  return env.ANTHROPIC_API_KEY ? "selective" : "off";
}
...
function selectiveRewriteDecision({
  ...
  if (!draftAnswer || /\b(?:I (?:do not|don't) have enough|could not extract|couldn't extract|couldn’t extract|won't guess|not enough evidence)\b/i.test(draftAnswer)) {
  ...
  if (["structured", "extractive", "official-clause", "deterministic"].includes(answerStrategy)) {
    return { eligible: true, reason: "grounded-answer-synthesis" };
  }
...
const SYSTEM_PROMPT = [
  ...
  "Sound like a helpful neighbor who already read the rulebook and is passing along the useful part.",
  ...
  "- A location mentioned only in the resident's question is context, not proof that an item may be placed there. If the cited sections do not establish that location, limit the answer to supported facts and say the placement is not confirmed.",
  ...
  "- Write the answer as natural prose. Do not use fixed labels such as \"Short answer\", \"What I found\", or \"Before you act\".",
  "- Include a next step only when it helps the resident act or prevents a mistake.",
  "",
  ...
  "- Avoid administrative phrases such as connected official sources, approved up-to-date sources, the requested detail, and I found official calendar events.",
...
function buildSourcesBlock(sources) {
  return budgetEvidenceText(sources, {
    maxSources: MAX_SOURCES,
    project: source => source.questionSpecificExcerpt && source.excerpt ? source.excerpt : source.text || source.excerpt || "",
  }).map(({ source, text: body, contextCoverage }, index) => {
      const label = [
        source.title,
        ...
        .filter(Boolean)
        .join(" | ");
      // Question-specific excerpts are approved projections of the cited
      // source. Prefer them for synthesis so a matching row in a long table
      // does not bring unrelated facts into the model's working context.
      // Final-answer grounding still validates against the complete sources.
      const labeledFacts = focusedDimensionDetails([source])
        .map(({ role, value }) => {
          const factLabel = role === "width" ? "Spread/width" : `${role.charAt(0).toUpperCase()}${role.slice(1)}`;
          const readableValue = String(value)
            .replace(/\bft\b/i, "feet")
            .replace(/\bin\b/i, "inches");
          return `${factLabel}: ${readableValue}`;
        });
      const explicitFacts = labeledFacts.length
        ? `\nExplicit labeled facts from this source: ${labeledFacts.join("; ")}.`
        : "";
...
// If the model is temporarily unavailable, remove the internal retrieval
// scaffold without changing its facts. This is a presentation fallback, not a
// second source of resident-facing information.
function naturalizeGroundedDraft(text) {
  const lines = String(text || "").split(/\r?\n/);
  const parts = [];
  for (const rawLine of lines) {
    const line = rawLine.trim();
    if (!line || /^What I found\s*:\s*$/i.test(line)) continue;
    let cleaned = line
      .replace(/^Short answer\s*:\s*/i, "")
      .replace(/^Allowed(?: choices from the selected source)?\s*:\s*/i, "")
      .replace(/^Before you act\s*:\s*/i, "")
      .replace(/^[-*]\s+/, "")
      .trim();
    const compactPredicate = cleaned.match(/^([^:.\n]{2,80}):\s*([a-z][^:\n]*[.!?])$/);
    if (compactPredicate && !/\b(?:is|are|was|were|has|have|must|may|can|shall|requires?)\b/i.test(compactPredicate[2])) {
      cleaned = `${compactPredicate[1]} is ${compactPredicate[2]}`;
    }
    if (!cleaned) continue;
    const normalized = cleaned.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
    if (!normalized) continue;
    if (parts.some((part) => part.normalized.includes(normalized))) continue;
    const containedIndex = parts.findIndex((part) => normalized.includes(part.normalized));
    if (containedIndex >= 0) parts[containedIndex] = { text: cleaned, normalized };
    else parts.push({ text: cleaned, normalized });
  }
  return parts.map((part) => part.text).join("\n\n").replace(/[ \t]{2,}/g, " ").trim();
}

// The deterministic source composer sometimes starts with exact official
// clauses. Turn only a few stable clause shapes into everyday instructions,
// while keeping each source-backed subject, date, limit, and action intact.
function composePlainEnglishFallback(text) {
  return naturalizeGroundedDraft(text)
    .split(/\n\s*\n/)
    .map((part) => {
      const value = part.trim();
      const approvedPeriod = value.match(/^(.+?)\s+during the following approved\s+[^:]+:\s*From\s+(.+?)\.?$/i);
      if (approvedPeriod) {
        const activity = (approvedPeriod[1].charAt(0).toLowerCase() + approvedPeriod[1].slice(1))
          .replace(/^install and energize\s+seasonal decorative lighting$/i, "put up and turn on seasonal decorative lights")
          .replace(/^install and energize\s+holiday lighting$/i, "put up and turn on holiday lights");
        return `You can ${activity} from ${approvedPeriod[2]}.`;
      }
      const removal = value.match(/^(.+?)\s+(?:is|are)\s+required to be removed\.?$/i);
      if (removal) return `You need to remove ${removal[1].charAt(0).toLowerCase() + removal[1].slice(1)}.`;
      const turnOff = value.match(/^(.+?)\s+must be turned off by\s+(.+?)\.?$/i);
      if (turnOff) return `Turn off ${turnOff[1].charAt(0).toLowerCase() + turnOff[1].slice(1)} by ${turnOff[2]}.`;
      const approvedFinish = value.match(/^The approved\s+(.+?)\s+on the\s+(.+?)\s+is\s+(.+?)\s+which matches\s+(.+?)\s+described in subsection\s+\([^)]+\)\s+of this section\.?$/i);
      if (approvedFinish) {
        const product = approvedFinish[3].replace(/,("?)$/, "$1");
        return `For the ${approvedFinish[2]}, the approved ${approvedFinish[1]} is ${product}. It matches ${approvedFinish[4]}.`;
      }
      const numberedClause = value.match(/^(.*?)\s+\(\d+\)\s+(.*)$/);
      const finishClause = (numberedClause ? numberedClause[1] : value).trim();
      const selectedFinish = finishClause.match(/^The\s+(.+?)\s+selected for\s+(.+?)\s+is\s+(.+?)\.?$/i);
      if (selectedFinish) {
        const product = selectedFinish[3].replace(/\.("?)$/, "$1");
        const followUp = (numberedClause ? numberedClause[2] : "")
          .replace(/the installer will need to cut to the appropriate length in the field/gi, "the installer will need to cut it to fit")
          .trim();
        return `${selectedFinish[2].charAt(0).toUpperCase() + selectedFinish[2].slice(1)} uses ${product}.${followUp ? ` ${followUp}` : ""}`;
      }
      if (/^Open the linked official section if you need the complete wording\.?$/i.test(value)) {
        return "Open the official source for the complete wording.";
      }
      return plainLanguageSourcePart(value);
    })
    .filter(Boolean)
    .join("\n\n");
}
...
async function rewriteAnswerWithLLM(question, draftAnswer, sources) {
  ...
  const unsupportedLocations = unsupportedQuestionLocations(question, sources);
  const locationScope = unsupportedLocations.length
    ? `\n\nEvidence scope boundary: The resident mentioned location details that the cited sections do not establish (${unsupportedLocations.map(({ label }) => label).join(", ")}). Do not approve placement there or say location-specific approval is unnecessary. State the supported facts, and make clear that the cited evidence does not confirm the requested placement.`
    : "";
    ...
    for (let attempt = 0; attempt < MAX_REWRITE_ATTEMPTS; attempt += 1) {
      ...
      headers: {
        ...
        system: SYSTEM_PROMPT,
          messages: [{ role: "user", content: retryPrompt || userContent }],
        }),
      ...
      // Use the same answer-level coverage check that runs at the end of the
      // rules path. That includes catalog-membership claims, which need a
      // correction while the writer still has the positive source evidence in
      // view rather than a later generic fallback.
      const issues = [...new Set([
        ...llmRewriteIssues(cleaned, draftAnswer, sources, question),
        ...answerCoverageIssues(question, cleaned, sources),
        ...residentVoiceIssues(cleaned),
      ])];
      if (!issues.length) {
      ...
      if (attempt + 1 >= MAX_REWRITE_ATTEMPTS) {
        console.warn(`Rules LLM rewrite rejected: ${issues.join("; ")}`);
        recordRulesLlmRejected({ reason: issues[0] });
        return null;
      }

      // Give the writer one bounded chance to correct a draft the deterministic
      // validator rejected. The original evidence is repeated below, and the
      // next result is put through the exact same validator before it can be
      // returned to a resident.
      retryPrompt =
        `Your previous candidate did not pass the factual grounding check. Rewrite it from scratch.\n\n` +
        `Candidate that was rejected:\n<<<CANDIDATE\n${cleaned}\nCANDIDATE>>>\n\n` +
        `Problems to correct:\n- ${issues.join("\n- ")}\n\n` +
        `Return only a natural, plain-English replacement. Keep every supported fact from the grounded draft. Do not add a yes/no answer, approval claim, placement claim, or other fact that the cited sections do not establish.\n\n` +
        userContent;
    }
    return null;
  } catch (error) {
