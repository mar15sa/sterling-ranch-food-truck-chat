...
const root=fileURLToPath(new URL('../../',import.meta.url)),[indexArg,attestationArg,outArg,representation='approved-text']=process.argv.slice(2);
...
const retrievalDocuments=corpus.documents.map(d=>semantic.retrievalDocument(d,representation));
...
const manifest={status:'initializing',isTest:true,startedAt:new Date().toISOString(),codeRevision:cp.execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),communityId:index.communityId,
 representation,answerProjectionVersion:1,retrievalRepresentationHash:hashTools.hash(retrievalDocuments),model:readiness.model,modelRevision:readiness.revision,dtype:'q8',pooling:'cls',dimensions:384,runtimeVersion:readiness.runtimeVersion,sourceSnapshotHash:hashTools.hash(index),corpusHash:hashTools.hash(corpus),casesHash:hashTools.hash(spec),design:jobs,completed:[],paidApiCalls:0,newSubscriptions:0,
 limits:{sourcesPerResult:4,rankCandidates:30},limitations:[...spec.limitations,representation==='approved-text'?'Approved projection text and title only.':'Exact approved action evidence context is retrieval-only; it is not approved answer facts.','Nearest neighbors do not establish relevance, permission, completeness or a resolved subject.','Desktop timings/RAM are not production hosting estimates.']};
...
try{
 ...
 const windowStart=Date.now(),units=[];for(const doc of retrievalDocuments)units.push(...await corpusTools.windowsForDocument(doc,async text=>(await tokenizer(text,{truncation:false,padding:false})).input_ids.dims.at(-1)));
 ...
 for(const [i,job] of jobs.entries()){
  ...
  const scanStart=Date.now(),dense=semantic.rankDense({corpus,currentIndex:index,now,units,vectors,queryVector:q.data,representation}),scanAndEligibilityMs=Date.now()-scanStart;
  const hybrid=semantic.fuse(keyword.slice(0,30),dense.slice(0,30),30);
  const serialize=rows=>rows.slice(0,10).map(r=>({id:r.document.id,title:r.document.title,sourceUrl:r.document.sourceUrl,sourceIdentity:semantic.projectionIdentity(r.document),text:r.document.text,actions:semantic.answerProjection(r.document).actions,score:r.score,fusionScore:r.fusionScore,ranks:r.ranks,...(r.window?{matchedWindow:r.window}:{})}));
