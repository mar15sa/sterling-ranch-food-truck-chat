...
function rentalAnswer(question, index) {
  ...
  if (park) {
    ...
    return { ...buildAnswerContract({
      directAnswer: "CAB park shelters are currently listed at $15 per hour. You can check a date and begin the reservation in the official rental catalog.",
      keyDetails: [
        ...
        "Have the park, date, start time, end time, and expected attendance ready.",
      ],
  ...
  return { ...buildAnswerContract({
    directAnswer: "Yes. The Great Hall is $100 per hour with a two-hour minimum ($200 minimum rental), plus a currently listed $250 refundable security deposit.",
    keyDetails: [
      ...
      "Pavilion rentals are limited to one per household per day and no more than two per calendar month.",
    ],
...
function proactiveCommunityAnswer(question, options = {}) {
  ...
  if (/\b(?:book|reserve|rent)\b.{0,40}\b(?:park|shelter|clubhouse|overlook|great hall|pavilion)\b|\b(?:park|shelter|clubhouse|overlook|great hall|pavilion)\b.{0,40}\b(?:book|reserve|rent)\b/i.test(text)) return rentalAnswer(text, index);
...
function enhanceProactiveRulesAnswer(question, answer, options = {}) {
  ...
  let directAnswer = "";
  let nextStep = "";
  if (/\b(?:when|time|today|now)\b.{0,35}\bwater|\bwater\b.{0,35}\b(?:lawn|irrigat)/i.test(text)) {
