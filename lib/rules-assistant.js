...
function simpleTopicOverviewAnswer(query, sources = [], residentQuestion = query) {
  ...
  if (
    ...
    const renewal = inlineText.match(/Owners of Qualified Residences may submit an Application for a new Annual Pass upon expiration of the initial or immediately preceding Annual Pass\./i)?.[0];
    ...
    const lead = asksReimbursement
      ...
      : asksHowToGet
        ? capitalizeFirstLetter(voucher || renewal)
