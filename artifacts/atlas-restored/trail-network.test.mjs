...
test('walking overview preserves full landscape and only offers walking areas',()=>{
 const h=walkOverview(t);assert.match(h,/assets\/full-landscape.png/);
 assert.equal((h.match(/class="walk-overview__area/g)||[]).length,4);
 for(const area of ['Providence','Ascent','Prospect'])assert.ok(h.includes(area));
 assert.doesNotMatch(h,/data-open|data-future|data-place/);
 const focused=walkOverview(t,'Ascent');assert.match(focused,/is-focused-ascent/);
 assert.match(focused,/data-walk-area="Ascent" aria-pressed="true"/);
 const css=fs.readFileSync(new URL('walk-overview.css',import.meta.url),'utf8');assert.match(css,/aspect-ratio:3\/2/);assert.match(css,/object-fit:contain/);
});
