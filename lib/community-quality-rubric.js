...
const VERSION = "resident-quality-rubric-v3-unpublished";
...
function evidenceAssessment(result = {}) {
  const audited = result?._requestContract?.assessment;
  if (audited && Array.isArray(audited.needs)) return audited;
  return { outcome: result?.completion?.outcome, needs: result?.completion?.needs };
}
...
function validCandidateShape(result = {}) {
  ...
  const auditedSelection = result?._requestContract?.candidate?.comparedDrafts === 2;
  return (["need-first-candidate", "need-first-ai-candidate", "need-first-candidate-boundary"].includes(result.answerMode) || auditedSelection)
...
function scoreProactivity(result, contractNeeds = []) {
  ...
  if (!required) {
    const irrelevant = ((result.actions || []).length > 0 && boundActions.length === 0)
      || displayedVoiceIssues(result).includes("unnecessary-source-handoff");
...
function scoreConcision(answer = "", needCount = 1) {
  const value = String(answer || "").trim();
  if (!value) return { value: 0, reason: "There is no answer to scan." };
  const paragraphs = value.split(/\n\s*\n/).map((part) => part.trim()).filter(Boolean);
  if (value.length > 1800 || paragraphs.length > Math.max(8, needCount * 5)) {
    return { value: 0, reason: "The answer buries the requested information in too much material." };
  }
  if (value.length > 1100 || paragraphs.length > Math.max(5, needCount * 3)) {
    return { value: 1, reason: "The answer is usable, but it includes more material than the request needs." };
  }
  return { value: 2, reason: "The requested information is easy to find without distracting additions." };
}

function ratingFor({ outcome, total, hardFailures, directness }) {
  if (hardFailures.length) return total >= 5 ? { rating: "Weak", score: 2 } : { rating: "Poor", score: 1 };
  if (COMPLETE_OUTCOMES.has(outcome)) {
    if (total === 12) return { rating: "Excellent", score: 5 };
    if (total >= 9) return { rating: "Good", score: 4 };
    if (total >= 6) return { rating: "Mixed", score: 3 };
...
function assessCommunityAnswerQuality(question, result = {}) {
  ...
  const voiceIssues = displayedVoiceIssues(result);
  if (voiceIssues.length) {
    humanFirst.value = Math.min(humanFirst.value, 1);
    humanFirst.issues = unique([...humanFirst.issues, ...voiceIssues]);
    humanFirst.reason = "The visible answer still contains wording that fails the resident voice checks.";
    hardFailures.push("resident-voice-failure");
  }
  const concision = scoreConcision(result.answer, contractNeeds.length);
  const dimensions = { directness, completeCoverage: coverage, specificity, usefulProactivity: proactivity, humanFirst, appropriateConcision: concision };
