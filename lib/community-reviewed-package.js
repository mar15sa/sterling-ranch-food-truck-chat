...
function buildReviewedSources(packageData) {
  return packageData.decisions.map(decision => {
    ...
    const facts = decision.facts.map(claim => ({
      ...review, id: claim.id, approvalClaim: claim.approvalClaim || claim.id, type: claim.type || 'information',
    ...
    const actions = decision.actions.map(action => ({ ...action, ...review, approvalClaim: action.approvalClaim || action.id,
