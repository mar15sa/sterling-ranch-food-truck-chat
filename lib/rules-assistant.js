...
function specialSourcesForQuestion(index, query) {
  ...
  if (/\b(?:approved (?:plants?|trees?)|plant list|trees? can (?:i|we) plant|kind of (?:plants?|trees?))\b/i.test(query)) {
...
async function answerRulesQuestion(query, options = {}) {
  ...
  const preferSpecialSources = Boolean(specialSources.length) && (
    ...
    /\b(?:chicken wire|pickle ?ball|sport court|rain(?:water)?(?: harvesting)? barrels?|artificial turf|synthetic turf|turf|air conditioner|ac unit|hvac|mini split|gazebo|pergola|fireworks?|approved plants?|approved trees?|plant list|trees? can (?:i|we) plant|tree lawn|fence stain|stain color|utility trailer|privacy film|tint film|window tint|long[-\s]?term rental|approved landscaper|own landscaping|redo backyard|concrete patio|extend concrete|rear landscaping|side yard|quantum fiber|internet provider|water usage|front yard tree|catio|easement)\b/i.test(routingQuery) ||
