...
function isFreshnessTrackedSource(source = {}) {
  ...
  return !isDynamicSource(source)
    ...
    && !["official-action", "owner-observation"].includes(source.connectorType);
}

function isOwnerObservation(source = {}) {
  return source.connectorType === "owner-observation" || source.provenanceType === "owner-observation";
}

module.exports = { isDynamicSource, isFreshnessTrackedSource, isOwnerObservation };
