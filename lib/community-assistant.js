...
function chooseAuditedAnswer(query, contract, baseline, baselineAssessment, candidate, candidateAssessment) {
  ...
  const baselineIsVerified = baseline?.confidence?.canAnswer === true
    && ["verified", "source-unavailable"].includes(String(baseline.answerStatus || ""));
  const simpleRequest = isSimpleResidentRequest(query, contract);
  const baselineWasPreviouslyEligible = simpleRequest
    && (baselineAssessment?.outcome === "complete" || baselineSupportedNeeds > 0 || baselineIsVerified);
  ...
  if (baselineWasPreviouslyEligible) {
    return {
      ...
      reason: baselineAssessment?.outcome === "complete"
        ? "baseline-fully-covers-simple-request"
        : baselineSupportedNeeds > 0
          ? "baseline-useful-simple-request"
          : "verified-simple-baseline-preserved",
    };
  }
