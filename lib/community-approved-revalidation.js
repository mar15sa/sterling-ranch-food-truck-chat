...
async function observeCanonicalSource(sourceUrl, approvedSources, { fetchImpl = globalThis.fetch, extractPdfTextImpl = extractPdfText, timeoutMs = 30_000 } = {}) {
  ...
  const scopedProjection = approvedSources.length === 1 && isScopedExactVersionProjection(approvedSources[0]);
  if (scopedProjection) {
    const projectionProof = projectedActionProof(approvedSources, observedLinks, text);
    return {
