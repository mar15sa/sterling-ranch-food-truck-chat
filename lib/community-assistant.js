...
function eventsAnswer(result, requested = [], routingPlan = {}) {
  ...
  const firstEventDate = firstEvent ? readableEventDate(firstEvent.date, result.range, result.checkedAt) : "";
  const firstEventTime = firstEvent ? readableEventTime(firstEvent.time) : "";
  const directAnswer = namedFilterLabel && firstEvent
    ? `The next ${namedFilterLabel} event is ${firstEvent.title} on ${firstEventDate}${firstEventTime ? ` at ${firstEventTime}` : ""}${firstEvent.location ? ` in ${firstEvent.location}` : ""}.`
