...
const { buildResidentRequestContract } = require("./community-request-contract");
...
async function answerCommunityQuestion(query, options = {}) {
  ...
  const requestContract = options.requestContractMode === "shadow"
    ? buildResidentRequestContract(query, state.plan || completedAnswer?.routingPlan)
    : null;
  ...
  if (!state.plan && !state.shadowPlan) return requestContract ? { ...answer, _requestContract: requestContract } : answer;
  if (state.plan && !answer.routingPlan) {
    return { ...answer, ...interpretationFields(state.plan, state.outcome, state.mode), ...(state.connectorDiagnostics && !answer._connectorDiagnostics ? { _connectorDiagnostics: state.connectorDiagnostics } : {}), ...(requestContract ? { _requestContract: requestContract } : {}) };
  }
  return {
    ...answer,
    ...
    ...(requestContract ? { _requestContract: requestContract } : {}),
  };
}
