...
function specialSourcesForQuestion(index, query) {
  ...
  if (isFlagQuery(query) || isOutdoorDecorativeObjectQuery(query)) {
    ...
    return [
      ...
      ...(/\bflagpoles?\b/i.test(query)
        ? sourcesByTitleAndText(
            index,
            query,
            /^Sec\. 21-22\. - General community standards/i,
            [/separate nighttime illumination of a flag/i],
            1
          )
        : []),
    ];
...
function thirdPartyOwnershipBoundaryAnswer(query, sources = []) {
  ...
  return structuredHelpfulAnswer(
    `The cited rule says this property is owned and maintained by ${owner}. It does not establish resident permission to alter it, so I could not verify that the requested change is allowed.`,
    [sources[0]?.excerpt || ""],
    `Confirm the change with ${owner} or the CAB before altering the property.`
  );
}
...
function sourceGroundedRuleAnswer(query, sources = [], index = null) {
  ...
  const illustrativeProjectClause = allClauses.find((entry) =>
    /\b(?:gazebo|pergola)s?\b/i.test(entry.text) && /\b(?:such as|including|includes)\b/i.test(entry.text)
  );
  if (projectRequest && illustrativeProjectClause) {
    return structuredHelpfulAnswer(
      "The official passage I found mentions the requested project only as an example in a different rule, so it does not establish whether the project itself is allowed.",
      [completeSentence(illustrativeProjectClause.text)],
      "Confirm the proposed structure, location, and review requirements with the CAB before buying materials or starting work."
    );
  }
  ...
  if (illustrativeOnly) {
    return structuredHelpfulAnswer(
      "The official passage I found mentions the requested project only as an example in a different rule, so it does not establish whether the project itself is allowed.",
      [primary],
      "Share more detail about the proposed location and structure, or confirm the project with the CAB before buying materials or starting work."
    );
  }
  ...
  if (/\b(?:list|directory)\b/i.test(query) || /\bapproved\s+[a-z]+s\b/i.test(query)) {
    ...
    if (!hasRequestedEntries) {
      ...
      if (registrationClauses.length) {
        return structuredHelpfulAnswer(
          "The selected rules establish registration requirements, but they do not contain the requested approved-provider list.",
          registrationClauses.map((entry) => completeSentence(entry.text)),
          "Use the current official directory link when available, and verify that the individual or company is still approved before hiring them."
        );
      }
    }
  }
  ...
  if (/\b(?:access|log in|login|portal|restore)\b/i.test(query) && !(isHomeAutomationAccessQuery(query) && allClauses.some((entry) =>
    ...
    return structuredHelpfulAnswer(
      "The selected official passages describe the system's capabilities, but they do not provide current resident access or login instructions.",
      [primary],
      "Use a separately approved current service page or contact before presenting a portal or login route."
    );
  }
  ...
  if (isHomeAutomationAccessQuery(query)) {
    ...
    if (supportDetails.length) {
      return structuredHelpfulAnswer(
        "The current home-automation section provides these customer-support options:",
        supportDetails,
        "Use the listed support route to restore access or confirm account details."
      );
    }
  }
  ...
  if (
    ...
    !/\b(?:build|install|permitted|allowed)\b/i.test(query) &&
    !(/\b(?:3[- ]?rail|three[- ]rail|style\s*3|gate)\b/i.test(query) && exactFenceFinishClause) &&
    fenceFinishClause &&
    !/\b(?:Sherwin|#[0-9]{3,}|product (?:name|number))\b/i.test(fenceFinishClause.text)
  ) {
    return structuredHelpfulAnswer(
      "The selected official passage requires an approved fence finish, but it does not name the exact stain product or color requested.",
      [completeSentence(fenceFinishClause.text)],
      "Use an exact, approved fence specification for the resident's fence type and village before naming a product or color."
    );
  }
  ...
  if (/\b(?:who owns?|ownership|belongs to)\b/i.test(query) || /^\s*does\b.{0,70}\bown\b/i.test(query)) {
    ...
    if (!ownershipClause && responsibilityClauses.length) {
      return structuredHelpfulAnswer(
        "The selected official passages establish maintenance responsibility, but they do not state who owns the property.",
        responsibilityClauses.map((entry) => completeSentence(entry.text)),
        "Check the recorded plat, survey, right-of-way, or easement documents before treating the area as privately or community owned."
      );
    }
  }
  ...
  if (removalRequest) {
    ...
    if (!directRemovalClause) {
      return structuredHelpfulAnswer(
        "The selected official passages discuss the related property area, but they do not state whether the requested removal is allowed.",
        [primary],
        "Use the linked sections as the confirmed facts, and verify the removal requirement with the CAB before doing the work."
      );
    }
  }
  ...
  const answer = structuredHelpfulAnswer(
    lead,
    findings,
    "Open the linked official section for the surrounding wording. If your situation differs from the facts described there, ask a more specific follow-up before acting."
  );
  if (answerCoverageIssues(query, answer, sources).includes("direct-permission-answer-missing")) {
    return structuredHelpfulAnswer(
      "The selected official passages describe this topic, but they do not state whether the resident choice in the question is required or allowed.",
      [primary, ...findings].slice(0, 4),
      "Use the linked sections as the confirmed facts, and verify the unanswered choice with the CAB or the current service provider before changing service or starting work."
    );
  }
  return answer;
}
...
function isSourceEvidenceBoundaryAnswer(answer = "") {
  return /Short answer:\s*(?:I don't have enough rulebook evidence to answer that confidently\.|The official passage I found mentions|The selected (?:official passages|rules|official passage).*(?:do not|does not|but))/i.test(
    String(answer || "")
  );
}
...
function questionSpecificSourceAnswer(query, sources = []) {
  ...
  return structuredHelpfulAnswer(
    ...
    "Open the linked official section for the complete wording and confirm any required approval before acting."
  );
}
...
function fenceAndShedOverviewAnswer(query, sources = [], index) {
  ...
  const sourceTextByTitle = (matcher, fallback) => (index?.documents || [])
    .filter((document) => matcher.test(document.title || ""))
    .map((document) => document.text || "")
    .filter(Boolean)
    .join("\n") || fallback.text || fullTextForSource(index, fallback);
  const fenceText = sourceTextByTitle(/^Sec\. 21-23\. - Fencing standards/i, fenceSource);
  const shedText = sourceTextByTitle(/^Sec\. 21-22.*Backyard utility sheds/i, shedSource);
  const clause = (text, pattern) => sentenceContaining(text, pattern) || String(text || "").match(new RegExp(`[^.]*${pattern.source}[^.]*\\.`, pattern.flags))?.[0].replace(/\s+/g, " ").trim() || "";
  const fenceApproval = clause(fenceText, /Approval must be obtained from the DRC prior to any construction or landscaping/i);
  const fenceStyle = clause(fenceText, /This is an open-rail fence consisting of three concrete rails/i);
  const shedApproval = clause(shedText, /Backyard utility sheds\.\s*DRC approval is required/i);
  const shedLimits = clause(shedText, /should not exceed an overall height of/i);
  const shedUtilities = clause(shedText, /Utilities to a backyard utility shed must be underground/i);
  const shedDetails = [shedApproval, shedLimits, shedUtilities].filter(Boolean).join(" ");

  if (fenceApproval && shedApproval) {
    return {
      available: true,
      answer: structuredHelpfulAnswer(
        fenceApproval,
        [
          `${capitalizeFirstLetter((String(query).match(/\bfence\b/i) || [""])[0])}: ${fenceApproval}`,
          fenceStyle ? `${capitalizeFirstLetter((String(query).match(/\bfence\b/i) || [""])[0])}: ${fenceStyle}` : "",
          `${capitalizeFirstLetter((String(query).match(/\bshed\b/i) || [""])[0])}: ${shedDetails}`,
        ].filter(Boolean),
        ""
      ),
...
function rvParkingOverviewAnswer(query, sources = []) {
  ...
  return structuredHelpfulAnswer(
    utilityTrailer
      ...
      : exceedsLimit
        ? `No. ${duration.requestedText.charAt(0).toUpperCase()}${duration.requestedText.slice(1)} exceeds the current temporary driveway-parking limit${hourLimit ? ` of ${hourLimit} hours` : ""}.`
  ...
  if (asksPoleHeight && /(?:DRC approval is required for freestanding flagpoles|DRC approved flagpole|flagpole requires DRC approval)/i.test(text)) {
    return structuredHelpfulAnswer(
      ...
      [
        ...
        /separate nighttime illumination of a flag/i.test(text) ? "Separate nighttime illumination of a flag also requires DRC approval." : "",
        /flags bearing commercial messages are prohibited/i.test(text) ? "Flags bearing commercial messages are prohibited." : "",
      ],
...
function specificCurrentFeeAnswer(query, sources = []) {
  ...
  if (/\b(?:water rates?|water fee|water charge)\b/i.test(query) && /(?:Monthly Fee Residential|residential water base-rate rows)/i.test(text)) {
    const base = amount(
      text.match(/Monthly Fee Residential[\s\S]{0,180}?Single Family Detached[^$]{0,80}\$\s*([\d,]+(?:\.\d{2})?)/i)
    ) || amount(text.match(/residential water base-rate rows:[^$]{0,280}\$\s*([\d,]+(?:\.\d{2})?)/i));
    const tierOne = amount(
      text.match(/Tier residential and non-residential Fee per 1,000 gallons Tier 1[^$]{0,100}\$\s*([\d,]+(?:\.\d{2})?)/i)
    ) || amount(text.match(/residential indoor-water tiers are\s*\$\s*([\d,]+(?:\.\d{2})?)/i));
    ...
    return structuredHelpfulAnswer(
      ...
      [
        ...
        "Other approved residential rows apply to dual indoor-and-outdoor meters and the listed home types.",
      ],
...
function currentEnforcementOverviewAnswer(query, sources = []) {
  ...
  if (isDelinquentAccountQuery(query) && /courtesy past[ -]?due notification/i.test(text)) {
    const courtesy = text.match(/Courtesy Past Due Notification\.\s*([A-Za-z]+)(?:\s*\(\d+\))?\s+days after/i)
      || text.match(/courtesy past[ -]?due notification\s+([A-Za-z]+) days after/i);
    const late = text.match(/within\s+([A-Za-z]+)(?:\s*\(\d+\))?\s+calendar days after the invoice due date/i)
      || text.match(/late fee after\s+([A-Za-z]+) calendar days/i);
    if (!courtesy || !late || !/last Wednesday of (?:the|that) month/i.test(text)) return "";
    return structuredHelpfulAnswer(
      ...
      "Use the linked collection policy promptly to review dispute and payment-plan options."
    );
  }
...
async function answerRulesQuestion(query, options = {}) {
  ...
  // Retired: topic-specific answer prose used to return before the grounded
  // composition path. Keep this block unreachable while its helper functions
  // are removed in a follow-up cleanup; ordinary rule questions must continue
  // to the shared source-derived draft and verified AI rewrite below.
  const staticTopicShortcutsEnabled = false;
  let earlyStructuredAnswer = "";
  if (staticTopicShortcutsEnabled) {
  ...
  let needsSourceDerivedFacts = summaryHasVolatileFacts(plainAnswer) || needsStructuredOverview || hasQuestionSpecificExcerpt;
