...
test('wider illustration retains full geographic arrangement and a directly comparable official plan',()=>{
 const html=villagePlanMarkup(),base=html.split('<div class="village-plan-sheet">')[1].split('<p class="plan-source-caption">')[0];
 assert.match(base,/<img class="village-plan-geography" src="assets\/wider-ranch-landscape-v1.png"/);
 assert.match(base,/alt="Approximate illustrated wider Ranch,/);
 assert.doesNotMatch(base,/<details|planning-paper/);
 assert.match(html,/<summary>Compare with official plan<\/summary>/);
 assert.match(html,/src="assets\/village-master-plan.jpg"/);
