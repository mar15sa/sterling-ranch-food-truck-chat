...
function main() {
  const packageData = read("data/community-source-approvals-v11.json");
  write("data/canonical-source-ledger.json", buildLedger());

  const index = read("data/community-index.json");
  const observedAt = [index.truthStatus?.generatedAt, packageData.decidedAt]
    .filter(Boolean)
    .sort()
    .at(-1);
  ...
  index.factLedger = buildFactLedger(index, { previousLedger: index.factLedger, observedAt });
  refreshTruthStatus(index, observedAt);
