...
function underEaveLightingOverviewAnswer(sources = []) {
  ...
  const lead = capitalizeFirstLetter(applicationClause.text);
  return structuredHelpfulAnswer(
    lead,
...
function fenceHeightOverviewAnswer(sources = []) {
  ...
  const lead = capitalizeFirstLetter(heightClause);
  return structuredHelpfulAnswer(
    lead,
...
function trashStorageOverviewAnswer(query, sources = []) {
  ...
  const lead = asksTiming
    ? `The controlling source does not specify the requested detail. ${storageClause}`
...
function rvParkingOverviewAnswer(query, sources = []) {
  ...
  const lead = /\bstreet\b/i.test(query) && streetClause
    ...
    : duration && hourLimit && duration.hours > hourLimit
      ? `No. ${capitalizeFirstLetter(duration.requestedText)} ${capitalizeFirstLetter(temporaryClause.text)}`
...
function sourceDerivedAnswerParts(query, sources, originalAnswer = "", residentQuestion = query) {
  ...
  if (isResidentFeeOverviewQuery(query)) {
    ...
    const overviewSources = sources.filter((source) =>
      /^20\d{2} (?:water, sanitary sewer, and stormwater|CAB service fees)/i.test(source.title || "")
    );
    if (overviewAnswer) return { available: true, answer: overviewAnswer, sources: overviewSources, strategy: "structured" };
  }
