...
function supportsKnownLiveCommunityRequest(question = "", plan = {}) {
  ...
  const foodTruck = /\bfood\s*trucks?\b/i.test(text)
    && ["schedule", "cost", "information"].some((goal) => goals.has(goal))
...
function normalizeKnownLiveCommunityPlan(question = "", plan = {}) {
  ...
  const intent = /\bfood\s*trucks?\b/i.test(text) ? "events" : "services";
  return { ...plan, intent, scope: "community", needsClarification: false, clarificationQuestion: "" };
}
...
function datedFacilityHoursAnswer(question, result, options = {}) {
  ...
  for (const source of result.sources || []) {
    ...
    const stale = source.staleAfter && new Date(source.staleAfter).getTime() < answerNow;
    ...
    return buildAnswerContract({
      directAnswer,
      keyDetails: stale
        ? [holidayNote, "This published schedule is awaiting a fresh confirmation, so please check the official facility page before you go."]
        : [holidayNote],
      ...
      // This shortcut used to bypass the normal source-freshness check. A
      // dated answer may describe stale evidence, but it must never present
      // that evidence to a resident as verified.
      status: stale ? "verified-incomplete" : "verified",
