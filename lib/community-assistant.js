...
function bestContactContext(question, sources) {
  ...
  return candidates.sort((a, b) => b.score - a.score)[0]?.context || "";
}
...
async function answerCommunityQuestion(query, options = {}) {
  ...
  if (/^(?:hi|hello|hey|thanks|thank you|how are you|tell me a joke)[!.?\s]*$/i.test(question)) {
    return buildAnswerContract({ directAnswer: "Hi! Ask me about community rules, services, forms, facilities, events, or current status.", status: "out-of-scope", answerMode: "conversation" });
  }
